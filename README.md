...
Level 1A adds research packets, bid/ask order-book-walk paper-fill simulation, and paper-trade records. Strategy-cycle paper execution can persist records to local Supabase/Postgres through `paper_trade_journal_records` when the DB sink is enabled. `PaperTradeJournal` JSONL is the legacy compatibility/export/replay path and remains the input for not-yet-migrated consumers such as NAV marking, outcome tracking, continuous-run NAV source handling, and cost audit. It remains paper-only: no account authentication, no private-key handling, no order placement, no order cancellation, no user WebSocket, no heartbeat, no live trading, and no compliance/legal/geographic-access analysis.
