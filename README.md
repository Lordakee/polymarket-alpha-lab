...
- Run from the CLI with `polymarket-alpha-lab run --starting-cash <Decimal> [--cycle-log <path>] [--nav-log <path>] [--repeat-interval <seconds>] [--max-iterations <n>] [--paper-execute --paper-journal <path>]`, which reuses the strategy-cycle scan/cycle args, maps `--repeat-interval 0` to single-shot, and prints completed/failed/skipped counts plus the time span. Continuous `run` keeps `--paper-journal` as the compatibility/NAV source path in this node. The DB-backed paper-trade read source applies only to the one-shot `portfolio-nav` CLI, not to continuous `run`; the DB-first no-default-journal behavior described above applies to the one-shot `strategy-cycle --paper-execute` path when the paper trade journal DB sink is enabled. There is no account reader, order API, wallet signer, live-execution client, scheduler daemon, or async runtime.
