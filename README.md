...
It also includes an optional local-only Strategy Risk Audit preflight for continuous paper runs and optional Strategy Risk Audit logging; the preflight reads existing paper logs and can pause the next paper run before any public client is constructed, while audit logging is an explicit opt-in local append-only JSONL evidence artifact.
