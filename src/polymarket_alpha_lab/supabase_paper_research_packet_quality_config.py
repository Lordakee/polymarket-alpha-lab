"""Environment boundary for paper research packet quality DB persistence."""
...
DEFAULT_PAPER_RESEARCH_PACKET_QUALITY_DB_TABLE = (
    DEFAULT_PAPER_RESEARCH_PACKET_QUALITY_REPORTS_TABLE
)
...
class SupabasePaperResearchPacketQualityConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_RESEARCH_PACKET_QUALITY_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_RESEARCH_PACKET_QUALITY_DB_DSN_ENV_VAR} must be set when DB is enabled",
            )
...
def from_paper_research_packet_quality_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperResearchPacketQualityConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_RESEARCH_PACKET_QUALITY_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_RESEARCH_PACKET_QUALITY_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_RESEARCH_PACKET_QUALITY_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_RESEARCH_PACKET_QUALITY_DB_TABLE,
    )
    try:
        return SupabasePaperResearchPacketQualityConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == _TABLE_NAME_ERROR:
            raise ValueError(
                f"{PAPER_RESEARCH_PACKET_QUALITY_DB_TABLE_ENV_VAR} must be a lowercase identifier with optional schema prefix",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_RESEARCH_PACKET_QUALITY_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_RESEARCH_PACKET_QUALITY_DB_DSN_ENV_VAR} must be a string",
        )
