"""Optional psycopg adapter for outcome tracking report storage."""
...
from polymarket_alpha_lab.outcome_tracking_store import (
...
def insert_outcome_tracking_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_outcome_tracking_report(
            connection,
            report,
...
def load_outcome_tracking_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_outcome_tracking_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the outcome tracking psycopg adapter; "
            "install the postgres extra.",
        ) from exc
    try:
        return psycopg.connect(dsn)
    except Exception:
        raise RuntimeError("failed to connect to the outcome tracking database") from None
...
__all__ = (
    ...
    "load_outcome_tracking_reports_with_psycopg",
)
