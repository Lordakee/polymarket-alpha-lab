"""Environment boundary for paper autonomous investment ledger DB persistence."""
...
class SupabasePaperAutonomousInvestmentLedgerConfig:
    enabled: bool
    dsn: str | None
...
def from_paper_autonomous_investment_ledger_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperAutonomousInvestmentLedgerConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_DSN_ENV_VAR)
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_DSN_ENV_VAR} must be a string",
        )
