"""Environment boundary for strategy candidate research queue DB persistence."""
...
class SupabaseStrategyCandidateResearchQueueConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_TABLE
...
def from_strategy_candidate_research_queue_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabaseStrategyCandidateResearchQueueConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_DSN_ENV_VAR)
    table_name = source.get(
        STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_TABLE_ENV_VAR,
        DEFAULT_STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_TABLE,
    )
    try:
        return SupabaseStrategyCandidateResearchQueueConfig(
            enabled=enabled,
            dsn=dsn,
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{STRATEGY_CANDIDATE_RESEARCH_QUEUE_DB_DSN_ENV_VAR} must be a string",
        )
    if not value:
        return None
