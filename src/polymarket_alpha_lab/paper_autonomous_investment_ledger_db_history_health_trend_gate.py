"""Pure gate reducer for investment ledger DB-history health trends."""
...
class PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig:
    ...
    max_watch_ledger_report_count_delta: int = 0
    max_blocked_ledger_report_count_delta: int = 0
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig:
            raise TypeError(
                "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig "
    ...
    def __post_init__(self) -> None:
        if type(self) is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig:
            raise ValueError(
        ...
        for field_name in (
            ...
            "max_watch_ledger_report_count_delta",
            "max_blocked_ledger_report_count_delta",
...
@dataclass(frozen=True)
class PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReasonCodeCount:
    reason_code: str
    report_count: int
    paper_only: bool = True
    report_only: bool = True
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if (
            cls
            is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReasonCodeCount
        ):
            raise TypeError(
                "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReasonCodeCount "
                "does not support subclassing",
...
class PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport:
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport:
            raise TypeError(
                "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport "
                "does not support subclassing",
            )
    ...
    def __post_init__(self) -> None:
        if type(self) is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport:
            raise ValueError(
        ...
        _require_optional_decimal(
            ...
            self.latest_total_submitted_notional_delta,
        )
        object.__setattr__(
            self,
            "reason_code_counts",
...
def build_paper_autonomous_investment_ledger_db_history_health_trend_gate_report(
    ...
    *,
    ...
    if type(trend_report) is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport:
        raise ValueError(
            "trend_report must be a "
            "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport",
        )
    if type(config) is not PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig:
        raise ValueError(
            "config must be a "
            "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig",
        )
    ...
    return PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport(
        ...
        watch_ledger_report_count_delta=trend_report.watch_ledger_report_count_delta,
        blocked_ledger_report_count_delta=(
            trend_report.blocked_ledger_report_count_delta
        ),
        latest_source_record_count_delta=(
            trend_report.latest_source_record_count_delta
        ),
        latest_submitted_count_delta=trend_report.latest_submitted_count_delta,
        latest_held_count_delta=trend_report.latest_held_count_delta,
        latest_blocked_count_delta=trend_report.latest_blocked_count_delta,
        latest_total_submitted_notional_delta=(
            trend_report.latest_total_submitted_notional_delta
        ),
        latest_source_generated_at_delta_seconds=(
            trend_report.latest_source_generated_at_delta_seconds
        ),
        latest_source_age_seconds_delta=(
            trend_report.latest_source_age_seconds_delta
        ),
        max_source_age_seconds_delta=trend_report.max_source_age_seconds_delta,
        duplicate_latest_generated_at_count_delta=(
            trend_report.duplicate_latest_generated_at_count_delta
        ),
...
def _gate_reason_codes(
    *,
    trend_report: PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport,
    trend_report_age_seconds: int,
    config: PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateConfig,
) -> tuple[str, ...]:
    ...
    if (
        trend_report.watch_ledger_report_count_delta is not None
        and trend_report.watch_ledger_report_count_delta
        > config.max_watch_ledger_report_count_delta
    ):
        reason_codes.append(
            "worsening_paper_autonomous_investment_ledger_db_history_health_trend_watch_count",
        )
    if (
        trend_report.blocked_ledger_report_count_delta is not None
        and trend_report.blocked_ledger_report_count_delta
        > config.max_blocked_ledger_report_count_delta
    ):
        reason_codes.append(
            "worsening_paper_autonomous_investment_ledger_db_history_health_trend_blocked_count",
        )
...
def _validate_gate_report(
    report: PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReport,
) -> None:
...
def _normalize_reason_code_counts(
    value: object,
) -> tuple[PaperAutonomousInvestmentLedgerDbHistoryHealthTrendGateReasonCodeCount, ...]:
