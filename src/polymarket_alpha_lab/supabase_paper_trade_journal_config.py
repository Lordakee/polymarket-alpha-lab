"""Environment boundary for paper trade journal DB persistence."""
...
def from_paper_trade_journal_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperTradeJournalConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(PAPER_TRADE_JOURNAL_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(PAPER_TRADE_JOURNAL_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_TRADE_JOURNAL_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_TRADE_JOURNAL_DB_TABLE,
    )
    try:
        return SupabasePaperTradeJournalConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                (
                    f"{PAPER_TRADE_JOURNAL_DB_TABLE_ENV_VAR} must be a simple "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_TRADE_JOURNAL_DB_ENABLED_ENV_VAR} must be true or false",
        )
    ...
    if normalized in _FALSE_VALUES:
        return False
    raise ValueError(f"{PAPER_TRADE_JOURNAL_DB_ENABLED_ENV_VAR} must be true or false")
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{PAPER_TRADE_JOURNAL_DB_DSN_ENV_VAR} must be a string")
