"""Optional psycopg adapter for candidate decision score report storage."""
...
from polymarket_alpha_lab.candidate_decision_score_store import (
...
def insert_candidate_decision_score_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_write_connection(
        dsn,
        lambda connection: insert_candidate_decision_score_report(
            connection,
            report,
...
def load_candidate_decision_score_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_read_connection(
        dsn,
        lambda connection: load_candidate_decision_score_reports(
            connection,
            action=action,
...
def _connect(dsn: str, *, autocommit: bool = False) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the candidate decision score psycopg "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the candidate decision score database",
        ) from None
