"""Minimal supplied-input equity indices team forecast workflow."""
...
PROMPT_VERSION = "equity-indices-supplied-input-v0"
TEAM_REASON_CODE = "team_equity_indices"
MEMORY_REFERENCES = ("equity_indices_supplied_evidence_only",)
KNOWN_FAILURE_MODES = (
    "index_gap_risk",
...
class EquityIndicesTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("equity indices team config", self)


@dataclass(frozen=True)
class EquityIndicesEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("equity indices evidence input", self)


def build_equity_indices_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[EquityIndicesEvidenceInput],
    ...
    if type(config) is not EquityIndicesTeamConfig:
        raise ValueError("config must be an EquityIndicesTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("equity indices team config", config)
...
def _normalize_evidence(
    evidence: Iterable[EquityIndicesEvidenceInput],
) -> tuple[EquityIndicesEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError(
            "evidence must be an iterable of EquityIndicesEvidenceInput values",
        )
    try:
    ...
    for item in items:
        if type(item) is not EquityIndicesEvidenceInput:
            raise ValueError(
                "evidence must contain only EquityIndicesEvidenceInput values",
            )
        require_paper_only_flags("equity indices evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[EquityIndicesEvidenceInput, ...],
...
def _average_weight(evidence: tuple[EquityIndicesEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
