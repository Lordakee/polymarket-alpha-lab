"""Environment boundary for paper autonomous readiness gate DB persistence."""
...
class SupabasePaperAutonomousReadinessGateConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_AUTONOMOUS_READINESS_GATE_DB_TABLE
...
def from_paper_autonomous_readiness_gate_db_env(
    env: Mapping[str, str | None] | None = None,
) -> SupabasePaperAutonomousReadinessGateConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_AUTONOMOUS_READINESS_GATE_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_AUTONOMOUS_READINESS_GATE_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_AUTONOMOUS_READINESS_GATE_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_AUTONOMOUS_READINESS_GATE_DB_TABLE,
    )
    try:
        return SupabasePaperAutonomousReadinessGateConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == _TABLE_NAME_ERROR:
            raise ValueError(
                f"{PAPER_AUTONOMOUS_READINESS_GATE_DB_TABLE_ENV_VAR} "
                f"{_TABLE_NAME_ERROR}",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_AUTONOMOUS_READINESS_GATE_DB_ENABLED_ENV_VAR} "
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_AUTONOMOUS_READINESS_GATE_DB_DSN_ENV_VAR} must be a string",
        )
