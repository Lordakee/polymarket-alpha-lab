"""Readonly paper report for tennis signal memory quality before forecasts."""
...
DEFAULT_RESEARCH_DOMAIN_TENNIS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION = (
    "research-domain-tennis-signal-memory-quality-report"
)
...
INPUT_FAMILIES = ("player", "injury", "surface", "form")
...
REPORT_REASON_CODES = (
    "no_tennis_memory_inputs_supplied",
...
class ResearchDomainTennisSignalMemoryQualityConfig:
    config_version: str = (
        DEFAULT_RESEARCH_DOMAIN_TENNIS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION
    )
    stale_player_after_seconds: Decimal = Decimal("604800.000000")
    stale_injury_after_seconds: Decimal = Decimal("172800.000000")
    stale_surface_after_seconds: Decimal = Decimal("1209600.000000")
    stale_form_after_seconds: Decimal = Decimal("604800.000000")
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in (
            "stale_player_after_seconds",
            "stale_injury_after_seconds",
            "stale_surface_after_seconds",
            "stale_form_after_seconds",
        ):
            object.__setattr__(
                self,
                field_name,
        ...
        _reject_unsafe_public_payload("tennis signal memory quality config", self)
        _require_hard_flags("ResearchDomainTennisSignalMemoryQualityConfig", self)


@dataclass(frozen=True)
class ResearchDomainTennisSignalMemoryInput:
    memory_ref: str
    team_ref: str
    event_ref: str
    input_family: str
    ...
    def __post_init__(self) -> None:
        for field_name in ("memory_ref", "team_ref", "event_ref"):
        ...
        _reject_unsafe_public_payload("tennis signal memory input", self)
        _require_hard_flags("ResearchDomainTennisSignalMemoryInput", self)


@dataclass(frozen=True)
class ResearchDomainTennisSignalMemoryQualityRow:
    memory_ref: str
    team_ref: str
    event_ref: str
    input_family: str
    ...
    def __post_init__(self) -> None:
        ...
        _validate_row(self)
        _reject_unsafe_public_payload("tennis signal memory quality row", self)
        _require_hard_flags("ResearchDomainTennisSignalMemoryQualityRow", self)


@dataclass(frozen=True)
class ResearchDomainTennisSignalMemoryQualityReport:
    ...
    rows: tuple[ResearchDomainTennisSignalMemoryQualityRow, ...]
    reason_codes: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        ...
        _reject_unsafe_public_payload("tennis signal memory quality report", self)
        _require_hard_flags("ResearchDomainTennisSignalMemoryQualityReport", self)
...
def build_research_domain_tennis_signal_memory_quality_report(
    rows: list[ResearchDomainTennisSignalMemoryInput]
    | tuple[ResearchDomainTennisSignalMemoryInput, ...],
    *,
    config: ResearchDomainTennisSignalMemoryQualityConfig,
    generated_at: datetime,
) -> ResearchDomainTennisSignalMemoryQualityReport:
    if type(config) is not ResearchDomainTennisSignalMemoryQualityConfig:
        raise ValueError(
            "config must be a ResearchDomainTennisSignalMemoryQualityConfig",
        )
    _require_hard_flags("ResearchDomainTennisSignalMemoryQualityConfig", config)
    ...
    return ResearchDomainTennisSignalMemoryQualityReport(
...
def research_domain_tennis_signal_memory_quality_report_payload(
    report: ResearchDomainTennisSignalMemoryQualityReport | dict[str, Any],
) -> dict[str, Any]:
    if type(report) is ResearchDomainTennisSignalMemoryQualityReport:
        _require_hard_flags("report", report)
        _reject_unsafe_public_payload("tennis signal memory quality report", report)
        payload = _json_ready(report)
    elif type(report) is dict:
        _reject_unsafe_public_payload("tennis signal memory quality payload", report)
        ...
        raise ValueError(
            "report must be a ResearchDomainTennisSignalMemoryQualityReport",
        )
    if type(payload) is not dict:
        raise ValueError("report payload must be a JSON object")
    _reject_unsafe_public_payload("tennis signal memory quality payload", payload)
    _require_hard_flags("payload", _DictFlags(payload))
    _validate_payload_digest(payload)
    return payload
...
def _normalize_source_rows(
    ...
    for row in rows:
        if type(row) is not ResearchDomainTennisSignalMemoryInput:
            raise ValueError(
                "rows must contain ResearchDomainTennisSignalMemoryInput values",
            )
        _require_hard_flags("ResearchDomainTennisSignalMemoryInput", row)
...
def _row_for_input(
    row: ResearchDomainTennisSignalMemoryInput,
    *,
    config: ResearchDomainTennisSignalMemoryQualityConfig,
    generated_at: datetime,
) -> ResearchDomainTennisSignalMemoryQualityRow:
    ...
    return ResearchDomainTennisSignalMemoryQualityRow(
...
def _freshness_limit(
    ...
    if input_family == "player":
        return config.stale_player_after_seconds
    if input_family == "injury":
        return config.stale_injury_after_seconds
    if input_family == "surface":
        return config.stale_surface_after_seconds
    if input_family == "form":
        return config.stale_form_after_seconds
...
def _report_reason_codes(
    ...
    if source_row_count == 0:
        return ("no_tennis_memory_inputs_supplied",)
    codes: list[str] = []
...
def _row_sort_key(row: ResearchDomainTennisSignalMemoryQualityRow) -> tuple[int, int, str, str]:
    return (
        _row_priority(row),
...
def _row_priority(row: ResearchDomainTennisSignalMemoryQualityRow) -> int:
...
def _validate_input(row: ResearchDomainTennisSignalMemoryInput) -> None:
...
def _validate_row(row: ResearchDomainTennisSignalMemoryQualityRow) -> None:
    if row.conflicting_evidence_count > row.evidence_count:
        raise ValueError("conflicting_evidence_count must not exceed evidence_count")
...
def _validate_report(report: ResearchDomainTennisSignalMemoryQualityReport) -> None:
...
def _normalize_report_rows(
    value: object,
) -> tuple[ResearchDomainTennisSignalMemoryQualityRow, ...]:
    if type(value) not in (list, tuple):
        raise ValueError("rows must be a list or tuple")
    rows = tuple(value)
    seen_refs: set[str] = set()
    for row in rows:
        if type(row) is not ResearchDomainTennisSignalMemoryQualityRow:
            raise ValueError(
                "rows must contain ResearchDomainTennisSignalMemoryQualityRow values",
            )
        _require_hard_flags("ResearchDomainTennisSignalMemoryQualityRow", row)
        if row.memory_ref in seen_refs:
            raise ValueError("duplicate memory_ref values are not allowed")
        seen_refs.add(row.memory_ref)
...
def _report_digest(report: ResearchDomainTennisSignalMemoryQualityReport) -> str:
    payload = _json_ready(report)
    if type(payload) is not dict:
        raise ValueError("report payload must be a JSON object")
...
__all__ = (
    "DEFAULT_RESEARCH_DOMAIN_TENNIS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION",
    "ResearchDomainTennisSignalMemoryInput",
    "ResearchDomainTennisSignalMemoryQualityConfig",
    "ResearchDomainTennisSignalMemoryQualityReport",
    "ResearchDomainTennisSignalMemoryQualityRow",
    "build_research_domain_tennis_signal_memory_quality_report",
    "research_domain_tennis_signal_memory_quality_report_payload",
)
