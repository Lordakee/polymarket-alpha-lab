...
def paper_research_packet_from_db_row(
    ...
    _reject_json_floats(row.packet_rows_json)
...
def _validate_row_payload_consistency(row: PaperResearchPacketDbRow) -> None:
    report = _report_from_payload_json(row.payload_json)
    _validate_row_matches_payload(row, _expected_row_values_from_report(report))


def _report_from_payload_json(payload_json: dict[str, Any]) -> PaperResearchPacketReport:
    _reject_json_floats(payload_json)
    _validate_payload_json_hard_flags(payload_json)
    _validate_json_hard_flags(payload_json, "payload_json")
    try:
        report = from_jsonable(PaperResearchPacketReport, payload_json)
...
def _expected_row_values_from_report(
    ...
    packet_rows_json = payload_json.get("packet_rows")
    if not isinstance(packet_rows_json, list):
    ...
    return {
        ...
        "medium_priority_count": report.medium_priority_count,
        "low_priority_count": report.low_priority_count,
        "packet_rows_json": list(packet_rows_json),
        "payload_json": payload_json,
        "paper_only": report.paper_only,
        "report_only": report.report_only,
        "readonly": report.readonly,
