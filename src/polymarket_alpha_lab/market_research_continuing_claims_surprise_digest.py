"""Pure Phase 1 continuing claims surprise research reducer."""
...
DEFAULT_MARKET_RESEARCH_CONTINUING_CLAIMS_SURPRISE_DIGEST_CONFIG_VERSION = (
    "market-research-continuing-claims-surprise-digest-v0"
)
...
REASON_PREFIX = "market_research_continuing_claims_surprise_digest_"
...
NEXT_STEPS = {
    STATUS_READY: "allow_report_only_market_research_continuing_claims_surprise_digest",
    STATUS_WATCH: "watch_report_only_market_research_continuing_claims_surprise_digest",
    STATUS_BLOCKED: "block_report_only_market_research_continuing_claims_surprise_digest",
}
...
class MarketResearchContinuingClaimsSurpriseDigestConfig:
    config_version: str = (
        DEFAULT_MARKET_RESEARCH_CONTINUING_CLAIMS_SURPRISE_DIGEST_CONFIG_VERSION
    )
    max_release_age_seconds: Decimal = Decimal("10800.000000")
    min_abs_surprise_claims: Decimal = Decimal("50000.000000")
    min_surprise_ratio: Decimal = Decimal("0.030000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not MarketResearchContinuingClaimsSurpriseDigestConfig:
            raise TypeError(
                "MarketResearchContinuingClaimsSurpriseDigestConfig does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchContinuingClaimsSurpriseDigestConfig:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketResearchContinuingClaimsSurpriseDigestSignal:
    condition_id: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchContinuingClaimsSurpriseDigestSignal:
            raise TypeError(
                "MarketResearchContinuingClaimsSurpriseDigestSignal does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchContinuingClaimsSurpriseDigestSignal:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketResearchContinuingClaimsSurpriseDigestRow:
    condition_id: str
    claims_market_key: str
    release_key: str
    digest_status: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchContinuingClaimsSurpriseDigestRow:
            raise TypeError(
                "MarketResearchContinuingClaimsSurpriseDigestRow does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchContinuingClaimsSurpriseDigestRow:
            raise ValueError(
                "row must be exactly MarketResearchContinuingClaimsSurpriseDigestRow",
            )
...
@dataclass(frozen=True)
class MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount:
    reason_code: str
    count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount:
            raise TypeError(
                "MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount does "
                "not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketResearchContinuingClaimsSurpriseDigestReport:
    ...
    rows: tuple[MarketResearchContinuingClaimsSurpriseDigestRow, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchContinuingClaimsSurpriseDigestReport:
            raise TypeError(
                "MarketResearchContinuingClaimsSurpriseDigestReport does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchContinuingClaimsSurpriseDigestReport:
            raise ValueError(
...
def build_market_research_continuing_claims_surprise_digest(
    signals: Iterable[MarketResearchContinuingClaimsSurpriseDigestSignal],
    *,
    config: MarketResearchContinuingClaimsSurpriseDigestConfig | None = None,
    generated_at: datetime,
) -> MarketResearchContinuingClaimsSurpriseDigestReport:
    cfg = config or MarketResearchContinuingClaimsSurpriseDigestConfig()
    if type(cfg) is not MarketResearchContinuingClaimsSurpriseDigestConfig:
        raise ValueError(
            "config must be a MarketResearchContinuingClaimsSurpriseDigestConfig",
        )
    ...
    return MarketResearchContinuingClaimsSurpriseDigestReport(
...
def market_research_continuing_claims_surprise_digest_payload(
    report: MarketResearchContinuingClaimsSurpriseDigestReport,
) -> dict[str, Any]:
    if type(report) is not MarketResearchContinuingClaimsSurpriseDigestReport:
        raise ValueError(
            "report must be a MarketResearchContinuingClaimsSurpriseDigestReport",
        )
    return _to_payload(asdict(report))


def _build_row(
    signal: MarketResearchContinuingClaimsSurpriseDigestSignal,
    *,
    config: MarketResearchContinuingClaimsSurpriseDigestConfig,
    generated_at: datetime,
) -> MarketResearchContinuingClaimsSurpriseDigestRow:
    ...
    return MarketResearchContinuingClaimsSurpriseDigestRow(
...
def _build_reason_code_counts(
    rows: tuple[MarketResearchContinuingClaimsSurpriseDigestRow, ...],
    signal_count: Decimal,
) -> tuple[MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount, ...]:
...
def _coerce_signals(
    signals: Iterable[MarketResearchContinuingClaimsSurpriseDigestSignal],
) -> tuple[MarketResearchContinuingClaimsSurpriseDigestSignal, ...]:
    ...
    for signal in coerced:
        if type(signal) is not MarketResearchContinuingClaimsSurpriseDigestSignal:
            raise ValueError(
                "signals must contain "
                "MarketResearchContinuingClaimsSurpriseDigestSignal items",
            )
    return coerced
...
def _normalize_rows(
    value: object,
) -> tuple[MarketResearchContinuingClaimsSurpriseDigestRow, ...]:
    if isinstance(value, (str, bytes)) or not isinstance(value, Iterable):
        raise ValueError("rows must be an iterable")
    rows = tuple(value)
    for row in rows:
        if type(row) is not MarketResearchContinuingClaimsSurpriseDigestRow:
            raise ValueError(
                "rows must contain MarketResearchContinuingClaimsSurpriseDigestRow "
                "items",
            )
    return rows
...
def _normalize_reason_code_counts(
    ...
    rows = tuple(value)
    for row in rows:
        if type(row) is not MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount:
            raise ValueError(
                "reason_code_counts must contain "
                "MarketResearchContinuingClaimsSurpriseDigestReasonCodeCount items",
            )
    return rows


def _row_sort_key(
    row: MarketResearchContinuingClaimsSurpriseDigestRow,
...
def _validate_report_consistency(
    ...
    expected_counts = {
        ...
        "stale_release_signal_count": _count_reason(
            report.rows,
            STALE_RELEASE_REASON,
        ),
        "low_absolute_surprise_signal_count": _count_reason(
