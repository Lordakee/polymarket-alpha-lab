"""Optional psycopg adapter for paper order lifecycle persistence."""
...
def insert_paper_order_lifecycle_record_with_psycopg(
    dsn: str,
    record: Any,
    *,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper order lifecycle "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper order lifecycle database",
        ) from None
