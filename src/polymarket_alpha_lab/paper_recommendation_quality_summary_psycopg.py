"""Optional psycopg adapter for paper recommendation quality summary storage."""
...
def insert_paper_recommendation_quality_summary_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_recommendation_quality_summary_report(
            connection,
            report,
...
def load_paper_recommendation_quality_summary_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_recommendation_quality_summary_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper recommendation quality summary "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper recommendation quality summary database",
        ) from None
...
from polymarket_alpha_lab.supabase_paper_recommendation_quality_summary_config import (
