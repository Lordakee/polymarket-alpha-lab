"""Optional psycopg adapter for paper recommendation readiness report storage."""
...
from polymarket_alpha_lab.paper_recommendation_readiness_store import (
    insert_paper_recommendation_readiness_report,
    load_paper_recommendation_readiness_reports,
)


DEFAULT_PAPER_RECOMMENDATION_READINESS_TABLE = (
    "paper_recommendation_readiness_reports"
)
_T = TypeVar("_T")


def insert_paper_recommendation_readiness_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    table_name: str = DEFAULT_PAPER_RECOMMENDATION_READINESS_TABLE,
) -> None:
    _with_owned_connection(
        dsn,
        lambda connection: insert_paper_recommendation_readiness_report(
            connection,
            report,
...
def load_paper_recommendation_readiness_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_recommendation_readiness_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper recommendation readiness "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper recommendation readiness database",
        ) from None
...
from polymarket_alpha_lab.supabase_paper_recommendation_readiness_config import (
