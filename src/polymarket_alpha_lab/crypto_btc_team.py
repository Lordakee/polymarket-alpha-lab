"""Minimal supplied-input BTC team forecast workflow."""
...
PROMPT_VERSION = "crypto-btc-supplied-input-v0"
TEAM_REASON_CODE = "team_crypto_btc"
MEMORY_REFERENCES = ("btc_supplied_evidence_only",)
KNOWN_FAILURE_MODES = ("btc_gap_risk", "crypto_weekend_liquidity")
...
class CryptoBtcTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("crypto btc team config", self)


@dataclass(frozen=True)
class CryptoBtcEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("crypto btc evidence input", self)


def build_crypto_btc_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[CryptoBtcEvidenceInput],
    ...
    require_paper_only_flags("crypto btc team config", config)
...
def _normalize_evidence(
    evidence: Iterable[CryptoBtcEvidenceInput],
) -> tuple[CryptoBtcEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError("evidence must be an iterable of CryptoBtcEvidenceInput values")
    try:
        items = tuple(evidence)
    except TypeError as exc:
        raise ValueError(
            "evidence must be an iterable of CryptoBtcEvidenceInput values",
        ) from exc
    if not items:
        raise ValueError("evidence must contain at least one item")
    for item in items:
        if type(item) is not CryptoBtcEvidenceInput:
            raise ValueError("evidence must contain only CryptoBtcEvidenceInput values")
        require_paper_only_flags("crypto btc evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[CryptoBtcEvidenceInput, ...],
...
def _average_weight(evidence: tuple[CryptoBtcEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[CryptoBtcEvidenceInput, ...]) -> Decimal:
