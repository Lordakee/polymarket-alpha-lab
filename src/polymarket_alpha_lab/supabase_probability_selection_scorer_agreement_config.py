"""Environment boundary for probability selection/scorer agreement persistence."""
...
class SupabaseProbabilitySelectionScorerAgreementConfig:
    enabled: bool
    dsn: str | None
...
def from_probability_selection_scorer_agreement_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabaseProbabilitySelectionScorerAgreementConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_DSN_ENV_VAR)
    table_name = source.get(
        PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_TABLE_ENV_VAR,
        DEFAULT_PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_TABLE,
    )
    try:
        return SupabaseProbabilitySelectionScorerAgreementConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                f"{PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_TABLE_ENV_VAR} "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_ENABLED_ENV_VAR} "
