"""Optional psycopg adapter for paper NAV snapshot storage."""
...
from polymarket_alpha_lab.paper_nav_snapshot_store import (
...
def insert_paper_nav_snapshot_with_psycopg(
    dsn: str,
    snapshot: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_nav_snapshot(
            connection,
            snapshot,
...
def load_paper_nav_snapshots_with_psycopg(
    dsn: str,
    *,
    limit: int | None = None,
    table_name: str = _DEFAULT_TABLE_NAME,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_nav_snapshots(
            connection,
            limit=limit,
            table_name=table_name,
        ),
...
def _with_owned_connection(dsn: str, operation: Callable[[Any], _T]) -> _T:
    validate_local_postgres_dsn(dsn, env_var_name=PAPER_NAV_SNAPSHOT_DB_DSN_ENV_VAR)
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper NAV snapshot psycopg adapter; "
            "install the postgres extra.",
        ) from exc
    try:
        return psycopg.connect(dsn)
    except Exception:
        raise RuntimeError("failed to connect to the paper NAV snapshot database") from None
...
__all__ = (
    ...
    "load_paper_nav_snapshots_with_psycopg",
)
