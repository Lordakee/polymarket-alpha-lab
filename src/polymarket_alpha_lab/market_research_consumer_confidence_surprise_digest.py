"""Pure Phase 1 consumer-confidence surprise research reducer."""
...
DEFAULT_MARKET_RESEARCH_CONSUMER_CONFIDENCE_SURPRISE_DIGEST_CONFIG_VERSION = (
    "market-research-consumer-confidence-surprise-digest-v0"
)
...
REASON_PREFIX = "market_research_consumer_confidence_surprise_digest_"
...
NEXT_STEPS = {
    STATUS_READY: "allow_report_only_market_research_consumer_confidence_surprise_digest",
    STATUS_WATCH: "watch_report_only_market_research_consumer_confidence_surprise_digest",
    STATUS_BLOCKED: "block_report_only_market_research_consumer_confidence_surprise_digest",
}
...
class MarketResearchConsumerConfidenceSurpriseDigestConfig:
    config_version: str = (
        DEFAULT_MARKET_RESEARCH_CONSUMER_CONFIDENCE_SURPRISE_DIGEST_CONFIG_VERSION
    )
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not MarketResearchConsumerConfidenceSurpriseDigestConfig:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestConfig does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchConsumerConfidenceSurpriseDigestConfig:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestConfig does not "
                "support subclassing",
            )
...
@dataclass(frozen=True)
class MarketResearchConsumerConfidenceSurpriseDigestSignal:
    condition_id: str
    research_key: str
    release_key: str
    survey_family: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchConsumerConfidenceSurpriseDigestSignal:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestSignal does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchConsumerConfidenceSurpriseDigestSignal:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestSignal does not "
                "support subclassing",
            )
        ...
        for field_name in ("expected_index", "actual_index"):
            object.__setattr__(
                self,
                field_name,
...
@dataclass(frozen=True)
class MarketResearchConsumerConfidenceSurpriseDigestRow:
    condition_id: str
    research_key: str
    release_key: str
    survey_family: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchConsumerConfidenceSurpriseDigestRow:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestRow does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchConsumerConfidenceSurpriseDigestRow:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestRow does not support "
                "subclassing",
            )
        ...
        for field_name in ("expected_index", "actual_index", "surprise_delta"):
            object.__setattr__(
                self,
                field_name,
                _require_finite_decimal(field_name, getattr(self, field_name)),
...
@dataclass(frozen=True)
class MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount:
    reason_code: str
    count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount does "
                "not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount does "
                "not support subclassing",
            )
...
@dataclass(frozen=True)
class MarketResearchConsumerConfidenceSurpriseDigestReport:
    ...
    rows: tuple[MarketResearchConsumerConfidenceSurpriseDigestRow, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchConsumerConfidenceSurpriseDigestReport:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestReport does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchConsumerConfidenceSurpriseDigestReport:
            raise TypeError(
                "MarketResearchConsumerConfidenceSurpriseDigestReport does not "
                "support subclassing",
            )
...
def build_market_research_consumer_confidence_surprise_digest(
    signals: Iterable[MarketResearchConsumerConfidenceSurpriseDigestSignal],
    *,
    config: MarketResearchConsumerConfidenceSurpriseDigestConfig,
    generated_at: datetime,
) -> MarketResearchConsumerConfidenceSurpriseDigestReport:
    if type(config) is not MarketResearchConsumerConfidenceSurpriseDigestConfig:
        raise ValueError(
            "config must be a MarketResearchConsumerConfidenceSurpriseDigestConfig",
        )
    ...
    return MarketResearchConsumerConfidenceSurpriseDigestReport(
        ...
        average_surprise_score=_average_decimal(
            row.surprise_score for row in rows
        ),
...
def market_research_consumer_confidence_surprise_digest_payload(
    report: MarketResearchConsumerConfidenceSurpriseDigestReport | dict[str, Any],
) -> dict[str, Any]:
    if isinstance(report, dict):
        return _payload_value(report)
    if type(report) is not MarketResearchConsumerConfidenceSurpriseDigestReport:
        raise ValueError(
            "report must be a MarketResearchConsumerConfidenceSurpriseDigestReport",
        )
    return _payload_value(asdict(report))


def _build_row(
    signal: MarketResearchConsumerConfidenceSurpriseDigestSignal,
    *,
    config: MarketResearchConsumerConfidenceSurpriseDigestConfig,
    generated_at: datetime,
) -> MarketResearchConsumerConfidenceSurpriseDigestRow:
    signal_age_seconds = _datetime_delta_seconds(generated_at, signal.observed_at)
    surprise_delta = _quantize_decimal(signal.actual_index - signal.expected_index)
    ...
    return MarketResearchConsumerConfidenceSurpriseDigestRow(
        condition_id=signal.condition_id,
        research_key=signal.research_key,
        release_key=signal.release_key,
        survey_family=signal.survey_family,
        digest_status=digest_status,
        observed_at=signal.observed_at,
        signal_age_seconds=signal_age_seconds,
        expected_index=signal.expected_index,
        actual_index=signal.actual_index,
...
def _reason_code_counts(
    reason_codes: tuple[str, ...],
    signal_count: Decimal,
) -> tuple[MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount, ...]:
...
def _row_reason_code_counts(
    ...
    signal_count = _decimal_count(len(rows))
    counts: list[MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount] = []
...
def _normalize_signals(
    signals: Iterable[MarketResearchConsumerConfidenceSurpriseDigestSignal],
) -> tuple[MarketResearchConsumerConfidenceSurpriseDigestSignal, ...]:
    if isinstance(signals, (str, bytes)) or not isinstance(signals, Iterable):
        raise ValueError("signals must be an iterable of consumer confidence signals")
    normalized = tuple(signals)
    for signal in normalized:
        if type(signal) is not MarketResearchConsumerConfidenceSurpriseDigestSignal:
            raise ValueError(
...
def _normalize_rows(
    rows: tuple[MarketResearchConsumerConfidenceSurpriseDigestRow, ...],
) -> tuple[MarketResearchConsumerConfidenceSurpriseDigestRow, ...]:
    if isinstance(rows, (str, bytes)) or not isinstance(rows, tuple):
        raise ValueError("rows must be a tuple")
    for row in rows:
        if type(row) is not MarketResearchConsumerConfidenceSurpriseDigestRow:
            raise ValueError(
...
def _normalize_reason_code_counts(
    ...
    for count in reason_code_counts:
        if type(count) is not MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount:
            raise ValueError(
                "reason_code_counts must contain exactly "
                "MarketResearchConsumerConfidenceSurpriseDigestReasonCodeCount",
            )
        _require_hard_flags("reason code count", count)
    if reason_code_counts != tuple(
...
def _normalize_reason_codes(
    ...
    *,
    ...
    if reason_codes != tuple(sorted(reason_codes, key=lambda code: sequence.index(code))):
        raise ValueError("reason_codes must be sorted deterministically")
    return reason_codes


def _validate_row_consistency(
    row: MarketResearchConsumerConfidenceSurpriseDigestRow,
) -> None:
    expected_delta = _quantize_decimal(row.actual_index - row.expected_index)
    if row.surprise_delta != expected_delta:
        raise ValueError("surprise_delta must equal actual_index minus expected_index")
    if READY_REASON in row.reason_codes and row.reason_codes != (READY_REASON,):
        raise ValueError("ready reason code cannot be combined")
...
def _validate_report_consistency(
    report: MarketResearchConsumerConfidenceSurpriseDigestReport,
) -> None:
    rows = report.rows
    signal_count = _decimal_count(len(rows))
...
def _row_sort_key(row: MarketResearchConsumerConfidenceSurpriseDigestRow) -> tuple[
    int,
    Decimal,
    Decimal,
    str,
