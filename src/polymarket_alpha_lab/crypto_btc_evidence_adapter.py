"""Pure BTC evidence adapter: M2 bundle rows to crypto_btc team inputs.

The adapter maps ready evidence to descriptive ``CryptoBtcEvidenceInput``
...
from .crypto_btc_team import CryptoBtcEvidenceInput


ADAPTER_VERSION = "crypto-btc-evidence-v1"
BTC_SPOT_ITEM = "btc_spot_price"
...
class BtcEvidenceAdaptation:
    inputs: tuple[CryptoBtcEvidenceInput, ...]
...
def adapt_btc_evidence(
    bundle: EvidenceBundle,
    *,
    observations_by_item: dict[str, NormalizedObservationRow],
    as_of: datetime,
) -> BtcEvidenceAdaptation:
    """Map one evidence bundle to canonical crypto_btc evidence inputs."""

    if type(bundle) is not EvidenceBundle:
        raise ValueError("bundle must be an EvidenceBundle")
    if bundle.team_id != "crypto_btc":
        raise ValueError("bundle must belong to crypto_btc")
    inputs: list[CryptoBtcEvidenceInput] = []
    reasons: list[str] = []
...
def _describe(item_name: str, value: object, selected: SelectedEvidence) -> str:
    if item_name == BTC_SPOT_ITEM and isinstance(value, dict):
        parts = [
            f"{key}={_plain(item)}"
        ...
        return (
            f"btc spot via {selected.source_family} at "
