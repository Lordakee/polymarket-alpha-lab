"""Minimal supplied-input gold commodities team forecast workflow."""
...
PROMPT_VERSION = "commodities-gold-supplied-input-v0"
TEAM_REASON_CODE = "team_commodities_gold"
MEMORY_REFERENCES = ("gold_supplied_evidence_only",)
KNOWN_FAILURE_MODES = ("gold_real_rates_usd_risk", "gold_session_liquidity_gap")
...
class CommoditiesGoldTeamConfig:
    config_version: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("commodities gold team config", self)


@dataclass(frozen=True)
class CommoditiesGoldEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("commodities gold evidence input", self)


def build_commodities_gold_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[CommoditiesGoldEvidenceInput],
    ...
    if type(config) is not CommoditiesGoldTeamConfig:
        raise ValueError("config must be a CommoditiesGoldTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("commodities gold team config", config)
...
def _normalize_evidence(
    evidence: Iterable[CommoditiesGoldEvidenceInput],
) -> tuple[CommoditiesGoldEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError(
            "evidence must be an iterable of CommoditiesGoldEvidenceInput values",
        )
    try:
    ...
    for item in items:
        if type(item) is not CommoditiesGoldEvidenceInput:
            raise ValueError(
                "evidence must contain only CommoditiesGoldEvidenceInput values",
            )
        require_paper_only_flags("commodities gold evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[CommoditiesGoldEvidenceInput, ...],
