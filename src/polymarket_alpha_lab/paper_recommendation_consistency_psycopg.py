"""Optional psycopg adapter for paper recommendation consistency storage."""
...
from polymarket_alpha_lab.paper_recommendation_consistency_store import (
...
def insert_paper_recommendation_consistency_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    table_name: str = DEFAULT_PAPER_RECOMMENDATION_CONSISTENCY_TABLE,
) -> None:
    _with_owned_connection(
        dsn,
        lambda connection: insert_paper_recommendation_consistency_report(
            connection,
            report,
...
def load_paper_recommendation_consistency_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_recommendation_consistency_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper recommendation consistency "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper recommendation consistency database",
        ) from exc
...
__all__ = (
    "insert_paper_recommendation_consistency_report_with_psycopg",
    "load_paper_recommendation_consistency_reports_with_psycopg",
)
