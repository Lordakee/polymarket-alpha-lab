"""Pure row codec for persisted paper recommendation health reports."""
...
class PaperRecommendationHealthDbRow:
    report_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        _require_decimal("average_net_probability_edge", self.average_net_probability_edge)
        ...
        _require_probability("min_recommend_share", self.min_recommend_share)
        object.__setattr__(
            self,
            "payload_json",
...
def paper_recommendation_health_to_db_row(
    report: PaperRecommendationHealthReport,
) -> PaperRecommendationHealthDbRow:
    if type(report) is not PaperRecommendationHealthReport:
        raise ValueError("report must be a PaperRecommendationHealthReport")
    ...
    return PaperRecommendationHealthDbRow(
        ...
        average_net_probability_edge=report.average_net_probability_edge,
        average_total_cost_per_share=report.average_total_cost_per_share,
        top_recommendation_score=report.top_recommendation_score,
        reason_code_counts_json=list(payload_json["reason_code_counts"]),
        max_average_cost_per_share=report.max_average_cost_per_share,
        min_recommend_share=report.min_recommend_share,
...
def paper_recommendation_health_from_db_row(
    ...
    if type(report) is not PaperRecommendationHealthReport:
        raise ValueError("payload_json must recover a PaperRecommendationHealthReport")
    _validate_report_tree(report)
    expected_row = paper_recommendation_health_to_db_row(report)
    _validate_row_matches_payload(row, expected_row)
    return report


def to_db_row(report: PaperRecommendationHealthReport) -> PaperRecommendationHealthDbRow:
    return paper_recommendation_health_to_db_row(report)


def from_db_row(row: PaperRecommendationHealthDbRow) -> PaperRecommendationHealthReport:
    return paper_recommendation_health_from_db_row(row)
...
def _validate_row_matches_payload(
    row: PaperRecommendationHealthDbRow,
    expected: PaperRecommendationHealthDbRow,
) -> None:
    for field_name in (
        "report_sha256",
        "generated_at",
...
def _json_ready(value: Any) -> Any:
    ...
    raise ValueError("health DB row values must be JSON serializable")
...
def _normalize_reason_code_counts_json(
    ...
    for item in value:
        ...
        reason_code = _required_mapping_value(item, "reason_code")
        count = _required_mapping_value(item, "count")
        _require_canonical_string("reason_code", reason_code)
        _require_positive_int("count", count)
        normalized.append({"reason_code": reason_code, "count": count})
    return normalized


def _required_mapping_value(value: dict[str, Any], field_name: str) -> Any:
...
paper_recommendation_health_report_to_db_row = paper_recommendation_health_to_db_row
paper_recommendation_health_report_from_db_row = paper_recommendation_health_from_db_row
