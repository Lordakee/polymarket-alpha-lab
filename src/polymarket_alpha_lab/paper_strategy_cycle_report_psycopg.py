"""Optional psycopg adapter for full paper strategy cycle report storage."""
...
from polymarket_alpha_lab.paper_strategy_cycle_report_store import (
...
def insert_paper_strategy_cycle_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_strategy_cycle_report(
            connection,
            report,
...
def load_paper_strategy_cycle_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_strategy_cycle_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper strategy cycle report "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper strategy cycle report database",
        ) from None
