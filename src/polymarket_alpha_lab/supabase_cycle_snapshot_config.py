"""Environment boundary for cycle snapshot DB persistence."""
...
CYCLE_SNAPSHOT_DB_DSN_ENV_VAR = "POLYMARKET_ALPHA_LAB_CYCLE_SNAPSHOT_DB_DSN"
CYCLE_SNAPSHOT_DB_TABLE_ENV_VAR = "POLYMARKET_ALPHA_LAB_CYCLE_SNAPSHOT_DB_TABLE"
DEFAULT_CYCLE_SNAPSHOT_DB_TABLE = "paper_recommendation_cycle_snapshots"
...
class SupabaseCycleSnapshotConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_CYCLE_SNAPSHOT_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(f"{CYCLE_SNAPSHOT_DB_DSN_ENV_VAR} must be set when DB is enabled")
...
def from_cycle_snapshot_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabaseCycleSnapshotConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(CYCLE_SNAPSHOT_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(CYCLE_SNAPSHOT_DB_DSN_ENV_VAR)
    table_name = source.get(
        CYCLE_SNAPSHOT_DB_TABLE_ENV_VAR,
        DEFAULT_CYCLE_SNAPSHOT_DB_TABLE,
    )
    try:
        return SupabaseCycleSnapshotConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                f"{CYCLE_SNAPSHOT_DB_TABLE_ENV_VAR} must be a simple lowercase identifier",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(f"{CYCLE_SNAPSHOT_DB_ENABLED_ENV_VAR} must be true or false")
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{CYCLE_SNAPSHOT_DB_DSN_ENV_VAR} must be a string")
