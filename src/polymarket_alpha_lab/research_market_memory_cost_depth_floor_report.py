"""Readonly memory cost depth floor research report."""
...
DEFAULT_RESEARCH_MARKET_MEMORY_COST_DEPTH_FLOOR_CONFIG_VERSION = (
    "research-market-memory-cost-depth-floor-v0"
)
...
EMPTY_REASON = "memory_cost_depth_floor_empty"
PASS_REASON = "memory_cost_depth_floor_pass"
WATCH_COST_REASON = "memory_cost_depth_floor_watch_cost"
BLOCK_COST_REASON = "memory_cost_depth_floor_block_cost"
INSUFFICIENT_MEMORY_REASON = "memory_cost_depth_floor_insufficient_memory"
THIN_DEPTH_REASON = "memory_cost_depth_floor_thin_depth"
...
class ResearchMarketMemoryCostDepthFloorConfig:
    watch_memory_cost_floor: Decimal = Decimal("0.030000")
    block_memory_cost_floor: Decimal = Decimal("0.075000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not ResearchMarketMemoryCostDepthFloorConfig:
            raise TypeError(
                "ResearchMarketMemoryCostDepthFloorConfig does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not ResearchMarketMemoryCostDepthFloorConfig:
            raise ValueError(
                "config must be exactly ResearchMarketMemoryCostDepthFloorConfig",
            )
        object.__setattr__(
            self,
            "watch_memory_cost_floor",
            _require_ratio("watch_memory_cost_floor", self.watch_memory_cost_floor),
        )
        object.__setattr__(
            self,
            "block_memory_cost_floor",
            _require_ratio("block_memory_cost_floor", self.block_memory_cost_floor),
        )
        object.__setattr__(
            self,
            "min_memory_points",
        ...
        if self.block_memory_cost_floor <= self.watch_memory_cost_floor:
            raise ValueError("block_memory_cost_floor must exceed watch_memory_cost_floor")
        object.__setattr__(
            self,
            "config_version",
            _require_public_identifier("config_version", self.config_version),
        )
        if self.config_version != DEFAULT_RESEARCH_MARKET_MEMORY_COST_DEPTH_FLOOR_CONFIG_VERSION:
            raise ValueError("config_version must be the supported config version")
        _require_hard_flags("config", self)


@dataclass(frozen=True)
class ResearchMarketMemoryCostDepthFloorObservation:
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not ResearchMarketMemoryCostDepthFloorObservation:
            raise TypeError(
                "ResearchMarketMemoryCostDepthFloorObservation does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not ResearchMarketMemoryCostDepthFloorObservation:
            raise ValueError(
        ...
        object.__setattr__(
            self,
            "memory_cost",
            _require_ratio("memory_cost", self.memory_cost),
        )
        object.__setattr__(
            self,
            "memory_point_count",
...
@dataclass(frozen=True)
class ResearchMarketMemoryCostDepthFloorRow:
    segment_key: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not ResearchMarketMemoryCostDepthFloorRow:
            raise TypeError(
                "ResearchMarketMemoryCostDepthFloorRow does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not ResearchMarketMemoryCostDepthFloorRow:
            raise ValueError(
                "row must be exactly ResearchMarketMemoryCostDepthFloorRow",
            )
        object.__setattr__(
            self,
            "segment_key",
...
@dataclass(frozen=True)
class ResearchMarketMemoryCostDepthFloorReport:
    generated_at: datetime
    config_version: str
    watch_memory_cost_floor: Decimal
    block_memory_cost_floor: Decimal
    ...
    depth_floor_breach_count: Decimal
    watch_ratio: Decimal
    max_memory_cost: Decimal
    min_depth_ratio_observed: Decimal
    status: str
    reason_codes: tuple[str, ...]
    rows: tuple[ResearchMarketMemoryCostDepthFloorRow, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not ResearchMarketMemoryCostDepthFloorReport:
            raise TypeError(
                "ResearchMarketMemoryCostDepthFloorReport does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not ResearchMarketMemoryCostDepthFloorReport:
            raise ValueError(
                "report must be exactly ResearchMarketMemoryCostDepthFloorReport",
            )
...
def build_research_market_memory_cost_depth_floor_report(
    observations: list[ResearchMarketMemoryCostDepthFloorObservation]
    | tuple[ResearchMarketMemoryCostDepthFloorObservation, ...],
    *,
    config: ResearchMarketMemoryCostDepthFloorConfig,
    generated_at: datetime,
) -> ResearchMarketMemoryCostDepthFloorReport:
    if type(config) is not ResearchMarketMemoryCostDepthFloorConfig:
        raise ValueError("config must be a ResearchMarketMemoryCostDepthFloorConfig")
    ...
    return ResearchMarketMemoryCostDepthFloorReport(
        generated_at=generated_at_utc,
        config_version=config.config_version,
        watch_memory_cost_floor=config.watch_memory_cost_floor,
        block_memory_cost_floor=config.block_memory_cost_floor,
        ...
        depth_floor_breach_count=_count(
            sum(1 for row in rows if THIN_DEPTH_REASON in row.reason_codes),
        ),
        watch_ratio=_ratio(watch_count + block_count, item_count),
        max_memory_cost=_max_decimal(row.memory_cost for row in rows),
...
def research_market_memory_cost_depth_floor_report_payload(
    report: ResearchMarketMemoryCostDepthFloorReport,
) -> dict[str, Any]:
    if type(report) is not ResearchMarketMemoryCostDepthFloorReport:
        raise ValueError("report must be a ResearchMarketMemoryCostDepthFloorReport")
    _require_hard_flags("report", report)
    _reject_unsafe_public_surface("report", report)
    payload = _json_ready(report)
    if type(payload) is not dict:
        raise ValueError("report payload must be a JSON object")
    validate_research_market_memory_cost_depth_floor_public_payload(payload)
    return payload


def validate_research_market_memory_cost_depth_floor_public_payload(
...
def _row_from_observation(
    observation: ResearchMarketMemoryCostDepthFloorObservation,
    *,
    config: ResearchMarketMemoryCostDepthFloorConfig,
) -> ResearchMarketMemoryCostDepthFloorRow:
    if type(observation) is not ResearchMarketMemoryCostDepthFloorObservation:
        raise ValueError(
            "observation must be a ResearchMarketMemoryCostDepthFloorObservation",
        )
    _require_hard_flags("observation", observation)
    reason_codes = _row_reason_codes(observation, config=config)
    return ResearchMarketMemoryCostDepthFloorRow(
        segment_key=observation.segment_key,
        reference_digest=_reference_digest(observation.private_reference),
        memory_cost=observation.memory_cost,
...
def _row_reason_codes(
    ...
    *,
    ...
    reasons: list[str] = []
    if observation.memory_cost >= config.block_memory_cost_floor:
        reasons.append(BLOCK_COST_REASON)
    elif observation.memory_cost >= config.watch_memory_cost_floor:
...
def _report_status(rows: tuple[ResearchMarketMemoryCostDepthFloorRow, ...]) -> str:
...
def _report_reason_codes(
    ...
    if not present:
        return (PASS_REASON,)
    return tuple(
        reason_code for reason_code in REPORT_TRIGGER_REASONS if reason_code in present
    )


def _row_sort_key(row: ResearchMarketMemoryCostDepthFloorRow) -> tuple[object, ...]:
...
def _normalize_observations(
    ...
    for observation in normalized:
        if type(observation) is not ResearchMarketMemoryCostDepthFloorObservation:
            raise ValueError(
                "observations must contain ResearchMarketMemoryCostDepthFloorObservation",
            )
        _require_hard_flags("observation", observation)
    return normalized
...
def _normalize_rows(
    rows: tuple[ResearchMarketMemoryCostDepthFloorRow, ...],
) -> tuple[ResearchMarketMemoryCostDepthFloorRow, ...]:
    if type(rows) is not tuple:
        raise ValueError("rows must be a tuple")
    normalized = tuple(rows)
    for row in normalized:
        if type(row) is not ResearchMarketMemoryCostDepthFloorRow:
            raise ValueError("rows must contain ResearchMarketMemoryCostDepthFloorRow")
...
def _validate_report_consistency(
    ...
    if report.item_count != item_count:
        raise ValueError("item_count is inconsistent with rows")
    ...
    if report.depth_floor_breach_count != _count(
        sum(1 for row in rows if THIN_DEPTH_REASON in row.reason_codes),
    ):
        raise ValueError("depth_floor_breach_count is inconsistent with rows")
    if report.watch_ratio != _ratio(watch_count + block_count, item_count):
        raise ValueError("watch_ratio is inconsistent with rows")
    if report.max_memory_cost != _max_decimal(row.memory_cost for row in rows):
        raise ValueError("max_memory_cost is inconsistent with rows")
