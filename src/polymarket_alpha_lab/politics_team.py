"""Minimal supplied-input politics team forecast workflow."""
...
PROMPT_VERSION = "politics-supplied-input-v0"
TEAM_REASON_CODE = "team_politics"
MEMORY_REFERENCES = ("politics_supplied_evidence_only",)
KNOWN_FAILURE_MODES = ("late_breaking_news", "polling_error", "turnout_model_error")
...
class PoliticsTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("politics team config", self)


@dataclass(frozen=True)
class PoliticsEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("politics evidence input", self)


def build_politics_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[PoliticsEvidenceInput],
    ...
    if type(config) is not PoliticsTeamConfig:
        raise ValueError("config must be a PoliticsTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("politics team config", config)
...
def _normalize_evidence(
    evidence: Iterable[PoliticsEvidenceInput],
) -> tuple[PoliticsEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError("evidence must be an iterable of PoliticsEvidenceInput values")
    try:
        items = tuple(evidence)
    except TypeError as exc:
        raise ValueError(
            "evidence must be an iterable of PoliticsEvidenceInput values",
        ) from exc
    if not items:
        raise ValueError("evidence must contain at least one item")
    for item in items:
        if type(item) is not PoliticsEvidenceInput:
            raise ValueError("evidence must contain only PoliticsEvidenceInput values")
        require_paper_only_flags("politics evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[PoliticsEvidenceInput, ...],
...
def _average_weight(evidence: tuple[PoliticsEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[PoliticsEvidenceInput, ...]) -> Decimal:
