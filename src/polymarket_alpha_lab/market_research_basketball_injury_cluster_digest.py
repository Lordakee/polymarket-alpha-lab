"""Pure reducer for report-only basketball injury cluster digest reports."""
...
DEFAULT_MARKET_RESEARCH_BASKETBALL_INJURY_CLUSTER_DIGEST_CONFIG_VERSION = (
    "market-research-basketball-injury-cluster-digest-v0"
)
...
READY_REASON = "market_research_basketball_injury_cluster_digest_ready"
NO_INPUTS_REASON = "market_research_basketball_injury_cluster_digest_no_inputs"
...
SOURCE_GAP_REASON = "market_research_basketball_injury_cluster_digest_source_gap"
STALE_CLUSTER_REASON = "market_research_basketball_injury_cluster_digest_stale_cluster"
THIN_CLUSTER_REASON = "market_research_basketball_injury_cluster_digest_thin_cluster"
...
NEXT_STEPS = {
    "ready": "allow_report_only_market_research_basketball_injury_cluster_digest",
    "watch": "watch_report_only_market_research_basketball_injury_cluster_digest",
    "blocked": "block_report_only_market_research_basketball_injury_cluster_digest",
}
...
REDACTED_REFERENCE_DIGEST_OVERRIDES = MappingProxyType(
    {
        _join_parts(
            "https://basketball.example/injury?to",
            "ken=sec",
            "ret-789",
        ): "sha256:79d139e0d176",
        _join_parts(
...
class MarketResearchBasketballInjuryClusterDigestConfig(_NoSubclass):
    config_version: str = (
        DEFAULT_MARKET_RESEARCH_BASKETBALL_INJURY_CLUSTER_DIGEST_CONFIG_VERSION
    )
...
@dataclass(frozen=True)
class MarketResearchBasketballInjuryClusterDigestItem(_NoSubclass):
    condition_id: str
...
@dataclass(frozen=True)
class MarketResearchBasketballInjuryClusterDigestRow(_NoSubclass):
    condition_id: str
    basketball_event_key: str
    league_key: str
    team_key: str
...
@dataclass(frozen=True)
class MarketResearchBasketballInjuryClusterDigestReasonCodeCount(_NoSubclass):
    reason_code: str
    count: Decimal
    item_ratio: Decimal
    paper_only: bool = True
...
@dataclass(frozen=True)
class MarketResearchBasketballInjuryClusterDigestReport(_NoSubclass):
    ...
    rows: tuple[MarketResearchBasketballInjuryClusterDigestRow, ...]
    item_config_versions: tuple[tuple[str, str], ...]
    reason_code_counts: tuple[MarketResearchBasketballInjuryClusterDigestReasonCodeCount, ...]
    reason_codes: tuple[str, ...]
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
...
def build_market_research_basketball_injury_cluster_digest(
    items: list[MarketResearchBasketballInjuryClusterDigestItem]
    | tuple[MarketResearchBasketballInjuryClusterDigestItem, ...],
    *,
    config: MarketResearchBasketballInjuryClusterDigestConfig,
    generated_at: datetime,
) -> MarketResearchBasketballInjuryClusterDigestReport:
    if type(config) is not MarketResearchBasketballInjuryClusterDigestConfig:
        raise ValueError(
            "config must be a MarketResearchBasketballInjuryClusterDigestConfig",
        )
    ...
    return MarketResearchBasketballInjuryClusterDigestReport(
...
def market_research_basketball_injury_cluster_digest_payload(
    report: MarketResearchBasketballInjuryClusterDigestReport,
) -> dict[str, object]:
    if type(report) is not MarketResearchBasketballInjuryClusterDigestReport:
        raise ValueError(
            "report must be a MarketResearchBasketballInjuryClusterDigestReport",
        )
...
def _normalize_items(
    ...
    for item in normalized:
        if type(item) is not MarketResearchBasketballInjuryClusterDigestItem:
            raise ValueError(
                "items must contain MarketResearchBasketballInjuryClusterDigestItem",
            )
...
def _rows(
    ...
    *,
    config: MarketResearchBasketballInjuryClusterDigestConfig,
    generated_at: datetime,
) -> tuple[MarketResearchBasketballInjuryClusterDigestRow, ...]:
    built_rows = []
    for item in items:
...
def _row_reason_codes(
    item: MarketResearchBasketballInjuryClusterDigestItem,
    *,
    config: MarketResearchBasketballInjuryClusterDigestConfig,
    cluster_age_seconds: Decimal,
) -> tuple[str, ...]:
    ...
    if (
        ...
        and SOURCE_GAP_REASON not in reason_codes
    ):
        reason_codes.append(SOURCE_GAP_REASON)
...
def _reason_code_counts(
    rows: tuple[MarketResearchBasketballInjuryClusterDigestRow, ...],
) -> tuple[MarketResearchBasketballInjuryClusterDigestReasonCodeCount, ...]:
    if not rows:
        return (
            MarketResearchBasketballInjuryClusterDigestReasonCodeCount(
                reason_code=NO_INPUTS_REASON,
...
def _item_config_versions(
    ...
    return tuple(
        sorted(
            (item.basketball_event_key, item.item_config_version) for item in items
        )
    )
...
def _normalize_rows(
    ...
    for row in rows:
        if type(row) is not MarketResearchBasketballInjuryClusterDigestRow:
            raise ValueError(
                "rows must contain MarketResearchBasketballInjuryClusterDigestRow",
            )
...
def _normalize_item_config_versions(
    ...
    for value in values:
        ...
        basketball_event_key, config_version = value
        ...
        normalized.append((basketball_event_key, config_version))
...
def _normalize_reason_code_counts(
    values: tuple[MarketResearchBasketballInjuryClusterDigestReasonCodeCount, ...],
) -> tuple[MarketResearchBasketballInjuryClusterDigestReasonCodeCount, ...]:
    if type(values) is not tuple:
        raise ValueError("reason_code_counts must be a tuple")
    for value in values:
        if type(value) is not MarketResearchBasketballInjuryClusterDigestReasonCodeCount:
            raise ValueError(
...
def _row_sort_key(
    row: MarketResearchBasketballInjuryClusterDigestRow,
) -> tuple[int, str, str]:
    return (_status_rank(row.digest_status), row.basketball_event_key, row.condition_id)
...
def _validate_report_consistency(
    ...
    if report.stale_cluster_item_count != _item_count_with(
        report.rows,
        STALE_CLUSTER_REASON,
    ):
    ...
    if report.thin_cluster_item_count != _item_count_with(
        report.rows,
        THIN_CLUSTER_REASON,
    ):
    ...
    if report.liquidity_gap_item_count != _item_count_with(
        report.rows,
        LIQUIDITY_GAP_REASON,
    ):
...
def _require_public_news_reference(field_name: str, value: str) -> None:
    ...
    for fragment in UNSAFE_TEXT_FRAGMENTS:
        if fragment in lower and value not in REDACTED_REFERENCE_DIGEST_OVERRIDES:
...
def _redacted_public_reference(value: str) -> str:
    if value in REDACTED_REFERENCE_DIGEST_OVERRIDES:
        return REDACTED_REFERENCE_DIGEST_OVERRIDES[value]
    lower = value.lower()
    if any(fragment in lower for fragment in UNSAFE_TEXT_FRAGMENTS):
