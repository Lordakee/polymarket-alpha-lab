"""Report-only probability fee/slippage gate."""
...
__all__ = (
    "DEFAULT_RESEARCH_MARKET_PROBABILITY_FEE_SLIPPAGE_GATE_CONFIG_VERSION",
    "ResearchMarketProbabilityFeeSlippageGateConfig",
    "ResearchMarketProbabilityFeeSlippageGateInput",
    "ResearchMarketProbabilityFeeSlippageGateReasonCodeCount",
    "ResearchMarketProbabilityFeeSlippageGateReport",
    "ResearchMarketProbabilityFeeSlippageGateRow",
    "build_research_market_probability_fee_slippage_gate_report",
    "research_market_probability_fee_slippage_gate_report_payload",
)


DEFAULT_RESEARCH_MARKET_PROBABILITY_FEE_SLIPPAGE_GATE_CONFIG_VERSION = (
    "research-market-probability-fee-slippage-gate-report-v1"
)
...
REASON_MISSING_INPUTS = "missing_probability_fee_slippage_gate_inputs"
REASON_PASS = "probability_fee_slippage_gate_pass"
REASON_WATCH = "probability_fee_slippage_gate_watch"
REASON_BLOCK = "probability_fee_slippage_gate_block"
...
REASON_TOTAL_DRAG_WATCH = "total_fee_slippage_drag_watch"
REASON_TOTAL_DRAG_BLOCK = "total_fee_slippage_drag_block"
REASON_DRAG_TO_EDGE_WATCH = "drag_to_edge_ratio_watch"
REASON_DRAG_TO_EDGE_BLOCK = "drag_to_edge_ratio_block"
REASON_SLIPPAGE_WATCH = "slippage_buffer_watch"
REASON_SLIPPAGE_BLOCK = "slippage_buffer_block"
REASON_CODES = (
...
class ResearchMarketProbabilityFeeSlippageGateConfig:
    config_version: str = (
        DEFAULT_RESEARCH_MARKET_PROBABILITY_FEE_SLIPPAGE_GATE_CONFIG_VERSION
    )
    pass_net_probability_edge: Decimal = Decimal("0.030000")
    watch_net_probability_edge: Decimal = Decimal("0.005000")
    watch_total_fee_slippage_drag_rate: Decimal = Decimal("0.030000")
    block_total_fee_slippage_drag_rate: Decimal = Decimal("0.080000")
    watch_drag_to_edge_ratio: Decimal = Decimal("0.500000")
    block_drag_to_edge_ratio: Decimal = Decimal("1.000000")
    watch_slippage_buffer_rate: Decimal = Decimal("0.020000")
    block_slippage_buffer_rate: Decimal = Decimal("0.060000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        raise TypeError(
            "ResearchMarketProbabilityFeeSlippageGateConfig does not support "
            "subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchMarketProbabilityFeeSlippageGateConfig, "config")
        ...
        for field_name in (
            "pass_net_probability_edge",
            "watch_net_probability_edge",
            "watch_total_fee_slippage_drag_rate",
            "block_total_fee_slippage_drag_rate",
            "watch_drag_to_edge_ratio",
            "block_drag_to_edge_ratio",
            "watch_slippage_buffer_rate",
            "block_slippage_buffer_rate",
        ):
            object.__setattr__(
                self,
                field_name,
        ...
        if self.pass_net_probability_edge < self.watch_net_probability_edge:
            raise ValueError(
                "pass_net_probability_edge must be at least watch_net_probability_edge",
            )
        _require_ascending(
            "total_fee_slippage_drag_rate",
            self.watch_total_fee_slippage_drag_rate,
            self.block_total_fee_slippage_drag_rate,
        )
        _require_ascending(
            "drag_to_edge_ratio",
        ...
        _require_ascending(
            ...
            self.watch_slippage_buffer_rate,
            self.block_slippage_buffer_rate,
        )
...
@dataclass(frozen=True)
class ResearchMarketProbabilityFeeSlippageGateInput:
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchMarketProbabilityFeeSlippageGateInput does not support "
            "subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchMarketProbabilityFeeSlippageGateInput, "input")
...
@dataclass(frozen=True)
class ResearchMarketProbabilityFeeSlippageGateRow:
    signal_digest: str
    observed_at: datetime
    model_probability: Decimal
    market_probability: Decimal
    ...
    slippage_buffer_rate: Decimal
    settlement_friction_rate: Decimal
    total_fee_slippage_drag_rate: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchMarketProbabilityFeeSlippageGateRow does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchMarketProbabilityFeeSlippageGateRow, "row")
...
@dataclass(frozen=True)
class ResearchMarketProbabilityFeeSlippageGateReasonCodeCount:
    reason_code: str
    count: Decimal
    paper_only: bool = True
    report_only: bool = True
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchMarketProbabilityFeeSlippageGateReasonCodeCount does not support "
            "subclassing",
        )
...
@dataclass(frozen=True)
class ResearchMarketProbabilityFeeSlippageGateReport:
    ...
    max_total_fee_slippage_drag_rate: Decimal
    max_drag_to_edge_ratio: Decimal
    reason_codes: tuple[str, ...]
    reason_code_counts: tuple[ResearchMarketProbabilityFeeSlippageGateReasonCodeCount, ...]
    rows: tuple[ResearchMarketProbabilityFeeSlippageGateRow, ...]
    derived_validation_digest: str = ""
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchMarketProbabilityFeeSlippageGateReport does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchMarketProbabilityFeeSlippageGateReport, "report")
        ...
        for field_name in (
            ...
            "max_total_fee_slippage_drag_rate",
...
def build_research_market_probability_fee_slippage_gate_report(
    inputs: Iterable[ResearchMarketProbabilityFeeSlippageGateInput],
    *,
    generated_at: datetime,
    config: ResearchMarketProbabilityFeeSlippageGateConfig | None = None,
) -> ResearchMarketProbabilityFeeSlippageGateReport:
    generated_at_utc = _as_utc("generated_at", generated_at)
    active_config = ResearchMarketProbabilityFeeSlippageGateConfig() if config is None else config
    if type(active_config) is not ResearchMarketProbabilityFeeSlippageGateConfig:
        raise ValueError("config must be ResearchMarketProbabilityFeeSlippageGateConfig")
    ...
    return ResearchMarketProbabilityFeeSlippageGateReport(
        ...
        average_net_probability_edge=_mean_decimal(row.net_probability_edge for row in rows),
        min_net_probability_edge=min(
            (row.net_probability_edge for row in rows),
            default=ZERO,
        ),
        max_total_fee_slippage_drag_rate=max(
            (row.total_fee_slippage_drag_rate for row in rows),
            default=ZERO,
        ),
...
def research_market_probability_fee_slippage_gate_report_payload(
    report: ResearchMarketProbabilityFeeSlippageGateReport | Mapping[str, Any],
) -> "FrozenJsonObject":
    if type(report) is ResearchMarketProbabilityFeeSlippageGateReport:
        ...
        raise ValueError(
            "report must be a ResearchMarketProbabilityFeeSlippageGateReport",
        )
...
def _row_from_input(
    item: ResearchMarketProbabilityFeeSlippageGateInput,
    *,
    config: ResearchMarketProbabilityFeeSlippageGateConfig,
) -> ResearchMarketProbabilityFeeSlippageGateRow:
    ...
    total_fee_slippage_drag_rate = _quantize(
        item.fee_rate
        + (item.spread_rate / TWO)
        + item.slippage_buffer_rate
        + item.settlement_friction_rate,
    )
    net_probability_edge = _quantize(
        gross_probability_edge - total_fee_slippage_drag_rate,
    )
    drag_to_edge_ratio = _safe_ratio(total_fee_slippage_drag_rate, gross_probability_edge)
    reason_codes = _row_reason_codes(
        gross_probability_edge=gross_probability_edge,
        net_probability_edge=net_probability_edge,
        total_fee_slippage_drag_rate=total_fee_slippage_drag_rate,
        drag_to_edge_ratio=drag_to_edge_ratio,
        slippage_buffer_rate=item.slippage_buffer_rate,
        config=config,
    )
    return ResearchMarketProbabilityFeeSlippageGateRow(
    ...
    *,
    ...
    detail_reasons.extend(
        _low_value_reason(
            ...
            watch_value=config.pass_net_probability_edge,
            block_value=config.watch_net_probability_edge,
    ...
    detail_reasons.extend(
        _high_value_reason(
            value=total_fee_slippage_drag_rate,
            watch_value=config.watch_total_fee_slippage_drag_rate,
            block_value=config.block_total_fee_slippage_drag_rate,
    ...
    detail_reasons.extend(
        _high_value_reason(
            value=slippage_buffer_rate,
            watch_value=config.watch_slippage_buffer_rate,
            block_value=config.block_slippage_buffer_rate,
...
def _report_status(rows: tuple[ResearchMarketProbabilityFeeSlippageGateRow, ...]) -> str:
    if not rows:
        return STATUS_BLOCK
...
def _reason_code_counts(
    rows: tuple[ResearchMarketProbabilityFeeSlippageGateRow, ...],
) -> tuple[ResearchMarketProbabilityFeeSlippageGateReasonCodeCount, ...]:
...
def _row_sort_key(
    row: ResearchMarketProbabilityFeeSlippageGateRow,
...
def _normalize_inputs(
    inputs: Iterable[ResearchMarketProbabilityFeeSlippageGateInput],
) -> tuple[ResearchMarketProbabilityFeeSlippageGateInput, ...]:
    ...
    for item in normalized:
        if type(item) is not ResearchMarketProbabilityFeeSlippageGateInput:
            raise ValueError(
                "inputs must contain ResearchMarketProbabilityFeeSlippageGateInput",
            )
...
def _normalize_rows(
    rows: object,
) -> tuple[ResearchMarketProbabilityFeeSlippageGateRow, ...]:
    if type(rows) not in (tuple, list):
        raise ValueError("rows must be a tuple or list")
    normalized = tuple(rows)
    seen: set[tuple[str, datetime]] = set()
    for row in normalized:
        if type(row) is not ResearchMarketProbabilityFeeSlippageGateRow:
            raise ValueError("rows must contain ResearchMarketProbabilityFeeSlippageGateRow")
...
def _normalize_reason_code_counts(
    ...
    seen: set[str] = set()
    for row in normalized:
        if type(row) is not ResearchMarketProbabilityFeeSlippageGateReasonCodeCount:
            raise ValueError(
...
def _validate_row_consistency(row: ResearchMarketProbabilityFeeSlippageGateRow) -> None:
    ...
    expected_total = _quantize(
        row.fee_rate
        + (row.spread_rate / TWO)
        + row.slippage_buffer_rate
        + row.settlement_friction_rate,
    )
    if row.total_fee_slippage_drag_rate != expected_total:
        raise ValueError("total_fee_slippage_drag_rate must match components")
    if row.net_probability_edge != _quantize(
        row.gross_probability_edge - row.total_fee_slippage_drag_rate,
    ):
        raise ValueError("net_probability_edge must match row fields")
    if row.drag_to_edge_ratio != _safe_ratio(
        row.total_fee_slippage_drag_rate,
...
def _validate_report_consistency(
    report: ResearchMarketProbabilityFeeSlippageGateReport,
) -> None:
    rows = report.rows
    expected_values = {
        "input_count": _decimal_from_count(len(rows)),
        ...
        "watch_count": _status_count(rows, STATUS_WATCH),
        "block_count": _status_count(rows, STATUS_BLOCK),
        "average_net_probability_edge": _mean_decimal(
            row.net_probability_edge for row in rows
        ),
        "min_net_probability_edge": min(
            (row.net_probability_edge for row in rows),
