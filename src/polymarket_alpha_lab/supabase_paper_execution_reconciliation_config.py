"""Environment boundary for paper execution reconciliation DB persistence."""
...
PAPER_EXECUTION_RECONCILIATION_DB_DSN_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_EXECUTION_RECONCILIATION_DB_DSN"
)
PAPER_EXECUTION_RECONCILIATION_DB_TABLE_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_EXECUTION_RECONCILIATION_DB_TABLE"
)
...
class SupabasePaperExecutionReconciliationConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_EXECUTION_RECONCILIATION_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(
                (
                    f"{PAPER_EXECUTION_RECONCILIATION_DB_DSN_ENV_VAR} must be set "
                    "when DB is enabled"
                ),
            )
...
def from_paper_execution_reconciliation_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperExecutionReconciliationConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_EXECUTION_RECONCILIATION_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_EXECUTION_RECONCILIATION_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_EXECUTION_RECONCILIATION_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_EXECUTION_RECONCILIATION_DB_TABLE,
    )
    try:
        return SupabasePaperExecutionReconciliationConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                (
                    f"{PAPER_EXECUTION_RECONCILIATION_DB_TABLE_ENV_VAR} must be a "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_EXECUTION_RECONCILIATION_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{PAPER_EXECUTION_RECONCILIATION_DB_DSN_ENV_VAR} must be a string")
