"""Minimal supplied-input oil commodities team forecast workflow."""
...
CATEGORY_ID = "finance.commodities.oil"
PROMPT_VERSION = "commodities-oil-supplied-input-v0"
TEAM_REASON_CODE = "team_commodities_oil"
MEMORY_REFERENCES = ("oil_supplied_evidence_only",)
KNOWN_FAILURE_MODES = (
...
class CommoditiesOilTeamConfig:
    config_version: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("commodities oil team config", self)


@dataclass(frozen=True)
class CommoditiesOilEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("commodities oil evidence input", self)


def build_commodities_oil_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[CommoditiesOilEvidenceInput],
    ...
    if type(config) is not CommoditiesOilTeamConfig:
        raise ValueError("config must be a CommoditiesOilTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("commodities oil team config", config)
...
def _normalize_evidence(
    evidence: Iterable[CommoditiesOilEvidenceInput],
) -> tuple[CommoditiesOilEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError(
            "evidence must be an iterable of CommoditiesOilEvidenceInput values",
        )
    try:
    ...
    for item in items:
        if type(item) is not CommoditiesOilEvidenceInput:
            raise ValueError(
                "evidence must contain only CommoditiesOilEvidenceInput values",
            )
        require_paper_only_flags("commodities oil evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[CommoditiesOilEvidenceInput, ...],
...
def _average_weight(evidence: tuple[CommoditiesOilEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[CommoditiesOilEvidenceInput, ...]) -> Decimal:
    ...
    if max_freshness_seconds <= 43200:
