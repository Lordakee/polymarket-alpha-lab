"""Optional psycopg adapter for paper recommendation health report storage."""
...
from polymarket_alpha_lab.paper_recommendation_health_store import (
    insert_paper_recommendation_health_report,
    load_paper_recommendation_health_reports,
)


DEFAULT_PAPER_RECOMMENDATION_HEALTH_TABLE = "paper_recommendation_health_reports"
_T = TypeVar("_T")


def insert_paper_recommendation_health_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    table_name: str = DEFAULT_PAPER_RECOMMENDATION_HEALTH_TABLE,
) -> None:
    _with_owned_connection(
        dsn,
        lambda connection: insert_paper_recommendation_health_report(
            connection,
            report,
...
def load_paper_recommendation_health_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_recommendation_health_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper recommendation health "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper recommendation health database",
        ) from exc
...
from polymarket_alpha_lab.supabase_paper_recommendation_health_config import (
