"""Minimal supplied-input ETH team forecast workflow."""
...
PROMPT_VERSION = "crypto-eth-supplied-input-v0"
TEAM_REASON_CODE = "team_crypto_eth"
MEMORY_REFERENCES = ("eth_supplied_evidence_only",)
KNOWN_FAILURE_MODES = ("eth_gas_fee_dislocation", "crypto_weekend_liquidity")
...
class CryptoEthTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("crypto eth team config", self)


@dataclass(frozen=True)
class CryptoEthEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("crypto eth evidence input", self)


def build_crypto_eth_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[CryptoEthEvidenceInput],
    ...
    if type(config) is not CryptoEthTeamConfig:
        raise ValueError("config must be a CryptoEthTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("crypto eth team config", config)
...
def _normalize_evidence(
    evidence: Iterable[CryptoEthEvidenceInput],
) -> tuple[CryptoEthEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError("evidence must be an iterable of CryptoEthEvidenceInput values")
    try:
        items = tuple(evidence)
    except TypeError as exc:
        raise ValueError(
            "evidence must be an iterable of CryptoEthEvidenceInput values",
        ) from exc
    if not items:
        raise ValueError("evidence must contain at least one item")
    for item in items:
        if type(item) is not CryptoEthEvidenceInput:
            raise ValueError("evidence must contain only CryptoEthEvidenceInput values")
        require_paper_only_flags("crypto eth evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[CryptoEthEvidenceInput, ...],
...
def _average_weight(evidence: tuple[CryptoEthEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[CryptoEthEvidenceInput, ...]) -> Decimal:
