"""Read-only local Postgres evidence persistence readiness report."""
...
DEFAULT_LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_READINESS_CONFIG_VERSION = (
    "local-supabase-evidence-persistence-readiness-report-v0"
)
...
REQUIRED_CHECK_COUNT = Decimal("8.000000")
DECIMAL_CONTEXT = Context(prec=64, rounding=ROUND_HALF_EVEN)
STATUS_RANK = {"ready": 0, "attention": 1, "blocker": 2}

SURFACE_MISSING_REASON_CODES = {
    surface: f"{surface}_persistence_plan_missing_blocker"
    for surface in LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES
}
...
REPORT_REASON_SEQUENCE = (
    SURFACE_MISSING_REASON_CODES["candidate_history"],
    SURFACE_MISSING_REASON_CODES["gate_history"],
    SURFACE_MISSING_REASON_CODES["source_history"],
    SURFACE_MISSING_REASON_CODES["memory_history"],
    SURFACE_MISSING_REASON_CODES["cost_history"],
    SURFACE_MISSING_REASON_CODES["operator_packet_history"],
    *PLAN_REASON_SEQUENCE,
)
...
class LocalSupabaseEvidencePersistencePlanSignal:
    evidence_surface: str
    local_supabase_postgres_planned: bool
    dsn_validator_name: str
    durable_jsonl_file_fallback_absent: bool
    ...
    def __post_init__(self) -> None:
        _require_surface(self.evidence_surface)
        _require_dsn_validator_name(self.dsn_validator_name)
        for field_name in (
            "local_supabase_postgres_planned",
            "durable_jsonl_file_fallback_absent",
...
@dataclass(frozen=True)
class LocalSupabaseEvidencePersistenceReadinessRow:
    evidence_surface: str
    status: str
    local_supabase_postgres_planned: bool
    dsn_validator_name: str
...
@dataclass(frozen=True)
class LocalSupabaseEvidencePersistenceReadinessReasonCodeCount:
    reason_code: str
    count: Decimal
    ratio: Decimal
    paper_only: bool = True
...
@dataclass(frozen=True)
class LocalSupabaseEvidencePersistenceReadinessReport:
    generated_at: datetime
    config_version: str
    evidence_surface_count: Decimal
    ...
    rows: tuple[LocalSupabaseEvidencePersistenceReadinessRow, ...]
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
...
def build_local_supabase_evidence_persistence_readiness_report(
    plans: Iterable[LocalSupabaseEvidencePersistencePlanSignal],
    *,
    generated_at: datetime,
) -> LocalSupabaseEvidencePersistenceReadinessReport:
    generated_at_utc = _as_utc("generated_at", generated_at)
    normalized_plans = _normalize_plans(plans, generated_at=generated_at_utc)
    rows = tuple(sorted((_row_from_plan(plan) for plan in normalized_plans), key=_row_sort_key))
    ...
    total_required = _count(len(LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES))

    return LocalSupabaseEvidencePersistenceReadinessReport(
        generated_at=generated_at_utc,
        config_version=DEFAULT_LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_READINESS_CONFIG_VERSION,
        evidence_surface_count=_count(len(rows)),
...
def local_supabase_evidence_persistence_readiness_report_payload(
    report: LocalSupabaseEvidencePersistenceReadinessReport,
) -> dict[str, object]:
    if type(report) is not LocalSupabaseEvidencePersistenceReadinessReport:
        raise ValueError(
            "report must be a LocalSupabaseEvidencePersistenceReadinessReport",
        )
...
def _row_from_plan(
    plan: LocalSupabaseEvidencePersistencePlanSignal,
) -> LocalSupabaseEvidencePersistenceReadinessRow:
    checks = (
        plan.local_supabase_postgres_planned,
        plan.dsn_validator_name == validate_local_postgres_dsn.__name__,
        plan.durable_jsonl_file_fallback_absent,
    ...
    return LocalSupabaseEvidencePersistenceReadinessRow(
        evidence_surface=plan.evidence_surface,
        status=_row_status(reason_codes),
        local_supabase_postgres_planned=plan.local_supabase_postgres_planned,
        dsn_validator_name=plan.dsn_validator_name,
        durable_jsonl_file_fallback_absent=plan.durable_jsonl_file_fallback_absent,
...
def _row_reason_codes(
    ...
    if not plan.local_supabase_postgres_planned:
        reason_codes.append("local_supabase_postgres_plan_missing_blocker")
    ...
    if not plan.durable_jsonl_file_fallback_absent:
        reason_codes.append("durable_jsonl_file_fallback_present_blocker")
    ...
    if not reason_codes:
        reason_codes.append("local_supabase_evidence_persistence_ready")
...
def _reason_code_counts(
    ...
    missing_required_surfaces: tuple[str, ...],
    reason_codes: tuple[str, ...],
) -> tuple[LocalSupabaseEvidencePersistenceReadinessReasonCodeCount, ...]:
    ...
    total = _count(len(LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES))
    return tuple(
        LocalSupabaseEvidencePersistenceReadinessReasonCodeCount(
...
def _missing_required_surfaces(
    ...
    found = {row.evidence_surface for row in rows}
    return tuple(
        surface
        for surface in LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES
...
def _row_sort_key(row: LocalSupabaseEvidencePersistenceReadinessRow) -> tuple[int, int]:
    return (
        -STATUS_RANK[row.status],
        LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES.index(row.evidence_surface),
    )
...
def _normalize_plans(
    ...
    *,
    generated_at: datetime,
) -> tuple[LocalSupabaseEvidencePersistencePlanSignal, ...]:
    ...
    for value in values:
        if type(value) is not LocalSupabaseEvidencePersistencePlanSignal:
            raise ValueError(
                "plans must contain LocalSupabaseEvidencePersistencePlanSignal values",
            )
        _require_hard_flags("plan signal", value)
        if value.evidence_surface in surfaces:
            raise ValueError("duplicate evidence_surface values are not allowed")
        surfaces.add(value.evidence_surface)
...
def _require_rows(
    rows: Iterable[LocalSupabaseEvidencePersistenceReadinessRow],
) -> tuple[LocalSupabaseEvidencePersistenceReadinessRow, ...]:
    ...
    for row in values:
        if type(row) is not LocalSupabaseEvidencePersistenceReadinessRow:
            raise ValueError(
                "rows must contain LocalSupabaseEvidencePersistenceReadinessRow values",
            )
        _require_hard_flags("row", row)
        if row.evidence_surface in surfaces:
            raise ValueError("rows must have unique evidence_surface values")
        surfaces.add(row.evidence_surface)
...
def _require_reason_code_counts(
    values: Iterable[LocalSupabaseEvidencePersistenceReadinessReasonCodeCount],
) -> tuple[LocalSupabaseEvidencePersistenceReadinessReasonCodeCount, ...]:
    ...
    seen: set[str] = set()
    for item in items:
        if type(item) is not LocalSupabaseEvidencePersistenceReadinessReasonCodeCount:
            raise ValueError(
                "reason_code_counts must contain "
                "LocalSupabaseEvidencePersistenceReadinessReasonCodeCount values",
            )
        _require_hard_flags("reason code count", item)
...
def _validate_report(report: LocalSupabaseEvidencePersistenceReadinessReport) -> None:
    rows = report.rows
    missing_required_surfaces = _missing_required_surfaces(rows)
    total_required = _count(len(LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES))
    if report.evidence_surface_count != _count(len(rows)):
        raise ValueError("evidence_surface_count must match rows")
...
def _require_missing_surfaces(values: Iterable[str]) -> tuple[str, ...]:
    ...
    for surface in surfaces:
        ...
        seen.add(surface)
        index = LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES.index(surface)
        if index <= previous_index:
            raise ValueError("missing_required_surfaces must be sorted deterministically")
        previous_index = index
    return surfaces
...
def _require_surface(value: object) -> None:
    if value not in LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_REQUIRED_SURFACES:
        raise ValueError("evidence_surface must be a supported history surface")
...
def _require_status(name: str, value: object) -> None:
    if value not in LOCAL_SUPABASE_EVIDENCE_PERSISTENCE_READINESS_STATUSES:
