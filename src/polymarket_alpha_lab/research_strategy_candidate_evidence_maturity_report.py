"""Pure evidence maturity reducer for screened research candidates."""
...
DEFAULT_RESEARCH_STRATEGY_CANDIDATE_EVIDENCE_MATURITY_REPORT_CONFIG_VERSION = (
    "research-strategy-candidate-evidence-maturity-report-v0"
)
...
NO_INPUTS_REASON = "candidate_evidence_maturity_no_inputs"
...
class ResearchStrategyCandidateEvidenceMaturityConfig(_FinalPublicDataclass):
    config_version: str = (
        DEFAULT_RESEARCH_STRATEGY_CANDIDATE_EVIDENCE_MATURITY_REPORT_CONFIG_VERSION
    )
    required_source_class_count: Decimal = Decimal("3.000000")
    stale_evidence_watch_seconds: Decimal = Decimal("172800.000000")
    stale_evidence_block_seconds: Decimal = Decimal("604800.000000")
    source_class_coverage_watch: Decimal = Decimal("0.800000")
    source_class_coverage_block: Decimal = Decimal("0.500000")
    claim_consistency_watch: Decimal = Decimal("0.800000")
    claim_consistency_block: Decimal = Decimal("0.600000")
    resolution_rule_clarity_watch: Decimal = Decimal("0.800000")
    resolution_rule_clarity_block: Decimal = Decimal("0.600000")
    manual_review_watch: Decimal = Decimal("0.400000")
    manual_review_block: Decimal = Decimal("0.750000")
    ...
    def __post_init__(self) -> None:
        ...
        _require_less_than(
            "source_class_coverage_block",
            self.source_class_coverage_block,
            self.source_class_coverage_watch,
        )
        _require_less_than(
            "claim_consistency_block",
            self.claim_consistency_block,
            self.claim_consistency_watch,
        )
        _require_less_than(
            "resolution_rule_clarity_block",
            self.resolution_rule_clarity_block,
            self.resolution_rule_clarity_watch,
        )
        _require_less_than(
            "manual_review_watch",
            self.manual_review_watch,
            self.manual_review_block,
        )
...
@dataclass(frozen=True)
class ResearchStrategyCandidateEvidenceMaturityInput(_FinalPublicDataclass):
    screening_key: str
    ...
    def __post_init__(self) -> None:
        ...
        _require_canonical_string("screening_key", self.screening_key)
        object.__setattr__(
            self,
            "source_class_count",
...
@dataclass(frozen=True)
class ResearchStrategyCandidateEvidenceMaturityRow(_FinalPublicDataclass):
    aggregate_row_number: Decimal
    aggregate_row_hash: str
    status: str
    source_class_count: Decimal
    source_class_coverage_score: Decimal
    evidence_observed_at: datetime
    evidence_age_seconds: Decimal
    evidence_age_pressure_score: Decimal
    claim_consistency_score: Decimal
    claim_consistency_gap_score: Decimal
    resolution_rule_clarity_score: Decimal
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "source_class_count",
            _normalize_nonnegative_decimal("source_class_count", self.source_class_count),
        )
        for field_name in (
            "source_class_coverage_score",
            "evidence_age_pressure_score",
            "claim_consistency_score",
            "claim_consistency_gap_score",
            "resolution_rule_clarity_score",
        ...
        if self.claim_consistency_gap_score != _quantize(
            ONE - self.claim_consistency_score,
        ):
            raise ValueError("claim_consistency_gap_score must match score")
        if self.resolution_rule_gap_score != _quantize(
            ONE - self.resolution_rule_clarity_score,
        ):
            raise ValueError("resolution_rule_gap_score must match score")
        _require_hard_flags("row", self)
        _reject_unsafe_public_payload("row", self)


@dataclass(frozen=True)
class ResearchStrategyCandidateEvidenceMaturityReasonCodeCount(_FinalPublicDataclass):
    reason_code: str
    count: Decimal
    paper_only: bool = True
    report_only: bool = True
...
@dataclass(frozen=True)
class ResearchStrategyCandidateEvidenceMaturityReport(_FinalPublicDataclass):
    ...
    watch_count: Decimal
    block_count: Decimal
    source_class_coverage_watch_count: Decimal
    stale_evidence_count: Decimal
    claim_consistency_watch_count: Decimal
    resolution_rule_clarity_watch_count: Decimal
    manual_review_urgent_count: Decimal
    mean_source_class_coverage_score: Decimal
    mean_evidence_age_seconds: Decimal
    max_evidence_age_seconds: Decimal
    mean_claim_consistency_score: Decimal
    mean_resolution_rule_clarity_score: Decimal
    mean_manual_review_urgency_score: Decimal
    ...
    rows: tuple[ResearchStrategyCandidateEvidenceMaturityRow, ...]
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in (
            ...
            "watch_count",
            "block_count",
            "source_class_coverage_watch_count",
            "stale_evidence_count",
            "claim_consistency_watch_count",
            "resolution_rule_clarity_watch_count",
        ...
        for field_name in (
            "mean_source_class_coverage_score",
            "mean_claim_consistency_score",
            "mean_resolution_rule_clarity_score",
            "mean_manual_review_urgency_score",
...
def build_research_strategy_candidate_evidence_maturity_report(
    candidates: Iterable[ResearchStrategyCandidateEvidenceMaturityInput],
    *,
    config: ResearchStrategyCandidateEvidenceMaturityConfig,
    generated_at: datetime,
) -> ResearchStrategyCandidateEvidenceMaturityReport:
    if type(config) is not ResearchStrategyCandidateEvidenceMaturityConfig:
        raise ValueError(
            "config must be a ResearchStrategyCandidateEvidenceMaturityConfig",
        )
    ...
    return ResearchStrategyCandidateEvidenceMaturityReport(
        **values,
        public_digest=_digest_from_mapping(values),
    )


def research_strategy_candidate_evidence_maturity_report_digest(
    report: ResearchStrategyCandidateEvidenceMaturityReport,
) -> str:
    if type(report) is not ResearchStrategyCandidateEvidenceMaturityReport:
        raise ValueError(
            "report must be a ResearchStrategyCandidateEvidenceMaturityReport",
        )
    _revalidate_report_for_payload(report)
    return _computed_report_digest(report)


def research_strategy_candidate_evidence_maturity_report_payload(
    report: ResearchStrategyCandidateEvidenceMaturityReport,
) -> dict[str, Any]:
    if type(report) is not ResearchStrategyCandidateEvidenceMaturityReport:
        raise ValueError(
...
def _row_values_for_candidate(
    candidate: ResearchStrategyCandidateEvidenceMaturityInput,
    *,
    config: ResearchStrategyCandidateEvidenceMaturityConfig,
    generated_at: datetime,
) -> _RowValues:
    ...
    source_class_coverage_score = _source_class_coverage_score(candidate, config)
    evidence_age_pressure_score = _evidence_age_pressure_score(
        evidence_age_seconds,
        config,
    )
    claim_consistency_gap_score = _quantize(ONE - candidate.claim_consistency_score)
    resolution_rule_gap_score = _quantize(ONE - candidate.resolution_rule_clarity_score)
    reason_codes = _row_reason_codes(
        candidate,
        config=config,
        source_class_coverage_score=source_class_coverage_score,
        evidence_age_seconds=evidence_age_seconds,
    )
    return _RowValues(
        aggregate_row_hash=_public_hash(candidate.screening_key),
        status=_row_status(reason_codes),
        source_class_count=candidate.source_class_count,
        source_class_coverage_score=source_class_coverage_score,
        evidence_observed_at=candidate.evidence_observed_at,
        evidence_age_seconds=evidence_age_seconds,
        evidence_age_pressure_score=evidence_age_pressure_score,
        claim_consistency_score=candidate.claim_consistency_score,
        claim_consistency_gap_score=claim_consistency_gap_score,
        resolution_rule_clarity_score=candidate.resolution_rule_clarity_score,
        resolution_rule_gap_score=resolution_rule_gap_score,
        manual_review_urgency_score=candidate.manual_review_pressure,
...
def _row_from_values(
    ...
    return ResearchStrategyCandidateEvidenceMaturityRow(
        aggregate_row_number=aggregate_row_number,
        aggregate_row_hash=values.aggregate_row_hash,
        status=values.status,
        source_class_count=values.source_class_count,
        source_class_coverage_score=values.source_class_coverage_score,
        evidence_observed_at=values.evidence_observed_at,
        evidence_age_seconds=values.evidence_age_seconds,
        evidence_age_pressure_score=values.evidence_age_pressure_score,
        claim_consistency_score=values.claim_consistency_score,
        claim_consistency_gap_score=values.claim_consistency_gap_score,
        resolution_rule_clarity_score=values.resolution_rule_clarity_score,
        resolution_rule_gap_score=values.resolution_rule_gap_score,
        manual_review_urgency_score=values.manual_review_urgency_score,
...
def _row_reason_codes(
    ...
    *,
    ...
    if source_class_coverage_score <= config.source_class_coverage_block:
        reason_codes.append("source_class_coverage_block")
    elif source_class_coverage_score < config.source_class_coverage_watch:
        reason_codes.append("source_class_coverage_watch")
    if evidence_age_seconds > config.stale_evidence_block_seconds:
        reason_codes.append("evidence_stale_block")
    elif evidence_age_seconds > config.stale_evidence_watch_seconds:
        reason_codes.append("evidence_stale_watch")
    if candidate.claim_consistency_score <= config.claim_consistency_block:
        reason_codes.append("claim_consistency_block")
    elif candidate.claim_consistency_score < config.claim_consistency_watch:
        reason_codes.append("claim_consistency_watch")
    if candidate.resolution_rule_clarity_score <= config.resolution_rule_clarity_block:
        reason_codes.append("resolution_rule_clarity_block")
    elif candidate.resolution_rule_clarity_score < config.resolution_rule_clarity_watch:
        reason_codes.append("resolution_rule_clarity_watch")
    if candidate.manual_review_pressure >= config.manual_review_block:
        reason_codes.append("manual_review_urgency_block")
    elif candidate.manual_review_pressure >= config.manual_review_watch:
        reason_codes.append("manual_review_urgency_watch")
    if not _has_attention_reason(reason_codes):
        reason_codes.append("evidence_maturity_clear")
...
def _report_reason_codes(
    ...
    reason_codes = [f"candidate_evidence_maturity_{status}"]
...
def _reason_code_counts(
    rows: tuple[ResearchStrategyCandidateEvidenceMaturityRow, ...],
) -> tuple[ResearchStrategyCandidateEvidenceMaturityReasonCodeCount, ...]:
    if not rows:
        return (
            ResearchStrategyCandidateEvidenceMaturityReasonCodeCount(
...
def _report_values(
    *,
    ...
    return {
        ...
        "source_class_coverage_watch_count": _kind_count(
            rows,
            "source_class_coverage_",
        ),
        "stale_evidence_count": _kind_count(rows, "evidence_stale_"),
        "claim_consistency_watch_count": _kind_count(rows, "claim_consistency_"),
        "resolution_rule_clarity_watch_count": _kind_count(
            rows,
            "resolution_rule_clarity_",
        ),
        "manual_review_urgent_count": _kind_count(rows, "manual_review_urgency_"),
        "mean_source_class_coverage_score": _mean(
            tuple(row.source_class_coverage_score for row in rows),
        ),
        "mean_evidence_age_seconds": _mean(
            tuple(row.evidence_age_seconds for row in rows),
        ),
        "max_evidence_age_seconds": _max_decimal(
            tuple(row.evidence_age_seconds for row in rows),
        ),
        "mean_claim_consistency_score": _mean(
            tuple(row.claim_consistency_score for row in rows),
        ),
        "mean_resolution_rule_clarity_score": _mean(
            tuple(row.resolution_rule_clarity_score for row in rows),
        ),
        "mean_manual_review_urgency_score": _mean(
...
def _normalize_inputs(
    candidates: Iterable[ResearchStrategyCandidateEvidenceMaturityInput],
) -> tuple[ResearchStrategyCandidateEvidenceMaturityInput, ...]:
    ...
    for row in rows:
        if type(row) is not ResearchStrategyCandidateEvidenceMaturityInput:
            raise ValueError(
                "candidates must contain ResearchStrategyCandidateEvidenceMaturityInput values",
            )
        _require_hard_flags("input", row)
        _reject_unsafe_public_payload("input", row)
        if row.screening_key in seen_keys:
            raise ValueError("candidates must not contain duplicate screening_key values")
        seen_keys.add(row.screening_key)
    return rows


def _normalize_rows(
    rows: Iterable[ResearchStrategyCandidateEvidenceMaturityRow],
) -> tuple[ResearchStrategyCandidateEvidenceMaturityRow, ...]:
    ...
    for row in values:
        if type(row) is not ResearchStrategyCandidateEvidenceMaturityRow:
            raise ValueError(
                "rows must contain ResearchStrategyCandidateEvidenceMaturityRow values",
            )
...
def _normalize_reason_code_counts(
    rows: Iterable[ResearchStrategyCandidateEvidenceMaturityReasonCodeCount],
) -> tuple[ResearchStrategyCandidateEvidenceMaturityReasonCodeCount, ...]:
    if isinstance(rows, (str, bytes)):
    ...
    for row in values:
        if type(row) is not ResearchStrategyCandidateEvidenceMaturityReasonCodeCount:
            raise ValueError(
                "reason_code_counts must contain ResearchStrategyCandidateEvidenceMaturityReasonCodeCount values",
            )
...
def _row_sort_key(
    row: ResearchStrategyCandidateEvidenceMaturityRow,
...
def _row_values_sort_key(values: _RowValues) -> tuple[Decimal, Decimal, Decimal, str]:
    return (
        ...
        -values.manual_review_urgency_score,
        -values.evidence_age_seconds,
...
def _validate_report(report: ResearchStrategyCandidateEvidenceMaturityReport) -> None:
    rows = report.rows
    ...
    if report.source_class_coverage_watch_count != _kind_count(
        rows,
        "source_class_coverage_",
    ):
        raise ValueError("source_class_coverage_watch_count must match rows")
    if report.stale_evidence_count != _kind_count(rows, "evidence_stale_"):
        raise ValueError("stale_evidence_count must match rows")
    if report.claim_consistency_watch_count != _kind_count(rows, "claim_consistency_"):
        raise ValueError("claim_consistency_watch_count must match rows")
    if report.resolution_rule_clarity_watch_count != _kind_count(
        rows,
        "resolution_rule_clarity_",
    ):
        raise ValueError("resolution_rule_clarity_watch_count must match rows")
    if report.manual_review_urgent_count != _kind_count(
        rows,
        "manual_review_urgency_",
    ):
        raise ValueError("manual_review_urgent_count must match rows")
    if report.mean_source_class_coverage_score != _mean(
        tuple(row.source_class_coverage_score for row in rows),
    ):
        raise ValueError("mean_source_class_coverage_score must match rows")
    ...
    if report.mean_claim_consistency_score != _mean(
        tuple(row.claim_consistency_score for row in rows),
    ):
        raise ValueError("mean_claim_consistency_score must match rows")
    if report.mean_resolution_rule_clarity_score != _mean(
        tuple(row.resolution_rule_clarity_score for row in rows),
    ):
        raise ValueError("mean_resolution_rule_clarity_score must match rows")
    if report.mean_manual_review_urgency_score != _mean(
        tuple(row.manual_review_urgency_score for row in rows),
    ):
        raise ValueError("mean_manual_review_urgency_score must match rows")
...
def _revalidate_report_for_payload(
    ...
    _require_exact_type(report, ResearchStrategyCandidateEvidenceMaturityReport, "report")
...
def _revalidate_row_for_payload(row: object) -> None:
    if type(row) is not ResearchStrategyCandidateEvidenceMaturityRow:
        raise ValueError("rows must contain ResearchStrategyCandidateEvidenceMaturityRow values")
    ...
    _require_nonnegative_six_decimal_decimal("source_class_count", row.source_class_count)
    for field_name in (
        "source_class_coverage_score",
        "evidence_age_pressure_score",
        "claim_consistency_score",
    ...
    _require_utc_datetime("evidence_observed_at", row.evidence_observed_at)
...
def _source_class_coverage_score(
    candidate: ResearchStrategyCandidateEvidenceMaturityInput,
    config: ResearchStrategyCandidateEvidenceMaturityConfig,
) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        coverage = candidate.source_class_count / config.required_source_class_count
    if coverage > ONE:
        coverage = ONE
    return _quantize(coverage)


def _evidence_age_seconds(evidence_observed_at: datetime, generated_at: datetime) -> Decimal:
    age_seconds = _quantize(
        Decimal(str((generated_at - _as_utc("evidence_observed_at", evidence_observed_at)).total_seconds())),
    )
    if age_seconds < ZERO:
...
def _evidence_age_pressure_score(
    evidence_age_seconds: Decimal,
    config: ResearchStrategyCandidateEvidenceMaturityConfig,
) -> Decimal:
    if evidence_age_seconds <= config.stale_evidence_watch_seconds:
        return ZERO
    if evidence_age_seconds >= config.stale_evidence_block_seconds:
        return ONE
    with localcontext(DECIMAL_CONTEXT):
        return _quantize(
            (evidence_age_seconds - config.stale_evidence_watch_seconds)
            / (
                config.stale_evidence_block_seconds
                - config.stale_evidence_watch_seconds
            ),
        )
...
def _computed_report_digest(report: ResearchStrategyCandidateEvidenceMaturityReport) -> str:
    values = {
...
def _require_less_than(label: str, value: Decimal, limit: Decimal) -> None:
    if value >= limit:
        raise ValueError(f"{label} must be below its watch level")
