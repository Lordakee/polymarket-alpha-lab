"""Optional psycopg adapter for paper execution reconciliation persistence."""
...
from polymarket_alpha_lab.supabase_paper_execution_reconciliation_config import (
...
def insert_paper_execution_reconciliation_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_execution_reconciliation_report_with_result(
            connection,
            report,
...
def load_paper_execution_reconciliation_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_execution_reconciliation_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper execution reconciliation "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper execution reconciliation database",
        ) from None
