"""Environment boundary for paper autonomous screening gate DB persistence."""
...
class SupabasePaperAutonomousScreeningDecisionSupportGateConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_DB_TABLE
...
def from_paper_autonomous_screening_decision_support_gate_db_env(
    ...
    dsn = source.get(PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_DB_TABLE,
    )
    try:
        return SupabasePaperAutonomousScreeningDecisionSupportGateConfig(
            enabled=enabled,
            dsn=dsn,
