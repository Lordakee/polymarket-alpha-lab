"""Decimal-only paper gate for resolution-risk cost floors."""
...
DEFAULT_STRATEGY_RECOMMENDATION_RESOLUTION_RISK_COST_FLOOR_V2_CONFIG_VERSION = (
    "strategy-recommendation-resolution-risk-cost-floor-v2"
)
...
EMPTY_REASON = "resolution_risk_cost_floor_empty"
...
REASON_RANK = {
    "resolution_risk_cost_floor_blocked": 0,
    "resolution_risk_cost_floor_watch": 1,
    "resolution_risk_cost_floor_pass": 2,
    "resolution_ambiguity_blocked": 3,
    "official_source_age_blocked": 4,
    "settlement_lag_blocked": 5,
    "taker_cost_blocked": 6,
    "spread_cost_blocked": 7,
    "slippage_cost_blocked": 8,
    "total_cost_blocked": 9,
    "liquidity_capacity_blocked": 10,
    "recommendation_floor_blocked": 11,
    "resolution_ambiguity_watch": 12,
    "official_source_age_watch": 13,
    "settlement_lag_watch": 14,
    "taker_cost_watch": 15,
    "spread_cost_watch": 16,
    "slippage_cost_watch": 17,
    "total_cost_watch": 18,
    "liquidity_capacity_watch": 19,
    "recommendation_floor_watch": 20,
    EMPTY_REASON: 21,
}
REPORT_KEYS = (
    "generated_at",
...
class StrategyRecommendationResolutionRiskCostFloorV2Config:
    config_version: str = (
        DEFAULT_STRATEGY_RECOMMENDATION_RESOLUTION_RISK_COST_FLOOR_V2_CONFIG_VERSION
    )
    resolution_ambiguity_watch: Decimal = Decimal("0.200000")
    resolution_ambiguity_block: Decimal = Decimal("0.350000")
    official_source_age_watch_seconds: Decimal = Decimal("3600.000000")
    official_source_age_block_seconds: Decimal = Decimal("7200.000000")
    settlement_lag_watch_seconds: Decimal = Decimal("86400.000000")
    settlement_lag_block_seconds: Decimal = Decimal("172800.000000")
    ...
    liquidity_capacity_ratio_watch: Decimal = Decimal("2.000000")
    liquidity_capacity_ratio_block: Decimal = Decimal("1.000000")
    ...
    def __post_init__(self) -> None:
        if type(self) is not StrategyRecommendationResolutionRiskCostFloorV2Config:
            raise ValueError(
                "config must be a StrategyRecommendationResolutionRiskCostFloorV2Config",
            )
        ...
        for field_name in (
            "official_source_age_watch_seconds",
            "official_source_age_block_seconds",
            "settlement_lag_watch_seconds",
            "settlement_lag_block_seconds",
            "liquidity_capacity_ratio_watch",
            "liquidity_capacity_ratio_block",
        ):
            object.__setattr__(
                self,
                field_name,
        ...
        _require_at_most(
            "official_source_age_watch_seconds",
            self.official_source_age_watch_seconds,
            self.official_source_age_block_seconds,
        )
        _require_at_most(
            "settlement_lag_watch_seconds",
            self.settlement_lag_watch_seconds,
            self.settlement_lag_block_seconds,
        )
        _require_at_most("taker_fee_rate_watch", self.taker_fee_rate_watch, self.taker_fee_rate_block)
        ...
        if self.liquidity_capacity_ratio_block > self.liquidity_capacity_ratio_watch:
            raise ValueError("liquidity_capacity_ratio_block must not exceed watch threshold")
        _require_at_most(
            "blocked_recommendation_floor",
...
@dataclass(frozen=True)
class StrategyRecommendationResolutionRiskCostFloorV2Input:
    ...
    expected_settlement_seconds: Decimal
    ...
    def __post_init__(self) -> None:
        if type(self) is not StrategyRecommendationResolutionRiskCostFloorV2Input:
            raise ValueError(
                "input must be a StrategyRecommendationResolutionRiskCostFloorV2Input",
            )
...
@dataclass(frozen=True)
class StrategyRecommendationResolutionRiskCostFloorV2Row:
    recommendation_id: str
    market_id: str
    recommendation_side: str
    base_confidence: Decimal
    ...
    expected_settlement_seconds: Decimal
    settlement_timeliness_score: Decimal
    taker_fee_rate: Decimal
    spread_cost: Decimal
    slippage_cost: Decimal
    total_cost: Decimal
    ...
    def __post_init__(self) -> None:
        if type(self) is not StrategyRecommendationResolutionRiskCostFloorV2Row:
            raise ValueError("row must be a StrategyRecommendationResolutionRiskCostFloorV2Row")
        for field_name in ("recommendation_id", "market_id", "recommendation_side"):
            _require_text(field_name, getattr(self, field_name))
        if self.recommendation_side not in SIDES:
            raise ValueError("recommendation_side must be yes or no")
...
@dataclass(frozen=True)
class StrategyRecommendationResolutionRiskCostFloorV2Report:
    ...
    status: str
    reason_codes: tuple[str, ...]
    rows: tuple[StrategyRecommendationResolutionRiskCostFloorV2Row, ...]
    derived_validation_digest: str = ""
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
    ...
    def __post_init__(self) -> None:
        if type(self) is not StrategyRecommendationResolutionRiskCostFloorV2Report:
            raise ValueError(
                "report must be a StrategyRecommendationResolutionRiskCostFloorV2Report",
            )
...
def build_strategy_recommendation_resolution_risk_cost_floor_v2_report(
    recommendations: object,
    *,
    config: StrategyRecommendationResolutionRiskCostFloorV2Config,
    generated_at: datetime,
) -> StrategyRecommendationResolutionRiskCostFloorV2Report:
    if type(config) is not StrategyRecommendationResolutionRiskCostFloorV2Config:
        raise ValueError(
            "config must be a StrategyRecommendationResolutionRiskCostFloorV2Config",
        )
    ...
    for item in items:
        if item.official_source_observed_at > generated_at:
            raise ValueError("official_source_observed_at must not be after generated_at")
    ...
    return StrategyRecommendationResolutionRiskCostFloorV2Report(
        generated_at=generated_at,
...
def strategy_recommendation_resolution_risk_cost_floor_v2_payload(
    report: StrategyRecommendationResolutionRiskCostFloorV2Report | dict[str, Any],
) -> dict[str, Any]:
    if type(report) is StrategyRecommendationResolutionRiskCostFloorV2Report:
    ...
    if type(report) is dict:
        ...
        return payload
    raise ValueError("report must be a StrategyRecommendationResolutionRiskCostFloorV2Report")


def _row_for_recommendation(
    item: StrategyRecommendationResolutionRiskCostFloorV2Input,
    *,
    config: StrategyRecommendationResolutionRiskCostFloorV2Config,
    generated_at: datetime,
) -> StrategyRecommendationResolutionRiskCostFloorV2Row:
    official_source_age_seconds = _age_seconds(
        "official_source_observed_at",
        generated_at,
        item.official_source_observed_at,
    )
    total_cost = _quantize(item.taker_fee_rate + item.spread_cost + item.slippage_cost)
    liquidity_capacity_ratio = _quantize(item.liquidity_capacity / item.paper_notional)
    resolution_clarity_score = _clamp_ratio(ONE - item.resolution_ambiguity_score)
    official_source_freshness_score = _inverse_threshold_score(
        official_source_age_seconds,
        config.official_source_age_block_seconds,
    )
    settlement_timeliness_score = _inverse_threshold_score(
        item.expected_settlement_seconds,
        config.settlement_lag_block_seconds,
    )
    cost_efficiency_score = _inverse_threshold_score(total_cost, config.total_cost_block)
    liquidity_capacity_score = _clamp_ratio(
        liquidity_capacity_ratio / config.liquidity_capacity_ratio_watch,
    )
    ...
    return StrategyRecommendationResolutionRiskCostFloorV2Row(
        ...
        official_source_observed_at=item.official_source_observed_at,
        official_source_age_seconds=official_source_age_seconds,
        official_source_freshness_score=official_source_freshness_score,
        expected_settlement_seconds=item.expected_settlement_seconds,
        settlement_timeliness_score=settlement_timeliness_score,
...
def _row_reason_codes(
    ...
    *,
    ...
    if item.resolution_ambiguity_score > config.resolution_ambiguity_block:
        block_reasons.append("resolution_ambiguity_blocked")
    elif item.resolution_ambiguity_score > config.resolution_ambiguity_watch:
        watch_reasons.append("resolution_ambiguity_watch")
    if official_source_age_seconds > config.official_source_age_block_seconds:
        block_reasons.append("official_source_age_blocked")
    elif official_source_age_seconds > config.official_source_age_watch_seconds:
        watch_reasons.append("official_source_age_watch")
    if item.expected_settlement_seconds > config.settlement_lag_block_seconds:
        block_reasons.append("settlement_lag_blocked")
    elif item.expected_settlement_seconds > config.settlement_lag_watch_seconds:
        watch_reasons.append("settlement_lag_watch")
    ...
    if liquidity_capacity_ratio < config.liquidity_capacity_ratio_block:
        block_reasons.append("liquidity_capacity_blocked")
    elif liquidity_capacity_ratio < config.liquidity_capacity_ratio_watch:
        watch_reasons.append("liquidity_capacity_watch")
    ...
    if block_reasons:
        reasons = ["resolution_risk_cost_floor_blocked", *block_reasons]
    elif watch_reasons:
        reasons = ["resolution_risk_cost_floor_watch", *watch_reasons]
    else:
        reasons = ["resolution_risk_cost_floor_pass"]
...
def _status_from_reasons(reason_codes: tuple[str, ...]) -> str:
    if "resolution_risk_cost_floor_blocked" in reason_codes:
        return "blocked"
    if "resolution_risk_cost_floor_watch" in reason_codes:
        return "watch"
    return "pass"
...
def _normalize_inputs(
    recommendations: object,
) -> tuple[StrategyRecommendationResolutionRiskCostFloorV2Input, ...]:
    ...
    seen: set[str] = set()
    for item in items:
        if type(item) is not StrategyRecommendationResolutionRiskCostFloorV2Input:
            raise ValueError(
                "recommendations must contain "
                "StrategyRecommendationResolutionRiskCostFloorV2Input",
            )
...
def _normalize_rows(
    rows: object,
) -> tuple[StrategyRecommendationResolutionRiskCostFloorV2Row, ...]:
    ...
    for item in items:
        if type(item) is not StrategyRecommendationResolutionRiskCostFloorV2Row:
            raise ValueError("rows must contain StrategyRecommendationResolutionRiskCostFloorV2Row")
...
def _row_sort_key(row: StrategyRecommendationResolutionRiskCostFloorV2Row) -> tuple[int, Decimal, str]:
...
def _report_status(rows: tuple[StrategyRecommendationResolutionRiskCostFloorV2Row, ...]) -> str:
...
def _check_row(row: StrategyRecommendationResolutionRiskCostFloorV2Row) -> None:
    ...
    if row.resolution_clarity_score != _clamp_ratio(ONE - row.resolution_ambiguity_score):
        raise ValueError("resolution_clarity_score must match resolution_ambiguity_score")
    if row.liquidity_capacity_ratio != _quantize(row.liquidity_capacity / row.paper_notional):
        raise ValueError("liquidity_capacity_ratio must match capacity and notional")
    ...
    expected_terminal = {
        "blocked": "resolution_risk_cost_floor_blocked",
        "watch": "resolution_risk_cost_floor_watch",
        "pass": "resolution_risk_cost_floor_pass",
    }[row.floor_status]
    if expected_terminal not in row.reason_codes:
        raise ValueError("reason_codes must include floor status reason")


def _check_report(report: StrategyRecommendationResolutionRiskCostFloorV2Report) -> None:
    rows = report.rows
...
def _row_digest(row: StrategyRecommendationResolutionRiskCostFloorV2Row) -> str:
    value = asdict(row)
    value.pop("derived_validation_digest", None)
    return _canonical_digest(value)


def _report_digest(report: StrategyRecommendationResolutionRiskCostFloorV2Report) -> str:
    value = asdict(report)
    value.pop("derived_validation_digest", None)
    return _canonical_digest(value)


def _require_row_digest(row: StrategyRecommendationResolutionRiskCostFloorV2Row) -> None:
    if row.derived_validation_digest != _row_digest(row):
        raise ValueError("derived_validation_digest mismatch")


def _require_report_digest(report: StrategyRecommendationResolutionRiskCostFloorV2Report) -> None:
...
__all__ = (
    "DEFAULT_STRATEGY_RECOMMENDATION_RESOLUTION_RISK_COST_FLOOR_V2_CONFIG_VERSION",
    "StrategyRecommendationResolutionRiskCostFloorV2Config",
    "StrategyRecommendationResolutionRiskCostFloorV2Input",
    "StrategyRecommendationResolutionRiskCostFloorV2Row",
    "StrategyRecommendationResolutionRiskCostFloorV2Report",
    "build_strategy_recommendation_resolution_risk_cost_floor_v2_report",
    "strategy_recommendation_resolution_risk_cost_floor_v2_payload",
)
