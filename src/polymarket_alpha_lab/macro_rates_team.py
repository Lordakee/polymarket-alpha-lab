"""Minimal supplied-input macro rates team forecast workflow."""
...
PROMPT_VERSION = "macro-rates-supplied-input-v0"
TEAM_REASON_CODE = "team_macro_rates"
MEMORY_REFERENCES = ("macro_rates_supplied_evidence_only",)
KNOWN_FAILURE_MODES = (
...
class MacroRatesTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("macro rates team config", self)


@dataclass(frozen=True)
class MacroRatesEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("macro rates evidence input", self)


def build_macro_rates_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[MacroRatesEvidenceInput],
    ...
    if type(config) is not MacroRatesTeamConfig:
        raise ValueError("config must be a MacroRatesTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("macro rates team config", config)
...
def _normalize_evidence(
    evidence: Iterable[MacroRatesEvidenceInput],
) -> tuple[MacroRatesEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError("evidence must be an iterable of MacroRatesEvidenceInput values")
    try:
        items = tuple(evidence)
    except TypeError as exc:
        raise ValueError(
            "evidence must be an iterable of MacroRatesEvidenceInput values",
        ) from exc
    if not items:
        raise ValueError("evidence must contain at least one item")
    for item in items:
        if type(item) is not MacroRatesEvidenceInput:
            raise ValueError("evidence must contain only MacroRatesEvidenceInput values")
        require_paper_only_flags("macro rates evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[MacroRatesEvidenceInput, ...],
...
def _average_weight(evidence: tuple[MacroRatesEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[MacroRatesEvidenceInput, ...]) -> Decimal:
