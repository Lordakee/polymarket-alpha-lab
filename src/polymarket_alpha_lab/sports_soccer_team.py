"""Minimal supplied-input soccer team forecast workflow."""
...
PROMPT_VERSION = "sports-soccer-supplied-input-v0"
TEAM_REASON_CODE = "team_sports_soccer"
MEMORY_REFERENCES = ("soccer_supplied_evidence_only",)
KNOWN_FAILURE_MODES = (
...
class SportsSoccerTeamConfig:
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("sports soccer team config", self)


@dataclass(frozen=True)
class SportsSoccerEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("sports soccer evidence input", self)


def build_sports_soccer_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[SportsSoccerEvidenceInput],
    ...
    if type(config) is not SportsSoccerTeamConfig:
        raise ValueError("config must be a SportsSoccerTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("sports soccer team config", config)
...
def _normalize_evidence(
    evidence: Iterable[SportsSoccerEvidenceInput],
) -> tuple[SportsSoccerEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError(
            "evidence must be an iterable of SportsSoccerEvidenceInput values",
        )
    try:
    ...
    for item in items:
        if type(item) is not SportsSoccerEvidenceInput:
            raise ValueError(
                "evidence must contain only SportsSoccerEvidenceInput values",
            )
        require_paper_only_flags("sports soccer evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[SportsSoccerEvidenceInput, ...],
