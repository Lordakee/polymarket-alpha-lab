"""Environment boundary for paper strategy cycle history gate DB persistence."""
...
class SupabasePaperStrategyCycleReportHistoryGateConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_DSN_ENV_VAR} "
...
def from_paper_strategy_cycle_report_history_gate_db_env(
    env: Mapping[str, str | None] | None = None,
) -> SupabasePaperStrategyCycleReportHistoryGateConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE,
    )
    try:
        return SupabasePaperStrategyCycleReportHistoryGateConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == _TABLE_NAME_ERROR:
            raise ValueError(
                f"{PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE_ENV_VAR} "
                f"{_TABLE_NAME_ERROR}",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_ENABLED_ENV_VAR} "
