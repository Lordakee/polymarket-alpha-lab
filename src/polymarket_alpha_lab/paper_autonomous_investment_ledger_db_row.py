...
def _validate_payload_dataclass_hard_flags(payload_json: dict[str, Any]) -> None:
    for payload_field_name in _PAYLOAD_DATACLASS_ARRAY_FIELDS:
        value = _payload_value(payload_json, payload_field_name)
        if value is _MISSING_PAYLOAD_VALUE or not isinstance(value, list):
            continue
        _require_json_array_hard_flags(f"payload_json {payload_field_name}", value)
...
def _require_json_array_hard_flags(field_name: str, value: list[Any]) -> None:
    for index, item in enumerate(value):
        if isinstance(item, dict):
            _require_json_hard_flags(f"{field_name} {index}", item)
...
_PAYLOAD_DATACLASS_ARRAY_FIELDS = ("reason_code_counts", "entries")
