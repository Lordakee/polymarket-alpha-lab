"""Environment boundary for paper probability selection summary DB persistence."""
...
class SupabasePaperProbabilitySelectionSummaryConfig:
    enabled: bool
    dsn: str | None
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR} "
...
def from_paper_probability_selection_summary_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperProbabilitySelectionSummaryConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE,
    )
    try:
        return SupabasePaperProbabilitySelectionSummaryConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                f"{PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE_ENV_VAR} "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR} "
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR} must be a string",
        )
