"""Readonly paper report for bonds/rates signal memory quality before forecasts."""
...
DEFAULT_RESEARCH_DOMAIN_BONDS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION = (
    "research-domain-bonds-signal-memory-quality-report"
)
...
INPUT_FAMILIES = ("auction", "curve", "duration", "issuance", "policy")
PUBLIC_STATUSES = ("pass", "watch", "block")
ROW_STATUSES = ("block", "watch", "pass")
ROW_REASON_CODES = (
    "missing_memory",
    "stale_memory",
    "conflicting_memory",
...
REPORT_REASON_CODES = (
    "no_bonds_memory_inputs_supplied",
...
class ResearchDomainBondsSignalMemoryQualityConfig:
    config_version: str = (
        DEFAULT_RESEARCH_DOMAIN_BONDS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION
    )
    stale_auction_after_seconds: Decimal = Decimal("604800.000000")
    stale_curve_after_seconds: Decimal = Decimal("172800.000000")
    stale_duration_after_seconds: Decimal = Decimal("1209600.000000")
    stale_issuance_after_seconds: Decimal = Decimal("604800.000000")
    stale_policy_after_seconds: Decimal = Decimal("604800.000000")
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in (
            "stale_auction_after_seconds",
            "stale_curve_after_seconds",
            "stale_duration_after_seconds",
            "stale_issuance_after_seconds",
            "stale_policy_after_seconds",
        ):
            object.__setattr__(
                self,
                field_name,
        ...
        _reject_unsafe_public_payload("bonds signal memory quality config", self)
        _require_hard_flags("ResearchDomainBondsSignalMemoryQualityConfig", self)


@dataclass(frozen=True)
class ResearchDomainBondsSignalMemoryInput:
    memory_ref: str
    team_ref: str
    handoff_ref: str
    input_family: str
    ...
    def __post_init__(self) -> None:
        for field_name in ("memory_ref", "team_ref", "handoff_ref"):
            _require_public_string(field_name, getattr(self, field_name))
        _require_member("input_family", self.input_family, INPUT_FAMILIES)
        ...
        _reject_unsafe_public_payload("bonds signal memory input", self)
        _require_hard_flags("ResearchDomainBondsSignalMemoryInput", self)


@dataclass(frozen=True)
class ResearchDomainBondsSignalMemoryQualityRow:
    memory_ref: str
    team_ref: str
    handoff_ref: str
    input_family: str
    ...
    def __post_init__(self) -> None:
        ...
        _validate_row(self)
        _reject_unsafe_public_payload("bonds signal memory quality row", self)
        _require_hard_flags("ResearchDomainBondsSignalMemoryQualityRow", self)


@dataclass(frozen=True)
class ResearchDomainBondsSignalMemoryQualityReport:
    ...
    rows: tuple[ResearchDomainBondsSignalMemoryQualityRow, ...]
    reason_codes: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        ...
        _reject_unsafe_public_payload("bonds signal memory quality report", self)
        _require_hard_flags("ResearchDomainBondsSignalMemoryQualityReport", self)
...
def build_research_domain_bonds_signal_memory_quality_report(
    rows: list[ResearchDomainBondsSignalMemoryInput]
    | tuple[ResearchDomainBondsSignalMemoryInput, ...],
    *,
    config: ResearchDomainBondsSignalMemoryQualityConfig,
    generated_at: datetime,
) -> ResearchDomainBondsSignalMemoryQualityReport:
    if type(config) is not ResearchDomainBondsSignalMemoryQualityConfig:
        raise ValueError(
            "config must be a ResearchDomainBondsSignalMemoryQualityConfig",
        )
    _require_hard_flags("ResearchDomainBondsSignalMemoryQualityConfig", config)
    ...
    return ResearchDomainBondsSignalMemoryQualityReport(
        ...
        stale_count=_reason_count(report_rows, "stale_memory"),
        conflict_count=_reason_count(report_rows, "conflicting_memory"),
        missing_count=_reason_count(report_rows, "missing_memory"),
...
def research_domain_bonds_signal_memory_quality_report_payload(
    report: ResearchDomainBondsSignalMemoryQualityReport | dict[str, Any],
) -> dict[str, Any]:
    if type(report) is ResearchDomainBondsSignalMemoryQualityReport:
        _require_hard_flags("report", report)
        _reject_unsafe_public_payload("bonds signal memory quality report", report)
        payload = _json_ready(report)
    elif type(report) is dict:
        _reject_unsafe_public_payload("bonds signal memory quality payload", report)
        ...
        raise ValueError(
            "report must be a ResearchDomainBondsSignalMemoryQualityReport",
        )
    if type(payload) is not dict:
        raise ValueError("report payload must be a JSON object")
    _reject_unsafe_public_payload("bonds signal memory quality payload", payload)
    _require_hard_flags("payload", _DictFlags(payload))
    _validate_payload_digest(payload)
    return payload
...
def _normalize_source_rows(
    ...
    for row in rows:
        if type(row) is not ResearchDomainBondsSignalMemoryInput:
            raise ValueError(
                "rows must contain ResearchDomainBondsSignalMemoryInput values",
            )
        _require_hard_flags("ResearchDomainBondsSignalMemoryInput", row)
        if row.memory_ref in seen_refs:
            raise ValueError("duplicate memory_ref values are not allowed")
        seen_refs.add(row.memory_ref)
    return rows


def _row_for_input(
    row: ResearchDomainBondsSignalMemoryInput,
    *,
    config: ResearchDomainBondsSignalMemoryQualityConfig,
    generated_at: datetime,
) -> ResearchDomainBondsSignalMemoryQualityRow:
    ...
    quality_score = (
        ...
        else _clamped_score(row.confidence_score - stale_penalty)
    )
    ...
    return ResearchDomainBondsSignalMemoryQualityRow(
...
def _freshness_limit(
    ...
    if input_family == "auction":
        return config.stale_auction_after_seconds
    if input_family == "curve":
        return config.stale_curve_after_seconds
    if input_family == "duration":
        return config.stale_duration_after_seconds
    if input_family == "issuance":
        return config.stale_issuance_after_seconds
    if input_family == "policy":
        return config.stale_policy_after_seconds
...
def _row_reason_codes(
    *,
    ...
    if not memory_present:
        codes.append("missing_memory")
    if stale_penalty > ZERO_SCORE:
        codes.append("stale_memory")
    if conflict_penalty > ZERO_SCORE:
        codes.append("conflicting_memory")
    if not codes:
        codes.append("memory_ready")
...
def _row_status(
    ...
    *,
    ...
    if "missing_memory" in reason_codes or "conflicting_memory" in reason_codes:
        return "block"
...
def _report_reason_codes(
    ...
    if source_row_count == 0:
        return ("no_bonds_memory_inputs_supplied",)
    codes: list[str] = []
    if _reason_count(rows, "missing_memory") > ZERO_COUNT:
        codes.append("missing_memory_present")
    if _reason_count(rows, "stale_memory") > ZERO_COUNT:
        codes.append("stale_memory_present")
    if _reason_count(rows, "conflicting_memory") > ZERO_COUNT:
        codes.append("conflicting_memory_present")
    if not codes:
        codes.append("memory_ready")
...
def _report_status(block_count: Decimal, watch_count: Decimal, row_count: int) -> str:
    if block_count > ZERO_COUNT:
        return "block"
    if watch_count > ZERO_COUNT or row_count == 0:
        return "watch"
    return "pass"
...
def _row_sort_key(row: ResearchDomainBondsSignalMemoryQualityRow) -> tuple[int, int, str, str]:
    return (
        _row_priority(row),
        INPUT_FAMILIES.index(row.input_family),
        row.handoff_ref,
        row.memory_ref,
    )


def _row_priority(row: ResearchDomainBondsSignalMemoryQualityRow) -> int:
    if "conflicting_memory" in row.reason_codes:
        return 0
    if "missing_memory" in row.reason_codes:
        return 1
    if "stale_memory" in row.reason_codes:
        return 2
    return 3


def _validate_input(row: ResearchDomainBondsSignalMemoryInput) -> None:
...
def _validate_row(row: ResearchDomainBondsSignalMemoryQualityRow) -> None:
    if row.conflicting_evidence_count > row.evidence_count:
        raise ValueError("conflicting_evidence_count must not exceed evidence_count")
    ...
    if row.row_status not in ROW_STATUSES:
        raise ValueError("row_status must be public")


def _validate_report(report: ResearchDomainBondsSignalMemoryQualityReport) -> None:
    ...
    if report.stale_count != _reason_count(report.rows, "stale_memory"):
        raise ValueError("stale_count must match rows")
    if report.conflict_count != _reason_count(report.rows, "conflicting_memory"):
        raise ValueError("conflict_count must match rows")
    if report.missing_count != _reason_count(report.rows, "missing_memory"):
...
def _normalize_report_rows(
    value: object,
) -> tuple[ResearchDomainBondsSignalMemoryQualityRow, ...]:
    if type(value) not in (list, tuple):
        raise ValueError("rows must be a list or tuple")
    rows = tuple(value)
    seen_refs: set[str] = set()
    for row in rows:
        if type(row) is not ResearchDomainBondsSignalMemoryQualityRow:
            raise ValueError(
                "rows must contain ResearchDomainBondsSignalMemoryQualityRow values",
            )
        _require_hard_flags("ResearchDomainBondsSignalMemoryQualityRow", row)
        if row.memory_ref in seen_refs:
            raise ValueError("duplicate memory_ref values are not allowed")
        seen_refs.add(row.memory_ref)
...
def _normalize_observed_at(
    *,
    ...
    if not memory_present:
        raise ValueError("observed_at must be omitted when memory_present is False")
    return _as_utc("observed_at", observed_at)
...
def _report_digest(report: ResearchDomainBondsSignalMemoryQualityReport) -> str:
    payload = _json_ready(report)
    if type(payload) is not dict:
        raise ValueError("report payload must be a JSON object")
...
__all__ = (
    "DEFAULT_RESEARCH_DOMAIN_BONDS_SIGNAL_MEMORY_QUALITY_CONFIG_VERSION",
    "ResearchDomainBondsSignalMemoryInput",
    "ResearchDomainBondsSignalMemoryQualityConfig",
    "ResearchDomainBondsSignalMemoryQualityReport",
    "ResearchDomainBondsSignalMemoryQualityRow",
    "build_research_domain_bonds_signal_memory_quality_report",
    "research_domain_bonds_signal_memory_quality_report_payload",
)
