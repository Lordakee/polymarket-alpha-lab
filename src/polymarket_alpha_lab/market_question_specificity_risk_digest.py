"""Pure report-only diagnostics for market question specificity risk."""
...
DEFAULT_MARKET_QUESTION_SPECIFICITY_RISK_DIGEST_CONFIG_VERSION = (
    "market-question-specificity-risk-digest-v0"
)

PASS_REASON = "market_question_specificity_risk_passed"
EMPTY_REASON = "market_question_specificity_risk_empty_findings"
RULE_UNCLEAR_REASON = "market_question_specificity_risk_rule_unclear"
ENDPOINT_NOT_MEASURABLE_REASON = (
    "market_question_specificity_risk_endpoint_not_measurable"
)
DEADLINE_UNCLEAR_REASON = "market_question_specificity_risk_deadline_unclear"
AMBIGUOUS_TERMS_REASON = "market_question_specificity_risk_ambiguous_terms"
AUTHORITATIVE_SOURCES_PARTIAL_REASON = (
...
NEXT_STEPS = {
    "pass": "allow_report_only_question_specificity_diagnostics",
    "watch": "review_report_only_question_specificity_sources",
    "blocked": "block_report_only_question_specificity_diagnostics",
}
ZERO_DECIMAL = Decimal("0")
ONE_DECIMAL = Decimal("1")
SIX_PLACES = Decimal("0.000001")
REDACTED_MARKERS = ("redacted", "anon", "masked", "synthetic", "sample")
SENSITIVE_MARKERS = ("0x", "@", "email", "key", "secret")
...
class MarketQuestionSpecificityRiskDigestConfig:
    config_version: str = DEFAULT_MARKET_QUESTION_SPECIFICITY_RISK_DIGEST_CONFIG_VERSION
    ...
    max_ambiguous_term_count: Decimal = Decimal("1")
    min_authoritative_source_coverage_ratio: Decimal = Decimal("0.500000")
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True

    def __post_init__(self) -> None:
        if type(self) is not MarketQuestionSpecificityRiskDigestConfig:
            raise ValueError(
                "config must be exactly MarketQuestionSpecificityRiskDigestConfig",
            )
        ...
        object.__setattr__(
            self,
            "min_authoritative_source_coverage_ratio",
            _require_ratio(
                "min_authoritative_source_coverage_ratio",
                self.min_authoritative_source_coverage_ratio,
            ),
        )
...
@dataclass(frozen=True)
class MarketQuestionSpecificityRiskFinding:
    ...
    def __post_init__(self) -> None:
        if type(self) is not MarketQuestionSpecificityRiskFinding:
            raise ValueError(
                "finding must be exactly MarketQuestionSpecificityRiskFinding",
            )
        ...
        object.__setattr__(
            ...
            _normalize_authoritative_source_mapping(
                self.authoritative_source_mapping,
            ),
        )
...
@dataclass(frozen=True)
class MarketQuestionSpecificityRiskDigestReasonCodeCount:
    reason_code: str
    count: Decimal
    paper_only: bool = True
    report_only: bool = True
    ...
    def __post_init__(self) -> None:
        if type(self) is not MarketQuestionSpecificityRiskDigestReasonCodeCount:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketQuestionSpecificityRiskDigestReport:
    ...
    authoritative_source_coverage_ratio: Decimal
    findings: tuple[MarketQuestionSpecificityRiskFinding, ...]
    source_config_versions: tuple[tuple[str, str], ...]
    reason_code_counts: tuple[MarketQuestionSpecificityRiskDigestReasonCodeCount, ...]
    reason_codes: tuple[str, ...]
    ...
    def __post_init__(self) -> None:
        if type(self) is not MarketQuestionSpecificityRiskDigestReport:
            raise ValueError(
                "report must be exactly MarketQuestionSpecificityRiskDigestReport",
            )
        ...
        object.__setattr__(
            ...
            _require_ratio(
                "authoritative_source_coverage_ratio",
                self.authoritative_source_coverage_ratio,
            ),
        )
...
def build_market_question_specificity_risk_digest(
    findings: list[MarketQuestionSpecificityRiskFinding]
    | tuple[MarketQuestionSpecificityRiskFinding, ...],
    *,
    config: MarketQuestionSpecificityRiskDigestConfig,
    generated_at: datetime,
) -> MarketQuestionSpecificityRiskDigestReport:
    if type(config) is not MarketQuestionSpecificityRiskDigestConfig:
        raise ValueError("config must be a MarketQuestionSpecificityRiskDigestConfig")
    ...
    return MarketQuestionSpecificityRiskDigestReport(
        ...
        authoritative_source_coverage_ratio=_average(
            tuple(
                _authoritative_source_coverage_ratio(row)
                for row in evaluated_findings
            ),
        ),
        ...
        source_config_versions=tuple(
            sorted(
                (row.market_reference, row.source_config_version)
                for row in evaluated_findings
            )
        ),
...
def market_question_specificity_risk_digest_payload(
    report: MarketQuestionSpecificityRiskDigestReport,
) -> dict[str, object]:
    if type(report) is not MarketQuestionSpecificityRiskDigestReport:
        raise ValueError(
            "report must be a MarketQuestionSpecificityRiskDigestReport",
        )
    ...
    return {
        ...
        "authoritative_source_coverage_ratio": _decimal_payload(
            report.authoritative_source_coverage_ratio,
        ),
        "findings": [_finding_payload(finding) for finding in report.findings],
        "source_config_versions": [
            [market_reference, source_config_version]
            for market_reference, source_config_version in report.source_config_versions
        ],
        "reason_code_counts": [
...
def _finding_payload(
    finding: MarketQuestionSpecificityRiskFinding,
) -> dict[str, object]:
    if type(finding) is not MarketQuestionSpecificityRiskFinding:
        raise ValueError("finding must be a MarketQuestionSpecificityRiskFinding")
    ...
    return {
        ...
        "authoritative_source_mapping": [
            ...
            for source_role, source_reference in finding.authoritative_source_mapping
        ],
...
def _reason_code_count_payload(
    count: MarketQuestionSpecificityRiskDigestReasonCodeCount,
) -> dict[str, object]:
    if type(count) is not MarketQuestionSpecificityRiskDigestReasonCodeCount:
        raise ValueError(
            "reason_code_count must be a MarketQuestionSpecificityRiskDigestReasonCodeCount",
        )
...
def _evaluate_finding(
    ...
    *,
    ...
    if (
        _authoritative_source_coverage_ratio(finding)
        < config.min_authoritative_source_coverage_ratio
    ):
        reason_codes.append(AUTHORITATIVE_SOURCES_PARTIAL_REASON)
    if not reason_codes:
    ...
    return MarketQuestionSpecificityRiskFinding(
        ...
        authoritative_source_mapping=finding.authoritative_source_mapping,
        source_config_version=finding.source_config_version,
...
def _finding_status(reason_codes: tuple[str, ...]) -> str:
    if any(
        ...
        return "blocked"
    if reason_codes == (AUTHORITATIVE_SOURCES_PARTIAL_REASON,):
        return "watch"
...
def _reason_code_counts_for_findings(
    findings: tuple[MarketQuestionSpecificityRiskFinding, ...],
) -> tuple[MarketQuestionSpecificityRiskDigestReasonCodeCount, ...]:
    reason_codes = tuple(
        reason_code
        for finding in findings
        for reason_code in finding.reason_codes
...
def _readiness_gap_count(finding: MarketQuestionSpecificityRiskFinding) -> Decimal:
...
def _authoritative_source_coverage_ratio(
    finding: MarketQuestionSpecificityRiskFinding,
) -> Decimal:
    if not finding.authoritative_source_mapping:
        return ZERO_DECIMAL
    observed_source_count = sum(
        1 for _, source_reference in finding.authoritative_source_mapping if source_reference
    )
    capped_source_count = min(observed_source_count, 2)
    return (Decimal(capped_source_count) / Decimal(2)).quantize(SIX_PLACES)
...
def _normalize_input_findings(
    ...
    seen_market_references: set[str] = set()
    for finding in normalized_findings:
        if type(finding) is not MarketQuestionSpecificityRiskFinding:
            raise ValueError(
                "findings must contain MarketQuestionSpecificityRiskFinding",
            )
        _require_hard_flags("finding", finding)
        if finding.market_reference in seen_market_references:
            raise ValueError("market_reference values must be unique")
        seen_market_references.add(finding.market_reference)
...
def _normalize_findings(
    ...
    if normalized_findings != tuple(
        sorted(normalized_findings, key=lambda row: row.market_reference)
    ):
        raise ValueError("findings must be sorted by market_reference")
    return normalized_findings


def _normalize_authoritative_source_mapping(
    authoritative_source_mapping: tuple[tuple[str, str], ...],
) -> tuple[tuple[str, str], ...]:
    if type(authoritative_source_mapping) not in (list, tuple):
        raise ValueError("authoritative_source_mapping must be a list or tuple")
    rows = tuple(authoritative_source_mapping)
    seen_source_roles: set[str] = set()
    for item in rows:
        if type(item) not in (list, tuple) or len(item) != 2:
            raise ValueError(
                "authoritative_source_mapping entries must be source role/reference pairs",
            )
        source_role, source_reference = item
        _require_canonical_string("authoritative_source_mapping source_role", source_role)
        _require_canonical_string(
            "authoritative_source_mapping source_reference",
            source_reference,
        )
        ...
        if source_role in seen_source_roles:
            raise ValueError("authoritative_source_mapping source roles must be unique")
        seen_source_roles.add(source_role)
    if rows != tuple(sorted(rows)):
        raise ValueError("authoritative_source_mapping must be sorted")
    return rows
...
def _normalize_source_config_versions(
    ...
    for item in versions:
        ...
        market_reference, source_config_version = item
        _require_canonical_string("source_config_versions market_reference", market_reference)
        _require_redacted_reference(
        ...
        if market_reference in seen_market_references:
            raise ValueError("source_config_versions market_reference values must be unique")
        seen_market_references.add(market_reference)
    if versions != tuple(sorted(versions)):
        raise ValueError("source_config_versions must be sorted by market_reference")
    return versions
...
def _validate_report_consistency(
    ...
    if report.authoritative_source_coverage_ratio != _average(
        tuple(_authoritative_source_coverage_ratio(row) for row in report.findings)
    ):
        raise ValueError("authoritative_source_coverage_ratio must match findings")
    ...
    if report.source_config_versions != tuple(
        sorted((row.market_reference, row.source_config_version) for row in report.findings)
    ):
...
def _require_redacted_reference(field_name: str, value: str) -> None:
    ...
    if "source" in lowered and not any(marker in lowered for marker in REDACTED_MARKERS):
        raise ValueError(f"{field_name} must be redacted")
