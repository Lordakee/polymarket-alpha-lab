"""Environment boundary for action-gated queue history DB persistence."""
...
class SupabaseActionGatedStrategyRecommendationQueueHistoryConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_ACTION_GATED_QUEUE_HISTORY_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.dsn is not None:
            validate_local_postgres_dsn(
                self.dsn,
                env_var_name=ACTION_GATED_QUEUE_HISTORY_DB_DSN_ENV_VAR,
            )
...
def from_action_gated_strategy_recommendation_queue_history_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabaseActionGatedStrategyRecommendationQueueHistoryConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(ACTION_GATED_QUEUE_HISTORY_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(ACTION_GATED_QUEUE_HISTORY_DB_DSN_ENV_VAR)
    table_name = source.get(
        ACTION_GATED_QUEUE_HISTORY_DB_TABLE_ENV_VAR,
        DEFAULT_ACTION_GATED_QUEUE_HISTORY_DB_TABLE,
    )
    try:
        return SupabaseActionGatedStrategyRecommendationQueueHistoryConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == _TABLE_NAME_ERROR:
            raise ValueError(
                f"{ACTION_GATED_QUEUE_HISTORY_DB_TABLE_ENV_VAR} {_TABLE_NAME_ERROR}",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{ACTION_GATED_QUEUE_HISTORY_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{ACTION_GATED_QUEUE_HISTORY_DB_DSN_ENV_VAR} must be a string")
    if not value:
        return None
