"""Environment boundary for paper trade attribution DB persistence."""
...
PAPER_TRADE_ATTRIBUTION_DB_ENABLED_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_TRADE_ATTRIBUTION_DB_ENABLED"
)
PAPER_TRADE_ATTRIBUTION_DB_DSN_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_TRADE_ATTRIBUTION_DB_DSN"
)
PAPER_TRADE_ATTRIBUTION_DB_TABLE_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_TRADE_ATTRIBUTION_DB_TABLE"
)
DEFAULT_PAPER_TRADE_ATTRIBUTION_DB_TABLE = "paper_trade_attribution_reports"
...
class SupabasePaperTradeAttributionConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_TRADE_ATTRIBUTION_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.dsn is not None:
            validate_local_postgres_dsn(
                self.dsn,
                env_var_name=PAPER_TRADE_ATTRIBUTION_DB_DSN_ENV_VAR,
            )
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_TRADE_ATTRIBUTION_DB_DSN_ENV_VAR} must be set when DB is enabled",
            )
...
def from_paper_trade_attribution_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperTradeAttributionConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(PAPER_TRADE_ATTRIBUTION_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(PAPER_TRADE_ATTRIBUTION_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_TRADE_ATTRIBUTION_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_TRADE_ATTRIBUTION_DB_TABLE,
    )
    try:
        return SupabasePaperTradeAttributionConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if message.startswith("table_name "):
            raise ValueError(
                f"{PAPER_TRADE_ATTRIBUTION_DB_TABLE_ENV_VAR} {message}",
            ) from exc
        raise
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_TRADE_ATTRIBUTION_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{PAPER_TRADE_ATTRIBUTION_DB_DSN_ENV_VAR} must be a string")
