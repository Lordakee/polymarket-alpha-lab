"""Pure Phase 1 gold options skew market research reducer."""
...
DEFAULT_MARKET_RESEARCH_GOLD_OPTIONS_SKEW_DIGEST_CONFIG_VERSION = (
    "market-research-gold-options-skew-digest-v0"
)

STATUS_READY = "ready"
STATUS_WATCH = "watch"
STATUS_BLOCKED = "blocked"
GOLD_OPTIONS_SKEW_DIGEST_STATUSES = (STATUS_READY, STATUS_WATCH, STATUS_BLOCKED)

READY_REASON = "market_research_gold_options_skew_digest_ready"
NO_INPUTS_REASON = "market_research_gold_options_skew_digest_no_inputs"
SKEW_PRESSURE_REASON = "market_research_gold_options_skew_digest_skew_pressure"
...
REFERENCE_GAP_REASON = "market_research_gold_options_skew_digest_reference_gap"
OPEN_INTEREST_GAP_REASON = (
    "market_research_gold_options_skew_digest_open_interest_gap"
)
VOLUME_GAP_REASON = "market_research_gold_options_skew_digest_volume_gap"
STALE_SNAPSHOT_REASON = "market_research_gold_options_skew_digest_stale_snapshot"
CONFIDENCE_GAP_REASON = "market_research_gold_options_skew_digest_confidence_gap"
...
class MarketResearchGoldOptionsSkewDigestConfig:
    config_version: str = (
        DEFAULT_MARKET_RESEARCH_GOLD_OPTIONS_SKEW_DIGEST_CONFIG_VERSION
    )
    max_snapshot_age_seconds: Decimal = Decimal("3600.000000")
    min_reference_count: Decimal = Decimal("2.000000")
    min_open_interest_usd: Decimal = Decimal("500000.000000")
    min_volume_usd: Decimal = Decimal("125000.000000")
    max_put_call_skew_abs: Decimal = Decimal("0.150000")
    max_put_wing_premium: Decimal = Decimal("0.110000")
    max_call_wing_premium: Decimal = Decimal("0.130000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not MarketResearchGoldOptionsSkewDigestConfig:
            raise TypeError(
                "MarketResearchGoldOptionsSkewDigestConfig "
    ...
    def __post_init__(self) -> None:
        if type(self) is not MarketResearchGoldOptionsSkewDigestConfig:
            raise ValueError(
                "config must be exactly MarketResearchGoldOptionsSkewDigestConfig",
            )
...
@dataclass(frozen=True)
class MarketResearchGoldOptionsSkewSnapshot:
    condition_id: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchGoldOptionsSkewSnapshot:
            raise TypeError(
                "MarketResearchGoldOptionsSkewSnapshot does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchGoldOptionsSkewSnapshot:
            raise ValueError(
                "snapshot must be exactly MarketResearchGoldOptionsSkewSnapshot",
            )
        for field_name in (
            "condition_id",
            "options_skew_key",
...
@dataclass(frozen=True)
class MarketResearchGoldOptionsSkewDigestRow:
    condition_id: str
    options_skew_key: str
    asset_symbol: str
    expiry_bucket: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchGoldOptionsSkewDigestRow:
            raise TypeError(
                "MarketResearchGoldOptionsSkewDigestRow does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchGoldOptionsSkewDigestRow:
            raise ValueError(
                "row must be exactly MarketResearchGoldOptionsSkewDigestRow",
            )
        for field_name in (
            "condition_id",
            "options_skew_key",
...
@dataclass(frozen=True)
class MarketResearchGoldOptionsSkewDigestReasonCodeCount:
    reason_code: str
    count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchGoldOptionsSkewDigestReasonCodeCount:
            raise TypeError(
                "MarketResearchGoldOptionsSkewDigestReasonCodeCount "
                "does not support subclassing",
            )
    ...
    def __post_init__(self) -> None:
        if type(self) is not MarketResearchGoldOptionsSkewDigestReasonCodeCount:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketResearchGoldOptionsSkewDigestReport:
    ...
    rows: tuple[MarketResearchGoldOptionsSkewDigestRow, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchGoldOptionsSkewDigestReport:
            raise TypeError(
                "MarketResearchGoldOptionsSkewDigestReport does not support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchGoldOptionsSkewDigestReport:
            raise ValueError(
                "report must be exactly MarketResearchGoldOptionsSkewDigestReport",
            )
...
def build_market_research_gold_options_skew_digest(
    snapshots: Iterable[MarketResearchGoldOptionsSkewSnapshot],
    *,
    config: MarketResearchGoldOptionsSkewDigestConfig | None = None,
    generated_at: datetime,
) -> MarketResearchGoldOptionsSkewDigestReport:
    cfg = config or MarketResearchGoldOptionsSkewDigestConfig()
    if type(cfg) is not MarketResearchGoldOptionsSkewDigestConfig:
        raise ValueError(
            "config must be exactly MarketResearchGoldOptionsSkewDigestConfig",
        )
    ...
    return MarketResearchGoldOptionsSkewDigestReport(
...
def market_research_gold_options_skew_digest_payload(
    report: MarketResearchGoldOptionsSkewDigestReport,
) -> MappingProxyType:
    if type(report) is not MarketResearchGoldOptionsSkewDigestReport:
        raise ValueError(
            "report must be exactly MarketResearchGoldOptionsSkewDigestReport",
        )
...
def _row_for_snapshot(
    snapshot: MarketResearchGoldOptionsSkewSnapshot,
    *,
    config: MarketResearchGoldOptionsSkewDigestConfig,
    generated_at: datetime,
) -> MarketResearchGoldOptionsSkewDigestRow:
    ...
    return MarketResearchGoldOptionsSkewDigestRow(
...
def _row_sort_value(row: MarketResearchGoldOptionsSkewDigestRow) -> tuple[int, Decimal]:
    status_rank = {
        STATUS_BLOCKED: 0,
        STATUS_WATCH: 1,
        STATUS_READY: 2,
...
def _report_status(rows: tuple[MarketResearchGoldOptionsSkewDigestRow, ...]) -> str:
    if not rows:
        return STATUS_WATCH
...
def _recommended_next_step(status: str) -> str:
    if status == STATUS_BLOCKED:
        return "block_report_only_market_research_gold_options_skew_digest"
    if status == STATUS_WATCH:
        return "review_report_only_market_research_gold_options_skew_digest"
    return "allow_report_only_market_research_gold_options_skew_digest"
...
def _reason_code_counts(
    rows: tuple[MarketResearchGoldOptionsSkewDigestRow, ...],
) -> tuple[MarketResearchGoldOptionsSkewDigestReasonCodeCount, ...]:
...
def _validate_row(row: MarketResearchGoldOptionsSkewDigestRow) -> None:
...
def _validate_report(report: MarketResearchGoldOptionsSkewDigestReport) -> None:
...
def _normalize_snapshots(
    snapshots: Iterable[MarketResearchGoldOptionsSkewSnapshot],
) -> tuple[MarketResearchGoldOptionsSkewSnapshot, ...]:
    snapshot_tuple = tuple(snapshots)
    seen_keys: set[str] = set()
    for snapshot in snapshot_tuple:
        if type(snapshot) is not MarketResearchGoldOptionsSkewSnapshot:
            raise ValueError(
                "snapshots must contain MarketResearchGoldOptionsSkewSnapshot",
            )
...
def _normalize_rows(
    rows: tuple[MarketResearchGoldOptionsSkewDigestRow, ...],
) -> tuple[MarketResearchGoldOptionsSkewDigestRow, ...]:
    if type(rows) is not tuple:
        raise ValueError("rows must be a tuple")
    for row in rows:
        if type(row) is not MarketResearchGoldOptionsSkewDigestRow:
            raise ValueError("rows must contain MarketResearchGoldOptionsSkewDigestRow")
        _require_hard_flags("row", row)
    return rows


def _normalize_reason_code_counts(
    items: tuple[MarketResearchGoldOptionsSkewDigestReasonCodeCount, ...],
) -> tuple[MarketResearchGoldOptionsSkewDigestReasonCodeCount, ...]:
    if type(items) is not tuple:
        raise ValueError("reason_code_counts must be a tuple")
    for item in items:
        if type(item) is not MarketResearchGoldOptionsSkewDigestReasonCodeCount:
...
def _require_status(field_name: str, value: str) -> None:
    if type(value) is not str or value not in GOLD_OPTIONS_SKEW_DIGEST_STATUSES:
