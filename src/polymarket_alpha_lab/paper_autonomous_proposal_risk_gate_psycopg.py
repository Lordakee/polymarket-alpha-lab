"""Optional psycopg adapter for paper autonomous proposal risk gate storage."""
...
from polymarket_alpha_lab.supabase_paper_autonomous_proposal_risk_gate_config import (
...
def load_paper_autonomous_proposal_risk_gate_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_autonomous_proposal_risk_gate_reports(
            connection,
            config_version=config_version,
            gate_status=gate_status,
            source_proposal_status=source_proposal_status,
            limit=limit,
            table_name=table_name,
        ),
    )
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper autonomous proposal risk gate "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper autonomous proposal risk gate database "
            f"({type(exc).__name__})",
        ) from exc
