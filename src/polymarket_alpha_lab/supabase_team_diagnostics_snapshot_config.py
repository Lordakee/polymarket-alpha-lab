"""Environment boundary for team diagnostics snapshot DB persistence."""
...
DEFAULT_TEAM_DIAGNOSTICS_SNAPSHOT_DB_TABLE = "team_diagnostics_snapshots"
...
class SupabaseTeamDiagnosticsSnapshotConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_TEAM_DIAGNOSTICS_SNAPSHOT_DB_TABLE
...
def from_team_diagnostics_snapshot_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabaseTeamDiagnosticsSnapshotConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(TEAM_DIAGNOSTICS_SNAPSHOT_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(TEAM_DIAGNOSTICS_SNAPSHOT_DB_DSN_ENV_VAR)
    table_name = source.get(
        TEAM_DIAGNOSTICS_SNAPSHOT_DB_TABLE_ENV_VAR,
        DEFAULT_TEAM_DIAGNOSTICS_SNAPSHOT_DB_TABLE,
    )
    try:
        return SupabaseTeamDiagnosticsSnapshotConfig(
            enabled=enabled,
            dsn=dsn,
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{TEAM_DIAGNOSTICS_SNAPSHOT_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{TEAM_DIAGNOSTICS_SNAPSHOT_DB_DSN_ENV_VAR} must be a string")
