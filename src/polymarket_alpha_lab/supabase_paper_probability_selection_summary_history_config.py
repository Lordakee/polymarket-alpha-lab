"""Environment boundary for paper probability selection summary history DB persistence."""
...
class SupabasePaperProbabilitySelectionSummaryHistoryConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_PROBABILITY_SELECTION_SUMMARY_HISTORY_DB_TABLE
...
def from_paper_probability_selection_summary_history_db_env(
    ...
    dsn = source.get(PAPER_PROBABILITY_SELECTION_SUMMARY_HISTORY_DB_DSN_ENV_VAR)
    ...
    try:
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                f"{PAPER_PROBABILITY_SELECTION_SUMMARY_HISTORY_DB_TABLE_ENV_VAR} "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_PROBABILITY_SELECTION_SUMMARY_HISTORY_DB_ENABLED_ENV_VAR} "
