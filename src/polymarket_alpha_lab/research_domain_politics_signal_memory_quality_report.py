"""Pure report-only reducer for politics signal memory quality."""
...
DEFAULT_RESEARCH_DOMAIN_POLITICS_SIGNAL_MEMORY_QUALITY_REPORT_CONFIG_VERSION = (
    "research-domain-politics-signal-memory-quality-report-v0"
)
POLITICS_SIGNAL_MEMORY_QUALITY_STATUSES = ("pass", "watch", "block")
POLITICS_SIGNAL_MEMORY_QUALITY_REASON_CODES = (
    "no_politics_signal_memory_quality_inputs",
    ...
    "politics_signal_memory_quality_pass",
)
...
_ROW_REASON_PRIORITY = POLITICS_SIGNAL_MEMORY_QUALITY_REASON_CODES[1:]
...
class ResearchDomainPoliticsSignalMemoryQualityConfig:
    config_version: str = (
        DEFAULT_RESEARCH_DOMAIN_POLITICS_SIGNAL_MEMORY_QUALITY_REPORT_CONFIG_VERSION
    )
    recent_memory_seconds: Decimal = Decimal("604800.000000")
    stale_memory_seconds: Decimal = Decimal("2592000.000000")
    coverage_pass_threshold: Decimal = Decimal("0.800000")
    coverage_watch_threshold: Decimal = Decimal("0.500000")
    ...
    completeness_weight: Decimal = Decimal("0.250000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        raise TypeError(
            "ResearchDomainPoliticsSignalMemoryQualityConfig does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type("config", self, ResearchDomainPoliticsSignalMemoryQualityConfig)
        ...
        if self.coverage_pass_threshold < self.coverage_watch_threshold:
            raise ValueError("coverage_pass_threshold must be at least watch threshold")
...
@dataclass(frozen=True)
class ResearchDomainPoliticsSignalMemoryQualityInput:
    team_label: str
    signal_family: str
    latest_memory_at: datetime
    expected_memory_count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchDomainPoliticsSignalMemoryQualityInput does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type("memory_input", self, ResearchDomainPoliticsSignalMemoryQualityInput)
        for field_name in ("team_label", "signal_family"):
        ...
        if self.expected_memory_count <= _ZERO_COUNT:
            raise ValueError("expected_memory_count must be positive")
        object.__setattr__(
            self,
            "coverage_score",
            _normalize_ratio("coverage_score", self.coverage_score),
        )
        if self.coverage_score != _bounded_ratio(
            self.available_memory_count,
            self.expected_memory_count,
        ):
            raise ValueError("coverage_score must match memory counts")
...
@dataclass(frozen=True)
class ResearchDomainPoliticsSignalMemoryQualityRow:
    team_label: str
    signal_family: str
    expected_memory_count: Decimal
    available_memory_count: Decimal
    missing_memory_count: Decimal
    conflicting_memory_count: Decimal
    stale_memory_count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchDomainPoliticsSignalMemoryQualityRow does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type("row", self, ResearchDomainPoliticsSignalMemoryQualityRow)
        for field_name in ("team_label", "signal_family"):
            _require_public_label(field_name, getattr(self, field_name))
        for field_name in (
            "expected_memory_count",
...
@dataclass(frozen=True)
class ResearchDomainPoliticsSignalMemoryQualityReport:
    ...
    rows: tuple[ResearchDomainPoliticsSignalMemoryQualityRow, ...]
    reason_codes: tuple[str, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        raise TypeError(
            "ResearchDomainPoliticsSignalMemoryQualityReport does not support subclassing",
        )

    def __post_init__(self) -> None:
        _require_exact_type("report", self, ResearchDomainPoliticsSignalMemoryQualityReport)
        ...
        object.__setattr__(
            ...
            _normalize_ratio("average_memory_quality_score", self.average_memory_quality_score),
        )
...
def build_research_domain_politics_signal_memory_quality_report(
    inputs: Iterable[ResearchDomainPoliticsSignalMemoryQualityInput],
    *,
    generated_at: datetime,
    config: ResearchDomainPoliticsSignalMemoryQualityConfig | None = None,
) -> ResearchDomainPoliticsSignalMemoryQualityReport:
    if config is None:
        config = ResearchDomainPoliticsSignalMemoryQualityConfig()
    _require_exact_type("config", config, ResearchDomainPoliticsSignalMemoryQualityConfig)
    _require_hard_flags(config)
    generated_at = _as_utc("generated_at", generated_at)
    normalized = _normalize_inputs(inputs)
    for value in normalized:
        if _seconds_between(value.latest_memory_at, generated_at) < _ZERO:
            raise ValueError("generated_at must be at or after latest_memory_at")
    rows = tuple(
        sorted(
    ...
    return ResearchDomainPoliticsSignalMemoryQualityReport(
        ...
        signal_family_count=_decimal_count(len({row.signal_family for row in rows})),
        pass_count=_status_count(rows, "pass"),
        watch_count=_status_count(rows, "watch"),
        block_count=_status_count(rows, "block"),
        stale_input_count=_count_if(
            rows,
            lambda row: _has_stale_memory(row, config),
        ),
        conflicting_input_count=_count_if(
            rows,
            lambda row: row.conflicting_memory_count > _ZERO_COUNT,
        ),
        missing_input_count=_count_if(
            rows,
            lambda row: row.missing_memory_count > _ZERO_COUNT,
        ),
        average_memory_quality_score=_average_score(
...
def research_domain_politics_signal_memory_quality_report_payload(
    report: ResearchDomainPoliticsSignalMemoryQualityReport | dict[str, Any],
) -> dict[str, Any]:
    if type(report) is ResearchDomainPoliticsSignalMemoryQualityReport:
    ...
    if type(report) is dict:
        ...
        return payload
    raise ValueError(
        "report must be a ResearchDomainPoliticsSignalMemoryQualityReport or payload",
    )
...
def _row_from_input(
    value: ResearchDomainPoliticsSignalMemoryQualityInput,
    generated_at: datetime,
    config: ResearchDomainPoliticsSignalMemoryQualityConfig,
) -> ResearchDomainPoliticsSignalMemoryQualityRow:
    memory_age_seconds = _seconds_between(value.latest_memory_at, generated_at)
    missing_memory_count = _missing_count(value)
    completeness_score = _bounded_ratio(
        value.available_memory_count,
        value.expected_memory_count,
    )
    freshness_score = _freshness_score(memory_age_seconds, config)
    non_conflict_score = _ZERO if value.conflicting_memory_count > _ZERO_COUNT else _ONE
    quality_score = _quality_score(
        freshness_score=freshness_score,
        coverage_score=value.coverage_score,
    ...
    reason_codes = _row_reason_codes(
        ...
        stale_memory_count=value.stale_memory_count,
        memory_age_seconds=memory_age_seconds,
        coverage_score=value.coverage_score,
        quality_score=quality_score,
        config=config,
    )
    return ResearchDomainPoliticsSignalMemoryQualityRow(
        team_label=value.team_label,
        signal_family=value.signal_family,
        expected_memory_count=value.expected_memory_count,
        available_memory_count=value.available_memory_count,
        missing_memory_count=missing_memory_count,
        conflicting_memory_count=value.conflicting_memory_count,
        stale_memory_count=value.stale_memory_count,
        memory_age_seconds=memory_age_seconds,
...
def _row_reason_codes(
    *,
    ...
    if conflicting_memory_count > _ZERO_COUNT:
        reasons.append("politics_signal_memory_conflicting_inputs")
    if missing_memory_count > _ZERO_COUNT:
        reasons.append("politics_signal_memory_missing_inputs")
    if stale_memory_count > _ZERO_COUNT or memory_age_seconds >= config.stale_memory_seconds:
        reasons.append("politics_signal_memory_stale_inputs")
    if coverage_score < config.coverage_pass_threshold and coverage_score >= (
        config.coverage_watch_threshold
    ):
        reasons.append("politics_signal_memory_watch_coverage")
    if (
        memory_age_seconds > config.recent_memory_seconds
        and memory_age_seconds < config.stale_memory_seconds
    ):
        reasons.append("politics_signal_memory_watch_recency")
    if conflicting_memory_count > _ZERO_COUNT or quality_score < config.quality_watch_threshold:
        reasons.append("politics_signal_memory_quality_block")
    elif (
        quality_score < config.quality_pass_threshold
        or missing_memory_count > _ZERO_COUNT
        or stale_memory_count > _ZERO_COUNT
        or memory_age_seconds > config.recent_memory_seconds
    ):
        reasons.append("politics_signal_memory_quality_watch")
    else:
        reasons.append("politics_signal_memory_quality_pass")
    return tuple(reason for reason in _ROW_REASON_PRIORITY if reason in reasons)


def _status_from_reason_codes(reason_codes: tuple[str, ...]) -> str:
    if "politics_signal_memory_quality_block" in reason_codes:
        return "block"
    if "politics_signal_memory_quality_watch" in reason_codes:
        return "watch"
    return "pass"
...
def _report_reason_codes(
    ...
    if not rows:
        return ("no_politics_signal_memory_quality_inputs",)
    present = frozenset(
        reason_code
        for row in rows
        for reason_code in row.reason_codes
        if reason_code != "politics_signal_memory_quality_pass"
    )
    if not present:
        return ("politics_signal_memory_quality_pass",)
    return tuple(reason_code for reason_code in _ROW_REASON_PRIORITY if reason_code in present)


def _normalize_inputs(
    values: Iterable[ResearchDomainPoliticsSignalMemoryQualityInput],
) -> tuple[ResearchDomainPoliticsSignalMemoryQualityInput, ...]:
    ...
    for value in normalized:
        _require_exact_type("memory_input", value, ResearchDomainPoliticsSignalMemoryQualityInput)
        _require_hard_flags(value)
        _reject_unsafe_public_surface("memory_input", value)
        key = (value.team_label, value.signal_family)
        if key in seen_keys:
            raise ValueError("inputs must use unique team and signal family labels")
        seen_keys.add(key)
    return normalized


def _normalize_rows(
    values: Iterable[ResearchDomainPoliticsSignalMemoryQualityRow],
) -> tuple[ResearchDomainPoliticsSignalMemoryQualityRow, ...]:
    if isinstance(values, (str, bytes)):
        raise ValueError("rows must contain ResearchDomainPoliticsSignalMemoryQualityRow values")
    try:
        rows = tuple(values)
    except TypeError as exc:
        raise ValueError(
            "rows must contain ResearchDomainPoliticsSignalMemoryQualityRow values",
        ) from exc
    for row in rows:
        _require_exact_type("row", row, ResearchDomainPoliticsSignalMemoryQualityRow)
        _require_hard_flags(row)
        _reject_unsafe_public_surface("row", row)
    if rows != tuple(sorted(rows, key=_row_sort_key)):
        raise ValueError("rows must use deterministic sort")
    if len(set((row.team_label, row.signal_family) for row in rows)) != len(rows):
        raise ValueError("rows must be unique")
    return rows


def _validate_row_consistency(row: ResearchDomainPoliticsSignalMemoryQualityRow) -> None:
    if row.expected_memory_count <= _ZERO_COUNT:
        raise ValueError("expected_memory_count must be positive")
    if row.missing_memory_count != max(
        row.expected_memory_count - row.available_memory_count,
        _ZERO_COUNT,
    ):
        raise ValueError("missing_memory_count must match memory counts")
    if row.coverage_score != _bounded_ratio(
        row.available_memory_count,
        row.expected_memory_count,
    ):
        raise ValueError("coverage_score must match memory counts")
    expected_non_conflict = _ZERO if row.conflicting_memory_count > _ZERO_COUNT else _ONE
    if row.non_conflict_score != expected_non_conflict:
        raise ValueError("non_conflict_score must match conflicting_memory_count")
    if row.completeness_score != _bounded_ratio(
        row.available_memory_count,
        row.expected_memory_count,
    ):
        raise ValueError("completeness_score must match memory counts")
    if row.row_status != _status_from_reason_codes(row.reason_codes):
        raise ValueError("row_status must match reason_codes")


def _validate_report_consistency(
    report: ResearchDomainPoliticsSignalMemoryQualityReport,
) -> None:
    ...
    if report.signal_family_count != _decimal_count(
        len({row.signal_family for row in report.rows}),
    ):
    ...
    if report.stale_input_count != _count_if(
        report.rows,
        lambda row: row.stale_memory_count > _ZERO_COUNT,
    ):
        raise ValueError("stale_input_count must match rows")
    if report.conflicting_input_count != _count_if(
        report.rows,
        lambda row: row.conflicting_memory_count > _ZERO_COUNT,
    ):
        raise ValueError("conflicting_input_count must match rows")
    if report.missing_input_count != _count_if(
        report.rows,
        lambda row: row.missing_memory_count > _ZERO_COUNT,
    ):
    ...
    if report.average_memory_quality_score != _average_score(
        tuple(row.memory_quality_score for row in report.rows),
    ):
        raise ValueError("average_memory_quality_score must match rows")


def _row_sort_key(
    row: ResearchDomainPoliticsSignalMemoryQualityRow,
...
def _missing_count(value: ResearchDomainPoliticsSignalMemoryQualityInput) -> Decimal:
    missing = value.expected_memory_count - value.available_memory_count
    if missing < _ZERO_COUNT:
        return _ZERO_COUNT
    return missing


def _has_stale_memory(
    row: ResearchDomainPoliticsSignalMemoryQualityRow,
    config: ResearchDomainPoliticsSignalMemoryQualityConfig,
) -> bool:
    return row.stale_memory_count > _ZERO_COUNT or row.memory_age_seconds > (
        config.recent_memory_seconds
    )
...
def _normalize_reason_codes_preserving_sequence(value: Iterable[str]) -> tuple[str, ...]:
    ...
    for reason_code in normalized:
        ...
        if reason_code not in POLITICS_SIGNAL_MEMORY_QUALITY_REASON_CODES:
...
def _require_status(field_name: str, value: object) -> None:
    if type(value) is not str or value not in POLITICS_SIGNAL_MEMORY_QUALITY_STATUSES:
