"""Optional psycopg adapter for action-gated queue history reports."""
...
from polymarket_alpha_lab.action_gated_strategy_recommendation_queue_history_store import (
...
def insert_paper_action_gated_strategy_recommendation_queue_history_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_action_gated_strategy_recommendation_queue_history_report(
            connection,
            report,
...
def load_paper_action_gated_strategy_recommendation_queue_history_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_action_gated_strategy_recommendation_queue_history_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the action-gated queue history "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the action-gated queue history database",
        ) from None
