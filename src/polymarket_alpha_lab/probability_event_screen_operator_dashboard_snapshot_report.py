"""Read-only operator dashboard snapshot report for probability event screens."""
...
PROBABILITY_EVENT_SCREEN_OPERATOR_DASHBOARD_SNAPSHOT_REPORT_VERSION = (
    "probability-event-screen-operator-dashboard-snapshot-v0"
)
...
SNAPSHOT_BANDS = ("ready", "watch", "blocked")
BLOCKED_REASON_BY_FIELD = {
    "final_review_ready": "final_review_not_ready",
    "operating_review_ready": "operating_review_not_ready",
    "audit_summary_ready": "audit_summary_not_ready",
...
class ProbabilityEventScreenOperatorDashboardSnapshotPublicPayload(dict[str, object]):
    """Immutable public payload for the operator dashboard snapshot report."""
...
class ProbabilityEventScreenOperatorDashboardSnapshotInput:
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not ProbabilityEventScreenOperatorDashboardSnapshotInput:
            raise TypeError(
                "ProbabilityEventScreenOperatorDashboardSnapshotInput "
    ...
    def __post_init__(self) -> None:
        if type(self) is not ProbabilityEventScreenOperatorDashboardSnapshotInput:
            raise ValueError(
...
@dataclass(frozen=True)
class ProbabilityEventScreenOperatorDashboardSnapshotReport:
    config_version: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not ProbabilityEventScreenOperatorDashboardSnapshotReport:
            raise TypeError(
                "ProbabilityEventScreenOperatorDashboardSnapshotReport "
                "does not support subclassing",
            )
    ...
    def __post_init__(self) -> None:
        if type(self) is not ProbabilityEventScreenOperatorDashboardSnapshotReport:
            raise ValueError(
                "report must be exactly "
                "ProbabilityEventScreenOperatorDashboardSnapshotReport",
            )
        _require_config_version(self.config_version)
        _require_bool("dashboard_snapshot_ready", self.dashboard_snapshot_ready)
        _require_snapshot_band(self.snapshot_band)
        for field_name in SIGNAL_FIELDS:
            _require_bool(field_name, getattr(self, field_name))
        object.__setattr__(
            self,
    ...
    def public_payload(
        self,
    ) -> ProbabilityEventScreenOperatorDashboardSnapshotPublicPayload:
        payload = ProbabilityEventScreenOperatorDashboardSnapshotPublicPayload(
...
def build_probability_event_screen_operator_dashboard_snapshot_report(
    inputs: ProbabilityEventScreenOperatorDashboardSnapshotInput,
) -> ProbabilityEventScreenOperatorDashboardSnapshotReport:
    """Build a deterministic paper-only operator dashboard snapshot report."""

    if type(inputs) is not ProbabilityEventScreenOperatorDashboardSnapshotInput:
        raise ValueError(
            "inputs must be a ProbabilityEventScreenOperatorDashboardSnapshotInput",
        )
    ...
    snapshot_band = _snapshot_band(blocked_reason_codes, attention_reason_codes)
    ...
    values: dict[str, object] = {
        ...
        "dashboard_snapshot_ready": snapshot_band == "ready",
    ...
    return ProbabilityEventScreenOperatorDashboardSnapshotReport(
        **values,
        digest=_payload_digest(_payload_values(values, digest="")),
    )


def probability_event_screen_operator_dashboard_snapshot_report_payload(
    report: ProbabilityEventScreenOperatorDashboardSnapshotReport,
) -> ProbabilityEventScreenOperatorDashboardSnapshotPublicPayload:
    if type(report) is not ProbabilityEventScreenOperatorDashboardSnapshotReport:
        raise ValueError(
            "report must be a ProbabilityEventScreenOperatorDashboardSnapshotReport",
        )
    _require_hard_flags(report)
    _validate_report(report)
    expected_digest = probability_event_screen_operator_dashboard_snapshot_report_digest(
        report,
    )
...
def probability_event_screen_operator_dashboard_snapshot_report_digest(
    report: ProbabilityEventScreenOperatorDashboardSnapshotReport,
) -> str:
    if type(report) is not ProbabilityEventScreenOperatorDashboardSnapshotReport:
        raise ValueError(
...
def _validate_report(
    ...
    if report.snapshot_band != snapshot_band:
        raise ValueError("snapshot_band must match reason codes")
    if report.dashboard_snapshot_ready != (snapshot_band == "ready"):
        raise ValueError("dashboard_snapshot_ready must match snapshot band")
...
def _validate_public_payload(
    ...
    if tuple(payload.keys()) != PAYLOAD_KEYS:
        raise ValueError("payload must match the canonical dashboard snapshot schema")
    ...
    _require_snapshot_band(payload["snapshot_band"])
...
def _report_values_without_digest(
    ...
    return {
        ...
        "snapshot_band": report.snapshot_band,
...
def _payload_values(values: Mapping[str, object], *, digest: str) -> dict[str, object]:
    return {
        ...
        "snapshot_band": values["snapshot_band"],
...
def _require_config_version(value: object) -> None:
    if value != PROBABILITY_EVENT_SCREEN_OPERATOR_DASHBOARD_SNAPSHOT_REPORT_VERSION:
...
def _require_snapshot_band(value: object) -> None:
    if type(value) is not str or value not in SNAPSHOT_BANDS:
        raise ValueError("snapshot_band must be supported")
