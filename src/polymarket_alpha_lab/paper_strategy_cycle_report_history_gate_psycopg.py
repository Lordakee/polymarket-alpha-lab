"""Optional psycopg adapter for paper strategy cycle history gate storage."""
...
def insert_paper_strategy_cycle_report_history_gate_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_write_connection(
        dsn,
        lambda connection: insert_paper_strategy_cycle_report_history_gate_report_with_result(
            connection,
            report,
...
def load_paper_strategy_cycle_report_history_gate_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_read_connection(
        dsn,
        lambda connection: load_paper_strategy_cycle_report_history_gate_reports(
            connection,
...
def _connect(dsn: str, *, autocommit: bool = False) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper strategy cycle history gate "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper strategy cycle history gate database",
        ) from None
