"""Optional psycopg adapter for paper recommendation risk budget report storage."""
...
from polymarket_alpha_lab.paper_recommendation_risk_budget_store import (
...
DEFAULT_PAPER_RECOMMENDATION_RISK_BUDGET_TABLE = (
    "paper_recommendation_risk_budget_reports"
)
_T = TypeVar("_T")


def insert_paper_recommendation_risk_budget_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    table_name: str = DEFAULT_PAPER_RECOMMENDATION_RISK_BUDGET_TABLE,
) -> None:
    _with_owned_connection(
        dsn,
        lambda connection: insert_paper_recommendation_risk_budget_report(
            connection,
            report,
...
def load_paper_recommendation_risk_budget_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_recommendation_risk_budget_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        if exc.name != "psycopg":
            raise
        raise RuntimeError(
            "psycopg is required to use the paper recommendation risk budget "
    ...
    try:
        ...
        raise RuntimeError(
            "failed to connect to the paper recommendation risk budget database",
        ) from None
...
__all__ = (
    ...
    "load_paper_recommendation_risk_budget_reports_with_psycopg",
)
