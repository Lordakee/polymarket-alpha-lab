"""Environment boundary for paper strategy cycle report DB persistence."""
...
class SupabasePaperStrategyCycleReportConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.dsn is not None:
            validate_local_postgres_dsn(
                self.dsn,
                env_var_name=PAPER_STRATEGY_CYCLE_REPORT_DB_DSN_ENV_VAR,
            )
        if self.enabled and self.dsn is None:
            raise ValueError(
                (
                    f"{PAPER_STRATEGY_CYCLE_REPORT_DB_DSN_ENV_VAR} must be set "
                    "when DB is enabled"
                ),
            )
...
def from_paper_strategy_cycle_report_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperStrategyCycleReportConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_STRATEGY_CYCLE_REPORT_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_STRATEGY_CYCLE_REPORT_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_STRATEGY_CYCLE_REPORT_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_DB_TABLE,
    )
    try:
        return SupabasePaperStrategyCycleReportConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                (
                    f"{PAPER_STRATEGY_CYCLE_REPORT_DB_TABLE_ENV_VAR} must be a "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_STRATEGY_CYCLE_REPORT_DB_ENABLED_ENV_VAR} must be true or false",
        )
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_STRATEGY_CYCLE_REPORT_DB_DSN_ENV_VAR} must be a string",
        )
