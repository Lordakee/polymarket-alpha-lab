"""Pure report-only history reducer for team research assignment reports."""
...
DEFAULT_TEAM_RESEARCH_ASSIGNMENT_HISTORY_CONFIG_VERSION = (
    "team-research-assignment-history-v0"
)
HISTORY_STATUSES = ("observed", "blocked")
ASSIGNMENT_REPORT_STATUSES = ("ready", "watch", "blocked")
REASON_CODES = ("insufficient_history", "duplicate_latest_generated_at")


@dataclass(frozen=True)
class TeamResearchAssignmentHistoryConfig:
    config_version: str = DEFAULT_TEAM_RESEARCH_ASSIGNMENT_HISTORY_CONFIG_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("TeamResearchAssignmentHistoryConfig", self)


@dataclass(frozen=True)
class TeamResearchAssignmentHistoryStatusRow:
    ...
    def __post_init__(self) -> None:
        _require_assignment_report_status("assignment_status", self.assignment_status)
        _require_nonnegative_int("report_count", self.report_count)
        require_paper_only_flags("TeamResearchAssignmentHistoryStatusRow", self)


@dataclass(frozen=True)
class TeamResearchAssignmentHistoryReport:
    ...
    status_rows: tuple[TeamResearchAssignmentHistoryStatusRow, ...]
    latest_assignment_status: str | None
    latest_assignment_count: int
    latest_assigned_count: int
    latest_watch_count: int
    latest_blocked_count: int
    assignment_count_delta: int
    assigned_count_delta: int
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _normalize_optional_assignment_report_status(
                "latest_assignment_status",
                self.latest_assignment_status,
            ),
        )
        for field_name in (
        ...
        require_paper_only_flags("TeamResearchAssignmentHistoryReport", self)
        _validate_report_consistency(self)


def build_team_research_assignment_history_report(
    reports: list[TeamResearchAssignmentReport] | tuple[TeamResearchAssignmentReport, ...],
    *,
    config: TeamResearchAssignmentHistoryConfig,
    generated_at: datetime,
) -> TeamResearchAssignmentHistoryReport:
    if type(config) is not TeamResearchAssignmentHistoryConfig:
        raise ValueError("config must be a TeamResearchAssignmentHistoryConfig")
    require_paper_only_flags("TeamResearchAssignmentHistoryConfig", config)
    ...
    return TeamResearchAssignmentHistoryReport(
        ...
        latest_assignment_status=(
            latest_report.assignment_status if latest_report is not None else None
        ),
        latest_assignment_count=_latest_int_value(latest_report, "assignment_count"),
        latest_assigned_count=_latest_int_value(latest_report, "assigned_count"),
        ...
        assignment_count_delta=_int_delta(ordered, "assignment_count"),
        assigned_count_delta=_int_delta(ordered, "assigned_count"),
...
def _normalize_reports(
    ...
    for report in source_reports:
        if type(report) is not TeamResearchAssignmentReport:
            raise ValueError("reports must contain TeamResearchAssignmentReport values")
        require_paper_only_flags("TeamResearchAssignmentReport", report)
    return source_reports


def _ordered_reports(
    reports: tuple[TeamResearchAssignmentReport, ...],
) -> tuple[tuple[datetime, TeamResearchAssignmentReport], ...]:
    indexed = tuple(
        (_as_utc("report generated_at", report.generated_at), index, report)
        for index, report in enumerate(reports)
    )
    return tuple((generated_at, report) for generated_at, _, report in sorted(indexed))


def _has_duplicate_latest_generated_at(
    ordered: tuple[tuple[datetime, TeamResearchAssignmentReport], ...],
) -> bool:
    if not ordered:
        return False
...
def _status_rows(
    reports: tuple[TeamResearchAssignmentReport, ...],
) -> tuple[TeamResearchAssignmentHistoryStatusRow, ...]:
    return tuple(
        TeamResearchAssignmentHistoryStatusRow(
            assignment_status=assignment_status,
            report_count=sum(
                1 for report in reports if report.assignment_status == assignment_status
            ),
        )
        for assignment_status in ASSIGNMENT_REPORT_STATUSES
    )


def _latest_int_value(report: TeamResearchAssignmentReport | None, field_name: str) -> int:
    if report is None:
        return 0
...
def _validate_report_consistency(report: TeamResearchAssignmentHistoryReport) -> None:
    ...
    if (
        report.latest_assigned_count
        + report.latest_watch_count
        + report.latest_blocked_count
        != report.latest_assignment_count
    ):
        raise ValueError("latest assignment counts must sum to latest_assignment_count")
    if report.report_count == 0:
        ...
        if report.latest_assignment_status is not None:
            raise ValueError("latest_assignment_status must be absent without reports")
        for field_name in (
            "latest_assignment_count",
            "latest_assigned_count",
            "latest_watch_count",
        ...
        if report.latest_assignment_status is None:
            raise ValueError("latest_assignment_status is required with reports")
...
def _normalize_status_rows(
    rows: tuple[TeamResearchAssignmentHistoryStatusRow, ...],
) -> tuple[TeamResearchAssignmentHistoryStatusRow, ...]:
    if type(rows) not in (list, tuple):
        raise ValueError("status_rows must be a list or tuple")
    normalized = tuple(rows)
    for row in normalized:
        if type(row) is not TeamResearchAssignmentHistoryStatusRow:
            raise ValueError(
                "status_rows must contain TeamResearchAssignmentHistoryStatusRow values",
            )
        require_paper_only_flags("TeamResearchAssignmentHistoryStatusRow", row)
    statuses = tuple(row.assignment_status for row in normalized)
    if statuses != ASSIGNMENT_REPORT_STATUSES:
        raise ValueError("status_rows must contain ready, watch, and blocked in order")
    return normalized
...
def _normalize_optional_assignment_report_status(
    field_name: str,
    value: str | None,
) -> str | None:
    if value is None:
        return None
    _require_assignment_report_status(field_name, value)
    return value
...
def _require_history_status(field_name: str, value: object) -> None:
    if type(value) is not str or value not in HISTORY_STATUSES:
        raise ValueError(f"{field_name} must be one of observed, blocked")


def _require_assignment_report_status(field_name: str, value: object) -> None:
    if type(value) is not str or value not in ASSIGNMENT_REPORT_STATUSES:
...
def _require_reason_code(field_name: str, value: object) -> None:
    if type(value) is not str or value not in REASON_CODES:
        raise ValueError(
            f"{field_name} must contain known team research assignment history reasons",
        )
