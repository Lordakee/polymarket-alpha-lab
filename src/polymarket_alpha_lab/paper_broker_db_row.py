...
def paper_broker_execution_record_from_db_row(
    ...
    try:
        record = from_jsonable(PaperBrokerExecutionRecord, row.payload_json)
    ...
    _require_hard_flags("record", record)
    expected = paper_broker_execution_record_to_db_row(record)
    _validate_row_matches_payload(row, expected)
    return record
...
def _validate_row_matches_payload(
    row: PaperBrokerExecutionDbRow,
    expected: PaperBrokerExecutionDbRow,
) -> None:
    for field_name in (
        "record_sha256",
        "generated_at",
