"""Pure row codec for paper autonomous readiness digest reports."""
...
class PaperAutonomousReadinessDigestDbRow:
    report_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "evidence_json",
            _normalize_json_object_array("evidence_json", self.evidence_json),
        )
        object.__setattr__(
            self,
            "source_config_versions_json",
...
def paper_autonomous_readiness_digest_to_db_row(
    report: PaperAutonomousReadinessDigestReport,
) -> PaperAutonomousReadinessDigestDbRow:
    if type(report) is not PaperAutonomousReadinessDigestReport:
        raise ValueError("report must be a PaperAutonomousReadinessDigestReport")
    _validate_report_tree(report)
    payload_json = _json_ready(asdict(report))
    if not isinstance(payload_json, dict):
        raise ValueError("payload_json must be a JSON object")
    evidence_json = payload_json.get("evidence")
    ...
    if not isinstance(evidence_json, list):
        raise ValueError("payload_json evidence must be a JSON array")
    ...
    return PaperAutonomousReadinessDigestDbRow(
        ...
        recommended_next_review_action=report.recommended_next_review_action,
...
def paper_autonomous_readiness_digest_from_db_row(
    row: PaperAutonomousReadinessDigestDbRow,
) -> PaperAutonomousReadinessDigestReport:
    if type(row) is not PaperAutonomousReadinessDigestDbRow:
        raise ValueError("row must be a PaperAutonomousReadinessDigestDbRow")
    _reject_json_floats(row.evidence_json)
    ...
    if row.evidence_json != row.payload_json.get("evidence"):
        raise ValueError("evidence_json must match payload_json")
    ...
    try:
        report = from_jsonable(PaperAutonomousReadinessDigestReport, row.payload_json)
    except (KeyError, TypeError, ValueError) as exc:
        raise ValueError(
            f"payload_json is not a valid readiness digest report: {exc}",
        ) from exc
    if type(report) is not PaperAutonomousReadinessDigestReport:
        raise ValueError("payload_json must recover a PaperAutonomousReadinessDigestReport")
    _validate_report_tree(report)
    expected_row = paper_autonomous_readiness_digest_to_db_row(report)
...
def to_db_row(
    ...
    return paper_autonomous_readiness_digest_to_db_row(report)


def from_db_row(
    row: PaperAutonomousReadinessDigestDbRow,
) -> PaperAutonomousReadinessDigestReport:
    return paper_autonomous_readiness_digest_from_db_row(row)
...
def _validate_row_matches_payload(
    row: PaperAutonomousReadinessDigestDbRow,
    expected: PaperAutonomousReadinessDigestDbRow,
) -> None:
...
def _validate_materialized_fields_match_payload(
    ...
    expected_values = {
        ...
        "digest_status": payload_json.get("digest_status"),
        "recommended_next_review_action": payload_json.get(
            "recommended_next_review_action",
        ),
        "evidence_json": payload_json.get("evidence"),
        "source_config_versions_json": payload_json.get("source_config_versions"),
    ...
    actual_values = {
        ...
        "recommended_next_review_action": row.recommended_next_review_action,
...
def _json_ready(value: Any) -> Any:
    ...
    raise ValueError("readiness digest DB row values must be JSON serializable")
