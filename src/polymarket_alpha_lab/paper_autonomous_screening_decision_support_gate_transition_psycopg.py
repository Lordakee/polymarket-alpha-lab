"""Optional psycopg adapter for paper autonomous screening transition reports."""
...
from polymarket_alpha_lab.supabase_paper_autonomous_screening_decision_support_gate_transition_config import (
...
def insert_paper_autonomous_screening_decision_support_gate_transition_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    table_name: str = (
...
def load_paper_autonomous_screening_decision_support_gate_transition_reports_with_psycopg(
    dsn: str,
    *,
...
def _with_owned_write_connection(dsn: str, operation: Callable[[Any], _T]) -> _T:
    validate_local_postgres_dsn(
        dsn,
        env_var_name=PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_DB_DSN_ENV_VAR,
    )
...
def _connect(dsn: str, **kwargs: Any) -> Any:
    try:
        ...
        raise RuntimeError(
            ...
            "decision-support gate transition adapter; install the postgres "
            "extra.",
        ) from exc
    try:
