"""Public-safe energy signal memory quality report."""
...
__all__ = (
    "DEFAULT_RESEARCH_DOMAIN_ENERGY_SIGNAL_MEMORY_QUALITY_REPORT_CONFIG_VERSION",
    "ResearchDomainEnergySignalMemoryInput",
    "ResearchDomainEnergySignalMemoryQualityConfig",
    "ResearchDomainEnergySignalMemoryQualityReasonCodeCount",
    "ResearchDomainEnergySignalMemoryQualityReport",
    "ResearchDomainEnergySignalMemoryQualityRow",
    "build_research_domain_energy_signal_memory_quality_report",
    "research_domain_energy_signal_memory_quality_report_payload",
)


DEFAULT_RESEARCH_DOMAIN_ENERGY_SIGNAL_MEMORY_QUALITY_REPORT_CONFIG_VERSION = (
    "research-domain-energy-signal-memory-quality-report-v0"
)
...
PASS_ROW_REASON_CODE = "energy_memory_clear"
MISSING_REQUIRED_TOPIC_REASON_CODE = "energy_memory_required_topic_missing"
BLOCK_REASON_CODES = (
...
REPORT_REASON_PRIORITY = (
    MISSING_REQUIRED_TOPIC_REASON_CODE,
    *BLOCK_REASON_CODES,
    *WATCH_REASON_CODES,
)
...
class ResearchDomainEnergySignalMemoryQualityConfig(_FinalPublicDataclass):
    config_version: str = (
        DEFAULT_RESEARCH_DOMAIN_ENERGY_SIGNAL_MEMORY_QUALITY_REPORT_CONFIG_VERSION
    )
    watch_stale_after_hours: Decimal = Decimal("24.000000")
    block_stale_after_hours: Decimal = Decimal("72.000000")
    ...
    required_memory_topics: tuple[str, ...] = ENERGY_MEMORY_TOPICS
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _require_memory_topics(
                "required_memory_topics",
                self.required_memory_topics,
            ),
        )
...
@dataclass(frozen=True)
class ResearchDomainEnergySignalMemoryInput(_FinalPublicDataclass):
    memory_topic: str
    age_hours: Decimal
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchDomainEnergySignalMemoryInput, "input")
        _require_memory_topic("memory_topic", self.memory_topic)
        object.__setattr__(
            self,
            "age_hours",
...
@dataclass(frozen=True)
class ResearchDomainEnergySignalMemoryQualityRow(_FinalPublicDataclass):
    memory_topic: str
    memory_status: str
    age_hours: Decimal
    conflict_count: Decimal
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchDomainEnergySignalMemoryQualityRow, "row")
        _require_memory_topic("memory_topic", self.memory_topic)
...
@dataclass(frozen=True)
class ResearchDomainEnergySignalMemoryQualityReasonCodeCount(_FinalPublicDataclass):
    reason_code: str
    count: Decimal
    memory_topic_ratio: Decimal
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "memory_topic_ratio",
            _require_ratio_decimal("memory_topic_ratio", self.memory_topic_ratio),
        )
        _require_hard_flags("reason_code_count", self)


@dataclass(frozen=True)
class ResearchDomainEnergySignalMemoryQualityReport(_FinalPublicDataclass):
    generated_at: datetime
    config_version: str
    memory_topic_count: Decimal
    required_memory_topic_count: Decimal
    missing_required_memory_topic_count: Decimal
    pass_count: Decimal
    watch_count: Decimal
    block_count: Decimal
    ...
    missing_memory_topics: tuple[str, ...]
    reason_codes: tuple[str, ...]
    reason_code_counts: tuple[ResearchDomainEnergySignalMemoryQualityReasonCodeCount, ...]
    rows: tuple[ResearchDomainEnergySignalMemoryQualityRow, ...]
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchDomainEnergySignalMemoryQualityReport, "report")
        ...
        for field_name in (
            "memory_topic_count",
            "required_memory_topic_count",
            "missing_required_memory_topic_count",
            "pass_count",
            "watch_count",
            "block_count",
        ...
        object.__setattr__(
            self,
            "missing_memory_topics",
            _require_memory_topics("missing_memory_topics", self.missing_memory_topics),
        )
        object.__setattr__(
            self,
            "reason_codes",
...
def build_research_domain_energy_signal_memory_quality_report(
    memory_inputs: Iterable[ResearchDomainEnergySignalMemoryInput],
    *,
    config: ResearchDomainEnergySignalMemoryQualityConfig,
    generated_at: datetime,
) -> ResearchDomainEnergySignalMemoryQualityReport:
    if type(config) is not ResearchDomainEnergySignalMemoryQualityConfig:
        raise ValueError(
            "config must be a ResearchDomainEnergySignalMemoryQualityConfig",
        )
    ...
    missing_topics = _missing_required_memory_topics(
        rows=rows,
        config=config,
    )
    status = _report_status(rows=rows, missing_topics=missing_topics)
    return ResearchDomainEnergySignalMemoryQualityReport(
        generated_at=generated_at_utc,
        config_version=config.config_version,
        memory_topic_count=_count(len(rows)),
        required_memory_topic_count=_count(len(config.required_memory_topics)),
        missing_required_memory_topic_count=_count(len(missing_topics)),
        ...
        missing_memory_topics=missing_topics,
        reason_codes=_report_reason_codes(rows=rows, missing_topics=missing_topics),
        reason_code_counts=_reason_code_counts(rows),
        rows=rows,
    )


def research_domain_energy_signal_memory_quality_report_payload(
    report: ResearchDomainEnergySignalMemoryQualityReport | dict[str, Any],
) -> dict[str, Any]:
    if type(report) is ResearchDomainEnergySignalMemoryQualityReport:
    ...
    if type(report) is dict:
        ...
        if supplied_digest != _payload_validation_digest(payload):
            raise ValueError("derived_validation_digest does not match report payload")
        return payload
    raise ValueError("report must be a ResearchDomainEnergySignalMemoryQualityReport")
...
def _row_from_input(
    item: ResearchDomainEnergySignalMemoryInput,
    *,
    config: ResearchDomainEnergySignalMemoryQualityConfig,
) -> ResearchDomainEnergySignalMemoryQualityRow:
    reason_codes = _row_reason_codes(item, config=config)
    return ResearchDomainEnergySignalMemoryQualityRow(
...
def _row_reason_codes(
    ...
    *,
    ...
    if item.age_hours >= config.block_stale_after_hours:
        reason_codes.append("energy_memory_stale_block")
    elif item.age_hours >= config.watch_stale_after_hours:
        reason_codes.append("energy_memory_stale_watch")

    if item.conflict_count >= config.block_conflict_count:
        reason_codes.append("energy_memory_conflict_block")
    elif item.conflict_count >= config.watch_conflict_count:
        reason_codes.append("energy_memory_conflict_watch")

    if item.missing_count >= config.block_missing_count:
        reason_codes.append("energy_memory_missing_block")
    elif item.missing_count >= config.watch_missing_count:
        reason_codes.append("energy_memory_missing_watch")
...
def _report_status(
    *,
    ...
    if missing_topics or any(row.memory_status == "block" for row in rows):
        return "block"
...
def _report_reason_codes(
    *,
    ...
    status = _report_status(rows=rows, missing_topics=missing_topics)
    reason_codes = [f"energy_memory_quality_{status}"]
    ...
    if missing_topics:
        reason_codes.append(MISSING_REQUIRED_TOPIC_REASON_CODE)
    for reason_code in REPORT_REASON_PRIORITY:
        if reason_code != MISSING_REQUIRED_TOPIC_REASON_CODE and reason_code in row_reasons:
...
def _reason_code_counts(
    rows: tuple[ResearchDomainEnergySignalMemoryQualityRow, ...],
) -> tuple[ResearchDomainEnergySignalMemoryQualityReasonCodeCount, ...]:
    if not rows:
        return ()
    ...
    return tuple(
        ResearchDomainEnergySignalMemoryQualityReasonCodeCount(
            ...
            count=count,
            memory_topic_ratio=_ratio(count, denominator),
        )
...
def _normalize_inputs(
    memory_inputs: Iterable[ResearchDomainEnergySignalMemoryInput],
) -> tuple[ResearchDomainEnergySignalMemoryInput, ...]:
    ...
    seen: set[str] = set()
    for item in items:
        if type(item) is not ResearchDomainEnergySignalMemoryInput:
            raise ValueError("memory_inputs must contain ResearchDomainEnergySignalMemoryInput")
        _require_hard_flags("input", item)
        if item.memory_topic in seen:
            raise ValueError("memory_topic values must be unique")
        seen.add(item.memory_topic)
    return items


def _missing_required_memory_topics(
    *,
    rows: tuple[ResearchDomainEnergySignalMemoryQualityRow, ...],
    config: ResearchDomainEnergySignalMemoryQualityConfig,
) -> tuple[str, ...]:
    present = {row.memory_topic for row in rows}
    return tuple(
        topic for topic in config.required_memory_topics if topic not in present
    )


def _validate_config(config: ResearchDomainEnergySignalMemoryQualityConfig) -> None:
...
def _validate_row(row: ResearchDomainEnergySignalMemoryQualityRow) -> None:
...
def _validate_report_materialized_fields(
    ...
    if report.memory_topic_count != _count(len(report.rows)):
        raise ValueError("memory_topic_count must match rows")
    ...
    if report.pass_count + report.watch_count + report.block_count != report.memory_topic_count:
        raise ValueError("status counts must match memory_topic_count")
    if report.missing_required_memory_topic_count != _count(
        len(report.missing_memory_topics),
    ):
        raise ValueError("missing_required_memory_topic_count must match missing topics")
    ...
    status = _report_status(rows=report.rows, missing_topics=report.missing_memory_topics)
    if report.status != status:
        raise ValueError("status must match rows and missing topics")
    if report.reason_codes != _report_reason_codes(
        rows=report.rows,
        missing_topics=report.missing_memory_topics,
    ):
        raise ValueError("reason_codes must match rows and missing topics")
    if report.reason_code_counts != _reason_code_counts(report.rows):
        raise ValueError("reason_code_counts must match rows")


def _require_rows(
    rows: Iterable[ResearchDomainEnergySignalMemoryQualityRow],
) -> tuple[ResearchDomainEnergySignalMemoryQualityRow, ...]:
    ...
    seen: set[str] = set()
    for row in values:
        if type(row) is not ResearchDomainEnergySignalMemoryQualityRow:
            raise ValueError("rows must contain ResearchDomainEnergySignalMemoryQualityRow")
        _require_hard_flags("row", row)
        if row.memory_topic in seen:
            raise ValueError("memory_topic values must be unique")
        seen.add(row.memory_topic)
    return tuple(sorted(values, key=_row_sort_key))


def _require_reason_code_counts(
    values: Iterable[ResearchDomainEnergySignalMemoryQualityReasonCodeCount],
) -> tuple[ResearchDomainEnergySignalMemoryQualityReasonCodeCount, ...]:
    ...
    for value in counts:
        if type(value) is not ResearchDomainEnergySignalMemoryQualityReasonCodeCount:
            raise ValueError(
...
def _require_memory_topics(field_name: str, values: Iterable[str]) -> tuple[str, ...]:
    if isinstance(values, (str, bytes)):
    ...
    seen: set[str] = set()
    for topic in topics:
        _require_memory_topic(field_name, topic)
        if topic in seen:
...
def _require_memory_topic(field_name: str, value: object) -> None:
    _require_public_string(field_name, value)
    if value not in ENERGY_MEMORY_TOPICS:
        raise ValueError(f"{field_name} must be a supported energy memory topic")
...
def _require_report_reason_codes(values: Iterable[str]) -> tuple[str, ...]:
    ...
    for reason_code in reason_codes:
        if (
            not reason_code.startswith("energy_memory_quality_")
...
def _row_sort_key(
    row: ResearchDomainEnergySignalMemoryQualityRow,
) -> tuple[Decimal, Decimal]:
    return (-_status_weight(row.memory_status), _memory_topic_weight(row.memory_topic))
...
def _memory_topic_weight(memory_topic: str) -> Decimal:
    return Decimal(ENERGY_MEMORY_TOPICS.index(memory_topic))
