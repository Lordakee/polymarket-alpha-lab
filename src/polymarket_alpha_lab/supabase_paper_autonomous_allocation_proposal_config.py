"""Environment boundary for paper autonomous allocation proposal persistence."""
...
class SupabasePaperAutonomousAllocationProposalConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_TABLE
...
def from_paper_autonomous_allocation_proposal_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperAutonomousAllocationProposalConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_TABLE,
    )
    try:
        return SupabasePaperAutonomousAllocationProposalConfig(
            enabled=enabled,
            dsn=dsn,
