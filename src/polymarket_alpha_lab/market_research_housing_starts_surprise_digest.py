"""Pure Phase 1 housing-starts surprise digest reducer."""
...
DEFAULT_MARKET_RESEARCH_HOUSING_STARTS_SURPRISE_DIGEST_CONFIG_VERSION = (
    "market-research-housing-starts-surprise-digest-v0"
)
...
REASON_PREFIX = "market_research_housing_starts_surprise_digest_"
...
NEXT_STEPS = {
    STATUS_READY: "allow_report_only_market_research_housing_starts_surprise_digest",
    STATUS_WATCH: "watch_report_only_market_research_housing_starts_surprise_digest",
    STATUS_BLOCKED: "block_report_only_market_research_housing_starts_surprise_digest",
}
...
class MarketResearchHousingStartsSurpriseDigestConfig:
    config_version: str = (
        DEFAULT_MARKET_RESEARCH_HOUSING_STARTS_SURPRISE_DIGEST_CONFIG_VERSION
    )
    fresh_release_max_age_seconds: Decimal = Decimal("7200.000000")
    min_source_count: Decimal = Decimal("2")
    material_surprise_threshold_units: Decimal = Decimal("50000.000000")
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        if cls is not MarketResearchHousingStartsSurpriseDigestConfig:
            raise TypeError(
                "MarketResearchHousingStartsSurpriseDigestConfig does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchHousingStartsSurpriseDigestConfig:
            raise ValueError(
        ...
        object.__setattr__(
            self,
            "material_surprise_threshold_units",
            _require_positive_decimal(
                "material_surprise_threshold_units",
                self.material_surprise_threshold_units,
            ),
        )
        object.__setattr__(
            self,
...
@dataclass(frozen=True)
class MarketResearchHousingStartsSurpriseDigestInputRow:
    research_key: str
    condition_id: str
    market_slug: str
    housing_report_key: str
    housing_report_family: str
    housing_report_reference: str
    released_at: datetime
    acknowledged_at: datetime | None
    source_count: Decimal
    forecast_starts_units: Decimal
    actual_starts_units: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchHousingStartsSurpriseDigestInputRow:
            raise TypeError(
                "MarketResearchHousingStartsSurpriseDigestInputRow does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchHousingStartsSurpriseDigestInputRow:
            raise ValueError(
        ...
        _require_public_string("housing_report_key", self.housing_report_key)
        _require_public_string("housing_report_family", self.housing_report_family)
        _require_reference("housing_report_reference", self.housing_report_reference)
        ...
        for field_name in ("forecast_starts_units", "actual_starts_units"):
            object.__setattr__(
                self,
                field_name,
...
@dataclass(frozen=True)
class MarketResearchHousingStartsSurpriseDigestRow:
    research_key: str
    condition_id: str
    market_slug: str
    housing_report_key: str
    ...
    forecast_starts_units: Decimal
    actual_starts_units: Decimal
    housing_starts_surprise_units: Decimal
    surprise_score: Decimal
    redacted_housing_report_reference: str
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchHousingStartsSurpriseDigestRow:
            raise TypeError(
                "MarketResearchHousingStartsSurpriseDigestRow does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchHousingStartsSurpriseDigestRow:
            raise ValueError(
                "row must be exactly MarketResearchHousingStartsSurpriseDigestRow",
            )
        _require_public_string("research_key", self.research_key)
        _require_public_string("condition_id", self.condition_id)
        _require_market_slug("market_slug", self.market_slug)
        ...
        for field_name in (
            "forecast_starts_units",
            "actual_starts_units",
            "housing_starts_surprise_units",
        ):
            object.__setattr__(
                self,
                field_name,
        ...
        object.__setattr__(
            self,
            "redacted_housing_report_reference",
            _require_redacted_reference(
                "redacted_housing_report_reference",
                self.redacted_housing_report_reference,
            ),
        )
        object.__setattr__(
            self,
...
@dataclass(frozen=True)
class MarketResearchHousingStartsSurpriseDigestReasonCodeCount:
    reason_code: str
    count: Decimal
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchHousingStartsSurpriseDigestReasonCodeCount:
            raise TypeError(
                "MarketResearchHousingStartsSurpriseDigestReasonCodeCount does not "
                "support subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchHousingStartsSurpriseDigestReasonCodeCount:
            raise ValueError(
...
@dataclass(frozen=True)
class MarketResearchHousingStartsSurpriseDigestReport:
    generated_at: datetime
    config_version: str
    digest_status: str
    recommended_next_step: str
    housing_report_count: Decimal
    ...
    rows: tuple[MarketResearchHousingStartsSurpriseDigestRow, ...]
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        super().__init_subclass__(**kwargs)
        if cls is not MarketResearchHousingStartsSurpriseDigestReport:
            raise TypeError(
                "MarketResearchHousingStartsSurpriseDigestReport does not support "
                "subclassing",
            )

    def __post_init__(self) -> None:
        if type(self) is not MarketResearchHousingStartsSurpriseDigestReport:
            raise ValueError(
                "report must be exactly MarketResearchHousingStartsSurpriseDigestReport",
            )
        object.__setattr__(
            self,
            "generated_at",
...
def build_market_research_housing_starts_surprise_digest(
    input_rows: list[MarketResearchHousingStartsSurpriseDigestInputRow]
    | tuple[MarketResearchHousingStartsSurpriseDigestInputRow, ...],
    *,
    config: MarketResearchHousingStartsSurpriseDigestConfig,
    generated_at: datetime,
) -> MarketResearchHousingStartsSurpriseDigestReport:
    if type(config) is not MarketResearchHousingStartsSurpriseDigestConfig:
        raise ValueError(
            "config must be a MarketResearchHousingStartsSurpriseDigestConfig",
        )
    ...
    housing_report_count = _count(len(ordered_rows))
    ...
    return MarketResearchHousingStartsSurpriseDigestReport(
        generated_at=generated_at_utc,
        config_version=config.config_version,
        digest_status=digest_status,
        recommended_next_step=NEXT_STEPS[digest_status],
        housing_report_count=housing_report_count,
...
def market_research_housing_starts_surprise_digest_payload(
    report: MarketResearchHousingStartsSurpriseDigestReport,
) -> dict[str, Any]:
    if type(report) is not MarketResearchHousingStartsSurpriseDigestReport:
        raise ValueError(
            "report must be a MarketResearchHousingStartsSurpriseDigestReport",
        )
...
def _normalize_input_rows(
    ...
    for input_row in normalized:
        if type(input_row) is not MarketResearchHousingStartsSurpriseDigestInputRow:
            raise ValueError(
                "input rows must contain "
                "MarketResearchHousingStartsSurpriseDigestInputRow values",
            )
        _require_hard_flags("input row", input_row)
        key = (input_row.research_key, input_row.condition_id, input_row.housing_report_key)
        if key in seen:
...
def _build_row(
    input_row: MarketResearchHousingStartsSurpriseDigestInputRow,
    *,
    config: MarketResearchHousingStartsSurpriseDigestConfig,
    generated_at: datetime,
) -> MarketResearchHousingStartsSurpriseDigestRow:
    ...
    surprise_units = _finite_decimal(
        input_row.actual_starts_units - input_row.forecast_starts_units,
    )
    reason_codes = _row_reason_codes(
        source_count=input_row.source_count,
        surprise_units=surprise_units,
        release_age_seconds=release_age_seconds,
        acknowledgement_lag_seconds=acknowledgement_lag_seconds,
        config=config,
    )
    return MarketResearchHousingStartsSurpriseDigestRow(
        research_key=input_row.research_key,
        condition_id=input_row.condition_id,
        market_slug=input_row.market_slug,
        housing_report_key=input_row.housing_report_key,
        housing_report_family=input_row.housing_report_family,
        ...
        acknowledgement_lag_seconds=acknowledgement_lag_seconds,
        source_count=input_row.source_count,
        forecast_starts_units=input_row.forecast_starts_units,
        actual_starts_units=input_row.actual_starts_units,
        housing_starts_surprise_units=surprise_units,
        surprise_score=input_row.surprise_score,
        redacted_housing_report_reference=_redacted_reference(
            input_row.housing_report_reference,
        ),
        reason_codes=reason_codes,
    )
...
def _row_reason_codes(
    *,
    source_count: Decimal,
    surprise_units: Decimal,
    release_age_seconds: Decimal,
    acknowledgement_lag_seconds: Decimal | None,
    config: MarketResearchHousingStartsSurpriseDigestConfig,
) -> tuple[str, ...]:
    reasons: list[str] = []
    if abs(surprise_units) >= config.material_surprise_threshold_units:
...
def _ranked_rows(
    rows: tuple[MarketResearchHousingStartsSurpriseDigestRow, ...],
) -> tuple[MarketResearchHousingStartsSurpriseDigestRow, ...]:
    return tuple(sorted(rows, key=_row_sort_key))


def _row_sort_key(
    row: MarketResearchHousingStartsSurpriseDigestRow,
) -> tuple[int, str, str]:
    return (
        {STATUS_BLOCKED: 0, STATUS_WATCH: 1, STATUS_READY: 2}[row.surprise_status],
        row.housing_report_key,
        row.research_key,
    )
...
def _reason_code_counts(
    rows: tuple[MarketResearchHousingStartsSurpriseDigestRow, ...],
) -> tuple[MarketResearchHousingStartsSurpriseDigestReasonCodeCount, ...]:
    counts: dict[str, int] = {}
    for row in rows:
...
def _normalize_rows(
    ...
    for row in normalized:
        if type(row) is not MarketResearchHousingStartsSurpriseDigestRow:
            raise ValueError(
                "rows must contain MarketResearchHousingStartsSurpriseDigestRow values",
            )
        _require_hard_flags("row", row)
        identity = (row.research_key, row.condition_id, row.housing_report_key)
        if identity in seen:
...
def _normalize_reason_code_counts(
    ...
    previous_order = -1
    for count in counts:
        if type(count) is not MarketResearchHousingStartsSurpriseDigestReasonCodeCount:
...
def _validate_row(row: MarketResearchHousingStartsSurpriseDigestRow) -> None:
    ...
    expected_units = _finite_decimal(row.actual_starts_units - row.forecast_starts_units)
    if row.housing_starts_surprise_units != expected_units:
        raise ValueError(
            "housing_starts_surprise_units must match actual minus forecast starts",
        )
...
def _validate_report(report: MarketResearchHousingStartsSurpriseDigestReport) -> None:
    if report.housing_report_count != _count(len(report.rows)):
        raise ValueError("housing_report_count must match rows")
