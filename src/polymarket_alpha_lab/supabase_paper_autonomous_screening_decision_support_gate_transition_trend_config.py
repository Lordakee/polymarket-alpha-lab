"""Environment boundary for paper autonomous screening transition trend persistence."""
...
class SupabasePaperAutonomousScreeningDecisionSupportGateTransitionTrendConfig:
    enabled: bool
    dsn: str | None
    table_name: str = (
        DEFAULT_PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_DB_TABLE
    ...
    def __post_init__(self) -> None:
        ...
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_DB_DSN_ENV_VAR} "
...
def from_paper_autonomous_screening_decision_support_gate_transition_trend_db_env(
    ...
    try:
        return SupabasePaperAutonomousScreeningDecisionSupportGateTransitionTrendConfig(
            enabled=enabled,
            dsn=dsn,
        ...
        if str(exc) == "table_name must be a simple lowercase identifier":
            raise ValueError(
                f"{PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_DB_TABLE_ENV_VAR} "
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_DB_ENABLED_ENV_VAR} "
