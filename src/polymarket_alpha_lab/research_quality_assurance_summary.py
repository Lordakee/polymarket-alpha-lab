"""Report-only research quality assurance summary.

This module combines already-redacted research quality signals into a
deterministic summary for human review. It does not fetch, persist, or mutate
external state.
"""
...
__all__ = (
    "DEFAULT_RESEARCH_QUALITY_ASSURANCE_SUMMARY_CONFIG_VERSION",
    "ResearchQualityAssuranceSummaryConfig",
    "ResearchQualityAssuranceComponentSignal",
    "ResearchQualityAssuranceSummaryRow",
    "ResearchQualityAssuranceSummaryReport",
    "build_research_quality_assurance_summary",
    "research_quality_assurance_summary_payload",
)


DEFAULT_RESEARCH_QUALITY_ASSURANCE_SUMMARY_CONFIG_VERSION = (
    "research-quality-assurance-summary-v0"
)
...
_COMPONENT_RANK = {
    "evidence_packet": Decimal("1.000000"),
    "source_quality_matrix": Decimal("2.000000"),
    "parsing_quality_matrix": Decimal("3.000000"),
    "manual_review_checklist": Decimal("4.000000"),
}

PASS_REASON = "research_quality_assurance_pass"
WATCH_REASON = "research_quality_assurance_watch"
BLOCK_REASON = "research_quality_assurance_block"
MISSING_COMPONENT_REASON = "missing_required_component"
LOW_COMPONENT_QUALITY_WATCH_REASON = "low_component_quality_watch"
LOW_COMPONENT_QUALITY_BLOCK_REASON = "low_component_quality_block"
COMPONENT_ISSUE_WATCH_REASON = "component_issue_watch"
COMPONENT_ISSUE_BLOCK_REASON = "component_issue_block"
...
ROW_REASON_CODES = (
    ...
    LOW_COMPONENT_QUALITY_BLOCK_REASON,
    COMPONENT_ISSUE_WATCH_REASON,
    INPUT_STATUS_WATCH_REASON,
    LOW_COMPONENT_QUALITY_WATCH_REASON,
    PASS_REASON,
)
REPORT_REASON_CODES = (
...
class ResearchQualityAssuranceSummaryConfig(_FinalPublicDataclass):
    config_version: str = DEFAULT_RESEARCH_QUALITY_ASSURANCE_SUMMARY_CONFIG_VERSION
    min_pass_component_score: Decimal = Decimal("0.750000")
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchQualityAssuranceSummaryConfig, "config")
        _require_public_identifier("config_version", self.config_version)
        if self.config_version != DEFAULT_RESEARCH_QUALITY_ASSURANCE_SUMMARY_CONFIG_VERSION:
        ...
        object.__setattr__(
            ...
            _require_ratio_decimal(
                "min_watch_component_score",
                self.min_watch_component_score,
            ),
        )
        if self.min_watch_component_score > self.min_pass_component_score:
            raise ValueError("min_watch_component_score must not exceed min_pass_component_score")
        _require_hard_flags("config", self)
        _reject_unsafe_public_payload("config", self)


@dataclass(frozen=True)
class ResearchQualityAssuranceComponentSignal(_FinalPublicDataclass):
    component: str
    status: str
    quality_score: Decimal
    checked_item_count: Decimal
    issue_count: Decimal
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchQualityAssuranceComponentSignal, "component signal")
        object.__setattr__(
            self,
            "component",
        ...
        if self.issue_count > self.checked_item_count:
            raise ValueError("issue_count must not exceed checked_item_count")
        _require_hard_flags("component signal", self)
        _reject_unsafe_public_payload("component signal", self)


@dataclass(frozen=True)
class ResearchQualityAssuranceSummaryRow(_FinalPublicDataclass):
    component_key: str
    component: str
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchQualityAssuranceSummaryRow, "row")
...
@dataclass(frozen=True)
class ResearchQualityAssuranceSummaryReport(_FinalPublicDataclass):
    config_version: str
    status: str
    ...
    watch_count: Decimal
    block_count: Decimal
    minimum_component_quality_score: Decimal
    average_component_quality_score: Decimal
    total_checked_item_count: Decimal
    total_issue_count: Decimal
    reason_codes: tuple[str, ...]
    rows: tuple[ResearchQualityAssuranceSummaryRow, ...]
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, ResearchQualityAssuranceSummaryReport, "report")
        _require_public_identifier("config_version", self.config_version)
        if self.config_version != DEFAULT_RESEARCH_QUALITY_ASSURANCE_SUMMARY_CONFIG_VERSION:
            raise ValueError("config_version must be the supported config version")
        object.__setattr__(self, "status", _require_member("status", self.status, STATUSES))
        ...
        for field_name in (
            "minimum_component_quality_score",
            "average_component_quality_score",
        ):
            object.__setattr__(
                self,
                field_name,
    ...
    def payload(self) -> dict[str, object]:
        return research_quality_assurance_summary_payload(self)
...
def build_research_quality_assurance_summary(
    components: Sequence[ResearchQualityAssuranceComponentSignal],
    *,
    config: ResearchQualityAssuranceSummaryConfig,
) -> ResearchQualityAssuranceSummaryReport:
    """Build a deterministic report-only summary from redacted QA signals."""

    if type(config) is not ResearchQualityAssuranceSummaryConfig:
        raise ValueError("config must be a ResearchQualityAssuranceSummaryConfig")
    _require_hard_flags("config", config)
    _reject_unsafe_public_payload("config", config)
    items = _normalize_component_signals(components)
    ...
    return ResearchQualityAssuranceSummaryReport(
        ...
        minimum_component_quality_score=min(
            (row.quality_score for row in rows),
            default=_ZERO,
        ),
        average_component_quality_score=_average_score(
            tuple(row.quality_score for row in rows),
        ),
        total_checked_item_count=sum((row.checked_item_count for row in rows), _ZERO),
...
def research_quality_assurance_summary_payload(
    value: ResearchQualityAssuranceSummaryReport | dict[str, object],
) -> dict[str, object]:
    if type(value) is ResearchQualityAssuranceSummaryReport:
        ...
        raise ValueError("value must be a ResearchQualityAssuranceSummaryReport or dict")
...
def _row_from_component(
    component: str,
    signal: ResearchQualityAssuranceComponentSignal,
    *,
    config: ResearchQualityAssuranceSummaryConfig,
    index: int,
) -> ResearchQualityAssuranceSummaryRow:
    ...
    return ResearchQualityAssuranceSummaryRow(
        component_key=f"redacted-research-check-{index:03d}",
        component=component,
        component_rank=_COMPONENT_RANK[component],
        status=status,
        status_rank=_STATUS_RANK[status],
        quality_score=signal.quality_score,
        checked_item_count=signal.checked_item_count,
...
def _row_reason_codes(
    ...
    *,
    ...
    if signal.quality_score < config.min_watch_component_score:
        reasons.append(LOW_COMPONENT_QUALITY_BLOCK_REASON)
    elif signal.quality_score < config.min_pass_component_score:
        reasons.append(LOW_COMPONENT_QUALITY_WATCH_REASON)
    reasons.extend(f"input_{reason_code}" for reason_code in signal.reason_codes)
    if not reasons:
        reasons.append(PASS_REASON)
    return _normalize_ordered_reason_codes(tuple(reasons), ROW_REASON_CODES)
...
def _report_reason_codes(
    ...
    if any(row.status == "block" for row in rows):
        ...
        return _normalize_ordered_reason_codes((*row_reasons, BLOCK_REASON), REPORT_REASON_CODES)
    if any(row.status == "watch" for row in rows):
        row_reasons = tuple(
            reason_code for row in rows for reason_code in row.reason_codes
            if reason_code != PASS_REASON
        )
        return _normalize_ordered_reason_codes((*row_reasons, WATCH_REASON), REPORT_REASON_CODES)
    return (PASS_REASON,)
...
def _normalize_component_signals(
    value: Sequence[ResearchQualityAssuranceComponentSignal],
) -> tuple[ResearchQualityAssuranceComponentSignal, ...]:
    if type(value) not in (list, tuple):
        raise ValueError("components must be a list or tuple")
    items = tuple(value)
    for item in items:
        if type(item) is not ResearchQualityAssuranceComponentSignal:
            raise ValueError(
                "components must contain ResearchQualityAssuranceComponentSignal items",
            )
...
def _component_signal_map(
    items: tuple[ResearchQualityAssuranceComponentSignal, ...],
) -> dict[str, ResearchQualityAssuranceComponentSignal]:
    signal_by_component: dict[str, ResearchQualityAssuranceComponentSignal] = {}
...
def _normalize_rows(
    value: Sequence[ResearchQualityAssuranceSummaryRow],
) -> tuple[ResearchQualityAssuranceSummaryRow, ...]:
    if type(value) not in (list, tuple):
        raise ValueError("rows must be a list or tuple")
    rows = tuple(value)
    seen: set[str] = set()
    for row in rows:
        if type(row) is not ResearchQualityAssuranceSummaryRow:
            raise ValueError("rows must contain ResearchQualityAssuranceSummaryRow items")
...
def _validate_row(row: ResearchQualityAssuranceSummaryRow) -> None:
    ...
    if row.issue_count > row.checked_item_count:
        raise ValueError("issue_count must not exceed checked_item_count")
    if row.status != _row_status(row.reason_codes):
        raise ValueError("status must match reason_codes")


def _validate_report(report: ResearchQualityAssuranceSummaryReport) -> None:
    ...
    expected_min = min((row.quality_score for row in report.rows), default=_ZERO)
    if report.minimum_component_quality_score != expected_min:
        raise ValueError("minimum_component_quality_score must match rows")
    if report.average_component_quality_score != _average_score(
        tuple(row.quality_score for row in report.rows),
    ):
        raise ValueError("average_component_quality_score must match rows")
    if report.total_checked_item_count != sum(
        (row.checked_item_count for row in report.rows),
        _ZERO,
    ):
        raise ValueError("total_checked_item_count must match rows")
...
def _report_payload(report: ResearchQualityAssuranceSummaryReport) -> dict[str, object]:
    return _json_ready(asdict(report))


def _report_digest(report: ResearchQualityAssuranceSummaryReport) -> str:
...
def _normalize_ordered_reason_codes(
    ...
    *,
    ...
    input_reasons = sorted(
        ...
        if type(reason_code) is str and reason_code.startswith("input_")
    )
    terminal_reasons = {PASS_REASON, WATCH_REASON, BLOCK_REASON}
    for reason_code in allowed:
        if (
            reason_code in value
            and reason_code in allowed_set
            and reason_code not in terminal_reasons
        ):
            normalized.append(reason_code)
    for reason_code in input_reasons:
        _require_public_identifier(field_name, reason_code)
        normalized.append(reason_code)
    for reason_code in allowed:
        if reason_code in value and reason_code in terminal_reasons:
            normalized.append(reason_code)
    if not normalized:
