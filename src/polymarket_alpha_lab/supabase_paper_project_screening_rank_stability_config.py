"""Environment boundary for paper project screening rank stability DB persistence."""
...
PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_DSN_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_DSN"
)
PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE"
)
DEFAULT_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE = (
    DEFAULT_PAPER_PROJECT_SCREENING_RANK_STABILITY_REPORTS_TABLE
)
...
class SupabasePaperProjectScreeningRankStabilityConfig:
    enabled: bool
    dsn: str | None
    table_name: str = DEFAULT_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE
...
def from_paper_project_screening_rank_stability_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperProjectScreeningRankStabilityConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(
        source.get(PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_ENABLED_ENV_VAR, ""),
    )
    dsn = source.get(PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_DSN_ENV_VAR)
    table_name = source.get(
        PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE_ENV_VAR,
        DEFAULT_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_TABLE,
    )
    try:
        return SupabasePaperProjectScreeningRankStabilityConfig(
            enabled=enabled,
            dsn=dsn,
...
def _normalize_optional_dsn(value: object) -> str | None:
    ...
    if type(value) is not str:
        raise ValueError(
            f"{PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_DSN_ENV_VAR} must be a string",
        )
