"""Optional psycopg adapter for strategy candidate research queue history reports."""
...
from polymarket_alpha_lab.strategy_candidate_research_queue_history_store import (
...
def insert_paper_strategy_candidate_research_queue_history_report_with_psycopg(
    dsn: str,
    report: Any,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: insert_paper_strategy_candidate_research_queue_history_report(
            connection,
            report,
...
def load_paper_strategy_candidate_research_queue_history_reports_with_psycopg(
    dsn: str,
    *,
    ...
    return _with_owned_connection(
        dsn,
        lambda connection: load_paper_strategy_candidate_research_queue_history_reports(
            connection,
...
def _connect(dsn: str) -> Any:
    try:
        ...
        raise RuntimeError(
            ...
            "history psycopg adapter; install the postgres extra.",
        ) from exc
    try:
        return psycopg.connect(dsn)
    except Exception:
        raise RuntimeError(
            "failed to connect to the strategy candidate research queue "
            "history database",
        ) from None
...
__all__ = (
    ...
    "load_paper_strategy_candidate_research_queue_history_reports_with_psycopg",
)
