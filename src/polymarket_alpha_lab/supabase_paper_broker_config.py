"""Environment boundary for paper broker execution record DB persistence."""
...
PAPER_BROKER_DB_ENABLED_ENV_VAR = "POLYMARKET_ALPHA_LAB_PAPER_BROKER_DB_ENABLED"
PAPER_BROKER_DB_DSN_ENV_VAR = "POLYMARKET_ALPHA_LAB_PAPER_BROKER_DB_DSN"
PAPER_BROKER_DB_TABLE_ENV_VAR = "POLYMARKET_ALPHA_LAB_PAPER_BROKER_DB_TABLE"
DEFAULT_PAPER_BROKER_DB_TABLE = "paper_broker_execution_records"
...
class SupabasePaperBrokerConfig:
    enabled: bool
    dsn: str | None
    ...
    def __post_init__(self) -> None:
        ...
        if self.dsn is not None:
            validate_local_postgres_dsn(
                self.dsn,
                env_var_name=PAPER_BROKER_DB_DSN_ENV_VAR,
            )
        if self.enabled and self.dsn is None:
            raise ValueError(
                f"{PAPER_BROKER_DB_DSN_ENV_VAR} must be set when DB is enabled",
            )
...
def from_paper_broker_db_env(
    env: Mapping[str, str] | None = None,
) -> SupabasePaperBrokerConfig:
    source = environ if env is None else env
    enabled = _parse_enabled(source.get(PAPER_BROKER_DB_ENABLED_ENV_VAR, ""))
    dsn = source.get(PAPER_BROKER_DB_DSN_ENV_VAR)
...
def _parse_enabled(value: object) -> bool:
    if type(value) is not str:
        raise ValueError(f"{PAPER_BROKER_DB_ENABLED_ENV_VAR} must be true or false")
...
def _normalize_optional_dsn(value: object) -> str | None:
    if value is None:
        return None
    if type(value) is not str:
        raise ValueError(f"{PAPER_BROKER_DB_DSN_ENV_VAR} must be a string")
