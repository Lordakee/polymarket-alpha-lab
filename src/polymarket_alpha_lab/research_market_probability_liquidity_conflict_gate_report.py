"""Readonly probability and liquidity conflict gate report."""
...
DEFAULT_RESEARCH_MARKET_PROBABILITY_LIQUIDITY_CONFLICT_GATE_CONFIG_VERSION = (
    "research-market-probability-liquidity-conflict-gate-v0"
)
...
PASS_REASON = "conflict_gate_pass"
WATCH_REASON = "conflict_gate_watch"
BLOCK_REASON = "conflict_gate_block"
...
LIQUIDITY_QUALITY_WATCH_REASON = "liquidity_quality_watch"
LIQUIDITY_QUALITY_BLOCK_REASON = "liquidity_quality_block"
...
CONFLICT_SCORE_WATCH_REASON = "conflict_score_watch"
CONFLICT_SCORE_BLOCK_REASON = "conflict_score_block"
...
ROW_REASON_CODES = (
    ...
    DEPTH_WATCH_REASON,
    DEPTH_BLOCK_REASON,
    LIQUIDITY_QUALITY_WATCH_REASON,
    LIQUIDITY_QUALITY_BLOCK_REASON,
    FEE_DRAG_WATCH_REASON,
    FEE_DRAG_BLOCK_REASON,
    VOLATILITY_WATCH_REASON,
    VOLATILITY_BLOCK_REASON,
    CONFLICT_SCORE_WATCH_REASON,
    CONFLICT_SCORE_BLOCK_REASON,
)
...
class MarketProbabilityLiquidityConflictGateConfig:
    watch_probability_gap: Decimal = Decimal("0.060000")
    block_probability_gap: Decimal = Decimal("0.250000")
    watch_spread_width: Decimal = Decimal("0.020000")
    block_spread_width: Decimal = Decimal("0.050000")
    watch_depth_score: Decimal = Decimal("0.600000")
    block_depth_score: Decimal = Decimal("0.300000")
    watch_liquidity_score: Decimal = Decimal("0.600000")
    block_liquidity_score: Decimal = Decimal("0.350000")
    watch_fee_drag: Decimal = Decimal("0.015000")
    block_fee_drag: Decimal = Decimal("0.030000")
    watch_volatility_score: Decimal = Decimal("0.600000")
    block_volatility_score: Decimal = Decimal("0.800000")
    watch_conflict_score: Decimal = Decimal("0.150000")
    block_conflict_score: Decimal = Decimal("0.600000")
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True

    def __post_init__(self) -> None:
        _require_exact_type(self, MarketProbabilityLiquidityConflictGateConfig, "config")
        for field_name in (
            ...
            "watch_depth_score",
            "block_depth_score",
            "watch_liquidity_score",
            "block_liquidity_score",
            "watch_fee_drag",
            "block_fee_drag",
        ...
        if self.block_liquidity_score >= self.watch_liquidity_score:
            raise ValueError("block_liquidity_score must be below watch_liquidity_score")
...
@dataclass(frozen=True)
class MarketProbabilityLiquidityConflictGateSignal:
    case_digest: str
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, MarketProbabilityLiquidityConflictGateSignal, "signal")
...
@dataclass(frozen=True)
class MarketProbabilityLiquidityConflictGateRow:
    case_digest: str
    model_probability: Decimal
    market_probability: Decimal
    ...
    spread_width: Decimal
    fee_drag: Decimal
    adjusted_probability_gap: Decimal
    depth_score: Decimal
    liquidity_score: Decimal
    ...
    validation_config: InitVar[
        MarketProbabilityLiquidityConflictGateConfig | None
    ] = None
    ...
    def __post_init__(
        self,
        validation_config: MarketProbabilityLiquidityConflictGateConfig | None,
    ) -> None:
        _require_exact_type(self, MarketProbabilityLiquidityConflictGateRow, "row")
        active_validation_config = (
            MarketProbabilityLiquidityConflictGateConfig()
            if validation_config is None
            else validation_config
        )
        if type(active_validation_config) is not MarketProbabilityLiquidityConflictGateConfig:
            raise ValueError(
                "validation_config must be MarketProbabilityLiquidityConflictGateConfig",
            )
        require_paper_only_flags("validation_config", active_validation_config)
        _require_digest("case_digest", self.case_digest)
        for field_name in (
            "model_probability",
            "market_probability",
...
@dataclass(frozen=True)
class MarketProbabilityLiquidityConflictGateReasonCodeCount:
    reason_code: str
    row_count: Decimal
    paper_only: bool = True
    report_only: bool = True
...
@dataclass(frozen=True)
class MarketProbabilityLiquidityConflictGateReport:
    ...
    max_adjusted_probability_gap: Decimal
    average_liquidity_shortfall_score: Decimal
    max_conflict_score: Decimal
    rows: tuple[MarketProbabilityLiquidityConflictGateRow, ...]
    reason_code_counts: tuple[MarketProbabilityLiquidityConflictGateReasonCodeCount, ...]
    derived_validation_digest: str = ""
    paper_only: bool = True
    report_only: bool = True
    readonly: bool = True
    ...
    def __post_init__(self) -> None:
        _require_exact_type(self, MarketProbabilityLiquidityConflictGateReport, "report")
        object.__setattr__(
            self,
            "generated_at",
        ...
        for field_name in (
            ...
            "max_adjusted_probability_gap",
            "average_liquidity_shortfall_score",
            "max_conflict_score",
        ):
            object.__setattr__(
                self,
...
def build_research_market_probability_liquidity_conflict_gate_report(
    signals: Iterable[MarketProbabilityLiquidityConflictGateSignal],
    *,
    generated_at: datetime,
    config: MarketProbabilityLiquidityConflictGateConfig | None = None,
) -> MarketProbabilityLiquidityConflictGateReport:
    generated_at_utc = _as_utc("generated_at", generated_at)
    active_config = (
        MarketProbabilityLiquidityConflictGateConfig() if config is None else config
    )
    if type(active_config) is not MarketProbabilityLiquidityConflictGateConfig:
        raise ValueError("config must be MarketProbabilityLiquidityConflictGateConfig")
    ...
    return MarketProbabilityLiquidityConflictGateReport(
        generated_at=generated_at_utc,
        config_version=(
            DEFAULT_RESEARCH_MARKET_PROBABILITY_LIQUIDITY_CONFLICT_GATE_CONFIG_VERSION
        ),
        gate_status=_report_status(report_rows),
        ...
        average_probability_gap=_mean_decimal(row.probability_gap for row in report_rows),
        max_adjusted_probability_gap=max(
            (row.adjusted_probability_gap for row in report_rows),
            default=ZERO,
        ),
        average_liquidity_shortfall_score=_mean_decimal(
            row.liquidity_shortfall_score for row in report_rows
        ),
        max_conflict_score=max((row.conflict_score for row in report_rows), default=ZERO),
        rows=report_rows,
        reason_code_counts=_reason_code_counts(report_rows),
    )


def research_market_probability_liquidity_conflict_gate_report_payload(
    report: MarketProbabilityLiquidityConflictGateReport | Mapping[str, Any],
) -> "FrozenJsonObject":
    if type(report) is MarketProbabilityLiquidityConflictGateReport:
        require_paper_only_flags("report", report)
        _reject_private_reference_fields("report", report)
        reject_unsafe_surface_fields("report", report)
        payload = json_ready_no_floats(report)
    elif type(report) is dict:
        _reject_private_reference_fields("payload", report)
        reject_unsafe_surface_fields("payload", report)
        _reject_public_numerics(report)
        require_paper_only_flags("payload", _MappingFlags(report))
        payload = json_ready_no_floats(report)
    else:
        raise ValueError(
            "report must be a MarketProbabilityLiquidityConflictGateReport",
        )
...
def _row_from_signal(
    signal: MarketProbabilityLiquidityConflictGateSignal,
    *,
    config: MarketProbabilityLiquidityConflictGateConfig,
) -> MarketProbabilityLiquidityConflictGateRow:
    probability_gap = _quantize(abs(signal.model_probability - signal.market_probability))
    adjusted_probability_gap = _clamp_floor_zero(
        probability_gap - signal.spread_width - signal.fee_drag,
    )
    liquidity_shortfall_score = _quantize(
        (ONE - signal.depth_score + ONE - signal.liquidity_score) / Decimal("2"),
    )
    conflict_score = _conflict_score(
        adjusted_probability_gap=adjusted_probability_gap,
    ...
    return MarketProbabilityLiquidityConflictGateRow(
        ...
        manual_review_required=gate_status != STATUS_PASS,
...
def _conflict_score(
    *,
    ...
    return _clamp_unit(
        (adjusted_probability_gap * Decimal("1.500000"))
        + (liquidity_shortfall_score * Decimal("0.250000"))
        + (volatility_score * Decimal("0.250000")),
    )
...
def _reason_codes_for_values(
    *,
    ...
    for reason in (
        _high_value_reason(
            value=adjusted_probability_gap,
            watch_value=config.watch_probability_gap,
            block_value=config.block_probability_gap,
            watch_reason=PROBABILITY_GAP_WATCH_REASON,
            block_reason=PROBABILITY_GAP_BLOCK_REASON,
        ),
        _high_value_reason(
            value=spread_width,
        ...
        _low_value_reason(
            ...
            watch_value=config.watch_liquidity_score,
            block_value=config.block_liquidity_score,
            watch_reason=LIQUIDITY_QUALITY_WATCH_REASON,
            block_reason=LIQUIDITY_QUALITY_BLOCK_REASON,
        ),
        _high_value_reason(
            value=fee_drag,
        ...
        _high_value_reason(
            ...
            watch_value=config.watch_conflict_score,
            block_value=config.block_conflict_score,
            watch_reason=CONFLICT_SCORE_WATCH_REASON,
            block_reason=CONFLICT_SCORE_BLOCK_REASON,
        ),
    ):
        if reason is None:
            continue
...
def _report_status(rows: tuple[MarketProbabilityLiquidityConflictGateRow, ...]) -> str:
...
def _normalize_signals(
    signals: Iterable[MarketProbabilityLiquidityConflictGateSignal],
) -> tuple[MarketProbabilityLiquidityConflictGateSignal, ...]:
    ...
    for signal in normalized:
        if type(signal) is not MarketProbabilityLiquidityConflictGateSignal:
            raise ValueError(
                "signals must contain MarketProbabilityLiquidityConflictGateSignal",
            )
...
def _normalize_rows(
    rows: object,
) -> tuple[MarketProbabilityLiquidityConflictGateRow, ...]:
    if type(rows) not in (tuple, list):
        raise ValueError("rows must be a tuple or list")
    normalized = tuple(rows)
    seen_case_digests: set[str] = set()
    for row in normalized:
        if type(row) is not MarketProbabilityLiquidityConflictGateRow:
            raise ValueError("rows must contain MarketProbabilityLiquidityConflictGateRow")
...
def _normalize_reason_code_counts(
    rows: object,
) -> tuple[MarketProbabilityLiquidityConflictGateReasonCodeCount, ...]:
    if type(rows) not in (tuple, list):
        raise ValueError("reason_code_counts must be a tuple or list")
    normalized = tuple(rows)
    seen_reason_codes: set[str] = set()
    for row in normalized:
        if type(row) is not MarketProbabilityLiquidityConflictGateReasonCodeCount:
            raise ValueError(
...
def _row_sort_key(
    row: MarketProbabilityLiquidityConflictGateRow,
) -> tuple[int, Decimal, Decimal, str]:
    return (
        STATUS_SORT_RANK[row.gate_status],
        -row.conflict_score,
        -row.adjusted_probability_gap,
        row.case_digest,
    )
...
def _status_count(
    ...
    return _decimal_from_int(sum(1 for row in rows if row.gate_status == status))
...
def _validate_row_consistency(
    ...
    *,
    ...
    if row.probability_gap != _quantize(
        abs(row.model_probability - row.market_probability),
    ):
        raise ValueError("probability_gap must match probabilities")
    expected_adjusted_gap = _clamp_floor_zero(
        row.probability_gap - row.spread_width - row.fee_drag,
    )
    if row.adjusted_probability_gap != expected_adjusted_gap:
        raise ValueError("adjusted_probability_gap must match adjusted values")
    expected_shortfall = _quantize(
        (ONE - row.depth_score + ONE - row.liquidity_score) / Decimal("2"),
    )
    if row.liquidity_shortfall_score != expected_shortfall:
        raise ValueError("liquidity_shortfall_score must match liquidity fields")
    expected_conflict_score = _conflict_score(
        adjusted_probability_gap=row.adjusted_probability_gap,
        liquidity_shortfall_score=row.liquidity_shortfall_score,
        volatility_score=row.volatility_score,
    )
    if row.conflict_score != expected_conflict_score:
        raise ValueError("conflict_score must match row fields")
    ...
    if row.manual_review_required != (row.gate_status != STATUS_PASS):
...
def _validate_report_consistency(
    report: MarketProbabilityLiquidityConflictGateReport,
) -> None:
    expected_values = {
        ...
        "average_probability_gap": _mean_decimal(
            row.probability_gap for row in report.rows
        ),
        "max_adjusted_probability_gap": max(
            (row.adjusted_probability_gap for row in report.rows),
            default=ZERO,
        ),
        "average_liquidity_shortfall_score": _mean_decimal(
            row.liquidity_shortfall_score for row in report.rows
        ),
        "max_conflict_score": max((row.conflict_score for row in report.rows), default=ZERO),
    }
