"""Minimal supplied-input basketball team forecast workflow."""
...
PROMPT_VERSION = "sports-basketball-supplied-input-v0"
TEAM_REASON_CODE = "team_sports_basketball"
MEMORY_REFERENCES = ("basketball_supplied_evidence_only",)
KNOWN_FAILURE_MODES = (
...
class SportsBasketballTeamConfig:
    config_version: str
    market_implied_probability_hint: Decimal = Decimal("0.500000")
    resolution_risk: Decimal = Decimal("0.100000")
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("sports basketball team config", self)


@dataclass(frozen=True)
class SportsBasketballEvidenceInput:
    source_id: str
    source_type: str
    evidence_text: str
    ...
    def __post_init__(self) -> None:
        ...
        require_paper_only_flags("sports basketball evidence input", self)


def build_sports_basketball_team_forecast(
    *,
    condition_id: str,
    market_slug: str,
    question: str,
    event_template: str,
    evidence: Iterable[SportsBasketballEvidenceInput],
    ...
    if type(config) is not SportsBasketballTeamConfig:
        raise ValueError("config must be a SportsBasketballTeamConfig")
    if type(generated_at) is not datetime:
        raise ValueError("generated_at must be a datetime")
    require_paper_only_flags("sports basketball team config", config)
...
def _normalize_evidence(
    evidence: Iterable[SportsBasketballEvidenceInput],
) -> tuple[SportsBasketballEvidenceInput, ...]:
    if isinstance(evidence, (str, bytes)):
        raise ValueError(
            "evidence must be an iterable of SportsBasketballEvidenceInput values",
        )
    try:
    ...
    for item in items:
        if type(item) is not SportsBasketballEvidenceInput:
            raise ValueError(
                "evidence must contain only SportsBasketballEvidenceInput values",
            )
        require_paper_only_flags("sports basketball evidence input", item)
    return items
...
def _build_evidence_packets(
    *,
    condition_id: str,
    market_slug: str,
    evidence: tuple[SportsBasketballEvidenceInput, ...],
...
def _average_weight(evidence: tuple[SportsBasketballEvidenceInput, ...]) -> Decimal:
    with localcontext(DECIMAL_CONTEXT):
        average = sum((item.weight for item in evidence), ZERO) / Decimal(len(evidence))
    return _clamp_probability(average)


def _data_freshness_score(evidence: tuple[SportsBasketballEvidenceInput, ...]) -> Decimal:
    max_freshness_seconds = max(item.data_freshness_seconds for item in evidence)
    if max_freshness_seconds <= 300:
        return Decimal("1.000000")
    if max_freshness_seconds <= 3600:
        return Decimal("0.750000")
    if max_freshness_seconds <= 21600:
