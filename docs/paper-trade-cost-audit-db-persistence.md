...
construction. There is no order submission. There is no exchange mutation.
...
insert or load paper trade cost audit rows only. These paths must not
authenticate exchange clients, read exchange accounts, construct orders, sign
payloads, submit orders, cancel orders, replace orders, or mutate the
exchange.
