...
def api():
    return importlib.import_module(
        "polymarket_alpha_lab.research_packet_source_ack_recheck_queue_report",
    )
...
def _config(**overrides: object):
    module = api()
    values = {
        "stale_after_seconds": d("1800.000000"),
        "repeated_miss_threshold_count": d("2"),
    }
    values.update(overrides)
    return module.ResearchPacketSourceAckRecheckQueueConfig(**values)
...
def _ack(
    ...
    *,
    ...
    module = api()
    return module.ResearchPacketSourceAck(
...
def _update(
    ...
    *,
    ...
    module = api()
    return module.ResearchPacketSourceAckRecheckUpdate(
        update_id=update_id,
        packet_id=packet_id,
        source_id=source_id,
...
def _build_report(*acks, updates=(), config=None):
    module = api()
    return module.build_research_packet_source_ack_recheck_queue_report(
        acks,
        updates,
...
def test_recheck_queue_flags_stale_contradiction_missing_owner_and_repeated_misses() -> None:
    ...
    assert report.queued_acknowledgement_count == d("2")
    assert report.urgent_acknowledgement_count == d("2")
    assert report.stale_acknowledgement_count == d("2")
    assert report.contradiction_count == d("2")
    assert report.missing_owner_count == d("1")
    assert report.later_official_update_count == d("1")
    assert report.later_proxy_update_count == d("1")
    assert report.repeated_team_category_miss_count == d("1")
    assert report.queued_acknowledgement_ratio == d("0.666667")
    ...
    alpha = report.rows[0]
    assert alpha.queue_status == "urgent"
    ...
    assert alpha.later_official_update_count == d("1")
    assert alpha.later_proxy_update_count == d("0")
    assert alpha.stale_acknowledgement is True
    assert alpha.contradiction_after_acknowledgement is True
    ...
    beta = report.rows[1]
    assert beta.queue_status == "urgent"
    ...
    assert clear.reason_codes == ("recheck_not_required",)

    assert report.team_category_miss_rows == (
        api().ResearchPacketSourceAckRecheckTeamCategoryMissRow(
            team_id="politics",
...
def test_rechecked_later_updates_clear_queue_and_sort_deterministically() -> None:
    module = api()
    config = module.ResearchPacketSourceAckRecheckQueueConfig(
        stale_after_seconds=d("99999.000000"),
    )
    ack_a = _ack(
        "ack-a",
        packet_id="packet-a",
    ...
    first = module.build_research_packet_source_ack_recheck_queue_report(
        (ack_a, ack_b),
        (update_b, update_a),
        config=config,
        generated_at=GENERATED_AT,
    )
    second = module.build_research_packet_source_ack_recheck_queue_report(
        (ack_b, ack_a),
        (update_a, update_b),
        config=config,
        generated_at=GENERATED_AT,
    ...
    assert first.queued_acknowledgement_ratio == d("0.000000")
    assert first.reason_codes == ("ack_recheck_queue_clear",)
    assert tuple(row.acknowledgement_id for row in first.rows) == ("ack-b", "ack-a")
    assert all(row.queue_status == "clear" for row in first.rows)
    assert all(row.reason_codes == ("recheck_not_required",) for row in first.rows)


def test_empty_queue_returns_readonly_decimal_report() -> None:
    report = _build_report()

    assert report.status == "empty"
    assert report.acknowledgement_count == d("0")
    assert report.update_count == d("0")
    assert report.queued_acknowledgement_count == d("0")
    assert report.urgent_acknowledgement_count == d("0")
    assert report.stale_acknowledgement_count == d("0")
    assert report.contradiction_count == d("0")
    assert report.missing_owner_count == d("0")
    assert report.later_official_update_count == d("0")
    assert report.later_proxy_update_count == d("0")
    assert report.queued_acknowledgement_ratio == d("0.000000")
    assert report.reason_codes == ("ack_recheck_queue_empty",)
    assert report.rows == ()
    assert report.team_category_miss_rows == ()
    assert report.paper_only is True
    assert report.report_only is True
...
def test_datetimes_decimals_flags_and_uniqueness_are_validated() -> None:
    ...
    assert type(report.update_count) is Decimal
    assert type(report.queued_acknowledgement_ratio) is Decimal
    ...
    with pytest.raises(FrozenInstanceError):
        report.rows[0].queue_status = "urgent"  # type: ignore[misc]
    with pytest.raises(ValueError, match="readonly"):
        replace(report, readonly=False)
    with pytest.raises(ValueError, match="stale_after_seconds must be a Decimal"):
        module.ResearchPacketSourceAckRecheckQueueConfig(
            stale_after_seconds=_DecimalSubclass("60.000000"),
        )
    ...
    with pytest.raises(ValueError, match="generated_at must be timezone-aware"):
        module.build_research_packet_source_ack_recheck_queue_report(
            (_ack("ack-valid"),),
            (),
            config=module.ResearchPacketSourceAckRecheckQueueConfig(),
    ...
    with pytest.raises(ValueError, match="acknowledged_at must be timezone-aware"):
        _ack(
            "ack-missing-offset",
            acknowledged_at=datetime(2026, 7, 2, 14, 0, tzinfo=_MissingOffsetTz()),
        )
    with pytest.raises(ValueError, match="config_version contains unsafe text"):
        module.ResearchPacketSourceAckRecheckQueueConfig(
            config_version="credential-report-v0",
        )
...
def test_payload_helper_is_json_ready_and_contains_no_float_values() -> None:
    ...
    payload = module.research_packet_source_ack_recheck_queue_report_to_payload(report)

    assert payload["generated_at"] == "2026-07-02T16:00:00+00:00"
    assert payload["acknowledgement_count"] == "1"
    assert payload["queued_acknowledgement_ratio"] == "1.000000"
    assert payload["rows"][0]["acknowledgement_age_seconds"] == "7200.000000"
