...
MODULE_PATH = REPO_ROOT / "src" / "polymarket_alpha_lab" / "cost_aware_snapshot_builder.py"
...
def snapshot_builder_exports(exports):
    return tuple(
        name
        for name in exports
        if name.startswith("PaperCostAwareSnapshot")
        or name == "build_paper_cost_aware_event_market_snapshot"
    )
...
def test_cost_aware_snapshot_builder_imports_only_allowed_dependencies():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert module_name in ALLOWED_IMPORT_MODULES, module_name


def test_cost_aware_snapshot_builder_does_not_import_live_or_loader_surfaces():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_cost_aware_snapshot_builder_public_exports_exactly_paper_report_api():
    tree = parse_module()
...
def test_cost_aware_snapshot_builder_does_not_define_forbidden_live_or_advice_surface_names():
    tree = parse_module()
    names = set()
    for node in ast.walk(tree):
...
def test_package_root_exports_snapshot_builder_api_only():
    tree = parse_module(PACKAGE_ROOT_PATH)
    package_exports = module_exports(tree)

    assert snapshot_builder_exports(package_exports) == EXPECTED_EXPORTS
    for name in snapshot_builder_exports(package_exports):
        assert not public_export_fragment_matches(name), name
    ...
    for node in ast.walk(tree):
        if (
            ...
            and node.module == "polymarket_alpha_lab.cost_aware_snapshot_builder"
        ):
            imported_names.update(alias.name for alias in node.names)
    assert imported_names == set(EXPECTED_EXPORTS)


def test_readme_snapshot_builder_sections_keep_paper_boundaries():
    readme = README_PATH.read_text(encoding="utf-8")
    status_start = readme.index("## Cost-Aware Snapshot Builder v0 Status")
    api_start = readme.index("## Cost-Aware Snapshot Builder v0 Python API", status_start)
    ...
    required_fragments = (
        "costawaresnapshotbuilderv0status",
        "costawaresnapshotbuilderv0pythonapi",
        "paperonly",
        "reportonly",
        "alreadybuiltpapercostawareeventmarketsnapshot",
        "nofetch",
        "noauth",
        "nowallet",
        "noorder",
