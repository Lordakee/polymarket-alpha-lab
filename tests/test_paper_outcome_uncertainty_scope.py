...
MODULE_PATH = (
    REPO_ROOT / "src" / "polymarket_alpha_lab" / "paper_outcome_uncertainty.py"
)
...
ALLOWED_FORBIDDEN_NAME_MATCHES = {
    "buildpaperoutcomeuncertaintyreport",
    "paperoutcomeuncertaintyconfig",
    "paperoutcomeuncertaintyinput",
    "paperoutcomeuncertaintyreport",
    "paperoutcomeuncertaintyrow",
...
def test_paper_outcome_uncertainty_imports_only_allowed_dependencies():
    tree = parse_module()
...
def test_paper_outcome_uncertainty_does_not_import_forbidden_surfaces():
    tree = parse_module()
...
def test_paper_outcome_uncertainty_exports_only_report_api():
    tree = parse_module()

    assert module_exports(tree) == EXPECTED_EXPORTS


def test_paper_outcome_uncertainty_defines_no_forbidden_live_names():
    tree = parse_module()
    normalized_names = {
...
def test_paper_outcome_uncertainty_does_not_perform_io_or_dynamic_execution():
    tree = parse_module()
    for node in ast.walk(tree):
