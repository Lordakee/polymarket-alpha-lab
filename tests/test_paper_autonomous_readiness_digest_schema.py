...
def _migration_path() -> Path:
    candidates = sorted(
        MIGRATIONS_DIR.glob("*paper_autonomous_readiness_digest_reports.sql"),
    )
    assert candidates, "paper autonomous readiness digest migration is missing"
...
def _table_body(sql: str) -> str:
    match = re.search(
        ...
        r"public\.paper_autonomous_readiness_digest_reports\s*\((.*?)\);",
        sql,
...
def _digest_status_check() -> str:
    statuses_sql = ", ".join(_sql_string(status) for status in DIGEST_STATUSES)
    return f"check (digest_status in ({statuses_sql}))"


def _next_review_action_check() -> str:
    cases = " ".join(
        "when "
        f"{_sql_string(status)} then "
        f"{_sql_string(NEXT_REVIEW_ACTION_BY_STATUS[status])}"
        for status in DIGEST_STATUSES
    )
    return (
        "check (recommended_next_review_action = case digest_status "
        f"{cases} end)"
    )


def test_readiness_digest_migration_uses_next_safe_version_and_unique_sequence() -> None:
...
def test_readiness_digest_migration_creates_report_table_with_contract_columns() -> None:
...
def test_readiness_digest_migration_constrains_status_action_json_and_flags() -> None:
    body = _without_space_after_open_paren(_table_body(_migration_text()))

    for expected_check in (
        "check (report_sha256 ~ '^[a-f0-9]{64}$')",
        _digest_status_check(),
        _next_review_action_check(),
...
def test_readiness_digest_migration_enforces_json_payload_consistency() -> None:
    body = _table_body(_migration_text())
...
def test_readiness_digest_migration_uses_jsonb_payloads_without_float_columns() -> None:
    body = _table_body(_migration_text())
...
def test_readiness_digest_migration_adds_expected_lookup_indexes() -> None:
    sql = _compact_index_sql(_migration_text())

    for index in (
        "create index if not exists pardr_generated_at_idx on public.paper_autonomous_readiness_digest_reports (generated_at desc);",
        "create index if not exists pardr_status_generated_idx on public.paper_autonomous_readiness_digest_reports (digest_status, generated_at desc);",
        "create index if not exists pardr_config_generated_idx on public.paper_autonomous_readiness_digest_reports (config_version, generated_at desc);",
        "create index if not exists pardr_source_versions_idx on public.paper_autonomous_readiness_digest_reports using gin (source_config_versions_json jsonb_path_ops);",
        "create index if not exists pardr_reason_codes_idx on public.paper_autonomous_readiness_digest_reports using gin (reason_codes_json jsonb_path_ops);",
        "create index if not exists pardr_payload_idx on public.paper_autonomous_readiness_digest_reports using gin (payload_json jsonb_path_ops);",
        "create index if not exists pardr_status_sort_idx on public.paper_autonomous_readiness_digest_reports (digest_status, generated_at desc, inserted_at desc, report_sha256 desc);",
    ):
        assert index in sql


def test_readiness_digest_migration_keeps_table_and_index_names_under_postgres_limit() -> None:
...
def test_readiness_digest_migration_avoids_forbidden_database_surfaces() -> None:
    text = _migration_text()
...
def test_readiness_digest_migration_stays_paper_only_without_live_account_surfaces() -> None:
    text = _migration_text()
