...
import polymarket_alpha_lab.supabase_action_gated_strategy_recommendation_queue_config as queue_config
from polymarket_alpha_lab.supabase_action_gated_strategy_recommendation_queue_config import (
    ...
    DEFAULT_ACTION_GATED_QUEUE_DB_TABLE,
...
def test_disabled_env_config_uses_default_table_and_accepts_absent_dsn() -> None:
    ...
    assert config == SupabaseActionGatedStrategyRecommendationQueueConfig(
        enabled=False,
        dsn=None,
...
def test_enabled_env_config_requires_dsn_without_echoing_secret() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        from_action_gated_strategy_recommendation_queue_db_env(
            {
                ACTION_GATED_QUEUE_DB_ENABLED_ENV_VAR: "true",
...
def test_enabled_env_config_rejects_padded_dsn_without_echoing_it() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        from_action_gated_strategy_recommendation_queue_db_env(
            {
                ACTION_GATED_QUEUE_DB_ENABLED_ENV_VAR: "1",
                ACTION_GATED_QUEUE_DB_DSN_ENV_VAR: f" {secret_dsn} ",
            },
        )
...
def test_enabled_env_config_rejects_non_strict_values(enabled_value: str) -> None:
    ...
    assert ACTION_GATED_QUEUE_DB_ENABLED_ENV_VAR in str(exc_info.value)
...
def test_enabled_env_config_reads_explicit_dsn_at_process_edge() -> None:
    ...
    config = from_action_gated_strategy_recommendation_queue_db_env(
        {
            ACTION_GATED_QUEUE_DB_ENABLED_ENV_VAR: "true",
            ACTION_GATED_QUEUE_DB_DSN_ENV_VAR: dsn,
            ACTION_GATED_QUEUE_DB_TABLE_ENV_VAR: "audit.action_gated_queue_reports",
        },
    )

    assert config.enabled is True
    assert config.dsn == dsn
    assert config.table_name == "audit.action_gated_queue_reports"
...
def test_env_table_name_error_mentions_variable_name() -> None:
    with pytest.raises(ValueError) as exc_info:
        from_action_gated_strategy_recommendation_queue_db_env(
            {
                ACTION_GATED_QUEUE_DB_TABLE_ENV_VAR: "ActionGatedQueueReports",
            },
        )

    assert ACTION_GATED_QUEUE_DB_TABLE_ENV_VAR in str(exc_info.value)
...
def test_migration_defines_action_gated_queue_report_table() -> None:
    ...
    assert "check (jsonb_typeof(reason_code_counts) = 'object')" in sql
    ...
    assert "check (candidate_count = ready_count + watch_count + blocked_count)" in sql
    assert (
        "check (action_status = 'research_ready' "
    ...
    for index_fragment in (
        ...
        "(source_config_version, action_status, generated_at desc, "
        "inserted_at desc, report_sha256 desc)",
    ):
