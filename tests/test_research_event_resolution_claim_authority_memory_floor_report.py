...
MODULE_NAME = (
    ...
    "research_event_resolution_claim_authority_memory_floor_report"
)
MODULE_PATH = (
    Path(__file__).resolve().parents[1]
    / "src"
    / "polymarket_alpha_lab"
    / "research_event_resolution_claim_authority_memory_floor_report.py"
)
...
RAW_PRIVATE_VALUES = (
    ...
    "https://example.test/private-resolution?token=secret-alpha",
    "raw source text says final outcome is yes",
    "postgres://user:pass@example.test:5432/private_claims",
    "resolution_claim_table",
    "wallet=0xabc; order=123; live trading disabled",
)
...
def cfg(**overrides: object) -> Any:
    module = api()
    values = {
        "config_version": "research-event-resolution-claim-authority-memory-floor-report-v0",
    ...
    return module.ResearchEventResolutionClaimAuthorityMemoryFloorConfig(**values)
...
def input_row(
    ...
    *,
    ...
    authority_memory_checked_at: datetime | None = None,
    authority_score: Decimal = d("0.950000"),
    parser_confidence: Decimal = d("0.930000"),
    memory_support_score: Decimal = d("0.900000"),
    resolution_confidence: Decimal = d("0.920000"),
    reason_codes: tuple[str, ...] = (),
) -> Any:
    module = api()
    return module.ResearchEventResolutionClaimAuthorityMemoryFloorInput(
        ...
        private_memory_ref=f"{RAW_PRIVATE_VALUES[1]}-{index}; {RAW_PRIVATE_VALUES[6]}",
        claim_observed_at=(
            claim_observed_at
        ...
        authority_memory_checked_at=(
            authority_memory_checked_at
            if authority_memory_checked_at is not None
...
def build_report(
    ...
    *,
    ...
    module = api()
    return module.build_research_event_resolution_claim_authority_memory_floor_report(
        rows,
...
def test_report_scores_pass_watch_block_without_public_raw_leakage() -> None:
    ...
    result = build_report(
        (
            input_row(
                ...
                claim_bucket="claim-pass",
            ),
            input_row(
                2,
                event_bucket="event-watch",
                claim_bucket="claim-watch",
                authority_memory_checked_at=GENERATED_AT - timedelta(hours=2),
            ...
            input_row(
                ...
                claim_bucket="claim-block",
                authority_memory_checked_at=GENERATED_AT - timedelta(days=2),
                authority_score=d("0.400000"),
                parser_confidence=d("0.450000"),
                memory_support_score=d("0.300000"),
                resolution_confidence=d("0.550000"),
    ...
    assert result.min_memory_floor_score == d("0.300000")
    assert result.average_memory_floor_score == d("0.616667")
    ...
    assert block_row.memory_floor_score == d("0.300000")
    assert block_row.reason_codes == (
        "claim_authority_memory_floor_block",
        "authority_score_block",
        "memory_age_block",
        "memory_support_block",
    ...
    assert watch_row.memory_floor_score == d("0.650000")
    assert watch_row.reason_codes == (
        "claim_authority_memory_floor_watch",
    ...
    assert pass_row.memory_floor_score == d("0.900000")
    assert pass_row.reason_codes == ("claim_authority_memory_floor_pass",)

    assert_public_numeric_values_are_decimal(result)

    payload = module.research_event_resolution_claim_authority_memory_floor_report_payload(
        result,
    )
    payload_again = module.research_event_resolution_claim_authority_memory_floor_report_payload(
    ...
    assert payload["rows"][0]["memory_floor_score"] == "0.300000"
...
def test_empty_report_is_block_readonly_and_decimal_zeroed() -> None:
    ...
    assert result.min_memory_floor_score == ZERO
    assert result.average_memory_floor_score == ZERO
    assert result.max_memory_age_seconds == ZERO
    assert result.status == "block"
    assert result.reason_codes == ("claim_authority_memory_floor_report_empty",)
    assert result.rows == ()
    assert result.paper_only is True
    assert result.report_only is True
    assert result.readonly is True
...
def test_dataclasses_are_frozen_decimal_only_and_validate_inputs() -> None:
    ...
    with pytest.raises(ValueError, match="authority_score"):
        input_row(1, authority_score=DecimalSubclass("0.900000"))
    ...
    with pytest.raises(ValueError, match="authority_memory_checked_at"):
        build_report(
            (
                input_row(
                    1,
                    authority_memory_checked_at=GENERATED_AT + timedelta(seconds=1),
                ),
            ),
        )
    with pytest.raises(ValueError, match="generated_at"):
        module.build_research_event_resolution_claim_authority_memory_floor_report(
            (),
...
def test_tamper_evident_digest_and_payload_validation() -> None:
    module = api()
    result = build_report((input_row(1), input_row(2, authority_score=d("0.650000"))))
    row = result.rows[0]

    with pytest.raises(ValueError, match="memory_floor_score"):
        replace(row, memory_floor_score=row.memory_floor_score + d("0.000001"))
    ...
    with pytest.raises(ValueError, match="readonly"):
        module.research_event_resolution_claim_authority_memory_floor_report_payload(
