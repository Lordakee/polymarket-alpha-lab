...
LOCAL_SUPABASE_DSN = "postgresql://worker:local-secret@localhost:54322/postgres"
REMOTE_SECRET_DSN = (
    "postgresql://worker:remote-token@db.remote.example.invalid:5432/polymarket"
)
STORE_MODULE_NAME = (
    "polymarket_alpha_lab."
    "paper_autonomous_allocation_proposal_db_history_health_store"
)
ADAPTER_MODULE_NAME = (
    "polymarket_alpha_lab."
    "paper_autonomous_allocation_proposal_db_history_health_psycopg"
)
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_AUTONOMOUS_ALLOCATION_PROPOSAL_DB_HISTORY_HEALTH_REPORTS_TABLE = (
        "paper_autonomous_allocation_proposal_db_history_health_reports"
    )
    store_module.insert_paper_autonomous_allocation_proposal_db_history_health_report_with_result = (
        insert_with_result
        if insert_with_result is not None
        else _unexpected_store_call("insert_with_result")
    )
    store_module.load_paper_autonomous_allocation_proposal_db_history_health_reports = (
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_autonomous_allocation_proposal_db_history_health_reports_with_psycopg(
            LOCAL_SUPABASE_DSN,
        )
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    report = FakeReport(health_status="pass")
    ...
    inserted = adapter_module.insert_paper_autonomous_allocation_proposal_db_history_health_report_with_psycopg(
        LOCAL_SUPABASE_DSN,
        report,
        table_name="db_history_health_archive",
    )
    ...
    assert store_table_name == "db_history_health_archive"
...
def test_successful_load_delegates_to_store_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_autonomous_allocation_proposal_db_history_health_reports_with_psycopg(
        LOCAL_SUPABASE_DSN,
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-autonomous-allocation-proposal-db-history-health-v0"
    assert health_status == "pass"
    assert latest_history_status == "pass"
    assert limit == 25
    assert table_name == "db_history_health_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.insert_paper_autonomous_allocation_proposal_db_history_health_report_with_psycopg(
            LOCAL_SUPABASE_DSN,
            FakeReport(health_status="pass"),
        )
...
def test_commit_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    def fake_load(
        ...
        *,
        ...
        assert latest_history_status is None
        return (report,)
...
def test_dict_and_list_params_are_wrapped_in_jsonb(
    ...
    assert params[1].value == [{"reason_code": "source_ready"}]
...
def test_json_cursor_exposes_rowcount_for_duplicate_detection(
    ...
    result = adapter_module.insert_paper_autonomous_allocation_proposal_db_history_health_report_with_psycopg(
        LOCAL_SUPABASE_DSN,
        FakeReport(health_status="pass"),
    )
