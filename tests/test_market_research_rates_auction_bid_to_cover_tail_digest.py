...
def api() -> ModuleType:
    return importlib.import_module(
        ...
        "market_research_rates_auction_bid_to_cover_tail_digest",
    )
...
def config(**overrides: object) -> Any:
    ...
    return module.RatesAuctionBidToCoverTailDigestConfig(**values)


def observation(
    source_id: str = "source-alpha",
    *,
    market_slug: str = "treasury-10y-auction-bid-to-cover-tail",
    ...
    module = api()
    return module.RatesAuctionBidToCoverTailObservation(
        source_id=source_id,
...
def digest(
    ...
    *,
    ...
    module = api()
    return module.build_market_research_rates_auction_bid_to_cover_tail_digest(
        inputs,
...
def test_high_risk_bid_to_cover_tail_digest_blocks_probability_screening() -> None:
    report = digest(
        (
            observation(
                "beta-watch",
                market_slug="treasury-5y-auction-tail-watch",
            ...
            observation(
                "alpha-calm",
                market_slug="treasury-2y-auction-tail-calm",
    ...
    assert report.max_tail_bps == d("5.500000")
    assert report.average_tail_pressure_score == d("0.484921")
    ...
    assert [
        ...
        ("zeta-blocked", "blocked", d("0.966667")),
        ("beta-watch", "watch", d("0.454762")),
        ("alpha-calm", "pass", d("0.033333")),
    ]
    ...
    assert blocked.reason_codes == (
        "auction_result_panel",
        "public_treasury_release",
        "rates_auction_bid_to_cover_tail_blocked",
        "rates_auction_bid_to_cover_tail_demand_weaker",
        "rates_auction_bid_to_cover_tail_shortfall_blocked",
        "rates_auction_bid_to_cover_tail_source_stale",
        "rates_auction_stop_out_tail_blocked",
    )
    ...
    assert watch.reason_codes == (
        ...
        "rates_auction_bid_to_cover_tail_demand_weaker",
        "rates_auction_bid_to_cover_tail_shortfall_watch",
        "rates_auction_bid_to_cover_tail_source_fresh",
        "rates_auction_bid_to_cover_tail_watch",
        "rates_auction_stop_out_tail_watch",
    )
    ...
    assert passed.reason_codes == (
        ...
        "rates_auction_bid_to_cover_tail_demand_stronger",
        "rates_auction_bid_to_cover_tail_source_fresh",
    )
    ...
    weaker = next(
        ...
        if value.reason_code == "rates_auction_bid_to_cover_tail_demand_weaker"
    )
...
def test_empty_digest_and_payload_are_report_only_readonly_decimal_stringed() -> None:
    module = api()
    report = digest(())
    payload = module.market_research_rates_auction_bid_to_cover_tail_digest_payload(
        report,
    )
    ...
    assert report.max_bid_to_cover_shortfall_ratio == d("0.000000")
    assert report.max_tail_bps == d("0.000000")
    assert report.average_tail_pressure_score == d("0.000000")
    assert report.digest_status == "blocked"
    assert report.reason_codes == ("rates_auction_bid_to_cover_tail_digest_empty",)
    assert report.reason_code_counts == (
        module.RatesAuctionBidToCoverTailReasonCodeCount(
...
def test_deterministic_sorting_reason_codes_and_payload_for_permuted_inputs() -> None:
    module = api()
    inputs = (
        observation("gamma", market_slug="treasury-7y-auction-tail-watch"),
        observation("alpha", market_slug="treasury-3y-auction-tail-watch"),
        observation(
            "beta",
            market_slug="treasury-5y-auction-tail-watch",
            upstream_reason_codes=(
    ...
    first_payload = module.market_research_rates_auction_bid_to_cover_tail_digest_payload(
        first,
    )
    second_payload = module.market_research_rates_auction_bid_to_cover_tail_digest_payload(
        second,
    )
...
def test_rejects_bad_public_types_datetimes_duplicates_future_rows_and_frozen_changes() -> None:
    ...
    with pytest.raises(ValueError, match="watch_bid_to_cover_shortfall_ratio"):
        config(watch_bid_to_cover_shortfall_ratio=Decimal("0.200000"))
    ...
    classes = (
        module.RatesAuctionBidToCoverTailDigestConfig,
        module.RatesAuctionBidToCoverTailObservation,
        module.RatesAuctionBidToCoverTailDigestRow,
        module.RatesAuctionBidToCoverTailReasonCodeCount,
        module.RatesAuctionBidToCoverTailDigestReport,
    )
...
def test_hard_flags_are_true_on_all_public_objects_and_cannot_be_downgraded() -> None:
    ...
    with pytest.raises(ValueError, match="report"):
        module.market_research_rates_auction_bid_to_cover_tail_digest_payload(
            {"paper_only": True, "report_only": True, "readonly": True},
        )


def test_non_default_thresholds_reclassify_same_bid_to_cover_tail_observation() -> None:
    ...
    assert "rates_auction_bid_to_cover_tail_shortfall_watch" in (
...
def test_module_scope_is_pure_in_memory_without_live_or_durable_surfaces() -> None:
    ...
    tree = ast.parse(
        Path(
            ...
            "market_research_rates_auction_bid_to_cover_tail_digest.py",
        ).read_text(),
    )
