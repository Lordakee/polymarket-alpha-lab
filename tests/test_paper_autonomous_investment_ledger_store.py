...
def test_insert_preserves_execute_exception_when_cursor_close_fails(
    store_module: types.ModuleType,
) -> None:
...
def test_insert_raises_cursor_close_error_after_successful_execute(
    store_module: types.ModuleType,
) -> None:
    close_error = RuntimeError("close failed")
...
def test_load_preserves_fetchall_exception_when_cursor_close_fails(
    store_module: types.ModuleType,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="fetchall failed") as exc_info:
        store_module.load_paper_autonomous_investment_ledger_reports(connection)

    assert exc_info.value is fetchall_error
    assert connection.cursor_instance.closed is True


def test_load_raises_cursor_close_error_after_successful_fetch(
    store_module: types.ModuleType,
) -> None:
    close_error = RuntimeError("close failed")
