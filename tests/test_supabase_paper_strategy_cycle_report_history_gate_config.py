...
MODULE_NAME = (
    "polymarket_alpha_lab.supabase_paper_strategy_cycle_report_history_gate_config"
)
ENABLED_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_STRATEGY_CYCLE_HISTORY_GATE_DB_ENABLED"
)
DSN_ENV_VAR = "POLYMARKET_ALPHA_LAB_STRATEGY_CYCLE_HISTORY_GATE_DB_DSN"
TABLE_ENV_VAR = "POLYMARKET_ALPHA_LAB_STRATEGY_CYCLE_HISTORY_GATE_DB_TABLE"
...
def test_public_constants_match_expected_env_surface_and_store_default() -> None:
    ...
    from polymarket_alpha_lab.paper_strategy_cycle_report_history_gate_store import (
        DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_REPORTS_TABLE,
    )

    assert module.PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_ENABLED_ENV_VAR == (
        ENABLED_ENV_VAR
    )
    assert module.PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_DSN_ENV_VAR == DSN_ENV_VAR
    assert (
        module.PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE_ENV_VAR
        == TABLE_ENV_VAR
    )
    assert (
        module.DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_DB_TABLE
        == DEFAULT_PAPER_STRATEGY_CYCLE_REPORT_HISTORY_GATE_REPORTS_TABLE
        == DEFAULT_TABLE
    )
...
def test_disabled_env_config_accepts_missing_dsn_and_default_table() -> None:
    ...
    config = module.from_paper_strategy_cycle_report_history_gate_db_env({})

    assert config == module.SupabasePaperStrategyCycleReportHistoryGateConfig(
        enabled=False,
        dsn=None,
    ...
    config = module.from_paper_strategy_cycle_report_history_gate_db_env(
        {
            ...
            DSN_ENV_VAR: dsn,
            TABLE_ENV_VAR: "paper_strategy_cycle_report_history_gate_archive",
        },
    )

    assert config.enabled is True
    assert config.dsn == dsn
    assert config.table_name == "paper_strategy_cycle_report_history_gate_archive"
...
def test_enabled_env_config_rejects_remote_postgres_hosts_without_echoing_secret(
    ...
    with pytest.raises(ValueError) as exc_info:
        module.from_paper_strategy_cycle_report_history_gate_db_env(
            {
                ENABLED_ENV_VAR: "true",
...
def test_dsn_normalizes_absent_blank_or_padded_values_to_none(
    ...
    config = module.SupabasePaperStrategyCycleReportHistoryGateConfig(
        enabled=False,
        dsn=dsn_value,
        table_name=DEFAULT_TABLE,
    )
...
def test_disabled_config_repr_shows_absent_dsn_without_secret_shape() -> None:
    ...
    rendered = repr(
        module.SupabasePaperStrategyCycleReportHistoryGateConfig(
            enabled=False,
            dsn=None,
            table_name=DEFAULT_TABLE,
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "paper_strategy_cycle_report_history_gate_archive",
        "gate_1_archive_2",
    ],
)
