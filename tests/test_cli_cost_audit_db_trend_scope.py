...
EXPECTED_BRANCH_CALLS = {
    ...
    "_run_cost_audit_db_trend",
    "_print_cost_audit_db_trend_summary",
...
EXPECTED_HELPER_IMPORT_MODULES = {
    "polymarket_alpha_lab.paper_trade_cost_audit_db_history_load",
    "psycopg",
}
...
def test_cost_audit_db_trend_parser_surface_is_exactly_read_only():
    tree = parse_cli()
...
def test_cost_audit_db_trend_branch_stays_read_only():
    tree = parse_cli()
...
def test_cost_audit_db_trend_helper_uses_only_read_path_wiring():
    tree = parse_cli()
    helper = _function_body(tree, "_run_cost_audit_db_trend")
    ...
    assert "cost-audit-db-trend-v0" in references
    _assert_no_forbidden_phase_one_escape(references)


def test_cost_audit_db_trend_summary_stays_aggregate_only():
    tree = parse_cli()
    summary = _function_body(tree, "_print_cost_audit_db_trend_summary")
