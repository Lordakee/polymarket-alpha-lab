...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    assert store_table_name == "consistency_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append(
            (connection_arg, config_version, consistency_status, limit, table_name),
        )
        return (report,)
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "recommendation-consistency-v1"
    assert consistency_status == "blocked"
    assert limit == 10
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    assert params[0].value == ["recommendation_consistency_passed"]
...
def test_load_delegates_filter_and_table_validation_to_store(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((config_version, consistency_status, limit, table_name))
        raise ValueError("consistency_status must be pass, watch, or blocked")
    ...
    assert store_calls == [(None, "selected", None, "consistency_archive;drop")]
    assert connection.cursor_count == 0
