...
LOADER_MODULE = (
    ...
    "paper_autonomous_investment_ledger_db_history_health_trend_gate_load"
)
SECRET_DSN = "postgresql://paper-ledger-trend-gate-secret.example.invalid/db"
SECRET_TABLE = "secret_schema.paper_ledger_secret_archive"
SECRET_PAYLOAD = '{"market_slug":"hidden-market","question":"hidden question"}'
SECRET_QUESTION = "Will hidden investment ledger trend gate resolve yes?"
...
def _enable_ledger_db(
    ...
    *,
    dsn: str = "postgresql://paper-ledger-trend-gate.example.invalid/db",
...
def _gate_report() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        config_version="paper-autonomous-investment-ledger-db-history-health-trend-gate-v0",
        source_config_version="paper-autonomous-investment-ledger-db-history-health-trend-v0",
        ...
        latest_health_generated_at=datetime(2026, 6, 25, 16, 0, tzinfo=UTC),
...
def test_investment_ledger_db_history_health_trend_gate_cli_requires_enabled_db_and_dsn_before_runner(
    enabled: str | None,
    dsn: str | None,
    ...
    def forbidden_runner(**_kwargs: Any) -> object:
        ...
        runner_calls += 1
        raise AssertionError("history health trend gate runner should not run")

    exit_code = main(
        [COMMAND],
        paper_autonomous_investment_ledger_db_history_health_trend_gate_runner=forbidden_runner,
    )
...
def test_investment_ledger_db_history_health_trend_gate_cli_rejects_non_positive_limit_before_env_runner_or_psycopg(
    limit: str,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_run_investment_ledger_db_history_health_trend_gate_rejects_invalid_limit_before_runner_or_psycopg(
    bad_limit: object,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    helper = getattr(
    ...
    with pytest.raises(ValueError, match=f"{COMMAND} limit must be positive"):
        helper(
            dsn="postgresql://paper-ledger-trend-gate.example.invalid/db",
...
def test_investment_ledger_db_history_health_trend_gate_cli_uses_injected_runner_and_prints_summary(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    dsn = "postgresql://paper-ledger-trend-gate.example.invalid/db"
    ...
    exit_code = main(
        [COMMAND, "--limit", "25"],
        paper_autonomous_investment_ledger_db_history_health_trend_gate_runner=fake_runner,
    )
    ...
    assert captured.out.splitlines() == [
        (
            ...
            "recommended_next_step="
            "throttle_paper_autonomous_investment_ledger_db_history_health_trend_review "
            "source_health_report_count=4 latest_health_status=watch "
            "latest_source_age_seconds=3600 duplicate_generated_at_count=1 "
            ...
            "duplicate_latest_generated_at_count_delta=1 "
            "reason_code_counts="
            "latest_paper_autonomous_investment_ledger_db_history_health_trend_watch=1"
        ),
    ]
    assert captured.err == ""


def test_investment_ledger_db_history_health_trend_gate_cli_uses_default_health_table_when_table_env_absent(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    dsn = "postgresql://paper-ledger-trend-gate.example.invalid/db"
...
def test_run_investment_ledger_db_history_health_trend_gate_default_loader_owns_connection_lifecycle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    helper = getattr(
        cli,
    ...
    class FakeConnection:
        ...
        def commit(self) -> None:
            raise AssertionError("read-only history health trend gate helper must not commit")

        def rollback(self) -> None:
            raise AssertionError("read-only history health trend gate helper must not rollback")
    ...
    loader_module.load_paper_autonomous_investment_ledger_db_history_health_trend_gate_report = (
        fake_loader
    )
...
def test_run_investment_ledger_db_history_health_trend_gate_connect_failure_names_health_report_db(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    helper = getattr(
        cli,
...
def test_investment_ledger_db_history_health_trend_gate_cli_runner_failure_redacts_sensitive_values(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    exit_code = main(
        [COMMAND],
        paper_autonomous_investment_ledger_db_history_health_trend_gate_runner=broken_runner,
    )
...
def test_run_investment_ledger_db_history_health_trend_gate_loader_failure_redacts_sensitive_values_and_closes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    helper = getattr(
        cli,
...
def test_investment_ledger_db_history_health_trend_gate_summary_printer_emits_aggregates_and_reason_counts_only(
    capsys: pytest.CaptureFixture[str],
) -> None:
    report = _gate_report()
