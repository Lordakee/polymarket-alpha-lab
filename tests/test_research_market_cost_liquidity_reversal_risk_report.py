...
def api():
    return importlib.import_module(
        ...
        "research_market_cost_liquidity_reversal_risk_report",
    )
...
def config(**overrides: object):
    ...
    values = {
        "config_version": (
            module.DEFAULT_RESEARCH_MARKET_COST_LIQUIDITY_REVERSAL_RISK_REPORT_CONFIG_VERSION
        ),
        "reversal_watch_pressure": d("0.350000"),
        "reversal_block_pressure": d("0.700000"),
        ...
        "reversal_weight": d("0.400000"),
        "liquidity_weight": d("0.250000"),
        "cost_weight": d("0.200000"),
        "freshness_weight": d("0.150000"),
        "max_pass_reversal_risk_score": d("0.300000"),
        "max_watch_reversal_risk_score": d("0.700000"),
    }
    values.update(overrides)
    return module.ResearchMarketCostLiquidityReversalRiskConfig(**values)


def risk_input(
    public_risk_ref: str = "risk-ref-pass",
    *,
    observed_at: datetime | None = None,
    prior_probability: Decimal = d("0.500000"),
    current_probability: Decimal = d("0.510000"),
    ...
    liquidity_reversal_pressure: Decimal = d("0.050000"),
    ...
    module = api()
    return module.ResearchMarketCostLiquidityReversalRiskInput(
        public_risk_ref=public_risk_ref,
        observed_at=(
            observed_at
        ...
        liquidity_reversal_pressure=liquidity_reversal_pressure,
...
def report(*items: object, cfg=None, generated_at: datetime = GENERATED_AT):
    module = api()
    return module.build_research_market_cost_liquidity_reversal_risk_report(
        items,
...
def test_scores_cost_liquidity_reversal_and_freshness_risk() -> None:
    module = api()
    ...
    combined = report(
        ...
        risk_input(
            ...
            current_probability=d("0.100000"),
            fee_rate=d("0.050000"),
            spread_ratio=d("0.040000"),
            available_depth=d("100.000000"),
            book_age_seconds=d("900.000000"),
            liquidity_reversal_pressure=d("0.750000"),
        ),
        risk_input(
            "risk-ref-watch",
            prior_probability=d("0.500000"),
            current_probability=d("0.900000"),
            fee_rate=d("0.020000"),
            spread_ratio=d("0.030000"),
            liquidity_reversal_pressure=d("0.200000"),
        ),
    )

    assert module.MARKET_COST_LIQUIDITY_REVERSAL_RISK_STATUSES == (
        "pass",
        "watch",
        "block",
    )
    assert type(combined) is module.ResearchMarketCostLiquidityReversalRiskReport
    ...
    assert combined.average_reversal_risk_score == d("0.393333")
    assert combined.max_reversal_risk_score == d("0.920000")
    assert combined.max_probability_delta == d("0.800000")
    ...
    assert tuple(row.public_risk_ref for row in combined.rows) == (
        "risk-ref-block",
        "risk-ref-watch",
        "risk-ref-pass",
    )
    ...
    assert block_row.probability_delta == d("0.800000")
    assert block_row.cost_ratio == d("0.090000")
    assert block_row.reversal_pressure_score == d("0.800000")
    assert block_row.liquidity_risk_score == d("1.000000")
    assert block_row.cost_risk_score == d("1.000000")
    assert block_row.freshness_risk_score == d("1.000000")
    assert block_row.reversal_risk_score == d("0.920000")
    ...
    assert watch_row.probability_delta == d("0.400000")
    assert watch_row.cost_ratio == d("0.050000")
    assert watch_row.cost_risk_score == d("0.400000")
    assert watch_row.reversal_risk_score == d("0.240000")
    ...
    assert pass_row.reversal_risk_score == d("0.020000")
    assert pass_row.reason_codes == (
        "input_manual_triage_context",
        "reversal_risk_pass",
    )

    payload = module.research_market_cost_liquidity_reversal_risk_report_payload(
        combined,
    )
    payload_json = json.dumps(payload, sort_keys=True).lower()
    assert payload["generated_at"] == "2026-07-09T14:00:00+00:00"
    assert payload["rows"][0]["reversal_risk_score"] == "0.920000"
...
def test_empty_report_blocks_and_payload_digest_is_stable() -> None:
    ...
    assert empty.average_reversal_risk_score is None
    assert empty.reason_codes == ("no_reversal_risk_inputs",)
    assert empty.reason_code_counts == (
        module.ResearchMarketCostLiquidityReversalRiskReasonCodeCount(
    ...
    left = report(risk_input("risk-ref-b"), risk_input("risk-ref-a"))
    right = report(risk_input("risk-ref-a"), risk_input("risk-ref-b"))
    left_payload = module.research_market_cost_liquidity_reversal_risk_report_payload(
        left,
    )
    right_payload = module.research_market_cost_liquidity_reversal_risk_report_payload(
        right,
    )
    digest = module.research_market_cost_liquidity_reversal_risk_report_digest(left)
    ...
    assert digest.payload == module.research_market_cost_liquidity_reversal_risk_report_payload(
        digest,
    )
...
def test_validates_decimal_exactness_dates_flags_and_status_values() -> None:
    ...
    with pytest.raises(ValueError, match="fee_rate"):
        risk_input(fee_rate=0.01)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="spread_ratio"):
        risk_input(spread_ratio=DecimalSubclass("0.010000"))
    with pytest.raises(ValueError, match="observed_at"):
        risk_input(observed_at=datetime(2026, 7, 9, 13, 59))
    ...
    with pytest.raises(ValueError, match="risk weights"):
        config(reversal_weight=d("0.300000"))
    with pytest.raises(ValueError, match="reversal_watch_pressure"):
        config(reversal_watch_pressure=d("0.800000"))
    with pytest.raises(ValueError, match="paper_only"):
        replace(config(), paper_only=False)
    with pytest.raises(ValueError, match="report_only"):
        risk_input(report_only=False)

    good = report(risk_input())
    with pytest.raises(ValueError, match="readonly"):
        replace(good, readonly=False)
    with pytest.raises(ValueError, match="status"):
        replace(good.rows[0], status="ready")
    with pytest.raises(FrozenInstanceError):
        good.rows[0].reversal_risk_score = d("0.000000")  # type: ignore[misc]
    ...
    assert module.research_market_cost_liquidity_reversal_risk_report_digest(
        good,
    ).paper_only is True
...
def test_rejects_public_identifier_and_payload_leakage_surfaces() -> None:
    ...
    for unsafe_value in (
        ...
        with pytest.raises(ValueError, match="unsafe"):
            risk_input(public_risk_ref=unsafe_value)
    ...
    for unsafe_codes in (
        ...
        with pytest.raises(ValueError, match="unsafe"):
            risk_input(reason_codes=unsafe_codes)

    good = report(risk_input())
    payload = module.research_market_cost_liquidity_reversal_risk_report_payload(good)
    unsafe_payload = dict(payload)
    unsafe_payload["market_id"] = "abc"
    with pytest.raises(ValueError, match="unsafe"):
        module.research_market_cost_liquidity_reversal_risk_report_payload(
            unsafe_payload,
        )
...
def test_owned_module_has_no_live_execution_or_storage_surface() -> None:
    module_path = (
        ...
        / "src"
        / "polymarket_alpha_lab"
        / "research_market_cost_liquidity_reversal_risk_report.py"
    )
