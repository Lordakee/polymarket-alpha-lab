...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_ACTION_GATED_STRATEGY_RECOMMENDATION_QUEUE_TABLE = (
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_action_gated_strategy_recommendation_queue_reports_with_psycopg(
            LOCAL_DSN,
        )
...
def test_owned_connection_validates_dsn_before_psycopg_helpers(
    ...
    assert keyword.value.id == "ACTION_GATED_QUEUE_DB_DSN_ENV_VAR"
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    report = FakeReport(config_version="action-gated-queue-v0")
    ...
    inserted = (
        adapter_module.insert_paper_action_gated_strategy_recommendation_queue_report_with_psycopg(
            LOCAL_DSN,
            report,
            table_name="paper_action_gated_queue_archive",
        )
    )
    ...
    assert store_table_name == "paper_action_gated_queue_archive"
...
def test_successful_load_delegates_to_store_commits_and_closes(
    ...
    assert connect_calls == [LOCAL_DSN]
    store_connection, source_config_version, action_status, limit, table_name = (
        store_calls[0]
    )
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert source_config_version == "cycle-action-gate-v0"
    assert action_status == "research_ready"
    assert limit == 25
    assert table_name == "paper_action_gated_queue_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.insert_paper_action_gated_strategy_recommendation_queue_report_with_psycopg(
            LOCAL_DSN,
            FakeReport(config_version="action-gated-queue-v0"),
        )
...
def test_dict_and_list_params_are_wrapped_in_jsonb(
    ...
    assert params[1].value == ["action_gate_research_ready"]
