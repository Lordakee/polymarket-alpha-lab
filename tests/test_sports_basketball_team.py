...
def basketball_evidence(
    *,
    source_id: str = "basketball_injury_report",
    source_type: str = "team_news",
    evidence_text: str = "Starting guard cleared without minutes restriction tonight.",
    data_timestamp: datetime = GENERATED_AT,
    data_freshness_seconds: int = 120,
    evidence_type: str = "injury_rotation",
    weight: Decimal = d("0.650000"),
    probability_impact: Decimal = d("0.040000"),
    reason_codes: tuple[str, ...] = ("basketball_injury_positive",),
    paper_only: bool = True,
    report_only: bool = True,
    readonly: bool = True,
) -> SportsBasketballEvidenceInput:
    return SportsBasketballEvidenceInput(
        source_id=source_id,
...
def build_forecast(
    *,
    condition_id: str = "condition-basketball",
    market_slug: str = "lakers-celtics-game-1",
    question: str = "Will Los Angeles win Game 1 against Boston?",
    event_template: str = "basketball_game_winner",
    evidence: tuple[SportsBasketballEvidenceInput, ...] | None = None,
    base_probability: Decimal = d("0.540000"),
    config: SportsBasketballTeamConfig | None = None,
    generated_at: datetime = GENERATED_AT,
) -> tuple[TeamForecastPacket, tuple[TeamForecastEvidencePacket, ...]]:
    return build_sports_basketball_team_forecast(
        condition_id=condition_id,
        market_slug=market_slug,
        question=question,
        event_template=event_template,
        evidence=evidence if evidence is not None else (basketball_evidence(),),
        base_probability=base_probability,
        config=config
        if config is not None
        else SportsBasketballTeamConfig(config_version="sports-basketball-team-v0"),
        generated_at=generated_at,
    )


def test_builds_supplied_input_basketball_forecast_and_evidence_packets():
    forecast, evidence_rows = build_sports_basketball_team_forecast(
        condition_id="condition-basketball",
        market_slug="lakers-celtics-game-1",
        question="Will Los Angeles win Game 1 against Boston?",
        event_template="basketball_game_winner",
        evidence=(
            SportsBasketballEvidenceInput(
                source_id="basketball_injury_report",
                source_type="team_news",
                evidence_text="Starting guard cleared without minutes restriction tonight.",
                data_timestamp=GENERATED_AT,
                data_freshness_seconds=120,
                evidence_type="injury_rotation",
                weight=d("0.650000"),
                probability_impact=d("0.040000"),
                reason_codes=("basketball_injury_positive",),
            ),
        ),
        base_probability=d("0.540000"),
        config=SportsBasketballTeamConfig(config_version="sports-basketball-team-v0"),
        generated_at=GENERATED_AT,
    )
    ...
    assert forecast.team_id == "sports_basketball"
    assert forecast.category_id == "sports.basketball"
    assert forecast.condition_id == "condition-basketball"
    assert forecast.market_slug == "lakers-celtics-game-1"
    assert forecast.question == "Will Los Angeles win Game 1 against Boston?"
    assert forecast.event_template == "basketball_game_winner"
    ...
    assert forecast.reason_codes == (
        "basketball_injury_positive",
        "team_sports_basketball",
    )
    assert forecast.source_references == ("basketball_injury_report",)
    assert forecast.memory_references == ("basketball_supplied_evidence_only",)
    assert forecast.known_failure_modes == (
        "basketball_late_injury_scratch",
        "basketball_rotation_volatility",
    )
    assert forecast.config_version == "sports-basketball-team-v0"
    assert forecast.prompt_version == "sports-basketball-supplied-input-v0"
    ...
    assert evidence_rows[0].evidence_id == "condition-basketball:basketball_injury_report"
    assert evidence_rows[0].team_id == "sports_basketball"
    assert evidence_rows[0].market_slug == "lakers-celtics-game-1"
    assert evidence_rows[0].weight == d("0.650000")
    assert evidence_rows[0].reason_codes == ("basketball_injury_positive",)
...
def test_forecast_probability_clamps_to_zero_and_one():
    high_forecast, _ = build_forecast(
        ...
        evidence=(basketball_evidence(probability_impact=d("0.250000")),),
    )
...
def test_data_freshness_score_uses_plan_thresholds(
    ...
    forecast, _ = build_forecast(
        evidence=(basketball_evidence(data_freshness_seconds=freshness_seconds),),
    )
...
def test_confidence_and_evidence_quality_average_weights_and_clamp_to_unit_interval():
    forecast, _ = build_forecast(
        evidence=(
            ...
            basketball_evidence(
                source_id="basketball_schedule_spot",
                evidence_type="schedule_fatigue",
                weight=d("0.300000"),
                probability_impact=d("-0.020000"),
                reason_codes=("basketball_schedule_fatigue",),
            ),
        ),
    )
    ...
    assert forecast.reason_codes == (
        "basketball_injury_positive",
        "basketball_schedule_fatigue",
        "team_sports_basketball",
    )
    assert forecast.source_references == (
        "basketball_injury_report",
        "basketball_schedule_spot",
    )
...
def test_selected_side_uses_market_implied_probability_hint_threshold():
    yes_forecast, _ = build_forecast(
        ...
        config=SportsBasketballTeamConfig(
            config_version="sports-basketball-team-v0",
            market_implied_probability_hint=d("0.620000"),
        ),
        evidence=(basketball_evidence(probability_impact=d("0.000000")),),
    )
...
def test_unsafe_side_and_config_flags_raise():
    ...
    unsafe_config = object.__new__(SportsBasketballTeamConfig)
    object.__setattr__(unsafe_config, "config_version", "sports-basketball-team-v0")
    object.__setattr__(
        unsafe_config,
    ...
    object.__setattr__(
        unsafe_config,
        "prompt_version",
        "sports-basketball-supplied-input-v0",
    )
...
def test_supplied_evidence_rejects_non_decimal_and_non_basketball_event_fields():
    with pytest.raises(ValueError, match="weight must be a Decimal"):
        basketball_evidence(weight=1)  # type: ignore[arg-type]
    ...
    with pytest.raises(ValueError, match="probability_impact must be a Decimal"):
        basketball_evidence(probability_impact="0.040000")  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="event_template"):
        build_forecast(event_template=" basketball_game_winner")

    with pytest.raises(ValueError, match="reason_codes"):
        basketball_evidence(reason_codes=("basketball_injury_positive", ""))


def test_input_dataclasses_are_frozen():
    config = SportsBasketballTeamConfig(config_version="sports-basketball-team-v0")
    evidence = basketball_evidence()
