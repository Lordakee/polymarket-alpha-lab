...
EVALUATED_AT = datetime(2026, 7, 2, 15, 45, tzinfo=UTC)
...
def api():
    return importlib.import_module(
        "polymarket_alpha_lab.paper_candidate_cash_lockup_digest",
    )
...
def config(**overrides: object):
    module = api()
    values = {
        "config_version": "paper-candidate-cash-lockup-digest-v0",
        "watch_lockup_ratio": d("0.250000"),
        "blocked_lockup_ratio": d("0.500000"),
        "watch_opportunity_cost": d("1.000000"),
        "annualized_opportunity_cost_proxy": d("0.073000"),
    }
    values.update(overrides)
    return module.PaperCandidateCashLockupDigestConfig(**values)
...
def candidate(**overrides: object):
    ...
    values = {
        ...
        "pending_cash_drag": d("50.000000"),
        "expected_days_to_close": d("10.000000"),
        "settlement_lag_days": d("2.000000"),
        "reason_codes": ("candidate_screened",),
    }
    values.update(overrides)
    return module.PaperCandidateCashLockupDigestCandidate(**values)


def report(*, candidates=(), cfg=None, generated_at: datetime = GENERATED_AT):
    module = api()
    return module.build_paper_candidate_cash_lockup_digest(
        candidates,
...
def test_digest_calculates_cash_lockup_metrics_and_reason_codes() -> None:
    result = report(
        candidates=(
            candidate(
            ...
            candidate(
                ...
                proposed_notional=d("300.000000"),
                available_cash=d("1000.000000"),
                pending_cash_drag=d("75.000000"),
            ),
            candidate(
                candidate_reference="secret-market-token-alpha",
                proposed_notional=d("600.000000"),
                available_cash=d("1000.000000"),
    ...
    assert result.config_version == "paper-candidate-cash-lockup-digest-v0"
    assert result.candidate_count == d("3")
    assert result.pass_count == d("1")
    assert result.watch_count == d("1")
    assert result.blocked_count == d("1")
    assert result.total_proposed_notional == d("1000.000000")
    assert result.total_pending_cash_drag == d("250.000000")
    assert result.total_opportunity_cost == d("2.400000")
    assert result.max_lockup_ratio == d("0.725000")
    assert result.max_cash_lockup_days == d("12.000000")
    assert result.status == "blocked"
    assert result.reason_codes == (
        "cash_lockup_ratio_blocked",
        "cash_lockup_ratio_watch",
        "cash_lockup_passed",
        "pending_cash_drag_present",
        "opportunity_cost_watch",
    )
    ...
    assert tuple(row.cash_lockup_status for row in result.rows) == (
        "blocked",
        "watch",
        "pass",
    )
    ...
    assert blocked.proposed_notional == d("600.000000")
    assert blocked.available_cash == d("1000.000000")
    assert blocked.pending_cash_drag == d("125.000000")
    assert blocked.expected_days_to_close == d("10.000000")
    assert blocked.settlement_lag_days == d("2.000000")
    assert blocked.cash_lockup_days == d("12.000000")
    assert blocked.lockup_ratio == d("0.725000")
    assert blocked.annualized_opportunity_cost_proxy == d("0.073000")
    assert blocked.opportunity_cost == d("1.440000")
    assert blocked.cash_lockup_status == "blocked"
    ...
    assert watched.lockup_ratio == d("0.375000")
    assert watched.opportunity_cost == d("0.720000")
    assert watched.cash_lockup_status == "watch"
    ...
    assert passed.lockup_ratio == d("0.150000")
    assert passed.opportunity_cost == d("0.240000")
    assert passed.cash_lockup_status == "pass"
...
def test_empty_report_is_watch_zeroed_decimal_and_readonly() -> None:
    ...
    assert empty.total_opportunity_cost == ZERO
    assert empty.max_lockup_ratio == ZERO
    assert empty.max_cash_lockup_days == ZERO
    assert empty.status == "watch"
    assert empty.reason_codes == ("cash_lockup_digest_empty",)
    assert empty.rows == ()
...
def test_payload_redacts_references_uses_decimal_strings_and_no_floats() -> None:
    ...
    result = report(
        ...
        generated_at=datetime(2026, 7, 2, 9, 0, tzinfo=timezone(timedelta(hours=-7))),
    )

    payload = module.paper_candidate_cash_lockup_digest_payload(result)
    rendered = repr(payload).lower()
    assert payload["generated_at"] == "2026-07-02T16:00:00+00:00"
    assert payload["candidate_count"] == "1"
    assert payload["total_opportunity_cost"] == "1.440000"
    assert payload["max_lockup_ratio"] == "0.725000"
    ...
    assert payload["rows"][0]["opportunity_cost"] == "1.440000"
...
def test_validation_rejects_bad_types_nonfinite_values_duplicates_and_flags() -> None:
    ...
    with pytest.raises(ValueError, match="config"):
        module.build_paper_candidate_cash_lockup_digest(
            (),
            config=object(),
            generated_at=GENERATED_AT,
        )
    ...
    with pytest.raises(ValueError, match="generated_at"):
        module.PaperCandidateCashLockupDigestReport(
            generated_at=_DatetimeSubclass(2026, 7, 2, 16, 0, tzinfo=UTC),
            config_version="paper-candidate-cash-lockup-digest-v0",
            ...
            total_opportunity_cost=ZERO,
            max_lockup_ratio=ZERO,
            max_cash_lockup_days=ZERO,
            status="watch",
            reason_codes=("cash_lockup_digest_empty",),
            rows=(),
        )
    ...
    with pytest.raises(ValueError, match="pending_cash_drag"):
        candidate(pending_cash_drag=Decimal("NaN"))
    ...
    with pytest.raises(ValueError, match="expected_days_to_close must be nonnegative"):
        candidate(expected_days_to_close=d("-1.000000"))
...
def test_datetimes_are_normalized_to_utc() -> None:
    ...
    assert result.rows[0].evaluated_at == datetime(2026, 7, 2, 15, 45, tzinfo=UTC)
    assert result.rows[0].evaluated_at.tzinfo is UTC


def test_rows_sort_deterministically_by_severity_ratio_and_reference() -> None:
    result = report(
        candidates=(
            candidate(
                candidate_reference="zulu-blocked",
                proposed_notional=d("600.000000"),
                pending_cash_drag=d("100.000000"),
            ),
            candidate(
    ...
    assert tuple(row.lockup_ratio for row in result.rows) == (
        d("0.725000"),
        d("0.700000"),
        d("0.350000"),
        d("0.100000"),
    )
    assert result.rows[0].redacted_candidate_reference != result.rows[1].redacted_candidate_reference


def test_module_has_no_external_io_or_unsafe_surface_terms() -> None:
    source_path = Path(
        "src/polymarket_alpha_lab/paper_candidate_cash_lockup_digest.py",
    )
