from __future__ import annotations

from dataclasses import FrozenInstanceError, replace
from datetime import UTC, datetime, timedelta, timezone
from decimal import Decimal
from importlib import import_module

import pytest

from polymarket_alpha_lab.paper_autonomous_investment_ledger_db_history_health import (
    PaperAutonomousInvestmentLedgerDbHistoryHealthReasonCodeCount,
    PaperAutonomousInvestmentLedgerDbHistoryHealthReport,
)


GENERATED_AT = datetime(2026, 6, 25, 20, 0, tzinfo=UTC)
BASE_AT = datetime(2026, 6, 25, 12, 0, tzinfo=UTC)
NEXT_STEP_BY_STATUS = {
    "pass": "allow_paper_autonomous_investment_ledger_review",
    "watch": "throttle_paper_autonomous_investment_ledger_review",
    "blocked": "block_paper_autonomous_investment_ledger_review",
}


class DatetimeSubclass(datetime):
    pass


def d(value: str) -> Decimal:
    return Decimal(value).quantize(Decimal("0.000001"))


def _api():
    return import_module(
        "polymarket_alpha_lab."
        "paper_autonomous_investment_ledger_db_history_health_trend",
    )


def _config(**overrides):
    values = {
        "config_version": (
            "paper-autonomous-investment-ledger-db-history-health-trend-v0"
        ),
    }
    values.update(overrides)
    return _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendConfig(**values)


def _reason_count(
    reason_code: str,
    report_count: int,
) -> PaperAutonomousInvestmentLedgerDbHistoryHealthReasonCodeCount:
    return PaperAutonomousInvestmentLedgerDbHistoryHealthReasonCodeCount(
        reason_code,
        report_count,
    )


def _health_report(
    *,
    generated_at: datetime,
    health_status: str = "pass",
    ledger_report_count: int | None = None,
    pass_ledger_report_count: int | None = None,
    watch_ledger_report_count: int | None = None,
    blocked_ledger_report_count: int | None = None,
    latest_ledger_status: str | None = None,
    latest_source_record_count: int | None = 1,
    latest_submitted_count: int | None = 1,
    latest_held_count: int | None = 0,
    latest_blocked_count: int | None = 0,
    latest_total_submitted_notional: Decimal | None = d("10.000000"),
    latest_source_generated_at: datetime | None = None,
    latest_source_age_seconds: int | None = 300,
    max_source_age_seconds: int | None = 600,
    duplicate_latest_generated_at_count: int = 0,
    reason_codes: tuple[str, ...] | None = None,
) -> PaperAutonomousInvestmentLedgerDbHistoryHealthReport:
    status_counts = {
        "pass": (3, 3, 0, 0),
        "watch": (3, 2, 1, 0),
        "blocked": (3, 2, 0, 1),
    }[health_status]
    ledger_report_count = (
        status_counts[0] if ledger_report_count is None else ledger_report_count
    )
    pass_ledger_report_count = (
        status_counts[1]
        if pass_ledger_report_count is None
        else pass_ledger_report_count
    )
    watch_ledger_report_count = (
        status_counts[2]
        if watch_ledger_report_count is None
        else watch_ledger_report_count
    )
    blocked_ledger_report_count = (
        status_counts[3]
        if blocked_ledger_report_count is None
        else blocked_ledger_report_count
    )
    if latest_ledger_status is None:
        latest_ledger_status = {
            "pass": "pass",
            "watch": "watch",
            "blocked": "blocked",
        }[health_status]
    if reason_codes is None:
        reason_codes = {
            "pass": ("paper_autonomous_investment_ledger_db_history_health_passed",),
            "watch": ("latest_paper_autonomous_investment_ledger_watch",),
            "blocked": ("latest_paper_autonomous_investment_ledger_blocked",),
        }[health_status]
    if latest_source_generated_at is None:
        latest_source_generated_at = generated_at - timedelta(seconds=300)
    return PaperAutonomousInvestmentLedgerDbHistoryHealthReport(
        generated_at=generated_at,
        config_version="paper-autonomous-investment-ledger-db-history-health-v0",
        health_status=health_status,
        recommended_next_step=NEXT_STEP_BY_STATUS[health_status],
        ledger_report_count=ledger_report_count,
        pass_ledger_report_count=pass_ledger_report_count,
        watch_ledger_report_count=watch_ledger_report_count,
        blocked_ledger_report_count=blocked_ledger_report_count,
        latest_ledger_status=latest_ledger_status,
        latest_source_record_count=latest_source_record_count,
        latest_submitted_count=latest_submitted_count,
        latest_held_count=latest_held_count,
        latest_blocked_count=latest_blocked_count,
        latest_total_submitted_notional=latest_total_submitted_notional,
        latest_source_generated_at=latest_source_generated_at,
        latest_source_age_seconds=latest_source_age_seconds,
        max_source_age_seconds=max_source_age_seconds,
        duplicate_latest_generated_at_count=duplicate_latest_generated_at_count,
        reason_code_counts=(
            _reason_count("paper_autonomous_investment_ledger_passed", 1),
        ),
        reason_codes=reason_codes,
    )


def test_health_trend_config_defaults_are_safe_phase1_defaults() -> None:
    api = _api()

    config = api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendConfig()

    assert config.config_version == (
        api.DEFAULT_PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_HISTORY_HEALTH_TREND_CONFIG_VERSION
    )
    assert config.config_version == (
        "paper-autonomous-investment-ledger-db-history-health-trend-v0"
    )
    assert config.paper_only is True
    assert config.report_only is True
    assert config.readonly is True


def test_health_trend_public_all_exports_constants_dataclasses_and_builder() -> None:
    api = _api()

    assert set(api.__all__) == {
        "DEFAULT_PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_HISTORY_HEALTH_TREND_CONFIG_VERSION",
        "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendConfig",
        "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow",
        "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendSnapshotSummary",
        "PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport",
        "build_paper_autonomous_investment_ledger_db_history_health_trend_report",
    }


def test_health_trend_empty_input_yields_valid_empty_report() -> None:
    trend = _api().build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (),
        config=_config(),
        generated_at=GENERATED_AT,
    )

    assert trend.generated_at == GENERATED_AT
    assert trend.config_version == (
        "paper-autonomous-investment-ledger-db-history-health-trend-v0"
    )
    assert trend.source_health_report_count == 0
    assert trend.first_generated_at is None
    assert trend.latest_generated_at is None
    assert trend.latest_health_status is None
    assert trend.health_status_counts == (("pass", 0), ("watch", 0), ("blocked", 0))
    assert trend.consecutive_latest_watch_count == 0
    assert trend.consecutive_latest_blocked_count == 0
    assert trend.duplicate_generated_at_count == 0
    assert trend.ledger_report_count_first is None
    assert trend.ledger_report_count_latest is None
    assert trend.ledger_report_count_delta is None
    assert trend.latest_source_record_count_first is None
    assert trend.latest_source_record_count_latest is None
    assert trend.latest_source_record_count_delta is None
    assert trend.latest_total_submitted_notional_first is None
    assert trend.latest_total_submitted_notional_latest is None
    assert trend.latest_total_submitted_notional_delta is None
    assert trend.latest_source_generated_at_first is None
    assert trend.latest_source_generated_at_latest is None
    assert trend.latest_source_generated_at_delta_seconds is None
    assert trend.latest_reason_code_counts == ()
    assert trend.total_reason_code_counts == ()
    assert trend.repeated_reason_code_counts == ()
    assert trend.reason_code_rows == ()
    assert trend.source_summaries == ()
    assert trend.paper_only is True
    assert trend.report_only is True
    assert trend.readonly is True


def test_health_trend_sorts_chronologically_and_computes_counts_deltas_and_reasons() -> None:
    first = _health_report(
        generated_at=BASE_AT,
        health_status="pass",
        ledger_report_count=3,
        pass_ledger_report_count=3,
        watch_ledger_report_count=0,
        blocked_ledger_report_count=0,
        latest_source_record_count=1,
        latest_submitted_count=1,
        latest_held_count=0,
        latest_blocked_count=0,
        latest_total_submitted_notional=d("10.000000"),
        latest_source_generated_at=BASE_AT - timedelta(minutes=10),
        latest_source_age_seconds=600,
        max_source_age_seconds=1_200,
        reason_codes=("paper_autonomous_investment_ledger_db_history_health_passed",),
    )
    second_same_time = _health_report(
        generated_at=BASE_AT + timedelta(hours=1),
        health_status="blocked",
        ledger_report_count=4,
        pass_ledger_report_count=3,
        watch_ledger_report_count=0,
        blocked_ledger_report_count=1,
        latest_source_record_count=2,
        latest_submitted_count=1,
        latest_held_count=0,
        latest_blocked_count=1,
        latest_total_submitted_notional=d("20.500000"),
        latest_source_generated_at=BASE_AT + timedelta(minutes=45),
        latest_source_age_seconds=900,
        max_source_age_seconds=1_500,
        reason_codes=(
            "latest_paper_autonomous_investment_ledger_blocked",
            "stale_paper_autonomous_investment_ledger_source_history",
        ),
    )
    latest_same_time = _health_report(
        generated_at=BASE_AT + timedelta(hours=1),
        health_status="watch",
        ledger_report_count=5,
        pass_ledger_report_count=3,
        watch_ledger_report_count=1,
        blocked_ledger_report_count=1,
        latest_source_record_count=4,
        latest_submitted_count=2,
        latest_held_count=1,
        latest_blocked_count=1,
        latest_total_submitted_notional=d("25.750000"),
        latest_source_generated_at=BASE_AT + timedelta(minutes=50),
        latest_source_age_seconds=800,
        max_source_age_seconds=1_600,
        duplicate_latest_generated_at_count=1,
        reason_codes=(
            "latest_paper_autonomous_investment_ledger_watch",
            "stale_paper_autonomous_investment_ledger_source_history",
        ),
    )

    trend = _api().build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (second_same_time, first, latest_same_time),
        config=_config(),
        generated_at=GENERATED_AT,
    )

    assert trend.source_health_report_count == 3
    assert trend.first_generated_at == BASE_AT
    assert trend.latest_generated_at == BASE_AT + timedelta(hours=1)
    assert trend.latest_health_status == "watch"
    assert trend.health_status_counts == (("pass", 1), ("watch", 1), ("blocked", 1))
    assert trend.consecutive_latest_watch_count == 1
    assert trend.consecutive_latest_blocked_count == 0
    assert trend.duplicate_generated_at_count == 1
    assert trend.ledger_report_count_first == 3
    assert trend.ledger_report_count_latest == 5
    assert trend.ledger_report_count_delta == 2
    assert trend.pass_ledger_report_count_first == 3
    assert trend.pass_ledger_report_count_latest == 3
    assert trend.pass_ledger_report_count_delta == 0
    assert trend.watch_ledger_report_count_first == 0
    assert trend.watch_ledger_report_count_latest == 1
    assert trend.watch_ledger_report_count_delta == 1
    assert trend.blocked_ledger_report_count_first == 0
    assert trend.blocked_ledger_report_count_latest == 1
    assert trend.blocked_ledger_report_count_delta == 1
    assert trend.latest_source_record_count_first == 1
    assert trend.latest_source_record_count_latest == 4
    assert trend.latest_source_record_count_delta == 3
    assert trend.latest_submitted_count_first == 1
    assert trend.latest_submitted_count_latest == 2
    assert trend.latest_submitted_count_delta == 1
    assert trend.latest_held_count_first == 0
    assert trend.latest_held_count_latest == 1
    assert trend.latest_held_count_delta == 1
    assert trend.latest_blocked_count_first == 0
    assert trend.latest_blocked_count_latest == 1
    assert trend.latest_blocked_count_delta == 1
    assert trend.latest_total_submitted_notional_first == d("10.000000")
    assert trend.latest_total_submitted_notional_latest == d("25.750000")
    assert trend.latest_total_submitted_notional_delta == d("15.750000")
    assert trend.latest_source_generated_at_first == BASE_AT - timedelta(minutes=10)
    assert trend.latest_source_generated_at_latest == BASE_AT + timedelta(minutes=50)
    assert trend.latest_source_generated_at_delta_seconds == 3_600
    assert trend.latest_source_age_seconds_first == 600
    assert trend.latest_source_age_seconds_latest == 800
    assert trend.latest_source_age_seconds_delta == 200
    assert trend.max_source_age_seconds_first == 1_200
    assert trend.max_source_age_seconds_latest == 1_600
    assert trend.max_source_age_seconds_delta == 400
    assert trend.duplicate_latest_generated_at_count_first == 0
    assert trend.duplicate_latest_generated_at_count_latest == 1
    assert trend.duplicate_latest_generated_at_count_delta == 1
    assert trend.latest_reason_code_counts == (
        ("latest_paper_autonomous_investment_ledger_watch", 1),
        ("stale_paper_autonomous_investment_ledger_source_history", 1),
    )
    assert trend.total_reason_code_counts == (
        ("stale_paper_autonomous_investment_ledger_source_history", 2),
        ("latest_paper_autonomous_investment_ledger_blocked", 1),
        ("latest_paper_autonomous_investment_ledger_watch", 1),
        ("paper_autonomous_investment_ledger_db_history_health_passed", 1),
    )
    assert trend.repeated_reason_code_counts == (
        ("stale_paper_autonomous_investment_ledger_source_history", 2),
    )
    assert trend.reason_code_rows == (
        _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
            "stale_paper_autonomous_investment_ledger_source_history",
            2,
            1,
            2,
        ),
        _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
            "latest_paper_autonomous_investment_ledger_blocked",
            1,
            0,
            1,
        ),
        _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
            "latest_paper_autonomous_investment_ledger_watch",
            1,
            1,
            1,
        ),
        _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
            "paper_autonomous_investment_ledger_db_history_health_passed",
            1,
            0,
            1,
        ),
    )
    assert tuple(summary.generated_at for summary in trend.source_summaries) == (
        BASE_AT,
        BASE_AT + timedelta(hours=1),
        BASE_AT + timedelta(hours=1),
    )
    assert tuple(summary.input_position for summary in trend.source_summaries) == (
        2,
        1,
        3,
    )
    assert trend.source_summaries[-1].health_status == "watch"


def test_health_trend_computes_latest_status_streaks() -> None:
    trend = _api().build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            _health_report(
                generated_at=BASE_AT,
                health_status="pass",
            ),
            _health_report(
                generated_at=BASE_AT + timedelta(hours=1),
                health_status="blocked",
            ),
            _health_report(
                generated_at=BASE_AT + timedelta(hours=2),
                health_status="blocked",
            ),
        ),
        config=_config(),
        generated_at=GENERATED_AT,
    )

    assert trend.latest_health_status == "blocked"
    assert trend.consecutive_latest_watch_count == 0
    assert trend.consecutive_latest_blocked_count == 2


def test_health_trend_dataclasses_are_frozen_and_validate_hard_flags() -> None:
    api = _api()
    config = _config()
    reason_row = api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
        "stale_paper_autonomous_investment_ledger_source_history",
        1,
        0,
        1,
    )
    summary = api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendSnapshotSummary(
        input_position=1,
        generated_at=BASE_AT,
        health_status="pass",
        ledger_report_count=3,
        pass_ledger_report_count=3,
        watch_ledger_report_count=0,
        blocked_ledger_report_count=0,
        latest_ledger_status="pass",
        latest_source_record_count=1,
        latest_submitted_count=1,
        latest_held_count=0,
        latest_blocked_count=0,
        latest_total_submitted_notional=d("10.000000"),
        latest_source_generated_at=BASE_AT - timedelta(minutes=5),
        latest_source_age_seconds=300,
        max_source_age_seconds=600,
        duplicate_latest_generated_at_count=0,
        reason_codes=("paper_autonomous_investment_ledger_db_history_health_passed",),
    )
    report = api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            _health_report(
                generated_at=BASE_AT,
                reason_codes=(
                    "stale_paper_autonomous_investment_ledger_source_history",
                ),
                health_status="watch",
            ),
        ),
        config=config,
        generated_at=GENERATED_AT,
    )

    with pytest.raises(FrozenInstanceError):
        config.config_version = "changed"  # type: ignore[misc]
    with pytest.raises(FrozenInstanceError):
        reason_row.total_count = 2  # type: ignore[misc]
    with pytest.raises(FrozenInstanceError):
        summary.health_status = "blocked"  # type: ignore[misc]
    with pytest.raises(FrozenInstanceError):
        report.latest_health_status = "blocked"  # type: ignore[misc]

    with pytest.raises(ValueError, match="config .*paper_only"):
        replace(config, paper_only=False)
    with pytest.raises(ValueError, match="reason code row .*report_only"):
        replace(reason_row, report_only=False)
    with pytest.raises(ValueError, match="snapshot summary .*readonly"):
        replace(summary, readonly=False)
    with pytest.raises(ValueError, match="trend report .*paper_only"):
        replace(report, paper_only=False)


def test_health_trend_public_dataclass_subclasses_cannot_be_constructed() -> None:
    api = _api()

    with pytest.raises(TypeError, match="TrendConfig .*subclassing"):
        class ConfigSubclass(api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendConfig):
            pass

    with pytest.raises(TypeError, match="TrendReasonCodeRow .*subclassing"):
        class ReasonCodeRowSubclass(
            api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow,
        ):
            pass

    with pytest.raises(TypeError, match="TrendSnapshotSummary .*subclassing"):
        class SnapshotSummarySubclass(
            api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendSnapshotSummary,
        ):
            pass

    with pytest.raises(TypeError, match="TrendReport .*subclassing"):
        class ReportSubclass(api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport):
            pass


def test_health_trend_rejects_wrong_types_naive_datetimes_and_corruption() -> None:
    api = _api()
    source = _health_report(generated_at=BASE_AT)

    with pytest.raises(ValueError, match="health_reports"):
        api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
            object(),
            config=_config(),
            generated_at=GENERATED_AT,
        )
    with pytest.raises(ValueError, match="config must be"):
        api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
            (source,),
            config=object(),
            generated_at=GENERATED_AT,
        )
    with pytest.raises(ValueError, match="generated_at must be a datetime"):
        api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
            (source,),
            config=_config(),
            generated_at=DatetimeSubclass(2026, 6, 25, 20, 0, tzinfo=UTC),
        )
    with pytest.raises(ValueError, match="generated_at must be timezone-aware"):
        api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
            (source,),
            config=_config(),
            generated_at=datetime(2026, 6, 25, 20, 0),
        )

    report = api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            source,
            _health_report(
                generated_at=BASE_AT + timedelta(hours=1),
                health_status="watch",
            ),
        ),
        config=_config(),
        generated_at=GENERATED_AT,
    )
    with pytest.raises(ValueError, match="latest_total_submitted_notional_delta"):
        replace(report, latest_total_submitted_notional_delta=25.0)
    with pytest.raises(ValueError, match="health_status_counts"):
        replace(
            report,
            health_status_counts=(("watch", 0), ("pass", 1), ("blocked", 0)),
        )
    with pytest.raises(ValueError, match="reason_code_rows"):
        replace(report, reason_code_rows=(object(),))
    with pytest.raises(ValueError, match="source_summaries"):
        replace(report, source_summaries=tuple(reversed(report.source_summaries)))


def test_health_trend_rejects_reason_code_row_snapshot_count_mismatch() -> None:
    report = _api().build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            _health_report(
                generated_at=BASE_AT,
                health_status="watch",
                reason_codes=(
                    "stale_paper_autonomous_investment_ledger_source_history",
                ),
            ),
            _health_report(
                generated_at=BASE_AT + timedelta(hours=1),
                health_status="watch",
                reason_codes=(
                    "stale_paper_autonomous_investment_ledger_source_history",
                ),
            ),
        ),
        config=_config(),
        generated_at=GENERATED_AT,
    )
    bad_row = replace(report.reason_code_rows[0], snapshot_count=1)

    with pytest.raises(ValueError, match="reason_code_rows must align with reason code counts"):
        _api().PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReport(
            generated_at=report.generated_at,
            config_version=report.config_version,
            source_health_report_count=report.source_health_report_count,
            first_generated_at=report.first_generated_at,
            latest_generated_at=report.latest_generated_at,
            latest_health_status=report.latest_health_status,
            health_status_counts=report.health_status_counts,
            consecutive_latest_watch_count=report.consecutive_latest_watch_count,
            consecutive_latest_blocked_count=report.consecutive_latest_blocked_count,
            duplicate_generated_at_count=report.duplicate_generated_at_count,
            ledger_report_count_first=report.ledger_report_count_first,
            ledger_report_count_latest=report.ledger_report_count_latest,
            ledger_report_count_delta=report.ledger_report_count_delta,
            pass_ledger_report_count_first=report.pass_ledger_report_count_first,
            pass_ledger_report_count_latest=report.pass_ledger_report_count_latest,
            pass_ledger_report_count_delta=report.pass_ledger_report_count_delta,
            watch_ledger_report_count_first=report.watch_ledger_report_count_first,
            watch_ledger_report_count_latest=report.watch_ledger_report_count_latest,
            watch_ledger_report_count_delta=report.watch_ledger_report_count_delta,
            blocked_ledger_report_count_first=report.blocked_ledger_report_count_first,
            blocked_ledger_report_count_latest=report.blocked_ledger_report_count_latest,
            blocked_ledger_report_count_delta=report.blocked_ledger_report_count_delta,
            latest_source_record_count_first=report.latest_source_record_count_first,
            latest_source_record_count_latest=report.latest_source_record_count_latest,
            latest_source_record_count_delta=report.latest_source_record_count_delta,
            latest_submitted_count_first=report.latest_submitted_count_first,
            latest_submitted_count_latest=report.latest_submitted_count_latest,
            latest_submitted_count_delta=report.latest_submitted_count_delta,
            latest_held_count_first=report.latest_held_count_first,
            latest_held_count_latest=report.latest_held_count_latest,
            latest_held_count_delta=report.latest_held_count_delta,
            latest_blocked_count_first=report.latest_blocked_count_first,
            latest_blocked_count_latest=report.latest_blocked_count_latest,
            latest_blocked_count_delta=report.latest_blocked_count_delta,
            latest_total_submitted_notional_first=(
                report.latest_total_submitted_notional_first
            ),
            latest_total_submitted_notional_latest=(
                report.latest_total_submitted_notional_latest
            ),
            latest_total_submitted_notional_delta=(
                report.latest_total_submitted_notional_delta
            ),
            latest_source_generated_at_first=report.latest_source_generated_at_first,
            latest_source_generated_at_latest=report.latest_source_generated_at_latest,
            latest_source_generated_at_delta_seconds=(
                report.latest_source_generated_at_delta_seconds
            ),
            latest_source_age_seconds_first=report.latest_source_age_seconds_first,
            latest_source_age_seconds_latest=report.latest_source_age_seconds_latest,
            latest_source_age_seconds_delta=report.latest_source_age_seconds_delta,
            max_source_age_seconds_first=report.max_source_age_seconds_first,
            max_source_age_seconds_latest=report.max_source_age_seconds_latest,
            max_source_age_seconds_delta=report.max_source_age_seconds_delta,
            duplicate_latest_generated_at_count_first=(
                report.duplicate_latest_generated_at_count_first
            ),
            duplicate_latest_generated_at_count_latest=(
                report.duplicate_latest_generated_at_count_latest
            ),
            duplicate_latest_generated_at_count_delta=(
                report.duplicate_latest_generated_at_count_delta
            ),
            latest_reason_code_counts=report.latest_reason_code_counts,
            total_reason_code_counts=report.total_reason_code_counts,
            repeated_reason_code_counts=report.repeated_reason_code_counts,
            reason_code_rows=(bad_row,),
            source_summaries=report.source_summaries,
        )


def test_health_trend_rejects_reason_code_aggregates_not_derived_from_summaries() -> None:
    api = _api()
    report = api.build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            _health_report(
                generated_at=BASE_AT,
                health_status="pass",
                reason_codes=(
                    "paper_autonomous_investment_ledger_db_history_health_passed",
                ),
            ),
        ),
        config=_config(),
        generated_at=GENERATED_AT,
    )
    invented_total_counts = (
        ("latest_paper_autonomous_investment_ledger_watch", 1),
    )
    invented_rows = (
        api.PaperAutonomousInvestmentLedgerDbHistoryHealthTrendReasonCodeRow(
            "latest_paper_autonomous_investment_ledger_watch",
            1,
            0,
            0,
        ),
    )

    with pytest.raises(
        ValueError,
        match="total_reason_code_counts must derive from source_summaries",
    ):
        replace(
            report,
            total_reason_code_counts=invented_total_counts,
            reason_code_rows=invented_rows,
        )

    with pytest.raises(
        ValueError,
        match="latest_reason_code_counts must derive from latest source summary",
    ):
        replace(
            report,
            latest_reason_code_counts=(),
        )


def test_health_trend_normalizes_aware_datetimes_to_utc() -> None:
    offset = timezone(timedelta(hours=-4))

    trend = _api().build_paper_autonomous_investment_ledger_db_history_health_trend_report(
        (
            _health_report(
                generated_at=datetime(2026, 6, 25, 12, 0, tzinfo=offset),
                latest_source_generated_at=datetime(
                    2026,
                    6,
                    25,
                    11,
                    55,
                    tzinfo=offset,
                ),
            ),
        ),
        config=_config(),
        generated_at=datetime(2026, 6, 25, 16, 0, tzinfo=offset),
    )

    assert trend.generated_at == GENERATED_AT
    assert trend.first_generated_at == datetime(2026, 6, 25, 16, 0, tzinfo=UTC)
    assert trend.latest_source_generated_at_latest == datetime(
        2026,
        6,
        25,
        15,
        55,
        tzinfo=UTC,
    )
