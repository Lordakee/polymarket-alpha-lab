...
TRANSITION_PATH = (
    REPO_ROOT / "src" / "polymarket_alpha_lab" / "phase_2_evidence_snapshot_transition.py"
)
...
def parse_transition_module():
    return ast.parse(TRANSITION_PATH.read_text(encoding="utf-8"))
...
def test_phase_2_evidence_snapshot_transition_module_imports_only_allowed_dependencies():
    tree = parse_transition_module()
...
def test_phase_2_evidence_snapshot_transition_module_does_not_import_forbidden_surfaces():
    tree = parse_transition_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_phase_2_evidence_snapshot_transition_uses_only_allowed_first_party_symbols():
    tree = parse_transition_module()
    assert imported_first_party_symbols(tree) == EXPECTED_FIRST_PARTY_IMPORTS


def test_phase_2_evidence_snapshot_transition_does_not_import_first_party_modules_wholesale():
    tree = parse_transition_module()
    for node in ast.walk(tree):
...
def test_phase_2_evidence_snapshot_transition_does_not_define_forbidden_names():
    tree = parse_transition_module()
    names = set()
    for node in ast.walk(tree):
...
def test_phase_2_evidence_snapshot_transition_public_exports_are_exact_and_report_only():
    tree = parse_transition_module()
    assert module_exports(tree) == EXPECTED_EXPORTS


def test_package_root_does_not_export_phase_2_evidence_snapshot_transition_names():
