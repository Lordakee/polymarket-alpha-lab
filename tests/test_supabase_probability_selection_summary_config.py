...
MODULE_NAME = "polymarket_alpha_lab.supabase_probability_selection_summary_config"
...
def test_public_constants_match_selection_summary_store_surface() -> None:
    ...
    assert module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR == (
        "POLYMARKET_ALPHA_LAB_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED"
    )
    assert module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR == (
        "POLYMARKET_ALPHA_LAB_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN"
    )
    assert module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE_ENV_VAR == (
        "POLYMARKET_ALPHA_LAB_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE"
    )
    assert (
        module.DEFAULT_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE
        == DEFAULT_PAPER_PROBABILITY_SELECTION_SUMMARY_REPORTS_TABLE
    ...
    assert module.__all__ == (
        ...
        "SupabasePaperProbabilitySelectionSummaryConfig",
...
def test_disabled_env_config_accepts_missing_dsn_and_default_table() -> None:
    ...
    assert config == module.SupabasePaperProbabilitySelectionSummaryConfig(
        enabled=False,
        dsn=None,
...
def test_enabled_env_config_reads_explicit_dsn_at_process_edge(
    ...
    config = module.from_paper_probability_selection_summary_db_env(
        {
            module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR: enabled_value,
            module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR: dsn,
            module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE_ENV_VAR: (
                "paper_probability_selection_summary_archive"
            ),
        },
    )
    ...
    assert config.table_name == "paper_probability_selection_summary_archive"
...
def test_disabled_env_config_parses_only_explicit_false_values(
    ...
    config = module.from_paper_probability_selection_summary_db_env(
        {
            module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR: enabled_value,
            module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR: " ",
        },
    )
    ...
    assert (
        config.table_name == module.DEFAULT_PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE
    )
...
def test_enabled_env_config_requires_dsn_without_echoing_secret() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        module.from_paper_probability_selection_summary_db_env(
            {
                module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR: "true",
                module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR: (
                    f" {secret_dsn} "
                ),
...
def test_invalid_enabled_env_value_names_variable_without_echoing_secret() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        module.from_paper_probability_selection_summary_db_env(
            {
                module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR: "yes",
                module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_DSN_ENV_VAR: secret_dsn,
            },
        )

    message = str(exc_info.value)
    assert module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_ENABLED_ENV_VAR in message
    assert secret_dsn not in message
    assert "secret" not in message.lower()
...
@pytest.mark.parametrize(
    "table_name",
    [
        "PaperProbabilitySelectionSummary",
        "paper-probability-selection-summary",
        "paper.probability.selection.summary",
        "_paper_probability_selection_summary_reports",
        "paper_probability_selection_summary_reports_",
        "",
    ],
)
...
def test_env_table_name_error_mentions_variable_name_without_echoing_secret() -> None:
    ...
    message = str(exc_info.value)
    assert module.PAPER_PROBABILITY_SELECTION_SUMMARY_DB_TABLE_ENV_VAR in message
    assert secret_dsn not in message
    assert "secret" not in message.lower()
