...
MODULE_NAME = "polymarket_alpha_lab.supabase_paper_autonomous_readiness_digest_config"
ENABLED_ENV_VAR = (
    "POLYMARKET_ALPHA_LAB_PAPER_AUTONOMOUS_READINESS_DIGEST_DB_ENABLED"
)
DSN_ENV_VAR = "POLYMARKET_ALPHA_LAB_PAPER_AUTONOMOUS_READINESS_DIGEST_DB_DSN"
TABLE_ENV_VAR = "POLYMARKET_ALPHA_LAB_PAPER_AUTONOMOUS_READINESS_DIGEST_DB_TABLE"
...
def test_public_constants_match_expected_env_surface_and_default_table() -> None:
    module = _config_module()

    assert (
        module.PAPER_AUTONOMOUS_READINESS_DIGEST_DB_ENABLED_ENV_VAR
        == ENABLED_ENV_VAR
    )
    assert module.PAPER_AUTONOMOUS_READINESS_DIGEST_DB_DSN_ENV_VAR == DSN_ENV_VAR
    assert module.PAPER_AUTONOMOUS_READINESS_DIGEST_DB_TABLE_ENV_VAR == TABLE_ENV_VAR
    assert module.DEFAULT_PAPER_AUTONOMOUS_READINESS_DIGEST_DB_TABLE == DEFAULT_TABLE


def test_disabled_env_config_accepts_missing_dsn_and_default_table() -> None:
    module = _config_module()

    config = module.from_paper_autonomous_readiness_digest_db_env({})

    assert config == module.SupabasePaperAutonomousReadinessDigestConfig(
        enabled=False,
        dsn=None,
    ...
    config = module.from_paper_autonomous_readiness_digest_db_env(
        {
            ...
            DSN_ENV_VAR: dsn,
            TABLE_ENV_VAR: "paper_autonomous_readiness_digest_archive",
        },
    )

    assert config.enabled is True
    assert config.dsn == dsn
    assert config.table_name == "paper_autonomous_readiness_digest_archive"
...
def test_enabled_env_config_rejects_remote_postgres_hosts_without_echoing_secret(
    ...
    with pytest.raises(ValueError) as exc_info:
        module.from_paper_autonomous_readiness_digest_db_env(
            {
                ENABLED_ENV_VAR: "true",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "postgresql://postgres:postgres@localhost:0/postgres",
        "postgresql://postgres:postgres@localhost:65536/postgres",
        "postgresql://postgres:postgres@localhost:not-a-port/postgres",
        "postgresql://[::1/postgres",
        "hostaddr=127.0.0.1 port=54322 dbname=postgres",
        "service=local-supabase",
        "host=localhost port=0 dbname=postgres",
        "host=localhost port=65536 dbname=postgres",
        "host=localhost port=not-a-port dbname=postgres",
        "host=localhost dbname=postgres unparseable-token",
    ],
    ids=[
        "uri-hostaddr",
        "uri-service",
        ...
        "keyword-service",
        "keyword-zero-port",
        "keyword-too-large-port",
        "keyword-nonnumeric-port",
        "keyword-malformed-token",
    ],
)
def test_enabled_env_config_rejects_hostaddr_service_bad_ports_and_malformed_values(
    dsn: str,
) -> None:
    module = _config_module()
...
def test_dsn_normalizes_absent_blank_or_padded_values_to_none(
    ...
    config = module.SupabasePaperAutonomousReadinessDigestConfig(
        enabled=False,
        dsn=dsn_value,
        table_name=DEFAULT_TABLE,
    )
...
def test_disabled_config_repr_shows_absent_dsn_without_secret_shape() -> None:
    ...
    rendered = repr(
        module.SupabasePaperAutonomousReadinessDigestConfig(
            enabled=False,
            dsn=None,
            table_name=DEFAULT_TABLE,
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "paper_autonomous_readiness_digest_archive",
        "digest_1_archive_2",
    ],
)
...
def test_env_table_name_error_mentions_variable_name_without_echoing_secret() -> None:
    ...
    message = str(exc_info.value)
    assert message == f"{TABLE_ENV_VAR} table_name must be a simple lowercase identifier"
    assert secret_dsn not in message
    assert "sensitive-token" not in message
