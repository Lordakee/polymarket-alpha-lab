...
def test_insert_quality_report_preserves_operation_exception_when_cursor_close_fails(
    store_module: types.ModuleType,
) -> None:
...
def test_insert_quality_report_propagates_cursor_close_error_after_success(
    store_module: types.ModuleType,
) -> None:
...
def test_load_quality_reports_preserves_fetchall_exception_when_cursor_close_fails(
    store_module: types.ModuleType,
) -> None:
    ...
    with pytest.raises(RuntimeError, match="fetchall failed") as exc_info:
        store_module.load_paper_research_packet_quality_reports(connection)

    assert exc_info.value is operation_error
    assert cursor.closed is True


def test_load_quality_reports_propagates_cursor_close_error_after_success(
    store_module: types.ModuleType,
) -> None:
    close_error = RuntimeError("close failed")
