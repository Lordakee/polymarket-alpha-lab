...
COMMAND = "paper-recommendation-cycle-review"
...
def test_paper_recommendation_cycle_review_branch_is_db_review_only():
    tree = parse_cli()
    branch = _single_command_branch(tree, COMMAND)
    call_names = _branch_call_names(branch)

    assert "_run_cycle_snapshot_db_review" in call_names
    assert "_print_cycle_snapshot_db_review_summary" in call_names
    _assert_no_forbidden_surface(_branch_references(branch))


def test_paper_recommendation_cycle_review_parser_surface_is_exact():
    tree = parse_cli()
