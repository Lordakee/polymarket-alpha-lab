...
HELPER = "_run_autonomous_market_scorer_history"
SUMMARY = "_print_autonomous_market_scorer_history_summary"
...
def _assert_summary_uses_sanitized_reason_formatting(call_names: set[str]) -> None:
    ...
    safe_count_formatters = {
        ...
        and "reasoncodecount" in normalize_identifier(name)
    }
    assert safe_count_formatters


def test_history_parser_surface_is_env_db_only_without_persist() -> None:
    tree = parse_cli()
    parser_variable = _command_parser_variable(tree, COMMAND)
...
def test_history_branch_uses_env_config_helper_and_summary_only() -> None:
    tree = parse_cli()
    ...
    assert "autonomous_market_scorer_db_sink" not in references
    _assert_no_forbidden_fragments(references, FORBIDDEN_PHASE_ESCAPE_FRAGMENTS)


def test_history_helper_loads_scorer_reports_and_builds_report_readonly() -> None:
    tree = parse_cli()
...
def test_history_summary_is_aggregate_only_with_safe_reason_formatting() -> None:
    tree = parse_cli()
