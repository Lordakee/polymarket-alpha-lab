...
STORE_MODULE_NAME = (
    ...
    "paper_autonomous_screening_decision_support_gate_transition_trend_store"
)
ADAPTER_MODULE_NAME = (
    "polymarket_alpha_lab."
    "paper_autonomous_screening_decision_support_gate_transition_trend_psycopg"
)
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_REPORTS_TABLE = (
        "paper_autonomous_screening_gate_transition_trend_reports"
    )
    store_module.insert_paper_autonomous_screening_decision_support_gate_transition_trend_report_with_result = (
        insert if insert is not None else _unexpected_store_call("insert")
    )
    store_module.load_paper_autonomous_screening_decision_support_gate_transition_trend_reports = (
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    inserted = (
        adapter_module.insert_paper_autonomous_screening_decision_support_gate_transition_trend_report_with_psycopg(
            LOCAL_DSN,
            report,
            table_name="paper_autonomous_screening_gate_transition_trend_archive",
        )
    )
    ...
    assert store_table_name == "paper_autonomous_screening_gate_transition_trend_archive"
...
def test_successful_load_uses_autocommit_connection_delegates_and_closes(
    ...
    loaded = (
        adapter_module.load_paper_autonomous_screening_decision_support_gate_transition_trend_reports_with_psycopg(
            LOCAL_DSN,
            config_version="trend-v0",
...
def test_local_dsn_is_validated_with_trend_env_var_before_connect(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeConnection()
    connect_calls: list[tuple[str, dict[str, object]]] = []
    ...
    def validate_spy(value: str, *, env_var_name: str) -> None:
        validation_calls.append((value, env_var_name))
    ...
    sys.modules.pop(ADAPTER_MODULE_NAME, None)
    adapter_module = importlib.import_module(ADAPTER_MODULE_NAME)
    monkeypatch.setattr(adapter_module, "validate_local_postgres_dsn", validate_spy)
    ...
    assert validation_calls == [(LOCAL_DSN, DSN_ENV_VAR)]
    assert connect_calls == [(LOCAL_DSN, {})]
...
def test_dict_and_list_params_are_wrapped_in_jsonb(
        ...
        try:
            cursor.execute(
                ...
                (
                    ...
                    ["screening_gate_transition_trend"],
                    "scalar",
                    1,
                ),
            )
    ...
    assert params[1].value == ["screening_gate_transition_trend"]
...
def test_public_exports_include_insert_and_load() -> None:
    import polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_psycopg as adapter_module
