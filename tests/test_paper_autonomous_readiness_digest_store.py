...
from tests.test_paper_autonomous_readiness_digest_db_row import _report
...
def _db_row():
    ...
    return paper_autonomous_readiness_digest_report_to_db_row(_report())
...
def test_insert_digest_report_uses_parameterized_insert() -> None:
    ...
    inserted = insert_paper_autonomous_readiness_digest_report(connection, report)
    ...
    assert normalize_sql(sql) == normalize_sql(
        """
        INSERT INTO paper_autonomous_readiness_digest_reports (
            report_sha256,
            generated_at,
            config_version,
            digest_status,
    ...
    assert "paper-autonomous-readiness-digest-test-v0" not in sql
    assert "review_watch_paper_autonomous_readiness_evidence" not in sql


def test_insert_digest_report_with_result_observes_duplicate() -> None:
    import polymarket_alpha_lab.paper_autonomous_readiness_digest_store as store

    report = _report()
    expected_row = _db_row()

    inserted = store.insert_paper_autonomous_readiness_digest_report_with_result(
        FakeConnection(rowcount=1),
        report,
    )
    duplicate = store.insert_paper_autonomous_readiness_digest_report_with_result(
...
def test_insert_digest_report_rejects_unexpected_rowcount() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
...
def test_load_digest_reports_filters_in_deterministic_order() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
    ...
    reports = load_paper_autonomous_readiness_digest_reports(
        connection,
        config_version="paper-autonomous-readiness-digest-test-v0",
        digest_status="watch",
        limit=25,
        table_name="readiness_digest_archive",
    )
    ...
    assert normalize_sql(sql) == normalize_sql(
        ...
        SELECT
            ...
            report_only,
            readonly
        FROM readiness_digest_archive
        WHERE config_version = %s AND digest_status = %s
        ORDER BY generated_at DESC, inserted_at DESC, report_sha256 DESC
        LIMIT %s
        """,
    )
    assert params == (
        "paper-autonomous-readiness-digest-test-v0",
        "watch",
        25,
    )
...
def test_load_preserves_fetchall_exception_when_cursor_close_fails() -> None:
    ...
    with pytest.raises(RuntimeError, match="fetchall failed") as exc_info:
        load_paper_autonomous_readiness_digest_reports(connection)
...
def test_load_digest_reports_accepts_positional_rows() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
        load_paper_autonomous_readiness_digest_reports,
    )
    ...
    reports = load_paper_autonomous_readiness_digest_reports(connection)

    assert reports == (_report(),)


def test_load_digest_reports_accepts_dict_rows_without_mutation() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
        load_paper_autonomous_readiness_digest_reports,
    )
    ...
    versions_before = deepcopy(row.source_config_versions_json)
    ...
    assert record["source_config_versions_json"] == versions_before


def test_load_digest_reports_accepts_namedtuple_like_rows() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
        load_paper_autonomous_readiness_digest_reports,
    )
...
def test_load_digest_reports_accepts_db_row_objects() -> None:
    from polymarket_alpha_lab.paper_autonomous_readiness_digest_store import (
        load_paper_autonomous_readiness_digest_reports,
    )
...
@pytest.mark.parametrize(
    "table_name",
    [
        "paper_autonomous_readiness_digest_reports; drop table users",
        "audit.paper_autonomous_readiness_digest_reports",
        "PaperAutonomousReadinessDigestReports",
        "_paper_autonomous_readiness_digest_reports",
        "paper_autonomous_readiness_digest_reports_",
        "a",
    ],
)
...
def test_load_accepts_simple_lowercase_table_names(table_name: str) -> None:
    ...
    load_paper_autonomous_readiness_digest_reports(connection, table_name=table_name)
...
@pytest.mark.parametrize(
    ("kwargs", "message"),
    (
        ({"table_name": "audit.readiness_digest_archive"}, "table_name"),
        ({"table_name": "_"}, "table_name"),
        ({"table_name": "a"}, "table_name"),
        ({"config_version": ""}, "config_version"),
        ({"config_version": " digest-v0"}, "config_version"),
        ({"digest_status": "paused"}, "digest_status"),
        ({"digest_status": True}, "digest_status"),
        ({"limit": 0}, "limit"),
        ({"limit": -1}, "limit"),
        ({"limit": True}, "limit"),
        ({"limit": "5"}, "limit"),
...
def test_load_rejects_invalid_query_inputs_before_cursor_creation(
    ...
    with pytest.raises(ValueError, match=message):
        load_paper_autonomous_readiness_digest_reports(connection, **kwargs)
...
def test_public_exports_include_default_table_and_store_functions() -> None:
    ...
    assert (
        store.DEFAULT_PAPER_AUTONOMOUS_READINESS_DIGEST_REPORTS_TABLE
        == "paper_autonomous_readiness_digest_reports"
    )
    assert "insert_paper_autonomous_readiness_digest_report" in store.__all__
    assert "load_paper_autonomous_readiness_digest_reports" in store.__all__
