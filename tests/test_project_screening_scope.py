...
MODULE_PATH = REPO_ROOT / "src" / "polymarket_alpha_lab" / "project_screening.py"
...
def project_screening_exports(exports):
    return tuple(
        name
        for name in exports
        if name.startswith("PaperProjectScreening")
        or name == "build_paper_project_screening_report"
    )
...
def test_project_screening_imports_only_allowed_dependencies():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert module_name in ALLOWED_IMPORT_MODULES, module_name


def test_project_screening_does_not_import_live_or_loader_surfaces():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_project_screening_public_exports_exactly_paper_report_api():
    tree = parse_module()
...
def test_project_screening_does_not_define_forbidden_live_or_advice_surface_names():
    tree = parse_module()
    names = set()
    for node in ast.walk(tree):
...
def test_package_root_exports_project_screening_api_only():
    tree = parse_module(PACKAGE_ROOT_PATH)
    package_exports = module_exports(tree)

    assert project_screening_exports(package_exports) == EXPECTED_EXPORTS
    for name in project_screening_exports(package_exports):
        assert not public_export_fragment_matches(name), name
    ...
    for node in ast.walk(tree):
        if (
            ...
            and node.module == "polymarket_alpha_lab.project_screening"
        ):
            imported_names.update(alias.name for alias in node.names)
    assert imported_names == set(EXPECTED_EXPORTS)


def test_readme_project_screening_sections_keep_paper_boundaries():
    readme = README_PATH.read_text(encoding="utf-8")
    status_start = readme.index("## Project Screening v0 Status")
    api_start = readme.index("## Project Screening v0 Python API", status_start)
    ...
    required_fragments = (
        "projectscreeningv0status",
        "projectscreeningv0pythonapi",
        "paperonly",
        "reportonly",
        "alreadybuiltpapercostawareeventstrategyreport",
        "humanresearchqueue",
        "notinvestmentrankings",
        "nottraderecommendations",
        "nofetch",
        "noauth",
        "nowallet",
        "noorder",
