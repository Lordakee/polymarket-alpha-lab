...
MODULE_PATH = (
    ...
    / "paper_execution_reconciliation_health_trend.py"
)
...
FORBIDDEN_IMPORT_PREFIXES = {
    ...
    "polymarket_alpha_lab.paper_execution_reconciliation_health_gate_load",
    "polymarket_alpha_lab.paper_execution_reconciliation_load",
...
ALLOWED_FORBIDDEN_NAME_MATCHES = {
    ...
    "missingpaperexecutionreconciliationhealthgatereports",
    "paperexecutionreconciliationhealthtrendconfig",
    "paperexecutionreconciliationhealthtrendreasoncodecount",
    "paperexecutionreconciliationhealthtrendreport",
    "readonly",
    "sourcehealthgatereports",
}
...
def test_health_trend_imports_only_allowed_dependencies() -> None:
    tree = parse_module()
...
def test_health_trend_does_not_import_forbidden_surfaces() -> None:
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_health_trend_uses_only_allowed_first_party_symbols() -> None:
    tree = parse_module()
    assert imported_first_party_symbols(tree) == EXPECTED_FIRST_PARTY_IMPORTS


def test_health_trend_exports_only_report_api() -> None:
    tree = parse_module()
    assert module_exports(tree) == EXPECTED_EXPORTS


def test_health_trend_defines_no_live_io_or_auth_names() -> None:
    tree = parse_module()
    normalized_names = {
...
def test_health_trend_does_not_perform_io_dynamic_execution_or_live_calls() -> None:
    tree = parse_module()
    for node in ast.walk(tree):
...
def test_health_trend_source_strings_do_not_reference_forbidden_live_surfaces() -> None:
    tree = parse_module()
    for node in ast.walk(tree):
