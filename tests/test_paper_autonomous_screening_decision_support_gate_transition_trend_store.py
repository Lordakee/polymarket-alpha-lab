...
from tests.test_paper_autonomous_screening_decision_support_gate_transition_trend_db_row import (
    _trend_report,
)
...
def test_insert_trend_report_uses_parameterized_insert() -> None:
    ...
    assert normalize_sql(sql) == normalize_sql(
        """
        INSERT INTO paper_autonomous_screening_gate_transition_trend_reports (
            report_sha256,
            generated_at,
            config_version,
            transition_report_count,
...
def test_insert_trend_report_with_result_observes_duplicate() -> None:
    import polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store as store

    report = _trend_report()
    expected_row = _db_row()

    inserted = (
        store.insert_paper_autonomous_screening_decision_support_gate_transition_trend_report_with_result(
...
def test_insert_trend_report_rejects_unexpected_rowcount() -> None:
    import polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store as store
...
def test_load_trend_reports_filters_by_config_statuses_and_limit() -> None:
    from polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store import (
    ...
    reports = (
        load_paper_autonomous_screening_decision_support_gate_transition_trend_reports(
            ...
            limit=25,
            table_name="autonomous_screening_gate_transition_trend_archive",
        )
    )

    assert reports == (_trend_report(),)
    assert connection.cursor_count == 1
    assert connection.commit_count == 0
    assert connection.rollback_count == 0
    assert connection.cursor_instance.closed is True
    ...
    assert normalize_sql(sql) == normalize_sql(
        ...
        SELECT
            ...
            report_only,
            readonly
        FROM autonomous_screening_gate_transition_trend_archive
...
def test_load_trend_reports_accepts_positional_rows() -> None:
    from polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store import (
        load_paper_autonomous_screening_decision_support_gate_transition_trend_reports,
    )
...
def test_load_trend_reports_accepts_dict_rows_without_mutation() -> None:
    from polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store import (
        load_paper_autonomous_screening_decision_support_gate_transition_trend_reports,
    )
...
def test_load_trend_reports_accepts_namedtuple_like_rows() -> None:
    from polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store import (
        load_paper_autonomous_screening_decision_support_gate_transition_trend_reports,
    )
...
def test_load_trend_reports_accepts_db_row_objects() -> None:
    from polymarket_alpha_lab.paper_autonomous_screening_decision_support_gate_transition_trend_store import (
        load_paper_autonomous_screening_decision_support_gate_transition_trend_reports,
    )
...
@pytest.mark.parametrize(
    "table_name",
    [
        "paper_autonomous_screening_gate_transition_trend_reports; drop table users",
        "audit.paper_autonomous_screening_gate_transition_trend_reports",
        "PaperAutonomousScreeningGateTransitionTrendReports",
        "_paper_autonomous_screening_gate_transition_trend_reports",
        "paper_autonomous_screening_gate_transition_trend_reports_",
        "a",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        "a0",
        "autonomous_screening_gate_transition_trend_archive",
        "gate_transition_trend_1_archive_2",
    ],
)
...
@pytest.mark.parametrize(
    ...
    (
        (
            {"table_name": "audit.autonomous_screening_gate_transition_trend_archive"},
            "table_name",
        ),
        ({"table_name": "_"}, "table_name"),
        ({"table_name": "a"}, "table_name"),
        ({"config_version": ""}, "config_version"),
        ({"config_version": " trend-v0"}, "config_version"),
...
def test_public_exports_include_default_table_and_store_functions() -> None:
    ...
    assert (
        store.DEFAULT_PAPER_AUTONOMOUS_SCREENING_DECISION_SUPPORT_GATE_TRANSITION_TREND_REPORTS_TABLE
