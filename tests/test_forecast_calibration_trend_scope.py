...
TREND_PATH = (
    REPO_ROOT / "src" / "polymarket_alpha_lab" / "forecast_calibration_trend.py"
)
...
def test_forecast_calibration_trend_module_imports_only_allowed_dependencies():
...
def test_forecast_calibration_trend_module_does_not_import_forbidden_surfaces():
    tree = parse_trend_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_forecast_calibration_trend_uses_only_allowed_first_party_symbols():
    tree = parse_trend_module()
    assert imported_first_party_symbols(tree) == EXPECTED_FIRST_PARTY_IMPORTS


def test_forecast_calibration_trend_does_not_import_first_party_modules_wholesale():
    tree = parse_trend_module()
    for node in ast.walk(tree):
...
def test_forecast_calibration_trend_does_not_define_forbidden_names():
    tree = parse_trend_module()
    names = set()
    for node in ast.walk(tree):
...
def test_forecast_calibration_trend_public_exports_are_exact_and_report_only():
    tree = parse_trend_module()
    assert module_exports(tree) == EXPECTED_EXPORTS


def test_package_root_does_not_export_forecast_calibration_trend_names():
