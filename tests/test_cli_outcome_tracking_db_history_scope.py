...
COMMAND = "outcome-tracking-db-history"
HELPER = "_run_outcome_tracking_db_history"
SUMMARY = "_print_outcome_tracking_db_history_summary"
...
FORBIDDEN_CLI_DB_FLAG_FRAGMENTS = {
    "--outcome-tracking-db-dsn",
    "--outcome-tracking-db-table",
    "--outcome-tracking-db-enabled",
}
...
EXPECTED_HELPER_IMPORT_MODULES = {
    "polymarket_alpha_lab.outcome_tracking_db_history_load",
    "psycopg",
}
...
def test_outcome_tracking_db_history_parser_surface_is_env_driven() -> None:
    tree = parse_cli()
...
def test_outcome_tracking_db_history_branch_stays_readonly_and_scoped() -> None:
    tree = parse_cli()
    ...
    assert "outcome-tracking-db-history limit must be positive" in references
    _assert_no_forbidden_fragments(references, FORBIDDEN_PHASE_ONE_ESCAPE_FRAGMENTS)


def test_outcome_tracking_db_history_helper_uses_readonly_db_loader_only() -> None:
    tree = parse_cli()
...
def test_outcome_tracking_db_history_summary_is_aggregate_only() -> None:
    tree = parse_cli()
    body = _function_body(tree, SUMMARY)
    references = _node_references(body)

    assert "  status_rows: " in references
