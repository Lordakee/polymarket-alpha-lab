...
MODULE_NAME = (
    ...
    "research_source_scrapling_claim_memory_confidence_report"
)
MODULE_PATH = (
    Path(__file__).resolve().parents[1]
    / "src"
    / "polymarket_alpha_lab"
    / "research_source_scrapling_claim_memory_confidence_report.py"
)
...
def config(**overrides: object):
    ...
    values: dict[str, object] = {
        ...
        "watch_memory_score_floor": d("0.650000"),
        "block_memory_score_floor": d("0.400000"),
        "watch_confidence_score_floor": d("0.700000"),
        "block_confidence_score_floor": d("0.450000"),
        "watch_claim_age_seconds": d("21600"),
        "block_claim_age_seconds": d("86400"),
        "min_memory_evidence_count": d("2"),
        "min_confidence_evidence_count": d("3"),
    ...
    return module.ResearchSourceScraplingClaimMemoryConfidenceConfig(**values)


def observation(
    *,
    observed_at: datetime = OBSERVED_AT,
    memory_score: Decimal = d("0.850000"),
    confidence_score: Decimal = d("0.800000"),
    claim_age_seconds: Decimal = d("3600"),
    memory_evidence_count: Decimal = d("3"),
    confidence_evidence_count: Decimal = d("4"),
    claim_revision_count: Decimal = d("1"),
    paper_only: bool = True,
    report_only: bool = True,
    readonly: bool = True,
):
    module = api()
    return module.ResearchSourceScraplingClaimMemoryConfidenceObservation(
...
def report(
    *observations: object,
    ...
    module = api()
    return module.build_research_source_scrapling_claim_memory_confidence_report(
        observations,
...
def redigest_payload(module: Any, payload: dict[str, Any]) -> dict[str, Any]:
    unsigned_payload = dict(payload)
    unsigned_payload.pop("derived_validation_digest", None)
    payload["derived_validation_digest"] = module._report_digest_from_values(
        unsigned_payload,
    )
    return payload
...
def test_empty_input_returns_pass_report_only_decimal_digest() -> None:
    ...
    assert empty_report.reason_codes == ("claim_memory_confidence_pass",)
    ...
    assert empty_report.low_memory_count == d("0")
    assert empty_report.low_confidence_count == d("0")
    assert empty_report.stale_memory_count == d("0")
    assert empty_report.sparse_memory_evidence_count == d("0")
    assert empty_report.sparse_confidence_evidence_count == d("0")
    assert empty_report.max_claim_age_seconds == d("0.000000")
    assert empty_report.min_memory_score is None
    assert empty_report.min_confidence_score is None
    ...
    payload = module.research_source_scrapling_claim_memory_confidence_report_payload(
        empty_report,
    )
    digest_value = module.research_source_scrapling_claim_memory_confidence_report_digest(
        empty_report,
    )
    ...
    assert (
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            payload,
        )
        is None
    )
...
def test_report_scores_claim_memory_confidence_watch_and_block() -> None:
    ...
    watched = observation(
        ...
        confidence_score=d("0.660000"),
        claim_age_seconds=d("30000"),
    )
    blocked = observation(
        observed_at=GENERATED_AT - timedelta(minutes=1),
        memory_score=d("0.350000"),
        confidence_score=d("0.400000"),
        claim_age_seconds=d("90000"),
        memory_evidence_count=d("1"),
        confidence_evidence_count=d("1"),
        claim_revision_count=d("4"),
    )
    ...
    assert built.low_memory_count == d("2")
    assert built.low_confidence_count == d("2")
    assert built.stale_memory_count == d("2")
    assert built.sparse_memory_evidence_count == d("1")
    assert built.sparse_confidence_evidence_count == d("1")
    assert built.max_claim_age_seconds == d("90000.000000")
    assert built.min_memory_score == d("0.350000")
    assert built.min_confidence_score == d("0.400000")
    assert tuple(row.status for row in built.rows) == ("block", "watch", "pass")

    blocked_row, watched_row, passed_row = built.rows
    assert blocked_row.row_label == "redacted-claim-memory-confidence-000001"
    assert blocked_row.memory_confidence_floor_score == d("0.350000")
    assert blocked_row.watch_signal_count == d("5")
    ...
    assert watched_row.watch_signal_count == d("3")
    ...
    assert passed_row.reason_codes == ("claim_memory_confidence_pass",)
...
def test_payload_is_decimal_string_sanitized_deterministic_and_digest_checked() -> None:
    ...
    assert payload["rows"][0]["memory_score"] == "0.350000"
    assert payload["rows"][1]["claim_age_seconds"] == "30000.000000"
    assert first.derived_validation_digest == second.derived_validation_digest
    assert payload["derived_validation_digest"] == first.derived_validation_digest
    assert module.research_source_scrapling_claim_memory_confidence_report_digest(first) == (
...
def test_public_payload_validation_rejects_schema_and_non_json_shapes() -> None:
    module = api()
    payload = report(observation()).payload

    missing_rows = dict(payload)
    missing_rows.pop("rows")
    redigest_payload(module, missing_rows)
    with pytest.raises(ValueError, match="missing public payload key"):
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            missing_rows,
        )

    unexpected_key = dict(payload)
    unexpected_key["extra_metric"] = "safe-public-value"
    redigest_payload(module, unexpected_key)
    ...
    redigest_payload(module, decimal_payload)
    with pytest.raises(ValueError, match="unsupported public payload value"):
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            decimal_payload,
        )
    ...
    redigest_payload(module, tuple_rows)
    with pytest.raises(ValueError, match="rows must be a public payload list"):
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            tuple_rows,
        )

    row_extra_key = dict(payload)
    row_extra_key["rows"] = [dict(payload["rows"][0], extra_metric="safe-public-value")]
    redigest_payload(module, row_extra_key)
    with pytest.raises(ValueError, match="unexpected public payload row key"):
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            row_extra_key,
        )

    inconsistent_count = dict(payload)
    inconsistent_count["observation_count"] = "2.000000"
    redigest_payload(module, inconsistent_count)
    ...
    second_payload = report(
        ...
        observation(observed_at=GENERATED_AT - timedelta(minutes=2)),
    ).payload
    noncanonical_rows = dict(second_payload)
    noncanonical_rows["rows"] = list(reversed(second_payload["rows"]))
    redigest_payload(module, noncanonical_rows)
    with pytest.raises(ValueError, match="derived_validation_digest"):
        module.validate_research_source_scrapling_claim_memory_confidence_public_payload(
            noncanonical_rows,
        )
...
def test_dataclasses_are_frozen_strict_decimal_only_and_enforce_flags() -> None:
    ...
    with pytest.raises(TypeError):
        ...
        class BadConfig(module.ResearchSourceScraplingClaimMemoryConfidenceConfig):
            pass
    ...
    with pytest.raises(ValueError, match="memory_score must be exactly Decimal"):
        observation(memory_score=_DecimalSubclass("0.400000"))  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="confidence_score must be a Decimal"):
        observation(confidence_score=0.4)  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="claim_age_seconds must be a whole Decimal"):
        observation(claim_age_seconds=d("1.5"))
    ...
    with pytest.raises(ValueError, match="block_memory_score_floor"):
        config(block_memory_score_floor=d("0.800000"))
...
def test_module_scope_has_no_forbidden_imports_or_payload_surfaces() -> None:
    ...
    for cls in (
        module.ResearchSourceScraplingClaimMemoryConfidenceRow,
        module.ResearchSourceScraplingClaimMemoryConfidenceReport,
    ):
