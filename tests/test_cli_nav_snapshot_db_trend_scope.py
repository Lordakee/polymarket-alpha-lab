...
COMMAND = "nav-snapshot-db-trend"
HELPER = "_run_nav_snapshot_db_trend"
SUMMARY = "_print_nav_snapshot_db_trend_summary"
...
FORBIDDEN_CLI_DB_FLAG_FRAGMENTS = {
    "--paper-nav-snapshot-db-dsn",
    "--paper-nav-snapshot-db-table",
    "--paper-nav-snapshot-db-enabled",
}
...
EXPECTED_HELPER_IMPORT_MODULES = {
    "polymarket_alpha_lab.paper_nav_snapshot_db_trend_load",
    "psycopg",
}
...
def test_nav_snapshot_db_trend_parser_surface_is_env_driven() -> None:
    tree = parse_cli()
...
def test_nav_snapshot_db_trend_branch_stays_readonly_and_scoped() -> None:
    tree = parse_cli()
    ...
    assert "nav-snapshot-db-trend limit must be positive" in references
    _assert_no_forbidden_fragments(references, FORBIDDEN_PHASE_ONE_ESCAPE_FRAGMENTS)


def test_nav_snapshot_db_trend_helper_uses_readonly_db_loader_only() -> None:
    tree = parse_cli()
...
def test_nav_snapshot_db_trend_summary_is_aggregate_only() -> None:
    tree = parse_cli()
