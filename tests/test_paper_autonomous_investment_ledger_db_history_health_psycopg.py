...
STORE_MODULE_NAME = (
    ...
    "paper_autonomous_investment_ledger_db_history_health_store"
)
ADAPTER_MODULE_NAME = (
    "polymarket_alpha_lab."
    "paper_autonomous_investment_ledger_db_history_health_psycopg"
)
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_HISTORY_HEALTH_REPORTS_TABLE = (
        "paper_autonomous_investment_ledger_db_history_health_reports"
    )
    store_module.insert_paper_autonomous_investment_ledger_db_history_health_report_with_result = (
        insert_with_result
        if insert_with_result is not None
        else _unexpected_store_call("insert_with_result")
    )
    store_module.load_paper_autonomous_investment_ledger_db_history_health_reports = (
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_autonomous_investment_ledger_db_history_health_reports_with_psycopg(
            SECRET_DSN,
        )
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    inserted = adapter_module.insert_paper_autonomous_investment_ledger_db_history_health_report_with_psycopg(
        SECRET_DSN,
        report,
...
def test_successful_load_delegates_to_store_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_autonomous_investment_ledger_db_history_health_reports_with_psycopg(
        SECRET_DSN,
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-autonomous-investment-ledger-db-history-health-v0"
    assert health_status == "pass"
    assert latest_ledger_status == "pass"
    assert limit == 25
...
def test_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.insert_paper_autonomous_investment_ledger_db_history_health_report_with_psycopg(
            SECRET_DSN,
...
def test_commit_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    def fake_load(
        ...
        *,
        ...
        assert latest_ledger_status is None
        return (report,)
...
def test_json_cursor_exposes_rowcount_for_duplicate_detection(
    ...
    result = adapter_module.insert_paper_autonomous_investment_ledger_db_history_health_report_with_psycopg(
        SECRET_DSN,
        FakeReport(health_status="pass"),
    )
