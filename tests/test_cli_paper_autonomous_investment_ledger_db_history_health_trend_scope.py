...
def test_investment_ledger_db_history_health_trend_cli_rejects_db_fast_live_auth_wallet_order_trade_execution_persist_config_and_status_flags(
    argv: list[str],
...
def test_investment_ledger_db_history_health_trend_cli_rejects_abbreviated_limit(
    capsys: pytest.CaptureFixture[str],
) -> None:
    with pytest.raises(SystemExit) as exc_info:
        main([COMMAND, "--lim", "7"])
