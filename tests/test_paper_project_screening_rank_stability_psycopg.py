...
STORE_MODULE_NAME = "polymarket_alpha_lab.paper_project_screening_rank_stability_store"
ADAPTER_MODULE_NAME = "polymarket_alpha_lab.paper_project_screening_rank_stability_psycopg"
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_PROJECT_SCREENING_RANK_STABILITY_REPORTS_TABLE = (
        "paper_project_screening_rank_stability_reports"
    )
    store_module.insert_paper_project_screening_rank_stability_report = (
        insert if insert is not None else _unexpected_store_call("insert")
    )
    store_module.load_paper_project_screening_rank_stability_reports = (
...
def test_importing_adapter_does_not_import_psycopg(
    ...
    assert hasattr(
        adapter_module,
        "insert_paper_project_screening_rank_stability_report_with_psycopg",
    )
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_project_screening_rank_stability_reports_with_psycopg(
            SECRET_DSN,
        )
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    inserted = adapter_module.insert_paper_project_screening_rank_stability_report_with_psycopg(
        SECRET_DSN,
        report,
    ...
    assert store_table_name == "paper_project_screening_rank_stability_archive"
...
def test_remote_dsn_is_rejected_before_psycopg_import_without_echoing_secret(
    ...
    message = str(exc_info.value)
    assert "POLYMARKET_ALPHA_LAB_PAPER_PROJECT_SCREENING_RANK_STABILITY_DB_DSN" in message
...
def test_successful_load_delegates_to_store_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_project_screening_rank_stability_reports_with_psycopg(
        SECRET_DSN,
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "project-screening-rank-stability-v0"
    assert stability_status == "blocked"
    assert limit == 25
    assert table_name == "paper_project_screening_rank_stability_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_successful_insert_close_failure_propagates_and_does_not_echo_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.insert_paper_project_screening_rank_stability_report_with_psycopg(
            SECRET_DSN,
            report,
        )
...
def test_connect_failure_raises_redacted_error_without_chained_secret_traceback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert "worker:secret@localhost" not in formatted
...
def test_public_exports_include_adapter_functions(
    ...
    assert adapter_module.__all__ == (
        "insert_paper_project_screening_rank_stability_report_with_psycopg",
        "load_paper_project_screening_rank_stability_reports_with_psycopg",
    )
