...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-recommendation-reason-trend-v0")
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-recommendation-reason-trend-v0"
    assert limit == 10
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    assert params[0].value == [{"reason_code": "insufficient_edge"}]
    assert isinstance(params[1], FakeJsonb)
    assert params[1].value == [{"market_slug": "market-a"}]
    assert isinstance(params[2], FakeJsonb)
    assert params[2].value == {"payload": {"readonly": True}}
    assert params[3] == Decimal("125.000000")
    assert not isinstance(params[3], FakeJsonb)
    assert params[4] == "paper-recommendation-reason-trend-v0"
    assert params[5] == 2
...
def test_load_delegates_filter_and_table_validation_to_store(
    ...
    connection = FakeCursorConnection()
    store_calls: list[tuple[str | None, int | None, str]] = []
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((config_version, limit, table_name))
    ...
    assert store_calls == [(None, None, "reason_trend_archive;drop")]
    assert connection.cursor_count == 0
