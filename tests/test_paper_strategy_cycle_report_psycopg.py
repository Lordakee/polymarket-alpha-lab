...
def adapter_module() -> types.ModuleType:
    sys.modules.pop("polymarket_alpha_lab.paper_strategy_cycle_report_psycopg", None)
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="strategy-cycle-v1")
    ...
    inserted = adapter_module.insert_paper_strategy_cycle_report_with_psycopg(
    ...
    assert store_table_name == "strategy_cycle_report_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_strategy_cycle_reports_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "strategy-cycle-v1"
    assert limit == 10
    assert table_name == "strategy_cycle_report_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
        ...
        try:
            cursor.execute(
                ...
                (
                    ...
                    {"payload": {"paper_only": True, "report_only": True}},
                    "scalar",
                    1,
                ),
            )
    ...
    adapter_module.insert_paper_strategy_cycle_report_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
        FakeReport(config_version="strategy-cycle-v1"),
    )
    ...
    assert params[0].value == [["blocked_fetch_error", 1]]
    assert isinstance(params[1], FakeJsonb)
    assert params[1].value == {"payload": {"paper_only": True, "report_only": True}}
...
def test_load_rolls_back_closes_and_reraises_commit_exception(
    ...
    with pytest.raises(RuntimeError, match="commit failed without dsn"):
        adapter_module.load_paper_strategy_cycle_reports_with_psycopg(
            "postgresql://user:secret@example.invalid/db",
        )
...
def test_connect_failure_raises_clean_error_without_dsn(
    ...
    assert str(exc_info.value) == "failed to connect to the paper strategy cycle report database"
...
def test_missing_psycopg_raises_clean_error_without_import_time_dependency(
    ...
    assert (
        str(exc_info.value)
        == "psycopg is required to use the paper strategy cycle report psycopg "
