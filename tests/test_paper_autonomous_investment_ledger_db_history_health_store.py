...
from tests.test_paper_autonomous_investment_ledger_db_history_health_db_row import (
    _report,
)
...
def test_insert_health_report_uses_parameterized_insert() -> None:
    ...
    inserted = insert_paper_autonomous_investment_ledger_db_history_health_report(
        connection,
        report,
    )
    ...
    assert normalize_sql(sql) == normalize_sql(
        f"""
        INSERT INTO paper_autonomous_investment_ledger_db_history_health_reports (
            {_columns_sql()}
    ...
    assert "paper_autonomous_investment_ledger_db_history_health_reports (" in sql
    assert "paper-autonomous-investment-ledger-db-history-health-v0" not in sql


def test_insert_health_report_with_result_observes_duplicate() -> None:
    import polymarket_alpha_lab.paper_autonomous_investment_ledger_db_history_health_store as store

    report = _report()
    expected_row = _db_row()

    inserted = (
        store.insert_paper_autonomous_investment_ledger_db_history_health_report_with_result(
...
def test_insert_ignores_cursor_close_errors() -> None:
    ...
    row = insert_paper_autonomous_investment_ledger_db_history_health_report(
        connection,
        _report(),
    )
...
def test_load_health_reports_filters_newest_first_and_excludes_inserted_at() -> None:
    ...
    reports = load_paper_autonomous_investment_ledger_db_history_health_reports(
        connection,
    ...
    assert normalize_sql(sql) == normalize_sql(
        ...
        SELECT
            {_columns_sql()}
        FROM health_archive
        WHERE config_version = %s AND health_status = %s AND latest_ledger_status = %s
...
@pytest.mark.parametrize(
    "table_name",
    [
        "paper_autonomous_investment_ledger_db_history_health_reports; drop table x",
        "audit..paper_autonomous_investment_ledger_db_history_health_reports",
        "audit.paper.autonomous_investment_ledger_db_history_health_reports",
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ({"latest_ledger_status": True}, "latest_ledger_status"),
        ({"limit": 0}, "limit"),
        ({"limit": -1}, "limit"),
        ({"limit": True}, "limit"),
        ({"limit": "5"}, "limit"),
...
def test_public_exports_include_default_table_and_store_functions() -> None:
    ...
    assert (
        store.DEFAULT_PAPER_AUTONOMOUS_INVESTMENT_LEDGER_DB_HISTORY_HEALTH_REPORTS_TABLE
