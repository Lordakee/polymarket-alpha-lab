...
def test_public_exports_include_insert_and_load(adapter_module: types.ModuleType) -> None:
    ...
    assert not hasattr(
        adapter_module,
        "DEFAULT_PAPER_RECOMMENDATION_QUALITY_HISTORY_TABLE",
    )
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-recommendation-quality-history-v0")
    ...
    assert store_table_name == "quality_history_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((connection_arg, config_version, history_status, limit, table_name))
        return (report,)
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-recommendation-quality-history-v0"
    assert history_status == "blocked"
    assert limit == 10
    assert table_name == "quality_history_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
