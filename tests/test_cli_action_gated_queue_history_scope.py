...
COMMAND = "action-gated-queue-history"
...
def test_action_gated_queue_history_parser_surface_is_exactly_read_only():
    tree = parse_cli()
...
def test_action_gated_queue_history_branch_is_read_only_report_only():
    tree = parse_cli()
...
def test_action_gated_queue_history_helper_uses_only_loader_and_builder():
    tree = parse_cli()
    helper = _function_body(tree, "_run_action_gated_queue_history")
...
def test_action_gated_queue_history_summary_prints_aggregate_fields_only():
    tree = parse_cli()
    ...
    assert "candidate_assessment_report" not in references
    assert "bundle_report" not in references
    assert "queue_summary_report" not in references
    assert "dsn" not in normalized_references
    assert "tablename" not in normalized_references
    _assert_no_forbidden_surface(references)
