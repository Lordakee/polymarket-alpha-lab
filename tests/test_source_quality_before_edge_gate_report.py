...
def test_official_anchor_age_preserves_exact_long_horizon_microseconds() -> None:
    ...
    result = report(
        candidate(official_anchor_observed_at=observed_at),
        cfg=config(max_official_anchor_age_seconds=exact_age_seconds),
        generated_at=generated_at,
    )

    assert result.rows[0].official_anchor_age_seconds == exact_age_seconds
    assert result.max_official_anchor_age_seconds == exact_age_seconds
