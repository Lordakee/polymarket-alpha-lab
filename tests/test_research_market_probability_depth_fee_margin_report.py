...
GENERATED_AT = datetime(2026, 7, 9, 20, 30, tzinfo=UTC)
OBSERVED_AT = datetime(2026, 7, 9, 20, 15, tzinfo=UTC)
...
def api() -> Any:
    return import_module(
        ...
        "research_market_probability_depth_fee_margin_report",
    )
...
def sample(reference: str, **overrides: object) -> Any:
    ...
    values = {
        ...
        "depth_cost_rate": d("0.006000"),
        "confidence_margin_rate": d("0.005000"),
        "visible_depth_rate": d("0.080000"),
        "minimum_depth_rate": d("0.020000"),
        "paper_only": True,
        "report_only": True,
        "readonly": True,
    }
    values.update(overrides)
    return module.ResearchMarketProbabilityDepthFeeMarginInput(**values)


def build_report(*inputs: object, config: object | None = None) -> Any:
    module = api()
    return module.build_research_market_probability_depth_fee_margin_report(
        inputs,
...
def test_depth_fee_margin_scores_sorts_and_summarizes_statuses() -> None:
    module = api()
    passed = sample(
    ...
    watched = sample(
        ...
        fee_rate=d("0.012000"),
        spread_rate=d("0.020000"),
        depth_cost_rate=d("0.014000"),
        confidence_margin_rate=d("0.004000"),
        visible_depth_rate=d("0.035000"),
        minimum_depth_rate=d("0.020000"),
    )
    blocked = sample(
        ...
        depth_cost_rate=d("0.060000"),
        confidence_margin_rate=d("0.020000"),
        visible_depth_rate=d("0.010000"),
        minimum_depth_rate=d("0.020000"),
    )
    ...
    assert type(report) is module.ResearchMarketProbabilityDepthFeeMarginReport
    ...
    assert report.average_net_probability_margin_rate == d("-0.020000")
    assert report.min_net_probability_margin_rate == d("-0.150000")
    assert report.max_total_depth_fee_margin_drag_rate == d("0.170000")
    assert report.min_depth_coverage_ratio == d("0.500000")
    assert report.max_drag_to_edge_ratio == d("8.500000")
    ...
    assert block_row.total_depth_fee_margin_drag_rate == d("0.170000")
    assert block_row.net_probability_margin_rate == d("-0.150000")
    assert block_row.depth_coverage_ratio == d("0.500000")
    assert block_row.drag_to_edge_ratio == d("8.500000")
    assert block_row.reason_codes == (
        "probability_depth_fee_margin_block",
        "net_probability_margin_block",
        "total_depth_fee_margin_drag_block",
    ...
    assert watch_row.total_depth_fee_margin_drag_rate == d("0.040000")
    assert watch_row.net_probability_margin_rate == d("0.020000")
    assert watch_row.depth_coverage_ratio == d("1.750000")
    assert watch_row.drag_to_edge_ratio == d("0.666667")
    assert watch_row.reason_codes == (
        "probability_depth_fee_margin_watch",
        "net_probability_margin_watch",
        "total_depth_fee_margin_drag_watch",
        "drag_to_edge_ratio_watch",
        "depth_coverage_watch",
    )

    assert pass_row.gross_probability_edge == d("0.090000")
    assert pass_row.total_depth_fee_margin_drag_rate == d("0.020000")
    assert pass_row.net_probability_margin_rate == d("0.070000")
    assert pass_row.depth_coverage_ratio == d("4.000000")
    assert pass_row.reason_codes == ("probability_depth_fee_margin_pass",)
...
def test_empty_input_blocks_without_row_leakage() -> None:
    ...
    assert report.average_net_probability_margin_rate == d("0.000000")
    assert report.min_net_probability_margin_rate == d("0.000000")
    assert report.max_total_depth_fee_margin_drag_rate == d("0.000000")
    assert report.min_depth_coverage_ratio == d("0.000000")
    assert report.max_drag_to_edge_ratio == d("0.000000")
    assert report.reason_codes == ("missing_probability_depth_fee_margin_inputs",)
...
def test_public_payload_is_json_ready_immutable_digest_bound_and_safe() -> None:
    ...
    payload = module.research_market_probability_depth_fee_margin_report_payload(report)
    ...
    assert payload["generated_at"] == "2026-07-09T20:30:00+00:00"
    assert payload["rows"][0]["signal_digest"] == report.rows[0].signal_digest
    assert payload["rows"][0]["total_depth_fee_margin_drag_rate"] == "0.170000"
    ...
    with pytest.raises(ValueError, match="derived_validation_digest"):
        module.research_market_probability_depth_fee_margin_report_payload(tampered)

    unsafe_payload = dict(payload)
    unsafe_payload["market_id"] = "leaked"
    with pytest.raises(ValueError, match="unsafe public field"):
        module.research_market_probability_depth_fee_margin_report_payload(
            unsafe_payload,
        )
...
def test_contracts_are_frozen_decimal_only_strict_and_flag_locked() -> None:
    ...
    for contract in (
        module.ResearchMarketProbabilityDepthFeeMarginConfig,
        module.ResearchMarketProbabilityDepthFeeMarginInput,
        module.ResearchMarketProbabilityDepthFeeMarginRow,
        module.ResearchMarketProbabilityDepthFeeMarginReasonCodeCount,
        module.ResearchMarketProbabilityDepthFeeMarginReport,
    ):
    ...
    with pytest.raises(ValueError, match="must be a Decimal"):
        sample("decimal-subclass", fee_rate=_DecimalSubclass("0.001000"))
    ...
    with pytest.raises(ValueError, match="depth coverage"):
        module.ResearchMarketProbabilityDepthFeeMarginConfig(
            pass_depth_coverage_ratio=d("1.000000"),
            block_depth_coverage_ratio=d("2.000000"),
        )
...
def test_source_excludes_database_network_wallet_action_and_advice_surfaces() -> None:
    ...
    assert "research_market_probability_depth_fee_margin_report_payload" in (
        module.__all__
    )
    for banned in (
        "api_key",
