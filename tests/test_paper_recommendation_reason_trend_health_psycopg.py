...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-recommendation-reason-trend-health-v0")
    ...
    inserted = adapter_module.insert_paper_recommendation_reason_trend_health_report_with_psycopg(
    ...
    assert store_table_name == "reason_trend_health_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_recommendation_reason_trend_health_reports_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-recommendation-reason-trend-health-v0"
    assert health_status == "blocked"
    assert limit == 10
    assert table_name == "reason_trend_health_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    adapter_module.insert_paper_recommendation_reason_trend_health_report_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
        FakeReport(config_version="paper-recommendation-reason-trend-health-v0"),
    )
...
def test_load_delegates_filter_and_table_validation_to_store(
    ...
    with pytest.raises(ValueError, match="health_status must be pass, watch, or blocked"):
        adapter_module.load_paper_recommendation_reason_trend_health_reports_with_psycopg(
            "postgresql://user:secret@example.invalid/db",
            health_status="selected",
            table_name="reason_trend_health_archive;drop",
        )

    assert store_calls == [(None, "selected", None, "reason_trend_health_archive;drop")]
...
def test_cleanup_failure_does_not_mask_commit_exception(
    ...
    def fake_load(
        ...
        *,
        ...
        return (FakeReport(config_version="paper-recommendation-reason-trend-health-v0"),)
