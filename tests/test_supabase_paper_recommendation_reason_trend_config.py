...
MODULE_PATH = (
    ...
    / "supabase_paper_recommendation_reason_trend_config.py"
)


def _config_module():
    import polymarket_alpha_lab.supabase_paper_recommendation_reason_trend_config as config_module
...
def test_public_constants_match_expected_env_surface() -> None:
    ...
    assert (
        config_module.PAPER_RECOMMENDATION_REASON_TREND_DB_ENABLED_ENV_VAR
        == ENABLED_ENV_VAR
    )
    assert config_module.PAPER_RECOMMENDATION_REASON_TREND_DB_DSN_ENV_VAR == DSN_ENV_VAR
    assert (
        config_module.PAPER_RECOMMENDATION_REASON_TREND_DB_TABLE_ENV_VAR
        == TABLE_ENV_VAR
    )
    assert (
        config_module.DEFAULT_PAPER_RECOMMENDATION_REASON_TREND_DB_TABLE
        == DEFAULT_TABLE
    )
    assert "SupabasePaperRecommendationReasonTrendConfig" in config_module.__all__
    assert "from_paper_recommendation_reason_trend_db_env" in config_module.__all__
...
def test_disabled_env_config_accepts_missing_dsn() -> None:
    ...
    assert config == config_module.SupabasePaperRecommendationReasonTrendConfig(
        enabled=False,
        dsn=None,
    ...
    config = config_module.from_paper_recommendation_reason_trend_db_env(
        {
            ENABLED_ENV_VAR: "1",
            DSN_ENV_VAR: dsn,
            TABLE_ENV_VAR: "paper_recommendation_reason_trend_archive",
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "paper_1_recommendation_2_reason_3_trend_4_reports",
    ],
)
