...
MODULE_NAME = (
    "polymarket_alpha_lab.research_event_authority_consensus_decay_watch_report"
)
...
def config(**overrides: object) -> Any:
    module = api()
    values = {
        "authority_signal_pass_count": d("3.000000"),
        "authority_signal_watch_count": d("2.000000"),
        "authority_family_pass_count": d("2.000000"),
        "authority_family_watch_count": d("1.000000"),
        "consensus_agreement_watch_below": d("0.750000"),
        "consensus_agreement_block_below": d("0.500000"),
    ...
    return module.ResearchEventAuthorityConsensusDecayWatchConfig(**values)


def event(
    seed: str,
    *,
    authority_signal_count: Decimal = d("3.000000"),
    authority_family_count: Decimal = d("2.000000"),
    consensus_agreement_score: Decimal = d("0.900000"),
    ...
    module = api()
    return module.ResearchEventAuthorityConsensusDecayWatchInput(
        event_digest=digest(f"{seed}-event"),
        consensus_digest=digest(f"{seed}-consensus"),
...
def build_report(*items: Any, cfg: Any | None = None) -> Any:
    module = api()
    return module.build_research_event_authority_consensus_decay_watch_report(
        items,
...
def test_empty_report_is_pass_readonly_digest_validated_and_decimal_only() -> None:
    ...
    assert type(result) is module.ResearchEventAuthorityConsensusDecayWatchReport
    ...
    assert result.low_authority_signal_event_count == d("0.000000")
    assert result.thin_authority_family_event_count == d("0.000000")
    assert result.low_consensus_agreement_event_count == d("0.000000")
    assert result.stale_authority_update_event_count == d("0.000000")
    assert result.contradiction_pressure_event_count == d("0.000000")
    assert result.extraction_uncertainty_event_count == d("0.000000")
    assert result.resolution_window_event_count == d("0.000000")
    assert result.lowest_consensus_health_score == d("0.000000")
    assert result.highest_consensus_decay_watch_score == d("0.000000")
    ...
    assert result.reason_codes == (
        "research_event_authority_consensus_decay_watch_empty",
    )
    assert result.rows == ()
    ...
    assert (
        ...
        == module.research_event_authority_consensus_decay_watch_report_digest(result)
    )
    module.validate_research_event_authority_consensus_decay_watch_report_digest(result)
    payload = result.payload
    assert payload == (
        module.research_event_authority_consensus_decay_watch_report_payload(result)
    )
...
def test_decay_watch_rows_classify_pass_watch_and_block_deterministically() -> None:
    result = build_report(
        event("official"),
        event(
            "watched",
            authority_signal_count=d("2.000000"),
            authority_family_count=d("1.000000"),
            consensus_agreement_score=d("0.700000"),
        ...
        event(
            "blocked",
            authority_signal_count=d("1.000000"),
            authority_family_count=d("0.000000"),
            consensus_agreement_score=d("0.400000"),
    ...
    assert result.low_authority_signal_event_count == d("2.000000")
    assert result.thin_authority_family_event_count == d("2.000000")
    assert result.low_consensus_agreement_event_count == d("2.000000")
    assert result.stale_authority_update_event_count == d("2.000000")
    assert result.contradiction_pressure_event_count == d("2.000000")
    assert result.extraction_uncertainty_event_count == d("2.000000")
    assert result.resolution_window_event_count == d("2.000000")
    assert result.lowest_consensus_health_score == d("0.185000")
    assert result.highest_consensus_decay_watch_score == d("0.815000")
    ...
    assert blocked.authority_signal_band == "missing"
    assert blocked.authority_family_band == "single"
    assert blocked.consensus_agreement_band == "fractured"
    assert blocked.authority_freshness_band == "expired"
    assert blocked.resolution_window_band == "immediate"
    assert blocked.consensus_health_score == d("0.185000")
    assert blocked.consensus_decay_watch_score == d("0.815000")
    ...
    assert watched.authority_signal_band == "thin"
    assert watched.authority_family_band == "thin"
    assert watched.consensus_agreement_band == "weak"
    assert watched.authority_freshness_band == "stale"
    assert watched.resolution_window_band == "near"
    assert watched.consensus_health_score == d("0.617500")
    assert watched.consensus_decay_watch_score == d("0.382500")
    ...
    assert passed.authority_signal_band == "sufficient"
    assert passed.authority_family_band == "diverse"
    assert passed.consensus_agreement_band == "aligned"
    assert passed.authority_freshness_band == "fresh"
    assert passed.consensus_health_score == d("0.943750")
    assert passed.consensus_decay_watch_score == d("0.056250")
    assert passed.reason_codes == ("event_authority_consensus_decay_clear",)
...
def test_payload_is_deterministic_public_safe_and_digest_validated() -> None:
    ...
    payload_a = module.research_event_authority_consensus_decay_watch_report_payload(
        report_a,
    )
    payload_b = module.research_event_authority_consensus_decay_watch_report_payload(
        report_b,
    )
    digest_a = module.research_event_authority_consensus_decay_watch_report_digest(
        report_a,
    )
    digest_b = module.research_event_authority_consensus_decay_watch_report_digest(
        report_b,
    )
    ...
    assert payload_a["rows"][0]["consensus_decay_watch_score"] == "0.815000"
    assert (
        module.research_event_authority_consensus_decay_watch_report_payload(payload_a)
        == payload_a
    )
    module.validate_research_event_authority_consensus_decay_watch_public_payload(
        payload_a,
    )
    ...
    with pytest.raises(FrozenInstanceError):
        report_a.rows[0].consensus_health_score = d("0.000000")  # type: ignore[misc]
    with pytest.raises(ValueError, match="unsafe public field"):
        module.research_event_authority_consensus_decay_watch_report_payload(
...
def test_validation_requires_digest_inputs_decimal_inputs_flags_and_safe_surface() -> None:
    ...
    with pytest.raises(ValueError, match="sha-256"):
        module.ResearchEventAuthorityConsensusDecayWatchInput(
            event_digest=_join_parts("raw-", "candidate", "-id"),
            consensus_digest=digest("consensus"),
            authority_signal_count=d("3.000000"),
            authority_family_count=d("2.000000"),
            consensus_agreement_score=d("0.900000"),
            ...
            extraction_uncertainty_score=d("0.050000"),
            resolution_window_seconds=d("3600.000000"),
        )
    with pytest.raises(ValueError, match="authority_signal_count must be a Decimal"):
        event("bad-decimal", authority_signal_count=3)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="consensus_agreement_score must be a Decimal"):
        event("bad-ratio", consensus_agreement_score=0.9)  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="consensus_agreement_score must be a Decimal"):
        event(
            "bad-subclass",
            consensus_agreement_score=_DecimalSubclass("0.900000"),
        )
    with pytest.raises(ValueError, match="six decimal places"):
        event("bad-scale", authority_signal_count=d("3.0"))
    with pytest.raises(ValueError, match="integer Decimal"):
        event("bad-count", authority_signal_count=d("1.500000"))
    with pytest.raises(ValueError, match="between zero and one"):
        event("bad-ratio-range", consensus_agreement_score=d("1.000001"))
    ...
    with pytest.raises(ValueError, match="authority_signal_watch_count"):
        config(authority_signal_watch_count=d("4.000000"))
    with pytest.raises(ValueError, match="consensus_agreement_block_below"):
        config(consensus_agreement_block_below=d("0.800000"))
    ...
    with pytest.raises(ValueError, match="generated_at must be timezone-aware"):
        module.build_research_event_authority_consensus_decay_watch_report(
            (),
    ...
    with pytest.raises(ValueError, match="consensus_decay_watch_score"):
        replace(result.rows[0], consensus_decay_watch_score=d("0.999999"))
...
def test_exports_frozen_dataclasses_status_vocabulary_and_module_scope() -> None:
    ...
    assert module.RESEARCH_EVENT_AUTHORITY_CONSENSUS_DECAY_WATCH_STATUSES == (
        "pass",
        "watch",
        "block",
    )
    ...
    with pytest.raises(FrozenInstanceError):
        config().authority_signal_pass_count = d("1.000000")
