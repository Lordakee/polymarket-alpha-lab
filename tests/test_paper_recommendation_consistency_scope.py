...
MODULE_PATH = (
    ...
    / "paper_recommendation_consistency.py"
)
...
FORBIDDEN_REDUCER_IMPORT_PREFIXES = {
...
def test_paper_recommendation_consistency_imports_only_allowed_dependencies():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert module_name in ALLOWED_IMPORT_MODULES, module_name


def test_paper_recommendation_consistency_does_not_import_reducers():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
            module_matches_prefix(module_name, forbidden)
            for forbidden in FORBIDDEN_REDUCER_IMPORT_PREFIXES
        ), module_name


def test_paper_recommendation_consistency_does_not_import_live_mutation_surfaces():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
            module_matches_prefix(module_name, forbidden)
...
def test_paper_recommendation_consistency_exports_only_standalone_api():
    tree = parse_module()

    assert module_exports(tree) == EXPECTED_EXPORTS


def test_paper_recommendation_consistency_does_not_define_forbidden_surfaces():
    tree = parse_module()
    normalized_names = {
...
def test_paper_recommendation_consistency_does_not_perform_io_or_dynamic_execution():
    tree = parse_module()
    for node in ast.walk(tree):
