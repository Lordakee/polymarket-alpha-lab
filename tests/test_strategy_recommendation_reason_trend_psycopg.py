...
def test_import_does_not_require_psycopg(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="strategy-recommendation-reason-trend-v0")
    ...
    assert store_table_name == "reason_trend_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((connection_arg, config_version, status, limit, table_name))
        return (report,)
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "strategy-recommendation-reason-trend-v0"
    assert status == "blocked"
    assert limit == 10
    assert table_name == "reason_trend_archive"
    assert connection.cursor_count == 0
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_load_delegates_filter_and_table_validation_to_store(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((config_version, status, limit, table_name))
        raise ValueError("status must be stable, watch, or blocked")
    ...
    assert store_calls == [(None, "pass", None, "reason_trend_archive;drop")]
    assert connection.cursor_count == 0
