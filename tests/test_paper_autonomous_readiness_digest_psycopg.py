...
STORE_MODULE_NAME = "polymarket_alpha_lab.paper_autonomous_readiness_digest_store"
ADAPTER_MODULE_NAME = "polymarket_alpha_lab.paper_autonomous_readiness_digest_psycopg"
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_AUTONOMOUS_READINESS_DIGEST_REPORTS_TABLE = (
        "paper_autonomous_readiness_digest_reports"
    )
    store_module.insert_paper_autonomous_readiness_digest_report_with_result = (
        insert_with_result
        if insert_with_result is not None
        else _unexpected_store_call("insert_with_result")
    )
    store_module.load_paper_autonomous_readiness_digest_reports = (
...
def test_importing_adapter_does_not_import_psycopg(
    ...
    assert hasattr(
        adapter_module,
        "load_paper_autonomous_readiness_digest_reports_with_psycopg",
    )
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_autonomous_readiness_digest_reports_with_psycopg(
            SECRET_DSN,
        )
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    report = FakeReport(digest_status="pass")
    ...
    inserted = adapter_module.insert_paper_autonomous_readiness_digest_report_with_psycopg(
        SECRET_DSN,
        report,
        table_name="paper_autonomous_readiness_digest_archive",
    )
    ...
    assert store_table_name == "paper_autonomous_readiness_digest_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1


def test_successful_load_connects_autocommit_delegates_closes_without_commit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeConnection()
    report = FakeReport(digest_status="watch")
    connect_calls: list[tuple[str, dict[str, Any]]] = []
    ...
    loaded = adapter_module.load_paper_autonomous_readiness_digest_reports_with_psycopg(
        SECRET_DSN,
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-autonomous-readiness-digest-v0"
    assert digest_status == "watch"
    assert limit == 25
    assert table_name == "paper_autonomous_readiness_digest_archive"
    assert connection.commit_count == 0
    assert connection.rollback_count == 0
    assert connection.close_count == 1


def test_write_store_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeConnection()
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.insert_paper_autonomous_readiness_digest_report_with_psycopg(
            SECRET_DSN,
            FakeReport(digest_status="pass"),
        )
...
def test_write_commit_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_read_failure_closes_without_commit_or_rollback(
    ...
    message = str(exc_info.value)
    assert "read failed without dsn" in message
    assert "postgresql://" not in message
    assert "secret" not in message
    assert "example.invalid" not in message
    assert connection.commit_count == 0
...
def test_connect_failure_raises_redacted_error(
    ...
    message = str(exc_info.value)
    assert message == "failed to connect to the paper autonomous readiness digest database"
...
def test_dict_and_list_params_are_wrapped_in_jsonb(
    ...
    assert params[1].value == [["source", "config-v0"]]
...
def test_json_cursor_exposes_fetchall_and_rowcount_for_store_delegates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeCursorConnection()
    ...
    result = adapter_module.insert_paper_autonomous_readiness_digest_report_with_psycopg(
        SECRET_DSN,
        FakeReport(digest_status="pass"),
    )
...
def test_public_adapter_functions_expose_domain_return_annotations(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter_module = _import_adapter(monkeypatch)
