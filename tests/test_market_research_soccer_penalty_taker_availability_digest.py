...
CONFIG_VERSION = "market-research-soccer-penalty-taker-availability-digest-test-v0"
MODULE_PATH = Path(
    "src/polymarket_alpha_lab/"
    "market_research_soccer_penalty_taker_availability_digest.py",
)
...
def api():
    return importlib.import_module(
        ...
        "market_research_soccer_penalty_taker_availability_digest",
    )
...
def signal(
    ...
    *,
    ...
    signal_ref: str = "local_penalty_taker_feed_v0",
    ...
    module = api()
    return module.MarketResearchSoccerPenaltyTakerAvailabilityDigestSignal(
        ...
        penalty_taker_status=penalty_taker_status,
...
def build_report(*signals: object, **overrides: object):
    ...
    values.update(overrides)
    return module.build_market_research_soccer_penalty_taker_availability_digest(
        **values,
    )
...
def test_digest_flags_blocked_watch_and_pass_penalty_taker_availability_risk() -> None:
    module = api()

    report = build_report(
        signal(
            "condition_watch",
            "will-player-score-penalty-watch",
            "cwc",
            "watch_match",
            "psg",
            "player_watch",
            ...
            penalty_taker_status="uncertain",
            player_availability_status="questionable",
            risk_score=d("0.550000"),
            source_count=d("2"),
            signal_config_version="soccer-penalty-taker-availability-feed-v1",
        ),
        signal(
            "condition_pass",
            "will-player-score-penalty-pass",
            "epl",
            "pass_match",
            "mci",
            "player_pass",
        ...
        signal(
            "condition_blocked",
            "will-player-score-penalty-blocked",
            "ucl",
            "blocked_match",
            "liv",
            "player_blocked",
            ...
            penalty_taker_status="removed",
            player_availability_status="unavailable",
            risk_score=d("0.920000"),
            source_count=d("1"),
            conflicting_source_count=d("1"),
            signal_config_version="soccer-penalty-taker-availability-feed-v2",
        ),
    )
    ...
    assert report.recommended_next_step == (
        "block_report_only_soccer_penalty_taker_availability_review"
    )
    ...
    assert report.penalty_taker_unavailable_signal_count == d("1.000000")
    assert report.penalty_taker_watch_signal_count == d("1.000000")
    ...
    assert report.reason_codes == (
        "soccer_penalty_taker_availability_conflicting_sources",
        "soccer_penalty_taker_availability_risk_score_blocked",
        "soccer_penalty_taker_availability_taker_unavailable",
        "soccer_penalty_taker_availability_player_unavailable",
        "soccer_penalty_taker_availability_source_coverage_gap",
        "soccer_penalty_taker_availability_risk_score_watch",
        "soccer_penalty_taker_availability_taker_watch",
        "soccer_penalty_taker_availability_player_watch",
    )
    assert report.reason_code_counts == (
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_conflicting_sources",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_risk_score_blocked",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_taker_unavailable",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_player_unavailable",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_source_coverage_gap",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_risk_score_watch",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_taker_watch",
            signal_count=d("1.000000"),
        ),
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestReasonCodeCount(
            reason_code="soccer_penalty_taker_availability_player_watch",
            signal_count=d("1.000000"),
        ),
    )
    assert report.signal_config_versions == (
        ("blocked_match", "player_blocked", "soccer-penalty-taker-availability-feed-v2"),
        ("pass_match", "player_pass", "soccer-penalty-taker-availability-feed-v0"),
        ("watch_match", "player_watch", "soccer-penalty-taker-availability-feed-v1"),
    )
    ...
    assert passed.reason_codes == ("soccer_penalty_taker_availability_clear",)
...
def test_empty_and_clear_inputs_pass_with_decimal_zeroes() -> None:
    ...
    assert empty_report.reason_codes == ("soccer_penalty_taker_availability_empty",)
    ...
    clear_report = build_report(
        signal(
            "condition_clear",
            "will-player-score-penalty-clear",
            "laliga",
            "clear_match",
            "rma",
            "player_clear",
    ...
    assert clear_report.reason_codes == ("soccer_penalty_taker_availability_passed",)
    assert clear_report.rows[0].digest_status == "pass"
    assert clear_report.rows[0].reason_codes == (
        "soccer_penalty_taker_availability_clear",
    )


def test_payload_helper_uses_six_decimal_strings_utc_datetimes_and_no_sensitive_terms() -> None:
    module = api()
    report = build_report(
        signal(
            "condition_payload",
            "will-player-score-penalty-payload",
            "bundesliga",
            "payload_match",
            "bayern",
            "player_payload",
    ...
    payload = module.market_research_soccer_penalty_taker_availability_digest_payload(
        report,
    )
...
def test_dataclasses_are_frozen_and_public_numeric_fields_are_decimal_only() -> None:
    ...
    with pytest.raises(ValueError, match="source_count must be a Decimal"):
        signal(source_count=1)  # type: ignore[arg-type]
...
def test_validation_rejects_unsafe_inputs_subclasses_duplicates_and_stale_inputs() -> None:
    ...
    with pytest.raises(ValueError, match="recommended_next_step must match digest_status"):
        replace(report, recommended_next_step="continue_report_only_soccer_penalty_review")
    ...
    with pytest.raises(ValueError, match="reason_codes must be sorted"):
        module.MarketResearchSoccerPenaltyTakerAvailabilityDigestRow(
            condition_id="condition_manual",
            market_slug="will-player-score-penalty-manual",
            competition_id="ucl",
