from __future__ import annotations

import ast
from dataclasses import FrozenInstanceError, fields, is_dataclass, replace
from datetime import UTC, datetime, timedelta, timezone
from decimal import Decimal
import importlib
from pathlib import Path
from typing import Any

import pytest


GENERATED_AT = datetime(2026, 7, 6, 13, 0, tzinfo=UTC)


def api() -> Any:
    return importlib.import_module(
        "polymarket_alpha_lab.research_packet_resolution_dispute_preflight_gate_v2",
    )


def d(value: str) -> Decimal:
    return Decimal(value)


def config(**overrides: object) -> object:
    module = api()
    values = {
        "config_version": "research-packet-resolution-dispute-preflight-gate-v2",
        "ambiguous_resolution_penalty_weight": d("0.350000"),
        "missing_official_source_penalty": d("0.250000"),
        "contradiction_penalty_weight": d("0.250000"),
        "dispute_history_weight": d("0.150000"),
        "review_threshold": d("0.350000"),
        "block_threshold": d("0.650000"),
    }
    values.update(overrides)
    return module.ResearchPacketResolutionDisputePreflightConfig(**values)


def input_row(**overrides: object) -> object:
    module = api()
    values = {
        "packet_id": "packet-alpha",
        "research_reference": "research-ref-alpha",
        "resolution_summary": "Official source resolves the question clearly.",
        "official_source_count": d("1"),
        "independent_source_count": d("2"),
        "ambiguity_score": d("0.000000"),
        "contradiction_score": d("0.000000"),
        "dispute_history_rate": d("0.000000"),
        "reason_codes": ("official_resolution_source_present", "resolution_language_clear"),
    }
    values.update(overrides)
    return module.ResearchPacketResolutionDisputePreflightInput(**values)


def report(*rows: object, cfg: object | None = None) -> object:
    module = api()
    return module.build_research_packet_resolution_dispute_preflight_gate_v2(
        rows,
        config=cfg or config(),
        generated_at=GENERATED_AT,
    )


def assert_no_runtime_numeric_values(value: object) -> None:
    if isinstance(value, dict):
        for item in value.values():
            assert_no_runtime_numeric_values(item)
    elif isinstance(value, list):
        for item in value:
            assert_no_runtime_numeric_values(item)
    else:
        assert type(value) not in (Decimal, float, int)


def test_dispute_preflight_scores_rows_and_report_counts() -> None:
    digest_report = report(
        input_row(
            packet_id="packet-clean",
            research_reference="research-clean",
        ),
        input_row(
            packet_id="packet-review",
            research_reference="research-review",
            resolution_summary="Resolution language is materially ambiguous.",
            ambiguity_score=d("0.700000"),
            contradiction_score=d("0.400000"),
            dispute_history_rate=d("0.500000"),
            reason_codes=(
                "official_resolution_source_present",
                "ambiguous_resolution_language",
                "conflicting_resolution_evidence",
                "prior_resolution_dispute_history",
            ),
        ),
        input_row(
            packet_id="packet-block",
            research_reference="research-block",
            resolution_summary="Ambiguous sources conflict and no official source is present.",
            official_source_count=d("0"),
            independent_source_count=d("1"),
            ambiguity_score=d("1.000000"),
            contradiction_score=d("0.800000"),
            dispute_history_rate=d("0.800000"),
            reason_codes=(
                "official_resolution_source_missing",
                "ambiguous_resolution_language",
                "conflicting_resolution_evidence",
                "prior_resolution_dispute_history",
            ),
        ),
    )

    assert is_dataclass(digest_report)
    assert digest_report.generated_at == GENERATED_AT
    assert digest_report.packet_count == d("3")
    assert digest_report.review_count == d("1")
    assert digest_report.block_count == d("1")
    assert digest_report.missing_official_source_count == d("1")
    assert digest_report.ambiguous_resolution_count == d("2")
    assert digest_report.max_dispute_preflight_score == d("0.920000")
    assert digest_report.average_dispute_preflight_score == d("0.446667")
    assert digest_report.reason_codes == (
        "resolution_dispute_preflight_block_recommended",
        "resolution_dispute_preflight_review_required",
        "official_source_missing_detected",
        "ambiguous_resolution_detected",
    )
    assert digest_report.paper_only is True
    assert digest_report.report_only is True
    assert digest_report.readonly is True

    first, second, third = digest_report.rows
    assert first.packet_id == "packet-block"
    assert first.dispute_preflight_score == d("0.920000")
    assert first.status == "block"
    assert first.official_source_missing is True
    assert first.reason_codes == (
        "official_source_missing",
        "ambiguous_resolution_penalty_applied",
        "conflicting_resolution_evidence_penalty_applied",
        "dispute_history_penalty_applied",
        "preflight_block",
    )
    assert second.packet_id == "packet-review"
    assert second.dispute_preflight_score == d("0.420000")
    assert second.status == "review"
    assert third.packet_id == "packet-clean"
    assert third.dispute_preflight_score == d("0.000000")
    assert third.status == "pass"


def test_ambiguous_resolution_penalties_can_drive_review_status() -> None:
    digest_report = report(
        input_row(
            ambiguity_score=d("0.800000"),
            reason_codes=(
                "official_resolution_source_present",
                "ambiguous_resolution_language",
            ),
        ),
        cfg=config(review_threshold=d("0.250000")),
    )

    row = digest_report.rows[0]
    assert row.ambiguity_penalty == d("0.280000")
    assert row.missing_official_source_penalty == d("0.000000")
    assert row.dispute_preflight_score == d("0.280000")
    assert row.status == "review"
    assert row.reason_codes == (
        "ambiguous_resolution_penalty_applied",
        "preflight_review",
    )


def test_missing_official_source_flags_and_penalty_are_reported() -> None:
    digest_report = report(
        input_row(
            official_source_count=d("0"),
            independent_source_count=d("3"),
            reason_codes=("official_resolution_source_missing", "resolution_language_clear"),
        ),
    )

    row = digest_report.rows[0]
    assert row.official_source_missing is True
    assert row.missing_official_source_penalty == d("0.250000")
    assert row.dispute_preflight_score == d("0.250000")
    assert row.reason_codes == ("official_source_missing", "preflight_pass")
    assert digest_report.missing_official_source_count == d("1")
    assert digest_report.reason_codes == ("official_source_missing_detected",)


def test_payload_serializes_decimal_values_as_strings_and_preserves_flags() -> None:
    module = api()
    digest_report = report(
        input_row(
            ambiguity_score=d("0.800000"),
            reason_codes=(
                "official_resolution_source_present",
                "ambiguous_resolution_language",
            ),
        ),
    )
    payload = module.research_packet_resolution_dispute_preflight_gate_v2_payload(
        digest_report,
    )
    payload_text = repr(payload)

    assert payload["generated_at"] == "2026-07-06T13:00:00+00:00"
    assert payload["packet_count"] == "1"
    assert payload["average_dispute_preflight_score"] == "0.280000"
    assert payload["rows"][0]["ambiguity_penalty"] == "0.280000"
    assert payload["rows"][0]["paper_only"] is True
    assert payload["paper_only"] is True
    assert payload["report_only"] is True
    assert payload["readonly"] is True
    assert "Decimal(" not in payload_text
    assert "datetime" not in payload_text.lower()
    assert_no_runtime_numeric_values(payload)


def test_dataclasses_are_frozen_and_require_decimal_public_numbers() -> None:
    module = api()
    digest_report = report(input_row())

    with pytest.raises(FrozenInstanceError):
        digest_report.rows[0].status = "review"  # type: ignore[misc]

    with pytest.raises(FrozenInstanceError):
        digest_report.packet_count = d("2")  # type: ignore[misc]

    with pytest.raises(ValueError, match="ambiguity_score must be a Decimal"):
        input_row(ambiguity_score=0.2)

    with pytest.raises(ValueError, match="official_source_count must be a Decimal"):
        input_row(official_source_count=1)

    with pytest.raises(ValueError, match="config must be paper_only"):
        config(paper_only=False)

    with pytest.raises(ValueError, match="input must be readonly"):
        input_row(readonly=False)

    assert {field.name for field in fields(module.ResearchPacketResolutionDisputePreflightRow)}
    assert {field.name for field in fields(module.ResearchPacketResolutionDisputePreflightReport)}


def test_derived_validation_digest_rejects_tampering() -> None:
    digest_report = report(
        input_row(
            ambiguity_score=d("0.800000"),
            reason_codes=(
                "official_resolution_source_present",
                "ambiguous_resolution_language",
            ),
        ),
    )

    assert len(digest_report.derived_validation_digest) == 64
    assert len(digest_report.rows[0].derived_validation_digest) == 64

    with pytest.raises(ValueError, match="derived_validation_digest mismatch"):
        replace(digest_report, derived_validation_digest="0" * 64)

    with pytest.raises(ValueError, match="derived_validation_digest mismatch"):
        replace(digest_report.rows[0], status="review")


def test_unsafe_payload_keys_values_and_flag_downgrades_are_rejected() -> None:
    module = api()
    unsafe_terms = (
        "live",
        "auth",
        "wallet",
        "order",
        "network",
        "database",
        "persist",
        "signing",
        "mutation",
        "buy",
        "sell",
        "trade",
    )

    with pytest.raises(ValueError, match="payload must be readonly"):
        module.research_packet_resolution_dispute_preflight_gate_v2_payload(
            {"paper_only": True, "report_only": True, "readonly": False},
        )

    with pytest.raises(ValueError, match="JSON value must not be a float"):
        module.research_packet_resolution_dispute_preflight_gate_v2_payload(
            {
                "paper_only": True,
                "report_only": True,
                "readonly": True,
                "score": 0.1,
            },
        )

    for term in unsafe_terms:
        with pytest.raises(ValueError, match="unsafe public"):
            module.research_packet_resolution_dispute_preflight_gate_v2_payload(
                {"paper_only": True, "report_only": True, "readonly": True, term: "x"},
            )
        with pytest.raises(ValueError, match="unsafe public"):
            module.research_packet_resolution_dispute_preflight_gate_v2_payload(
                {
                    "paper_only": True,
                    "report_only": True,
                    "readonly": True,
                    "note": f"contains {term} term",
                },
            )


def test_public_module_surface_excludes_execution_side_effects() -> None:
    module = api()
    source_path = Path(
        "src/polymarket_alpha_lab/research_packet_resolution_dispute_preflight_gate_v2.py",
    )
    source = source_path.read_text(encoding="utf-8")
    lowered = source.lower()

    assert module.__all__ == (
        "ResearchPacketResolutionDisputePreflightConfig",
        "ResearchPacketResolutionDisputePreflightInput",
        "ResearchPacketResolutionDisputePreflightRow",
        "ResearchPacketResolutionDisputePreflightReport",
        "build_research_packet_resolution_dispute_preflight_gate_v2",
        "research_packet_resolution_dispute_preflight_gate_v2_payload",
    )
    public_names = {name.lower() for name in module.__all__}
    public_fields = {
        field.name.lower()
        for cls_name in module.__all__
        if cls_name.startswith("ResearchPacket")
        for field in fields(getattr(module, cls_name))
    }
    for forbidden in (
        "live",
        "auth",
        "wallet",
        "order",
        "network",
        "database",
        "persist",
        "signing",
        "mutation",
        "buy",
        "sell",
        "trade",
    ):
        assert all(forbidden not in name for name in public_names)
        assert all(forbidden not in name for name in public_fields)

    for forbidden_fragment in (
        "requests",
        "httpx",
        "urllib",
        "socket",
        "sqlite",
        "sqlalchemy",
        "subprocess",
        "open(",
        "write(",
        "delete(",
    ):
        assert forbidden_fragment not in lowered

    tree = ast.parse(source)
    forbidden_import_roots = {
        "requests",
        "httpx",
        "urllib",
        "socket",
        "sqlite3",
        "sqlalchemy",
        "subprocess",
    }
    for node in ast.walk(tree):
        if isinstance(node, ast.Constant):
            assert type(node.value) is not float
        if isinstance(node, ast.Import):
            assert all(alias.name.split(".")[0] not in forbidden_import_roots for alias in node.names)
        if isinstance(node, ast.ImportFrom) and node.module:
            assert node.module.split(".")[0] not in forbidden_import_roots
