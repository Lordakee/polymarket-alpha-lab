...
def test_exact_watch_threshold_is_classified_as_watch() -> None:
    boundary_report = report(
        spread_probability=d("0.050000"),
        recent_price_move_probability=ZERO,
        depth_imbalance_probability=ZERO,
        source_freshness_age_hours=ZERO,
        forecast_age_hours=ZERO,
    )

    assert boundary_report.adverse_selection_status == "watch"
    assert boundary_report.reason_codes == ("spread_probability_watch",)
    assert boundary_report.manual_next_step == (
        "manual_review_market_maker_adverse_selection"
    )
