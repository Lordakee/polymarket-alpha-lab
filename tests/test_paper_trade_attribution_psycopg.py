...
def adapter_module() -> types.ModuleType:
    sys.modules.pop("polymarket_alpha_lab.paper_trade_attribution_psycopg", None)
    return importlib.import_module("polymarket_alpha_lab.paper_trade_attribution_psycopg")
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-trade-attribution-db-v0")
    ...
    inserted = adapter_module.insert_paper_trade_attribution_report_with_psycopg(
        LOCAL_DSN,
        report,
        table_name="paper_trade_attribution_archive",
    )
    ...
    assert store_table_name == "paper_trade_attribution_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_trade_attribution_reports_with_psycopg(
        LOCAL_DSN,
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-trade-attribution-db-v0"
    assert limit == 10
    assert table_name == "paper_trade_attribution_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    adapter_module.insert_paper_trade_attribution_report_with_psycopg(
        LOCAL_DSN,
        FakeReport(config_version="paper-trade-attribution-db-v0"),
    )
...
def test_load_rolls_back_closes_and_reraises_commit_exception(
    ...
    with pytest.raises(RuntimeError, match="commit failed without dsn"):
        adapter_module.load_paper_trade_attribution_reports_with_psycopg(LOCAL_DSN)
...
def test_remote_dsn_is_rejected_before_connect_without_echoing_secret(
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.load_paper_trade_attribution_reports_with_psycopg(
            REMOTE_SECRET_DSN,
        )
