...
MODULE_PATH = (
    ...
    / "paper_recommendation_gate_summary.py"
)
...
def test_paper_recommendation_gate_summary_imports_only_allowed_dependencies():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert module_name in ALLOWED_IMPORT_MODULES, module_name


def test_paper_recommendation_gate_summary_does_not_import_live_mutation_surfaces():
    tree = parse_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_paper_recommendation_gate_summary_exports_only_report_api():
    tree = parse_module()

    assert module_exports(tree) == EXPECTED_EXPORTS


def test_paper_recommendation_gate_summary_does_not_define_forbidden_surfaces():
    tree = parse_module()
    names = set()
    for node in ast.walk(tree):
...
def test_paper_recommendation_gate_summary_does_not_perform_io_or_dynamic_execution():
    tree = parse_module()
    for node in ast.walk(tree):
