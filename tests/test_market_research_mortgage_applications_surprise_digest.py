...
def api():
    return importlib.import_module(
        "polymarket_alpha_lab.market_research_mortgage_applications_surprise_digest",
    )
...
def observation(
    ...
    *,
    region_id: str = "us",
    market_slug: str = "mba-mortgage-applications-above-consensus",
    observed_applications: str | Decimal = "421.000000",
    expected_applications: str | Decimal = "410.000000",
    surprise_ratio: str | Decimal = "0.026829",
    source_row_count: str | Decimal = "2",
    data_timestamp: datetime = datetime(2026, 7, 5, 11, 30, tzinfo=UTC),
    reason_codes: tuple[str, ...] = ("mortgage_applications_positive_surprise",),
):
    digest = api()
    return digest.MortgageApplicationsSurpriseObservation(
        source_id=source_id,
        region_id=region_id,
        market_slug=market_slug,
        observed_applications=decimal_arg(observed_applications),
        expected_applications=decimal_arg(expected_applications),
...
def report(*rows):
    digest = api()
    return digest.build_market_research_mortgage_applications_surprise_digest(
        rows,
        config=digest.MortgageApplicationsSurpriseDigestConfig(),
...
def test_empty_digest_blocks_as_missing_evidence() -> None:
    ...
    assert digest_report.recommended_next_step == (
        "block_report_only_market_research_mortgage_applications_surprise_digest"
    )
    assert digest_report.reason_codes == ("no_inputs",)
    assert digest_report.reason_code_counts == (
        api().MortgageApplicationsSurpriseReasonCodeCount(
...
def test_digest_reduces_mortgage_applications_surprises_with_deterministic_rows() -> None:
    digest_report = report(
        observation(
            "source-inline",
            region_id="midwest",
            market_slug="midwest-mortgage-applications-inline",
            observed_applications="99.000000",
            expected_applications="100.000000",
            surprise_ratio="-0.010000",
            source_row_count="1",
            data_timestamp=datetime(2026, 7, 5, 8, 15, tzinfo=timezone(timedelta(hours=-4))),
            reason_codes=("mortgage_applications_inline",),
        ),
        observation(
            "source-blocked",
            region_id="west",
            market_slug="west-mortgage-applications-spike",
            observed_applications="112.000000",
            expected_applications="100.000000",
            surprise_ratio="0.120000",
            source_row_count="3",
            data_timestamp=datetime(2026, 7, 5, 12, 45, tzinfo=UTC),
            reason_codes=("mortgage_applications_positive_surprise",),
        ),
        observation(
            "source-watch",
            region_id="south",
            market_slug="south-mortgage-applications-miss",
            observed_applications="188.000000",
            expected_applications="200.000000",
            surprise_ratio="-0.060000",
            source_row_count="2",
            data_timestamp=datetime(2026, 7, 5, 11, 30, tzinfo=UTC),
            reason_codes=("mortgage_applications_negative_surprise",),
        ),
    )
    ...
    assert digest_report.reason_code_counts == (
        api().MortgageApplicationsSurpriseReasonCodeCount(
            reason_code="mortgage_applications_large_surprise_present",
            count=d("1.000000"),
            observation_ratio=d("0.333333"),
        ),
        api().MortgageApplicationsSurpriseReasonCodeCount(
            reason_code="mortgage_applications_mixed_surprises_present",
            count=d("2.000000"),
    ...
    assert tuple(row.market_slug for row in digest_report.surprise_rows) == (
        "west-mortgage-applications-spike",
        "south-mortgage-applications-miss",
        "midwest-mortgage-applications-inline",
    )
...
def test_digest_rejects_float_inputs_false_flags_duplicates_and_mutation() -> None:
    ...
    with pytest.raises(ValueError, match="observed_applications must be a Decimal"):
        digest.MortgageApplicationsSurpriseObservation(
            source_id="source-float",
            region_id="us",
            market_slug="mortgage-applications-float",
            observed_applications=421.0,
            expected_applications=d("410.000000"),
            surprise_ratio=d("0.026829"),
            source_row_count=d("1"),
            data_timestamp=datetime(2026, 7, 5, 12, 0, tzinfo=UTC),
            reason_codes=("mortgage_applications_positive_surprise",),
        )
    with pytest.raises(ValueError, match="observed_applications must be a Decimal"):
        observation("source-decimal-subclass", observed_applications=_DecimalSubclass("421"))
    with pytest.raises(ValueError, match="expected_applications must be positive"):
        observation("source-zero-expected", expected_applications="0.000000")
    ...
    with pytest.raises(FrozenInstanceError):
        row.observed_applications = d("200")  # type: ignore[misc]
    ...
    with pytest.raises(ValueError, match="config paper_only must be True"):
        digest.MortgageApplicationsSurpriseDigestConfig(paper_only=False)
    with pytest.raises(ValueError, match="config report_only must be True"):
        digest.MortgageApplicationsSurpriseDigestConfig(report_only=False)
    with pytest.raises(ValueError, match="config readonly must be True"):
        digest.MortgageApplicationsSurpriseDigestConfig(readonly=False)
...
def test_digest_validates_timestamp_reason_codes_and_surprise_math() -> None:
    with pytest.raises(ValueError, match="data_timestamp must be timezone-aware"):
        observation(
            "source-naive",
            data_timestamp=datetime(2026, 7, 5, 12, 0),
        )
    with pytest.raises(ValueError, match="data_timestamp must be timezone-aware"):
        observation(
            "source-missing-offset",
            data_timestamp=datetime(2026, 7, 5, 12, 0, tzinfo=_MissingOffsetTimezone()),
        )
    with pytest.raises(ValueError, match="generated_at must be a datetime"):
        api().build_market_research_mortgage_applications_surprise_digest(
            (),
            config=api().MortgageApplicationsSurpriseDigestConfig(),
    ...
    with pytest.raises(ValueError, match="reason_codes must match surprise direction"):
        observation(
            "source-bad-reason",
            observed_applications="421.000000",
            expected_applications="410.000000",
...
def test_manual_report_and_constructor_ordering_rejects_drift() -> None:
    digest_report = report(
        observation(
        ...
        observation(
            "source-watch-consistency",
            observed_applications="94.000000",
            expected_applications="100.000000",
            surprise_ratio="-0.060000",
            reason_codes=("mortgage_applications_negative_surprise",),
        ),
    ...
    inline_row = report(
        observation(
            ...
            observed_applications="100.000000",
            expected_applications="100.000000",
    ...
    with pytest.raises(ValueError, match="recommended_next_step must match"):
        replace(
            ...
            recommended_next_step=(
                "allow_report_only_market_research_mortgage_applications_surprise_digest"
            ),
        )
...
def test_payload_is_json_ready_and_omits_live_or_durable_surfaces() -> None:
    payload = api().market_research_mortgage_applications_surprise_digest_payload(
        report(
            observation(
                "source-json",
                observed_applications="421.000000",
    ...
    assert payload["surprise_rows"][0]["observed_applications"] == "421.000000"
    ...
    module_path = (
        ...
        / "market_research_mortgage_applications_surprise_digest.py"
    )
...
def test_public_numeric_count_and_ratio_fields_are_decimal_only() -> None:
    ...
    for value in (
        digest.MortgageApplicationsSurpriseDigestConfig(),
...
def _is_public_numeric_field(field_name: str) -> bool:
    return (
        ...
        or field_name in {"observed_applications", "expected_applications"}
    )
