...
def other_sports_evidence(
    *,
    source_id: str = "other_sports_odds_reference",
    source_type: str = "market_data",
    evidence_text: str = "Consensus odds and matchup notes favor Fighter A.",
    data_timestamp: datetime = GENERATED_AT,
    data_freshness_seconds: int = 600,
    evidence_type: str = "odds_matchup",
    weight: Decimal = d("0.650000"),
    probability_impact: Decimal = d("0.030000"),
    reason_codes: tuple[str, ...] = ("other_sports_odds_fresh",),
    paper_only: bool = True,
    report_only: bool = True,
    readonly: bool = True,
) -> SportsOtherEvidenceInput:
    return SportsOtherEvidenceInput(
        source_id=source_id,
...
def build_forecast(
    *,
    condition_id: str = "condition-other-sports",
    market_slug: str = "ufc-main-event-winner",
    question: str = "Will Fighter A win the main event?",
    event_template: str = "other_sports_match_winner",
    evidence: tuple[SportsOtherEvidenceInput, ...] | None = None,
    base_probability: Decimal = d("0.570000"),
    config: SportsOtherTeamConfig | None = None,
    generated_at: datetime = GENERATED_AT,
) -> tuple[TeamForecastPacket, tuple[TeamForecastEvidencePacket, ...]]:
    return build_sports_other_team_forecast(
        condition_id=condition_id,
        market_slug=market_slug,
        question=question,
        event_template=event_template,
        evidence=evidence if evidence is not None else (other_sports_evidence(),),
        base_probability=base_probability,
        config=config
        if config is not None
        else SportsOtherTeamConfig(config_version="sports-other-team-v0"),
        generated_at=generated_at,
    )


def test_builds_supplied_input_other_sports_forecast_and_evidence_packets():
    forecast, evidence_rows = build_sports_other_team_forecast(
        condition_id="condition-other-sports",
        market_slug="ufc-main-event-winner",
        question="Will Fighter A win the main event?",
        event_template="other_sports_match_winner",
        evidence=(
            SportsOtherEvidenceInput(
                source_id="other_sports_odds_reference",
                source_type="market_data",
                evidence_text="Consensus odds and matchup notes favor Fighter A.",
                data_timestamp=GENERATED_AT,
                data_freshness_seconds=600,
                evidence_type="odds_matchup",
                weight=d("0.650000"),
                probability_impact=d("0.030000"),
                reason_codes=("other_sports_odds_fresh",),
            ),
        ),
        base_probability=d("0.570000"),
        config=SportsOtherTeamConfig(config_version="sports-other-team-v0"),
        generated_at=GENERATED_AT,
    )
    ...
    assert forecast.team_id == "sports_other"
    assert forecast.category_id == "sports.other"
    assert forecast.condition_id == "condition-other-sports"
    assert forecast.market_slug == "ufc-main-event-winner"
    assert forecast.question == "Will Fighter A win the main event?"
    assert forecast.event_template == "other_sports_match_winner"
    ...
    assert forecast.reason_codes == (
        "other_sports_odds_fresh",
        "team_sports_other",
    )
    assert forecast.source_references == ("other_sports_odds_reference",)
    assert forecast.memory_references == ("sports_other_supplied_evidence_only",)
    ...
    assert forecast.config_version == "sports-other-team-v0"
    assert forecast.prompt_version == "sports-other-supplied-input-v0"
    ...
    assert evidence_rows[0].evidence_id == (
        "condition-other-sports:other_sports_odds_reference"
    )
    assert evidence_rows[0].team_id == "sports_other"
    assert evidence_rows[0].market_slug == "ufc-main-event-winner"
    assert evidence_rows[0].source_id == "other_sports_odds_reference"
    assert evidence_rows[0].source_type == "market_data"
    assert evidence_rows[0].data_timestamp == GENERATED_AT
    assert evidence_rows[0].data_freshness_seconds == 600
    assert evidence_rows[0].evidence_type == "odds_matchup"
    assert evidence_rows[0].evidence_text == (
        "Consensus odds and matchup notes favor Fighter A."
    )
    assert evidence_rows[0].weight == d("0.650000")
    assert evidence_rows[0].reason_codes == ("other_sports_odds_fresh",)
...
def test_forecast_probability_clamps_to_zero_and_one():
    high_forecast, _ = build_forecast(
        ...
        evidence=(other_sports_evidence(probability_impact=d("0.250000")),),
    )
...
def test_data_freshness_score_uses_other_sports_thresholds(
    freshness_seconds: int,
    expected_score: Decimal,
):
    forecast, _ = build_forecast(
        evidence=(other_sports_evidence(data_freshness_seconds=freshness_seconds),),
    )
...
def test_confidence_and_evidence_quality_average_weights_and_clamp_to_unit_interval():
    forecast, _ = build_forecast(
        evidence=(
            other_sports_evidence(
                source_id="other_sports_odds_reference",
                weight=d("0.900000"),
                probability_impact=d("0.010000"),
            ),
            other_sports_evidence(
                source_id="other_sports_scout_reference",
                evidence_type="scout_matchup",
                weight=d("0.300000"),
                probability_impact=d("-0.020000"),
                reason_codes=("other_sports_matchup_soft",),
            ),
        ),
    )
    ...
    assert forecast.reason_codes == (
        "other_sports_matchup_soft",
        "other_sports_odds_fresh",
        "team_sports_other",
    )
    assert forecast.source_references == (
        "other_sports_odds_reference",
        "other_sports_scout_reference",
    )
...
def test_selected_side_uses_market_implied_probability_hint_threshold():
    yes_forecast, _ = build_forecast(
        ...
        config=SportsOtherTeamConfig(
            config_version="sports-other-team-v0",
            market_implied_probability_hint=d("0.620000"),
        ),
        evidence=(other_sports_evidence(probability_impact=d("0.000000")),),
    )
...
def test_unsafe_side_and_config_flags_raise():
    ...
    with pytest.raises(ValueError, match="paper_only"):
        SportsOtherTeamConfig(config_version="sports-other-team-v0", paper_only=False)
    ...
    unsafe_config = object.__new__(SportsOtherTeamConfig)
    object.__setattr__(unsafe_config, "config_version", "sports-other-team-v0")
    ...
    object.__setattr__(
        unsafe_config,
        "prompt_version",
        "sports-other-supplied-input-v0",
    )
...
def test_supplied_evidence_rejects_non_decimals_and_noncanonical_fields():
    with pytest.raises(ValueError, match="weight must be a Decimal"):
        other_sports_evidence(weight="0.650000")  # type: ignore[arg-type]
    ...
    with pytest.raises(ValueError, match="probability_impact must be a Decimal"):
        other_sports_evidence(probability_impact="0.030000")  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="event_template"):
        build_forecast(event_template=" other_sports_match_winner")

    with pytest.raises(ValueError, match="source_id"):
        other_sports_evidence(source_id=" other_sports_odds_reference")


def test_input_dataclasses_are_frozen():
    config = SportsOtherTeamConfig(config_version="sports-other-team-v0")
    evidence = other_sports_evidence()
