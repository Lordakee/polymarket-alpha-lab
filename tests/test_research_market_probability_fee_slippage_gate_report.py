...
GENERATED_AT = datetime(2026, 7, 9, 18, 15, tzinfo=UTC)
OBSERVED_AT = datetime(2026, 7, 9, 18, 0, tzinfo=UTC)
...
def api() -> Any:
    return import_module(
        ...
        "research_market_probability_fee_slippage_gate_report",
    )
...
def sample(reference: str, **overrides: object) -> Any:
    ...
    values = {
        ...
        "slippage_buffer_rate": d("0.004000"),
    ...
    return module.ResearchMarketProbabilityFeeSlippageGateInput(**values)


def build_report(*inputs: object, config: object | None = None) -> Any:
    module = api()
    return module.build_research_market_probability_fee_slippage_gate_report(
        inputs,
...
def test_fee_slippage_gate_scores_sorts_and_summarizes_statuses() -> None:
    module = api()
    passed = sample(
    ...
    blocked = sample(
        ...
        slippage_buffer_rate=d("0.070000"),
    ...
    assert type(report) is module.ResearchMarketProbabilityFeeSlippageGateReport
    ...
    assert report.average_net_probability_edge == d("-0.024333")
    assert report.min_net_probability_edge == d("-0.160000")
    assert report.max_total_fee_slippage_drag_rate == d("0.180000")
    assert report.max_drag_to_edge_ratio == d("9.000000")
    ...
    assert block_row.total_fee_slippage_drag_rate == d("0.180000")
    assert block_row.net_probability_edge == d("-0.160000")
    assert block_row.drag_to_edge_ratio == d("9.000000")
    assert block_row.reason_codes == (
        "probability_fee_slippage_gate_block",
        "net_probability_edge_block",
        "total_fee_slippage_drag_block",
        "drag_to_edge_ratio_block",
        "slippage_buffer_block",
    )

    assert watch_row.gross_probability_edge == d("0.060000")
    assert watch_row.total_fee_slippage_drag_rate == d("0.038000")
    assert watch_row.net_probability_edge == d("0.022000")
    assert watch_row.drag_to_edge_ratio == d("0.633333")
    assert watch_row.reason_codes == (
        "probability_fee_slippage_gate_watch",
        "net_probability_edge_watch",
        "total_fee_slippage_drag_watch",
        "drag_to_edge_ratio_watch",
    )

    assert pass_row.gross_probability_edge == d("0.080000")
    assert pass_row.total_fee_slippage_drag_rate == d("0.015000")
    assert pass_row.net_probability_edge == d("0.065000")
    assert pass_row.reason_codes == ("probability_fee_slippage_gate_pass",)
...
def test_empty_input_blocks_without_row_leakage() -> None:
    ...
    assert report.average_net_probability_edge == d("0.000000")
    assert report.min_net_probability_edge == d("0.000000")
    assert report.max_total_fee_slippage_drag_rate == d("0.000000")
    assert report.max_drag_to_edge_ratio == d("0.000000")
    assert report.reason_codes == ("missing_probability_fee_slippage_gate_inputs",)
...
def test_public_payload_is_json_ready_immutable_digest_bound_and_safe() -> None:
    ...
    payload = module.research_market_probability_fee_slippage_gate_report_payload(report)
    ...
    assert payload["generated_at"] == "2026-07-09T18:15:00+00:00"
    assert payload["rows"][0]["signal_digest"] == report.rows[0].signal_digest
    assert payload["rows"][0]["total_fee_slippage_drag_rate"] == "0.180000"
    ...
    with pytest.raises(ValueError, match="derived_validation_digest"):
        module.research_market_probability_fee_slippage_gate_report_payload(tampered)

    unsafe_payload = dict(payload)
    unsafe_payload["market_id"] = "leaked"
    with pytest.raises(ValueError, match="unsafe public field"):
        module.research_market_probability_fee_slippage_gate_report_payload(
            unsafe_payload,
        )
...
def test_contracts_are_frozen_decimal_only_strict_and_flag_locked() -> None:
    ...
    for contract in (
        module.ResearchMarketProbabilityFeeSlippageGateConfig,
        module.ResearchMarketProbabilityFeeSlippageGateInput,
        module.ResearchMarketProbabilityFeeSlippageGateRow,
        module.ResearchMarketProbabilityFeeSlippageGateReasonCodeCount,
        module.ResearchMarketProbabilityFeeSlippageGateReport,
    ):
    ...
    with pytest.raises(ValueError, match="block threshold"):
        module.ResearchMarketProbabilityFeeSlippageGateConfig(
            watch_drag_to_edge_ratio=d("1.000000"),
            block_drag_to_edge_ratio=d("0.500000"),
        )
...
def test_source_excludes_database_network_wallet_action_and_advice_surfaces() -> None:
    ...
    assert "research_market_probability_fee_slippage_gate_report_payload" in (
        module.__all__
    )
    for banned in (
        "api_key",
