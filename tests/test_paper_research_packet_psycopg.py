...
def adapter_module() -> types.ModuleType:
    sys.modules.pop("polymarket_alpha_lab.paper_research_packet_psycopg", None)
    return importlib.import_module("polymarket_alpha_lab.paper_research_packet_psycopg")
...
def test_import_does_not_require_psycopg(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    module = importlib.import_module("polymarket_alpha_lab.paper_research_packet_psycopg")

    assert module.__name__ == "polymarket_alpha_lab.paper_research_packet_psycopg"
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-research-packet-v0")
    ...
    inserted = adapter_module.insert_paper_research_packet_report_with_psycopg(
    ...
    assert store_table_name == "research_packet_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    loaded = adapter_module.load_paper_research_packet_reports_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
    ...
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-research-packet-v0"
    assert limit == 10
    assert table_name == "research_packet_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    adapter_module.insert_paper_research_packet_report_with_psycopg(
        "postgresql://user:secret@example.invalid/db",
        FakeReport(config_version="paper-research-packet-v0"),
    )

    _, params = connection.cursor_instance.calls[0]
    assert isinstance(params[0], FakeJsonb)
    assert params[0].value == [{"packet_rank": 1}]
    ...
    assert params[3] == "paper-research-packet-v0"
    assert params[4] == 2
    assert params[5] is None
    assert connection.commit_count == 1


def test_insert_exposes_cursor_rowcount_to_store(
    monkeypatch: pytest.MonkeyPatch,
    adapter_module: types.ModuleType,
) -> None:
    connection = FakeCursorConnection()
    _install_fake_psycopg(monkeypatch, connect=lambda dsn: connection)
    observed_rowcounts: list[int] = []

    def fake_insert(connection_arg: Any, report_arg: Any, *, table_name: str) -> FakeRow:
        cursor = connection_arg.cursor()
        try:
            cursor.execute("insert", ())
            observed_rowcounts.append(cursor.rowcount)
        finally:
            cursor.close()
        return FakeRow(report_sha256="a" * 64)
    ...
    assert inserted == FakeRow(report_sha256="a" * 64)
    assert observed_rowcounts == [1]
    assert connection.commit_count == 1
...
def test_connect_failure_raises_clean_error_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_research_packet_reports_with_psycopg(
            "postgresql://user:secret@example.invalid/db",
        )
