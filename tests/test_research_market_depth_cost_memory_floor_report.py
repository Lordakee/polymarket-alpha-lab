...
MODULE_NAME = "polymarket_alpha_lab.research_market_depth_cost_memory_floor_report"
MODULE_PATH = (
    Path(__file__).resolve().parents[1]
    / "src"
    / "polymarket_alpha_lab"
    / "research_market_depth_cost_memory_floor_report.py"
)
...
def config(**overrides: object) -> Any:
    module = api()
    values = {
        "watch_depth_cost_floor": d("0.030000"),
        "block_depth_cost_floor": d("0.075000"),
        "min_memory_points": d("2.000000"),
        "min_depth_ratio": d("0.500000"),
    }
    values.update(overrides)
    return module.ResearchMarketDepthCostMemoryFloorConfig(**values)
...
def observation(
    ...
    *,
    ...
    module = api()
    return module.ResearchMarketDepthCostMemoryFloorObservation(
...
def build_report(*observations: Any, cfg: Any | None = None) -> Any:
    module = api()
    return module.build_research_market_depth_cost_memory_floor_report(
        observations,
...
def test_empty_report_is_pass_readonly_decimal_digest_and_json_safe() -> None:
    ...
    assert type(report) is module.ResearchMarketDepthCostMemoryFloorReport
    ...
    assert report.max_depth_cost == d("0.000000")
    assert report.min_depth_ratio_observed == d("0.000000")
    assert report.reason_codes == ("depth_cost_memory_floor_empty",)
    assert report.rows == ()
    ...
    payload = module.research_market_depth_cost_memory_floor_report_payload(report)
    ...
    module.validate_research_market_depth_cost_memory_floor_public_payload(payload)
...
def test_status_rollups_rows_redaction_and_payload_are_deterministic() -> None:
    ...
    assert report.max_depth_cost == d("0.090000")
    assert report.min_depth_ratio_observed == d("0.450000")
    assert report.reason_codes == (
        "depth_cost_memory_floor_watch_cost",
        "depth_cost_memory_floor_block_cost",
        "depth_cost_memory_floor_insufficient_memory",
        "depth_cost_memory_floor_thin_depth",
    )
    ...
    payload_again = module.research_market_depth_cost_memory_floor_report_payload(
        build_report(
            observation(
                "candidate-source-raw-market",
                segment_key="delta-memory-block",
...
def test_dataclasses_are_frozen_exact_decimal_only_and_flags_are_hard() -> None:
    ...
    for cls in (
        module.ResearchMarketDepthCostMemoryFloorConfig,
        module.ResearchMarketDepthCostMemoryFloorObservation,
        module.ResearchMarketDepthCostMemoryFloorRow,
        module.ResearchMarketDepthCostMemoryFloorReport,
    ):
    ...
    with pytest.raises(ValueError, match="watch_depth_cost_floor"):
        config(watch_depth_cost_floor=0.03)
    with pytest.raises(ValueError, match="block_depth_cost_floor"):
        config(block_depth_cost_floor=_DecimalSubclass("0.075000"))
...
def test_public_payload_validator_rejects_leaks_numerics_and_digest_tampering() -> None:
    module = api()
    payload = module.research_market_depth_cost_memory_floor_report_payload(
        build_report(
            observation(
    ...
    with pytest.raises(ValueError, match="unsafe public field"):
        module.validate_research_market_depth_cost_memory_floor_public_payload(
