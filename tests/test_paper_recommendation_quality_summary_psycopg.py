...
def test_public_exports_include_insert_and_load(adapter_module: types.ModuleType) -> None:
    assert sorted(adapter_module.__all__) == [
        "insert_paper_recommendation_quality_summary_report_with_psycopg",
        "load_paper_recommendation_quality_summary_reports_with_psycopg",
    ]
    assert not hasattr(
        adapter_module,
        "DEFAULT_PAPER_RECOMMENDATION_QUALITY_SUMMARY_TABLE",
    )
...
def test_insert_opens_psycopg_connection_delegates_commits_and_closes(
    ...
    report = FakeReport(config_version="paper-recommendation-quality-summary-v0")
    ...
    inserted = adapter_module.insert_paper_recommendation_quality_summary_report_with_psycopg(
    ...
    assert store_table_name == "quality_summary_archive"
...
def test_load_opens_psycopg_connection_delegates_query_options_commits_and_closes(
    ...
    def fake_load(
        ...
        *,
        ...
        store_calls.append((connection_arg, config_version, summary_status, limit, table_name))
        return (report,)
    ...
    loaded = adapter_module.load_paper_recommendation_quality_summary_reports_with_psycopg(
        "postgresql://postgres:postgres@localhost:54322/postgres",
    ...
    assert connect_calls == ["postgresql://postgres:postgres@localhost:54322/postgres"]
    store_connection, config_version, summary_status, limit, table_name = store_calls[0]
    assert store_connection is not connection
    assert store_connection.connection is connection
    assert config_version == "paper-recommendation-quality-summary-v0"
    assert summary_status == "blocked"
    assert limit == 10
    assert table_name == "quality_summary_archive"
    assert connection.commit_count == 1
    assert connection.rollback_count == 0
    assert connection.close_count == 1
...
def test_insert_adapts_json_values_for_psycopg_without_wrapping_scalars(
    ...
    adapter_module.insert_paper_recommendation_quality_summary_report_with_psycopg(
        "postgresql://postgres:postgres@localhost:54322/postgres",
        FakeReport(config_version="paper-recommendation-quality-summary-v0"),
    )

    _, params = connection.cursor_instance.calls[0]
    assert isinstance(params[0], FakeJsonb)
    assert params[0].value == ["wide_spread"]
    assert isinstance(params[1], FakeJsonb)
    assert params[1].value == {"payload": {"readonly": True}}
    assert isinstance(params[2], FakeJsonb)
    assert params[2].value == [{"report_name": "health"}]
...
def test_cleanup_failure_does_not_mask_commit_exception(
    ...
    def fake_load(
        ...
        *,
        ...
        return (FakeReport(config_version="paper-recommendation-quality-summary-v0"),)
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_recommendation_quality_summary_reports_with_psycopg(
            "postgresql://postgres:postgres@localhost:54322/postgres",
        )
