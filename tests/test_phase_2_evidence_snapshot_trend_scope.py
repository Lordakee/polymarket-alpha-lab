...
TREND_PATH = REPO_ROOT / "src" / "polymarket_alpha_lab" / "phase_2_evidence_snapshot_trend.py"
...
def test_phase_2_evidence_snapshot_trend_module_imports_only_allowed_dependencies():
...
def test_phase_2_evidence_snapshot_trend_module_does_not_import_forbidden_surfaces():
    tree = parse_trend_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_phase_2_evidence_snapshot_trend_uses_only_allowed_first_party_symbols():
    tree = parse_trend_module()
    assert imported_first_party_symbols(tree) == EXPECTED_FIRST_PARTY_IMPORTS


def test_phase_2_evidence_snapshot_trend_does_not_import_first_party_modules_wholesale():
    tree = parse_trend_module()
    for node in ast.walk(tree):
...
def test_phase_2_evidence_snapshot_trend_does_not_define_forbidden_names():
    tree = parse_trend_module()
    names = set()
    for node in ast.walk(tree):
...
def test_package_root_does_not_export_phase_2_evidence_snapshot_trend_names():
