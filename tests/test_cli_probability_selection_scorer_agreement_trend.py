...
COMMAND = "probability-selection-scorer-agreement-trend"
...
def test_parser_help_includes_probability_selection_scorer_agreement_trend(
...
def test_agreement_trend_command_help_declares_readonly_report_only_boundary(
    capsys: pytest.CaptureFixture[str],
) -> None:
    with pytest.raises(SystemExit) as exc_info:
        main([COMMAND, "--help"])
...
def test_agreement_trend_cli_rejects_db_file_live_wallet_order_and_auth_flags(
    flag: str,
    capsys: pytest.CaptureFixture[str],
) -> None:
    with pytest.raises(SystemExit) as exc_info:
...
def test_agreement_trend_disabled_db_config_does_not_connect_or_run(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def forbidden_runner(**kwargs: Any) -> object:
        ...
        runner_calls += 1
        raise AssertionError("agreement trend runner should not run without DB")
    ...
    exit_code = main(
        [COMMAND],
        probability_selection_scorer_agreement_trend_runner=forbidden_runner,
...
def test_agreement_trend_enabled_db_without_dsn_does_not_connect_or_run(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setenv(PROBABILITY_SELECTION_SCORER_AGREEMENT_DB_ENABLED_ENV_VAR, "true")
    ...
    def forbidden_runner(**kwargs: Any) -> object:
        nonlocal runner_calls
        runner_calls += 1
        raise AssertionError("agreement trend runner should not run without DSN")
...
def test_agreement_trend_cli_rejects_non_positive_limit_before_env_runner_or_connect(
    raw_limit: str,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def forbidden_runner(**kwargs: Any) -> object:
        nonlocal runner_calls
        runner_calls += 1
        raise AssertionError("agreement trend runner should not run")
...
def test_agreement_trend_cli_uses_agreement_env_config_and_prints_aggregate_summary(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    exit_code = main(
        [COMMAND, "--limit", "25"],
        probability_selection_scorer_agreement_trend_runner=fake_runner,
        client_factory=lambda: (_ for _ in ()).throw(
            AssertionError("client should not be constructed"),
        ),
    )
    ...
    for stable_field in (
        "source_report_count=4",
        "first_generated_at=2026-06-28T08:00:00+00:00",
        "latest_generated_at=2026-06-28T11:30:00+00:00",
        "history_span_seconds=12600",
        "latest_agreement_status=low_overlap",
        "latest_status_streak=2",
        "aligned_report_count=1",
        "low_overlap_report_count=2",
    ...
    assert "latest_agreement_low_overlap,recurring_agreement_reason_codes" in captured.out
    assert (
        "recurring_reason_code_counts: "
...
def test_agreement_trend_cli_rejects_injected_runner_result_that_is_not_trend_report(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    captured = capsys.readouterr()
    assert f"{COMMAND} failed:" in captured.err
    assert "ProbabilitySelectionScorerAgreementTrendReport" in captured.err
    assert "payload-json-secret" not in captured.out
    assert "payload-json-secret" not in captured.err


def test_agreement_trend_cli_read_errors_redact_dsn_table_payload_and_hash(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    dsn = (
    ...
    exit_code = main(
        [COMMAND],
        probability_selection_scorer_agreement_trend_runner=broken_runner,
    )
...
def test_agreement_trend_summary_sanitizes_reason_code_output(
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    getattr(cli, "_print_probability_selection_scorer_agreement_trend_summary")(report)
...
def test_agreement_trend_helper_default_load_path_reads_reports_chronologically(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    dsn = "postgresql://agreement:secret@localhost:54322/postgres"
    table_name = "probability_selection_scorer_agreement_reports"
    ...
    helper = getattr(cli, "_run_probability_selection_scorer_agreement_trend")
    result = helper(
        dsn=dsn,
        table_name=table_name,
        limit=7,
...
def test_agreement_trend_cli_default_load_errors_close_and_redact(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    dsn = (
...
def test_agreement_trend_cli_rejects_remote_db_dsn_before_connecting_or_running(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    remote_dsn = (
    ...
    def forbidden_runner(**kwargs: Any) -> object:
        nonlocal runner_calls
        runner_calls += 1
        raise AssertionError("agreement trend runner should not run with remote DSN")
