...
STORE_MODULE_NAME = "polymarket_alpha_lab.paper_autonomous_readiness_gate_store"
ADAPTER_MODULE_NAME = "polymarket_alpha_lab.paper_autonomous_readiness_gate_psycopg"
...
def _install_fake_store(
    ...
    *,
    ...
    store_module.DEFAULT_PAPER_AUTONOMOUS_READINESS_GATE_REPORTS_TABLE = (
        "paper_autonomous_readiness_gate_reports"
    )
    store_module.insert_paper_autonomous_readiness_gate_report_with_result = (
        insert_with_result
        if insert_with_result is not None
        else _unexpected_store_call("insert_with_result")
    )
    store_module.load_paper_autonomous_readiness_gate_reports = (
...
def test_missing_psycopg_error_mentions_install_extra_without_dsn(
    ...
    with pytest.raises(RuntimeError) as exc_info:
        adapter_module.load_paper_autonomous_readiness_gate_reports_with_psycopg(
            SECRET_DSN,
        )
...
def test_successful_insert_delegates_to_store_commits_and_closes(
    ...
    report = FakeReport(readiness_status="pass")
    ...
    inserted = adapter_module.insert_paper_autonomous_readiness_gate_report_with_psycopg(
        SECRET_DSN,
        report,
        table_name="paper_autonomous_readiness_gate_archive",
    )
    ...
    assert store_table_name == "paper_autonomous_readiness_gate_archive"
...
def test_successful_load_connects_autocommit_delegates_closes_without_commit(
    ...
    connection = FakeConnection()
    report = FakeReport(readiness_status="watch")
    connect_calls: list[tuple[str, dict[str, Any]]] = []
    store_calls: list[
        tuple[
            Any,
    ...
    loaded = adapter_module.load_paper_autonomous_readiness_gate_reports_with_psycopg(
        SECRET_DSN,
    ...
    assert limit == 25
    assert table_name == "paper_autonomous_readiness_gate_archive"
...
def test_write_store_failure_rolls_back_closes_reraises_and_does_not_echo_dsn(
    ...
    with pytest.raises(ValueError) as exc_info:
        adapter_module.insert_paper_autonomous_readiness_gate_report_with_psycopg(
            SECRET_DSN,
            FakeReport(readiness_status="pass"),
        )
...
def test_connect_failure_raises_redacted_error(
    ...
    message = str(exc_info.value)
    assert message == "failed to connect to the paper autonomous readiness gate database"
...
def test_json_cursor_exposes_fetchall_and_rowcount_for_store_delegates(
    ...
    result = adapter_module.insert_paper_autonomous_readiness_gate_report_with_psycopg(
        SECRET_DSN,
        FakeReport(readiness_status="pass"),
    )
