...
def parse_trend_batch_health_module():
    return ast.parse(TREND_BATCH_HEALTH_PATH.read_text(encoding="utf-8"))
...
def test_trend_batch_health_module_imports_only_allowed_dependencies():
    tree = parse_trend_batch_health_module()
...
def test_trend_batch_health_module_does_not_import_forbidden_surfaces():
    tree = parse_trend_batch_health_module()
    for module_name in imported_modules(tree):
        assert not any(
...
def test_trend_batch_health_uses_only_allowed_first_party_symbols():
    tree = parse_trend_batch_health_module()
    assert imported_first_party_symbols(tree) == EXPECTED_FIRST_PARTY_IMPORTS


def test_trend_batch_health_does_not_import_first_party_modules_wholesale():
    tree = parse_trend_batch_health_module()
    for node in ast.walk(tree):
...
def test_trend_batch_health_does_not_define_forbidden_names():
    tree = parse_trend_batch_health_module()
    names = set()
    for node in ast.walk(tree):
...
def test_trend_batch_health_public_exports_are_report_only():
    tree = parse_trend_batch_health_module()
...
def test_package_root_exports_batch_health_trend_batch_health_names_only_for_node_14():
...
def test_readme_level_2_node_14_section_keeps_report_only_boundaries():
    readme = README_PATH.read_text(encoding="utf-8")
    start = readme.index("## Level 2 Node 14 Status")
    end = readme.index("## Automation Roadmap", start)
    normalized = normalize_identifier(readme[start:end])
    required_fragments = (
        "level2node14status",
        "level2node14pythonapi",
        "reportonly",
        "suppliedtradeproposalevidencecomparisonhistorybatchhealthtrendbatchreport",
        "trendbatchstatusfrequencies",
        "node13gatestatusfrequencies",
...
def test_readme_repository_layout_lists_level_2_node_14_artifacts():
    readme = README_PATH.read_text(encoding="utf-8")
