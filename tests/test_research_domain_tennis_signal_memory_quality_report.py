...
MODULE_PATH = (
    ...
    / "research_domain_tennis_signal_memory_quality_report.py"
)
...
def api():
    return importlib.import_module(
        "polymarket_alpha_lab.research_domain_tennis_signal_memory_quality_report",
    )
...
def config(**overrides: object):
    module = api()
    values = {
        "config_version": "tennis-signal-memory-quality-test",
        "stale_player_after_seconds": d("604800.000000"),
        "stale_injury_after_seconds": d("172800.000000"),
        "stale_surface_after_seconds": d("1209600.000000"),
        "stale_form_after_seconds": d("604800.000000"),
    ...
    return module.ResearchDomainTennisSignalMemoryQualityConfig(**values)


def memory(**overrides: object):
    module = api()
    values = {
        "memory_ref": "tennis-memory-player-a",
        "team_ref": "tennis-research-team",
        "event_ref": "tennis-event-a",
        "input_family": "player",
        "memory_present": True,
    ...
    values.update(overrides)
    return module.ResearchDomainTennisSignalMemoryInput(**values)


def build_report(*rows: object, cfg=None, generated_at: datetime = GENERATED_AT):
    module = api()
    return module.build_research_domain_tennis_signal_memory_quality_report(
        rows,
...
def test_tennis_memory_quality_identifies_stale_conflicting_and_missing_inputs() -> None:
    report = build_report(
        memory(memory_ref="tennis-memory-player-ready", input_family="player"),
        memory(
            memory_ref="tennis-memory-injury-stale",
            input_family="injury",
            observed_at=GENERATED_AT - timedelta(days=4),
            confidence_score=d("0.760000"),
        ),
        memory(
            memory_ref="tennis-memory-surface-conflict",
            input_family="surface",
            conflicting_evidence_count=d("1"),
            confidence_score=d("0.880000"),
        ),
        memory(
            memory_ref="tennis-memory-form-missing",
            input_family="form",
    ...
    assert report.average_quality_score == d("0.552500")
...
def test_empty_report_watches_before_forecast_handoff() -> None:
    ...
    assert report.reason_codes == ("no_tennis_memory_inputs_supplied",)
...
def test_payload_serializes_decimal_strings_and_validates_sha256_digest() -> None:
    ...
    payload = module.research_domain_tennis_signal_memory_quality_report_payload(report)
    ...
    payload_again = module.research_domain_tennis_signal_memory_quality_report_payload(
        report,
    )
    ...
    with pytest.raises(ValueError, match="derived_validation_digest"):
        module.research_domain_tennis_signal_memory_quality_report_payload(tampered)
...
def test_report_rejects_derived_validation_digest_tampering() -> None:
    ...
    with pytest.raises(ValueError, match="derived_validation_digest"):
        module.ResearchDomainTennisSignalMemoryQualityReport(**values)
...
def test_unsafe_public_payload_keys_values_and_flag_downgrades_rejected(
    ...
    with pytest.raises(ValueError):
        module.research_domain_tennis_signal_memory_quality_report_payload(payload)
...
def test_input_validation_rejects_raw_identifiers_and_invalid_status_surfaces() -> None:
    ...
    with pytest.raises(ValueError, match="unsafe public value"):
        memory(event_ref="market-ref-raw")
    with pytest.raises(ValueError, match="unsafe public value"):
        memory(memory_ref="live-player-memory")
    ...
    with pytest.raises(ValueError, match="row_status"):
        module.ResearchDomainTennisSignalMemoryQualityRow(
            memory_ref="tennis-memory-row",
            team_ref="tennis-research-team",
            event_ref="tennis-event-a",
            input_family="player",
            row_status="blocked",
