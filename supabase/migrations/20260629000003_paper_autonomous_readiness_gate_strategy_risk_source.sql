...
begin
    for constraint_name in
        ...
        where conrelid = 'public.paper_autonomous_readiness_gate_reports'::regclass
          and contype = 'c'
          and (
              lower(pg_get_constraintdef(oid)) like '%jsonb_array_length(source_statuses_json) in (3, 4)%'
              or lower(pg_get_constraintdef(oid)) like '%jsonb_array_length(source_statuses_json) = any (array[3, 4])%'
          )
    loop
        execute format(
    ...
    for constraint_name in
        ...
        where conrelid = 'public.paper_autonomous_readiness_gate_reports'::regclass
          and contype = 'c'
          and (
              lower(pg_get_constraintdef(oid)) like '%jsonb_array_length(source_config_versions_json) in (3, 4)%'
              or lower(pg_get_constraintdef(oid)) like '%jsonb_array_length(source_config_versions_json) = any (array[3, 4])%'
          )
    loop
        execute format(
            'alter table public.paper_autonomous_readiness_gate_reports drop constraint if exists %I',
